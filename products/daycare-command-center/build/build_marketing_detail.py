"""Detailed marketing image set for Daycare & Childcare Provider Command Center™ (07-10)."""
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR PROVIDERS", "Price It, Fill It, Keep It",
                "Four ways it turns a full house into an actual income")
    feats = [
        ("$", "Live Care Score", "donut", (90, "90%", "healthy"),
         "Spots filled, margin, break-even cover, ratios, your hourly rate and your tax reserve roll into one number \u2014 the health of the whole program at a glance."),
        ("%", "Rate & Break-Even Engine", "table", None,
         "What one child actually nets you after food and supplies, and how many children it takes just to cover the house. Everyone after that number is your pay."),
        ("box", "Run the Whole Program", "stock", None,
         "Children and families, tuition with who's behind, attendance, the CACFP food program, ratios by age group, compliance files and supplies \u2014 all in one place."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack \u2014 enrollment form, emergency & allergy card, daily sheet for parents, attendance, CACFP meal count and a compliance checklist."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("Tuition per child", "$1,061"), ("Net per child", "$904"), ("Break-even children", "5")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["THE ENGINE", "VALUE"], [0.0, 0.62]):
            ...
            od = ImageDraw.Draw(img)
            items = [("Occupancy", "91.7%", MINT_BG, PRIMARY), ("Margin", "55.7%", MINT_BG, PRIMARY),
                     ("Ratio", "5.5 : 1", MINT_BG, PRIMARY), ("Tax reserve", "40%", SURFACE, (110, 88, 58))]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Sign-In Sheet",
                "Everything a basic daycare printable pack leaves out \u2014 built in")
    rows = [
        ("Children logged in and out", True, True),
        ("Net per child after food & supplies", False, True),
        ("Break-even child count", False, True),
        ("What an empty spot costs you", False, True),
        ("Tuition owed vs collected", False, True),
        ("CACFP meal count & reimbursement", False, True),
        ("Ratios by age group", False, True),
        ("Compliance files, flagged incomplete", False, True),
        ("Your pay per hour actually open", False, True),
        ("Tax set-aside tracked all year", False, True),
    ...
    d.text((c1, top + hh // 2), "A Printable\nPack", font=fs(18), fill=(210, 210, 205), anchor="mm")
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "SET UP IN AN AFTERNOON", "Know Your Numbers in 4 Steps",
                "No spreadsheet skills needed \u2014 if you can type, you can run it")
    steps = [
        ("1", "List what it costs", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack, then list your assistant, insurance, home share, software and supplies \u2014 everything that bills you whether you have eleven children or six."),
        ("2", "Find your net per child", "Your weekly rate becomes a monthly figure, then food and supplies come out, and you finally see what one child actually contributes."),
        ("3", "Find your break-even", "Fixed costs divided by net per child gives the number of children who only pay the bills. Everyone enrolled after that number is your pay."),
        ("4", "Run the whole program", "Track tuition, attendance, the food program, ratios and compliance files, and let the Care Score tell you whether you're running a business or just a very full house."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "EVERYTHING YOU GET", "One System, Whole Program",
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + Care Score",
        "Rate & break-even engine", "Children, families & tuition",
        "Attendance & late-pickup log", "CACFP food program claim",
        "Ratios, compliance files & tax", "Start-Here quick-start guide",
    ]
    ...
    who = ["Licensed home daycare providers", "In-home & family childcare", "Small centres and preschools",
           "Nannies & nanny-share providers", "Providers opening their first program", "Anyone raising their rates this year"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "PRICE IT \u00b7 FILL IT \u00b7 KEEP IT", font=fs(27), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "You're not babysitting. You're running a business \u2014 run it like one.",
       fs(23, bold=False), TEXT_MUTED)
    square_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "DAYCARE", 58, max_w=1150)
    img.convert("RGB").save(out, "PNG", optimize=True)
