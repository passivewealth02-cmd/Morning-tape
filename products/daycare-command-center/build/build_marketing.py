"""Marketing image set for Daycare & Childcare Provider Command Center™ (6 images, 2000x2000)."""
...
TABS = ["Dashboard", "Rate & Enrollment", "Children & Families", "Tuition & Payments", "Attendance",
        "Costs & Expenses", "Food Program", "Ratios & Schedule", "Compliance & Files", "Supplies",
        "Tax Set-Aside", "Monthly Summary", "Settings"]

FILE_LABEL = "Daycare_Command_Center.xlsx — Little Acorns Home Childcare · Dana"
...
def acorn_crest(c, cx, cy, r=56, glow=True):
    """Brand crest — an acorn: small thing, grown carefully."""
    if glow:
    ...
    ov = Image.new("RGBA", c.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    # nut body
    od.pieslice((cx - r * 0.40, cy - r * 0.30, cx + r * 0.40, cy + r * 0.62), 0, 180,
                fill=GOLD_HI, outline=(150, 120, 70))
    od.rectangle((cx - r * 0.40, cy - r * 0.30, cx + r * 0.40, cy - r * 0.14), fill=GOLD_HI)
    # cap
    od.rounded_rectangle((cx - r * 0.46, cy - r * 0.40, cx + r * 0.46, cy - r * 0.12), radius=int(r * 0.10),
                         fill=PRIMARY_DK, outline=GOLD_HI, width=3)
    # cap texture
    for k in range(-2, 3):
        xx = cx + k * r * 0.17
        od.line((xx, cy - r * 0.38, xx, cy - r * 0.14), fill=GOLD_LT, width=2)
    # stalk
    od.line((cx, cy - r * 0.40, cx, cy - r * 0.58), fill=HIGHLIGHT, width=max(3, int(r * 0.08)))
    # two little leaves
    od.ellipse((cx + r * 0.02, cy - r * 0.66, cx + r * 0.28, cy - r * 0.50), fill=HIGHLIGHT)
    od.ellipse((cx - r * 0.28, cy - r * 0.66, cx - r * 0.02, cy - r * 0.50), fill=HIGHLIGHT)
    c.alpha_composite(ov)


square_crest = acorn_crest
road_crest = acorn_crest
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "THE PROGRAM", font=fs(15), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("WEEKLY RATE", "$245.00", "full time"),
    ("TUITION / CHILD", "$1,061", "per month"),
    ("COST / CHILD", "$157.00", "food & supplies"),
    ("NET PER CHILD", "$904", "what one child nets"),
    ("ENROLLED", "11", "of 12 licensed"),
    ("OCCUPANCY", "91.7%", "spots filled"),
    ("MONTHLY REVENUE", "$12,539", "tuition + food program"),
    ("MONTHLY COSTS", "$5,552", "fixed + per child"),
    ("YOUR PAY", "$6,987", "before tax"),
    ("BREAK-EVEN", "5", "children, just for the house"),
    ("YOUR $ / HOUR", "$32.27", "$24.20 after tax"),
    ("CARE SCORE", "90%", "healthy"),
]

REV_MONTHS = [("Feb", 10940), ("Mar", 11480), ("Apr", 12060), ("May", 12510), ("Jun", 12880), ("Jul", 12539)]


def content_dashboard(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Provider Dashboard", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 64), "Little Acorns Home Childcare \u00b7 Dana  \u00b7  you're not babysitting",
           font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 210, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 105, y0 + 44), "\u25cf $904 per child", font=fs(14), fill=PRIMARY, anchor="mm")
    ...
    d.text((gx, cy_top), "PROGRAM HEALTH \u00b7 WHERE THE MONTH GOES \u00b7 REVENUE BY MONTH", font=fs(19), fill=ACCENT, anchor="lt")
    ...
    d.text((px + 16, panels_y + 14), "Program Health", font=fs(17), fill=ACCENT, anchor="lt")
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("Spots filled", 1.0, "100%"), ("Margin healthy", 1.0, "100%"),
           ("House more than covered", 1.0, "100%"), ("Ratios comfortable", 1.0, "100%"),
           ("Your hourly on target", 1.0, "100%"), ("Tax reserve funded", 0.40, "40%")],
          color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Where the Month Goes", font=fs(15), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.44, panels_y + panel_h * 0.46, min(panel_h * 0.25, pw * 0.25),
          [(55.7, PRIMARY), (30.5, ACCENT), (11.8, SURFACE), (2.0, GOLD_LT)], "56%", "your pay")
    legend(d, px + pw * 0.06, panels_y + panel_h - 148,
           [(PRIMARY, "Your pay $6,987"), (ACCENT, "Fixed costs $3,825"),
            (SURFACE, "Food $1,485"), (GOLD_LT, "Supplies $242")], 14, 30)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Revenue by Month", font=fs(17), fill=ACCENT, anchor="lt")
    vbars(img, d, (px + 18, panels_y + 60, px + pw - 14, panels_y + panel_h - 10), REV_MONTHS, 12880)
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Care Score", font=fs(17), fill=ACCENT, anchor="lt")
...
def content_children(img, cbox):
    rows = [
        ("Zoe K. \u2014 infant", "Full time", "$285", "$1,234", "Paid"),
        ("Mia R. \u2014 preschool", "Full time", "$245", "$1,061", "Paid"),
        ("Ruby C. \u2014 toddler", "Full time", "$245", "$1,061", "Partial"),
        ("Owen H. \u2014 toddler", "Full time", "$245", "$1,061", "Late"),
        ("Isaac D. \u2014 school age", "Before/after", "$145", "$628", "Paid"),
    ]
    _table(img, cbox, "Children, Families & Tuition",
           "Every child, their schedule and their rate \u2014 and exactly who is behind.",
           ["CHILD", "SCHEDULE", "WEEKLY", "MONTHLY", "STATUS"],
           [0.0, 0.44, 0.60, 0.74, 0.89], rows,
           total_row=("11 enrolled", "91.7% full", "", "$11,063", "$1,422 out"))


def content_rate(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "The Rate & Break-Even Engine", font=fs(30), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 60), "What one child actually nets you \u2014 and how many it takes just to cover the house.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("Full-time weekly rate \u00d7 4.33 weeks", "$1,061", PRIMARY),
            ("\u2212 Food for that child", "$135.00", ACCENT),
            ("\u2212 Supplies & activities", "$22.00", ACCENT),
            ("= NET PER CHILD, PER MONTH", "$904", PRIMARY)]
    ...
    d.text((x0 + pad + 14, ry + rh / 2), "$3,825 fixed costs \u00f7 $904 = BREAK-EVEN", font=fs(18), fill=PRIMARY, anchor="lm")
    d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.80, ry + rh / 2), "5 children", font=fs(18), fill=PRIMARY, anchor="mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2 - 20), "Five just pay the bills. Everyone after that is your pay.", font=fs(22), fill=WHITE, anchor="mm")
    od.text(((x0 + x1) / 2, by + callout_h / 2 + 24), "ONE EMPTY SPOT COSTS YOU $10,846 A YEAR", font=fs(23), fill=GOLD_HI, anchor="mm")
    img.alpha_composite(ov)
...
def render_hero(out):
    ...
    acorn_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE COMPLETE HOME CHILDCARE SYSTEM", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "DAYCARE", 86, max_w=1550)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 546), "Rates, enrollment, tuition, ratios, the food program & tax \u2014 one system.",
       fs(20, bold=False), (224, 213, 190))
    chips = [("14", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("RATE", "& BREAK-EVEN ENGINE")]
    cw = 440
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not a sign-in sheet \u2014 a complete price-it, fill-it, keep-it system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Start Here", "how it all works"), ("Dashboard", "your pay & score"),
        ("Rate & Enrollment", "net per child"), ("Children & Families", "everyone enrolled"),
        ("Tuition & Payments", "who's behind"), ("Attendance", "in, out, late"),
        ("Costs & Expenses", "what it all costs"), ("Food Program", "claim every meal"),
        ("Ratios & Schedule", "stay covered"), ("Compliance & Files", "before they ask"),
        ("Supplies", "never run out"), ("Tax Set-Aside", "move it early"),
        ("Monthly Summary", "is it working?"), ("Settings", "set it once"),
        ("+ 12 Printable Pages", "forms & daily sheets"), ("Live Care Score", "it all rolls up"),
    ]
    cols = 4
    margin = 88
    gx, gy = 22, 20
...
def render_rate(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "AN EMPTY SPOT COSTS YOU", font=fs(29), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "Rate & Break-Even", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "What one child really nets \u2014 and how many it takes just to cover the house",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 1, content_rate)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_children(out):
    ...
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "EVERY CHILD, EVERY PAYMENT", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "Your Whole Roster", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Schedules, rates and exactly who is behind \u2014 flagged red automatically",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 2, content_children)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_engine(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 110, "PRICE IT \u00b7 FILL IT", font=fs(32), pad_x=52, pad_y=22)
    tc(d, (SIZE // 2, 224), "The Provider Engine", fserif(44), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 1, content_rate)
    app_window(img, (60, 1180, SIZE - 60, SIZE - 60), 2, content_children)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_printables(out):
    ...
    d = ImageDraw.Draw(img)
    acorn_crest(img, SIZE // 2, 110, r=48)
    pill(img, SIZE // 2, 214, "PLUS 12 PRINTABLE PAGES", font=fs(30), pad_x=48, pad_y=20)
    tc(d, (SIZE // 2, 300), "Print, Fill & Hand to Parents", fserif(42), WHITE)
    gold_divider(img, SIZE // 2, 362, width=480)
    tc(d, (SIZE // 2, 404), "A matching print-ready pack \u2014 enrollment form, emergency card, daily sheet, meal count & more",
       fs(21, bold=False), (226, 214, 190))
    labels = ["Rate & Break-Even", "Monthly Costs", "Enrollment Form", "Emergency & Allergy",
              "Daily Sheet", "Attendance Sheet", "Tuition Log", "CACFP Meal Count",
              "Ratios & Schedule", "Compliance Files", "Tax Set-Aside", "Monthly Review"]
...
def main():
    ...
    targets = [
        ...
        ("03_rate.png", render_rate),
        ("04_children.png", render_children),
