"""Detailed marketing image set for Tot-School Command Center™ (4 images, 2000x2000).

  07_features.png   - feature spotlights (what it does + why it matters)
  08_compare.png    - "basic toddler printable" vs Tot-School Command Center™
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR REAL TODDLER DAYS", "Where Little Ones Bloom",
                "Four ways it turns everyday play into visible progress")
    feats = [
        ("$", "Live Growing Score", "donut", (69, "69%", "growing"),
         "Milestones, first words, themes, trays & the ready-for-preschool checklist roll into one number — see at a glance how they're growing."),
        ("%", "A Toddler Milestones Tracker", "table", None,
         "Gross & fine motor, language, social-emotional, self-help & cognitive — mark each milestone Met, Emerging or Not Yet, at their own pace."),
        ("box", "First Words, Concepts & Trays", "stock", None,
         "Track spoken words, colors, shapes & animal sounds, plus a tot tray a day tied to a real skill — the little leaps, made visible."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready PDF pack — weekly theme plan, milestone checklist, tot-tray planner, sensory bank, portfolio of firsts & more."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("Language", "Met"), ("Fine Motor", "Met"), ("Cognitive", "Emerging")]
            ...
            od = ImageDraw.Draw(img)
            items = [("Words spoken", "45", MINT_BG, PRIMARY), ("Milestones met", "26/35", MINT_BG, PRIMARY),
                     ("Themes done", "5/12", SURFACE, (110, 88, 58)), ("Board books", "24", MINT_BG, PRIMARY)]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Milestone Chart",
                "Everything a basic toddler printable leaves out — built in")
    rows = [
        ("A gentle theme & tot-tray plan", True, True),
        ("Whole-child milestones tracker", False, True),
        ("First words & concepts tracker", False, True),
        ("Board-book read-aloud log", False, True),
        ("Messy & sensory play bank", False, True),
        ("Outings & nature memories", False, True),
        ("Portfolio of firsts", False, True),
        ("Supplies budget", False, True),
        ("Ready-for-preschool checklist", False, True),
        ("Live Growing Score", False, True),
...
def render_howitworks(out):
    ...
    steps = [
        ...
        ("2", "Add your little ones", "In Settings, add your family & tots, then fill a quick profile for each — age, personality & what they love."),
        ("3", "Pick a theme & set a tray", "Choose a gentle theme and set out a tot tray or two from the play bank. Themes, trays & books all roll into one dashboard."),
        ("4", "Play & watch them grow", "Mark milestones, first words & trays as they go — the Growing Score shows exactly how they're blossoming."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + Growing Score",
        "Whole-child milestones tracker", "First words, concepts & themes",
        "Tot-tray & sensory activity planner", "Board-book read-aloud log",
        "Portfolio of firsts, outings & supplies", "Start-Here guide + free updates",
    ]
    ...
    who = ["At-home tot-school & toddler families", "Homeschool & preschool-prep parents", "First-time & experienced moms",
           "Play-based · Montessori-inspired · gentle", "Nannies & in-home care providers", "Anyone loving the toddler years"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "NO PRESSURE · ALL PLAY", font=fs(28), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Slow down and play — the little years are the big ones.",
       fs(24, bold=False), TEXT_MUTED)
    book_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "TOT-SCHOOL COMMAND CENTER", 50, max_w=1560)
    img.convert("RGB").save(out, "PNG", optimize=True)
