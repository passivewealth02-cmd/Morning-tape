"""Marketing image set for Tot-School Command Center™ (6 images, 2000x2000).

  01_hero.png       - branded hero + live tot-school dashboard
  02_inside.png     - "everything inside — 17 connected tabs"
  03_milestones.png - the toddler milestones tracker (distinct sheet)
  04_words.png      - first words & concepts (the concept-tile shot)
  05_plan.png       - tot-tray planner + ready for preschool
...
TABS = ["Dashboard", "Tot Profiles", "Milestones", "First Words & Concepts", "Weekly Themes",
        "Daily Rhythm", "Tot Trays", "Board-Book Log", "Sensory Play", "Outings",
        "Attendance", "Portfolio", "Supplies", "Ready for Preschool", "Goals", "Settings"]

FILE_LABEL = "Tot_School_Command_Center.xlsx — The Bennett Family · 2026–2027 · Theo & Nora"
...
def teddy_crest(c, cx, cy, r=56, glow=True):
    """Brand crest — a little teddy-bear face: the icon of the toddler years."""
    if glow:
    ...
    c.alpha_composite(ov)
    ov = Image.new("RGBA", c.size, (0, 0, 0, 0)); d = ImageDraw.Draw(ov)
    er = r * 0.24            # ear radius
    ex = r * 0.52
    ey = cy - r * 0.42
    for sx in (-1, 1):
        d.ellipse((cx + sx * ex - er, ey - er, cx + sx * ex + er, ey + er), fill=GOLD_HI, outline=(150, 120, 70))
        d.ellipse((cx + sx * ex - er * 0.5, ey - er * 0.5, cx + sx * ex + er * 0.5, ey + er * 0.5), fill=SURFACE)
    hr = r * 0.56            # head radius
    d.ellipse((cx - hr, cy - hr + r * 0.06, cx + hr, cy + hr + r * 0.06), fill=GOLD_HI, outline=(150, 120, 70))
    # eyes
    eyr = r * 0.07
    for sx in (-1, 1):
        d.ellipse((cx + sx * r * 0.2 - eyr, cy - r * 0.12 - eyr, cx + sx * r * 0.2 + eyr, cy - r * 0.12 + eyr), fill=PRIMARY_DK)
    # snout
    sr = r * 0.26
    sy = cy + r * 0.2
    d.ellipse((cx - sr, sy - sr * 0.75, cx + sr, sy + sr * 0.75), fill=SURFACE, outline=(150, 120, 70))
    d.ellipse((cx - r * 0.08, sy - r * 0.12, cx + r * 0.08, sy - r * 0.12 + r * 0.14), fill=PRIMARY_DK)
    c.alpha_composite(ov)


# keep the name used by the detail-image module
book_crest = teddy_crest
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "TOT-SCHOOL", font=fs(16), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("TOTS", "2", "in the system"),
    ("TOT-SCHOOL DAYS", "72", "of 120"),
    ("WORDS SPOKEN", "45", "and counting"),
    ("MILESTONES MET", "26", "of 35"),
    ("THEMES DONE", "5", "of 12"),
    ("TRAYS DONE", "62%", "checked off"),
    ("BOARD BOOKS", "24", "in rotation"),
    ("OUTINGS", "6", "little trips"),
    ("SUPPLIES SPENT", "$259", "of $300"),
    ("PRESCHOOL-READY", "88%", "checklist"),
    ("GOALS", "71%", "avg progress"),
    ("GROWING SCORE", "69%", "blended"),
]
...
def content_dashboard(img, cbox):
    ...
    d.text((x0 + pad, y0 + 22), "Tot-School Dashboard", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 64), "The Bennett Family · Theo (2) & Nora (14 mo)  ·  no pressure, all play", font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 180, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 90, y0 + 44), "● 69% growing", font=fs(16), fill=PRIMARY, anchor="mm")
    ...
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("Milestones", 0.74, "74%"), ("Words", 0.75, "75%"), ("Themes", 0.42, "42%"),
           ("Trays", 0.62, "62%"), ("Ready", 0.88, "88%")], color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    ...
    donut(d, px + pw * 0.42, panels_y + panel_h * 0.52, min(panel_h * 0.28, pw * 0.27),
          [(42, PRIMARY), (8, ACCENT), (50, SURFACE)], "5/12", "done")
    legend(d, px + pw * 0.04, panels_y + panel_h - 104, [(PRIMARY, "Done 5"), (ACCENT, "In progress 1"), (SURFACE, "Upcoming 6")], 15, 30)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Coming Up", font=fs(17), fill=ACCENT, anchor="lt")
    tasks = [("Things That Go", WARN_BG), ("On the Farm", MINT_BG), ("Farm visit outing", MINT_BG),
             ("Weather & Sky", MINT_BG), ("Bugs & Crawlies", MINT_BG), ("Nature-trail stroll", MINT_BG)]
    ty = panels_y + 58
    ...
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Growing Score", font=fs(17), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.5, panels_y + panel_h * 0.54, min(panel_h * 0.30, pw * 0.30),
          [(69, PRIMARY), (31, SURFACE)], "69%", "growing")
...
def content_milestones(img, cbox):
    rows = [
        ...
        ("Social-Emotional", "Plays alongside others", "Theo", "Met"),
        ("Self-Help", "Shows potty interest", "Theo", "Emerging"),
        ("Cognitive", "Finds a hidden object", "Nora", "Met"),
    ]
    _table(img, cbox, "Toddler Milestones",
           "The whole-child picture — 26 of 35 milestones met across 6 domains",
           ["DOMAIN", "MILESTONE", "TOT", "STATUS"],
           [0.0, 0.26, 0.64, 0.83], rows,
           status_col=3, status_map={"Met": (MINT_BG, PRIMARY), "Emerging": (WARN_BG, ACCENT),
                                     "Not Yet": ((235, 230, 222), TEXT_MUTED)})


def content_trays(img, cbox):
    rows = [
        ("Fine Motor", "Pom-pom drop into a bottle", "Done"),
        ("Practical Life", "Water pouring station", "Done"),
        ("Language", "Animal-sound matching", "Done"),
        ("Cognitive", "Color-sorting cups", "Done"),
        ("Sensory", "Rainbow rice sensory bin", "Done"),
        ("Cognitive", "Shape sorter", "Planned"),
    ]
    _table(img, cbox, "Tot Trays & Activities",
           "Simple invitations to play, tagged to a skill — 62% of this term's trays done",
           ["SKILL AREA", "TOT TRAY / ACTIVITY", "STATUS"],
           [0.0, 0.30, 0.83], rows,
           status_col=2, status_map={"Done": (MINT_BG, PRIMARY), "Planned": (WARN_BG, ACCENT),
                                     "Skipped": (RED_BG, DANGER)})


def content_readiness(img, cbox):
    rows = [
        ("Uses words to ask for things", "Ready"),
        ("Feeds self finger foods", "Ready"),
        ("Follows a simple routine", "Ready"),
        ("Shows interest in the potty", "Not yet"),
        ("Plays near other children", "Ready"),
        ("Sits for a short story", "Ready"),
    ]
    _table(img, cbox, "Ready for Preschool",
           "The gentle 'getting there' list, never a test — 14 of 16 already there",
...
CONCEPT_TILES = [
    ("Red", True), ("Blue", True), ("Yellow", True), ("Green", False), ("Circle", True), ("Square", False),
    ("Nose", True), ("Eyes", True), ("Mouth", True), ("Ears", True), ("Hands", True), ("Feet", True),
    ("Tummy", False), ("Hair", True), ("Dog woof", True), ("Cat meow", True), ("Cow moo", True), ("Duck quack", True),
    ("Sheep baa", False), ("Pig oink", False), ("Count to 3", True), ("Count to 5", False), ("Waves hi", True), ("Says more", True),
]


def content_concepts(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "First Words & Concepts", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 62), "Colors, shapes, body parts, animal sounds & first counting.  18 of 24 so far",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 300, y0 + 24, x1 - pad, y0 + 60), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 150, y0 + 42), "● 45 words spoken", font=fs(15), fill=PRIMARY, anchor="mm")
    cols = 6
    gx = x0 + pad; gy = y0 + 116; gw = x1 - x0 - 2 * pad; gap = 14
    tile = (gw - (cols - 1) * gap) / cols
    for i, (lab, known) in enumerate(CONCEPT_TILES):
        r, c = divmod(i, cols)
        ...
        lf = fit_font(d, lab, tile - 20, 24, serif=False)
        d.text((tx + tile / 2, ty + tile / 2 + 4), lab, font=lf,
               fill=(PRIMARY if known else TEXT_MUTED), anchor="mm")
        if known:
            d.ellipse((tx + tile - 38, ty + 10, tx + tile - 10, ty + 38), fill=GREEN)
            d.text((tx + tile - 24, ty + 24), "✓", font=fs(int(tile * 0.16)), fill=WHITE, anchor="mm")
    cy = gy + 4 * (tile + gap) + 10
    d.text((gx, cy), "WORDS & FIRST NUMBERS", font=fs(18), fill=ACCENT, anchor="lt")
    chips = [("45", "words spoken"), ("18/24", "concepts got"), ("to 3", "counts aloud"), ("+8", "new this month")]
    cw = (gw - 3 * 18) / 4
    for i, (big, small) in enumerate(chips):
        ...
        d.text((cx0 + cw / 2, cyy + 46), big, font=fserif(38), fill=PRIMARY, anchor="mm")
...
def render_hero(out):
    ...
    teddy_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE COMPLETE AT-HOME TOT-SCHOOL SYSTEM", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "TOT-SCHOOL COMMAND CENTER", 84, max_w=1860)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 550), "Milestones, first words, gentle play & sweet firsts — one calm system for toddlers 18mo–3yr.",
       fs(20, bold=False), (224, 213, 190))
    chips = [("17", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("18mo", "TO 3 YEARS")]
    cw = 440
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not a printable pack — a complete at-home tot-school planning & keepsake system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Start Here", "how it all works"), ("Tot-School Dashboard", "live Growing Score"),
        ("Tot Profiles", "a profile per tot"), ("Milestones", "the whole-child tracker"),
        ("First Words & Concepts", "colors, sounds & counting"), ("Weekly Themes", "a gentle theme a week"),
        ("Daily Rhythm", "meals, naps & play"), ("Tot Trays", "invitations to play"),
        ("Board-Book Log", "the heart of it"), ("Sensory Play", "messy-play idea bank"),
        ("Outings & Nature", "little adventures"), ("Attendance & Days", "a light record"),
        ("Portfolio of Firsts", "photos & keepsakes"), ("Supplies & Budget", "low-cost, low-stress"),
        ("Ready for Preschool", "the gentle checklist"), ("Goals & Little Wins", "the tiny victories"),
...
def render_milestones(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "THE TODDLER MILESTONES TRACKER", font=fs(26), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "Every Little Leap, One Place", fserif(46), WHITE)
    tc(d, (SIZE // 2, 300), "Gross & fine motor, language, social-emotional, self-help & cognitive — at their own pace",
       fs(21, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 2, content_milestones)
    img.convert("RGB").save(out, "PNG", optimize=True)


def render_words(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "FIRST WORDS & CONCEPTS", font=fs(28), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "Watch the First Words Add Up", fserif(46), WHITE)
    tc(d, (SIZE // 2, 300), "Colors, shapes, body parts & animal sounds — celebrate every new one",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 3, content_concepts)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_plan(out):
    ...
    tc(d, (SIZE // 2, 224), "Tot Trays & Readiness, Handled", fserif(44), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 6, content_trays)
...
def render_printables(out):
    ...
    d = ImageDraw.Draw(img)
    teddy_crest(img, SIZE // 2, 110, r=48)
    ...
    tc(d, (SIZE // 2, 404), "A matching print-ready PDF pack for the wall, the diaper bag & your keepsake box",
       fs(21, bold=False), (226, 214, 190))
    labels = ["Tot-at-a-Glance", "Weekly Theme Plan", "Daily Rhythm", "Milestones Checklist",
              "First Words & Concepts", "Tot-Tray Planner", "Board-Book Log", "Outings & Nature",
              "Sensory Play Bank", "Ready for Preschool", "Tot Goals", "Portfolio of Firsts"]
...
def main():
    ...
    targets = [
        ...
        ("03_milestones.png", render_milestones),
        ("04_words.png", render_words),
