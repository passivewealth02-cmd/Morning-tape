"""Detailed marketing image set for Flip Command Center™ (4 images, 2000x2000).

  07_features.png   - feature spotlights (what it does + why it matters)
  08_compare.png    - "basic flip spreadsheet" vs Flip Command Center™
  09_howitworks.png - run a flip in 4 steps
  10_value.png      - what's included, who it's for & the guarantee

Reuses the branded helpers from build_marketing.py.
Run: python3 build_marketing_detail.py   (run build_pdf.py + build_marketing.py first)
"""
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR REAL FLIPS", "Where Flippers Win",
                "Four ways it protects your profit from offer to sold")
    feats = [
        ("$", "The Deal Analyzer", "donut", (83, "83%", "deal score"),
         "ARV, purchase, rehab, holding & selling roll into all-in cost, profit, ROI and a 70%-rule BUY/PASS verdict — before you ever make the offer."),
        ("%", "A Budget That Catches Overruns", "table", None,
         "Planned vs actual for every category — over-budget lines flag red the day they happen, so a $2K surprise never becomes a $20K one."),
        ("box", "The Whole Job, Coordinated", "stock", None,
         "Scope of work, contractors, draw schedule, materials & timeline — every trade, dollar and date in one place, demo to done."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready PDF pack — deal analyzer, rehab budget, scope, draw schedule, punch list & before/after log for the job-site binder."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("Kitchen", "52%", MINT_BG, PRIMARY), ("Demo & dumpster", "111%", RED_BG, DANGER), ("Paint", "28%", MINT_BG, PRIMARY)]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["CATEGORY", "% USED"], [0.0, 0.62]):
                od.text((tx0 + 14 + (tx1 - tx0) * fx, ty + 17), h, font=fs(15), fill=WHITE, anchor="lm")
            for ri, (a, b, bg, fg) in enumerate(rows):
                ry = ty + 34 + ri * 42
                if ri % 2:
                    od.rectangle((tx0, ry, tx1, ry + 42), fill=ROW_ALT)
                od.text((tx0 + 14, ry + 21), a, font=fs(17), fill=PRIMARY, anchor="lm")
                od.rounded_rectangle((tx1 - 96, ry + 8, tx1 - 12, ry + 34), radius=13, fill=bg)
                od.text((tx1 - 54, ry + 21), b, font=fs(15), fill=fg, anchor="mm")
        elif kind == "stock":
            od = ImageDraw.Draw(img)
            items = [("Projected profit", "$73,198", MINT_BG, PRIMARY), ("Cash-on-cash ROI", "77%", MINT_BG, PRIMARY),
                     ("Budget used", "76%", SURFACE, (110, 88, 58)), ("Scope done", "56%", MINT_BG, PRIMARY)]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Rehab Checklist",
                "Everything a basic flip spreadsheet leaves out — built in")
    rows = [
        ("A rehab task checklist", True, True),
        ("Deal analyzer (70% rule, ROI, profit)", False, True),
        ("Rehab budget: planned vs actual", False, True),
        ("Contractor directory & bids", False, True),
        ("Draw schedule & payment log", False, True),
        ("Holding-cost calculator", False, True),
        ("Financing / capital stack", False, True),
        ("Comps & ARV worksheet", False, True),
        ("Selling & exit net sheet", False, True),
        ("Live profit + Deal Score dashboard", False, True),
    ...
    d.text((c1, top + hh // 2), "Basic\nSpreadsheet", font=fs(18), fill=(210, 210, 205), anchor="mm")
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "FROM OFFER TO SOLD", "Run a Flip in 4 Steps",
                "No spreadsheet skills needed — if you can type a number, you can run it")
    steps = [
        ("1", "Analyze the deal", "Enter ARV, purchase price & rehab budget — the 70% rule, all-in cost, profit & ROI give you an instant BUY / PASS."),
        ("2", "Build the plan", "Load the rehab budget by category, the scope of work room by room, plus your contractors, materials & timeline."),
        ("3", "Run the job", "As work happens, log actual costs & payments and check off tasks — the dashboard tracks budget-used and profit live."),
        ("4", "Sell & bank it", "The exit net sheet shows your real profit at the closing table, and the before/after log builds your flipping brand."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "EVERYTHING YOU GET", "One System, Offer to Sold",
    ...
    includes = [
        "17-tab Google Sheets system", "Excel (.xlsx) edition",
        "12-page printable PDF pack", "Deal analyzer (70% rule + ROI)",
        "Rehab budget: planned vs actual", "Scope, contractors & draw schedule",
        "Holding-cost & financing calcs", "Comps, ARV & exit net sheet",
        "Live profit + Deal Score dashboard", "Start-Here guide + free updates",
    ]
    ...
    who = ["New & experienced house flippers", "BRRRR & buy-hold investors", "Wholesalers analyzing deals",
           "DIY & whole-home renovators", "Investor-friendly agents & pros", "Anyone renovating to sell or refi"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "KNOW YOUR NUMBER · PROTECT YOUR PROFIT", font=fs(26), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Analyze it, fund it, flip it — and hit your number every time.",
       fs(24, bold=False), TEXT_MUTED)
    book_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "FLIP COMMAND CENTER", 54, max_w=1200)
    img.convert("RGB").save(out, "PNG", optimize=True)
