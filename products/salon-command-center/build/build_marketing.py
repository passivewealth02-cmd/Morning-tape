"""Marketing image set for Salon, Barber & Booth Renter Command Center™ (6 images, 2000x2000)."""
...
TABS = ["Dashboard", "Service Pricing", "Chair & Rent", "Services Menu", "Client Book", "Appointments",
        "Retail & Backbar", "Income & Tips", "Expenses", "Rebooking & Retention", "Inventory",
        "Monthly Summary", "Settings"]

FILE_LABEL = "Salon_Command_Center.xlsx — Gilded Chair Studio · Remi"
...
def scissors_crest(c, cx, cy, r=56, glow=True):
    """Brand crest — shears: the icon of the chair."""
    if glow:
    ...
    ov = Image.new("RGBA", c.size, (0, 0, 0, 0)); d = ImageDraw.Draw(ov)
    # two crossing blades
    d.line((cx - r * 0.34, cy + r * 0.30, cx + r * 0.30, cy - r * 0.46), fill=GOLD_HI, width=max(3, int(r * 0.09)))
    d.line((cx + r * 0.34, cy + r * 0.30, cx - r * 0.30, cy - r * 0.46), fill=GOLD_HI, width=max(3, int(r * 0.09)))
    # blade tips, brighter
    d.line((cx + r * 0.06, cy - r * 0.14, cx + r * 0.30, cy - r * 0.46), fill=(238, 248, 245), width=max(2, int(r * 0.06)))
    d.line((cx - r * 0.06, cy - r * 0.14, cx - r * 0.30, cy - r * 0.46), fill=(238, 248, 245), width=max(2, int(r * 0.06)))
    # pivot screw
    d.ellipse((cx - r * 0.09, cy - r * 0.15, cx + r * 0.09, cy + r * 0.03), fill=HIGHLIGHT, outline=(150, 120, 70))
    # ring handles
    rr = r * 0.20
    for sx in (-1, 1):
        hx = cx + sx * r * 0.36; hy = cy + r * 0.42
        d.ellipse((hx - rr, hy - rr, hx + rr, hy + rr), outline=GOLD_HI, width=max(3, int(r * 0.08)))
    c.alpha_composite(ov)


square_crest = scissors_crest
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "THE CHAIR", font=fs(15), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("TICKET PRICE", "$65.00", "what they pay"),
    ("BACKBAR / PRODUCT", "$6.50", "used on them"),
    ("CARD FEE", "$2.19", "2.9% + 30\u00a2"),
    ("RENT / CHAIR-HOUR", "$7.19", "booked or not"),
    ("YOU ACTUALLY KEEP", "$47.32", "per service"),
    ("TRUE MARGIN", "72.8%", "after the chair"),
    ("CLIENTS / MONTH", "92", "served"),
    ("MONTHLY REVENUE", "$6,652", "service + retail"),
    ("MONTHLY PROFIT", "$4,339", "after everything"),
    ("BREAK-EVEN CLIENTS", "21", "just for the chair"),
    ("CHAIR UTILIZATION", "70%", "of open hours"),
    ("CHAIR SCORE", "90%", "healthy"),
]

REV_MONTHS = [("Feb", 5420), ("Mar", 5780), ("Apr", 6090), ("May", 6280), ("Jun", 6470), ("Jul", 6652)]


def content_dashboard(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Chair Dashboard", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 64), "Gilded Chair Studio \u00b7 Remi  \u00b7  price the chair, not just the cut",
           font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 210, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 105, y0 + 44), "\u25cf you keep $47.32", font=fs(14), fill=PRIMARY, anchor="mm")
    ...
    d.text((gx, cy_top), "CHAIR HEALTH \u00b7 WHERE THE $65 GOES \u00b7 REVENUE BY MONTH", font=fs(19), fill=ACCENT, anchor="lt")
    ...
    d.text((px + 16, panels_y + 14), "Chair Health", font=fs(17), fill=ACCENT, anchor="lt")
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("True margin healthy", 1.0, "100%"), ("Clients rebooking", 1.0, "100%"), ("Retail attaching", 1.0, "100%"),
           ("Chair more than covered", 1.0, "100%"), ("No-shows controlled", 1.0, "100%"), ("New clients coming in", 0.40, "40%")],
          color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Where the $65 Goes", font=fs(15), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.44, panels_y + panel_h * 0.46, min(panel_h * 0.25, pw * 0.25),
          [(72.8, PRIMARY), (13.8, ACCENT), (10.0, SURFACE), (3.4, GOLD_LT)], "73%", "you keep")
    legend(d, px + pw * 0.06, panels_y + panel_h - 148,
           [(PRIMARY, "You keep $47.32"), (ACCENT, "Chair rent $8.99"),
            (SURFACE, "Backbar $6.50"), (GOLD_LT, "Card fee $2.19")], 14, 30)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Revenue by Month", font=fs(17), fill=ACCENT, anchor="lt")
    vbars(img, d, (px + 18, panels_y + 60, px + pw - 14, panels_y + panel_h - 10), REV_MONTHS, 6652)
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Chair Score", font=fs(17), fill=ACCENT, anchor="lt")
...
def content_menu(img, cbox):
    rows = [
        ("Men's cut", "$35", "30 min", "$27.59", "$55.18"),
        ("Deep conditioning add-on", "$25", "20 min", "$16.58", "$49.73"),
        ("Balayage", "$210", "3.5 hr", "$146.45", "$41.84"),
        ("Women's cut & style", "$65", "1.25 hr", "$47.33", "$37.86"),
        ("Root touch-up", "$95", "1.75 hr", "$65.36", "$37.35"),
    ]
    _table(img, cbox, "Services Menu \u2014 Costed by the Hour",
           "Net of product, card fees and chair rent \u2014 the $35 cut out-earns the $95 colour.",
           ["SERVICE", "PRICE", "TIME", "YOU KEEP", "PER HOUR"],
           [0.0, 0.42, 0.56, 0.72, 0.89], rows,
           total_row=("10 services", "best / hr", "$55.18", "worst / hr", "$37.35"))


def content_pricing(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "The True-Ticket Engine", font=fs(30), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 60), "Product, card fees and the rent your chair charges by the hour \u2014 all stripped out.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("Service price (what the client pays)", "$65.00", PRIMARY),
            ("\u2212 Backbar / product used", "$6.50", ACCENT),
            ("\u2212 Card processing (2.9% + 30\u00a2)", "$2.19", ACCENT),
            ("\u2212 Rent this service owes (1.25 hrs)", "$8.99", DANGER)]
    ...
    d.text((x0 + pad + 14, ry + rh / 2), "= YOU ACTUALLY KEEP", font=fs(18), fill=PRIMARY, anchor="lm")
    d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.80, ry + rh / 2), "$47.32", font=fs(18), fill=PRIMARY, anchor="mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2 - 20), "72.8% true margin  \u00b7  $37.86 an hour, for real", font=fs(22), fill=WHITE, anchor="mm")
    od.text(((x0 + x1) / 2, by + callout_h / 2 + 24), "THE CHAIR TAKES $8.99 EVEN WHEN IT'S EMPTY", font=fs(23), fill=GOLD_HI, anchor="mm")
    img.alpha_composite(ov)
...
def render_hero(out):
    ...
    scissors_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE COMPLETE SALON & BOOTH RENTER SYSTEM", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "SALON & BARBER", 86, max_w=1550)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 546), "True-ticket pricing, chair rent, retail, rebooking & tips \u2014 one system.",
       fs(20, bold=False), (224, 213, 190))
    chips = [("14", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("TRUE", "TICKET ENGINE")]
    cw = 440
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not a price list \u2014 a complete price-it, book-it, keep-it system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Start Here", "how it all works"), ("Dashboard", "profit & score"),
        ("Service Pricing", "what you really keep"), ("Chair & Rent", "your break-even"),
        ("Services Menu", "costed by the hour"), ("Client Book", "who's worth what"),
        ("Appointments", "kept & no-showed"), ("Retail & Backbar", "attach rate"),
        ("Income & Tips", "your take-home"), ("Expenses", "what the chair costs"),
        ("Rebooking", "cheapest marketing"), ("Inventory", "never run out"),
        ("Monthly Summary", "is it working?"), ("Settings", "set it once"),
        ("+ 12 Printable Pages", "forms & logs"), ("Live Chair Score", "it all rolls up"),
    ]
    cols = 4
    margin = 88
    gx, gy = 22, 20
...
def render_pricing(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "YOUR $65 ISN'T $65", font=fs(29), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "The True-Ticket Engine", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Product, card fees and the rent your chair charges by the hour \u2014 what you actually keep",
...
def render_menu(out):
    ...
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "EVERY SERVICE, BY THE HOUR", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "Your Menu, Costed", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "The $35 cut out-earns the $95 colour per hour \u2014 you'd never know without this",
       fs(22, bold=False), (226, 214, 190))
...
def render_engine(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 110, "PRICE IT \u00b7 KEEP IT", font=fs(32), pad_x=52, pad_y=22)
    tc(d, (SIZE // 2, 224), "The Chair Engine", fserif(44), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 1, content_pricing)
    app_window(img, (60, 1180, SIZE - 60, SIZE - 60), 3, content_menu)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_printables(out):
    ...
    d = ImageDraw.Draw(img)
    scissors_crest(img, SIZE // 2, 110, r=48)
    pill(img, SIZE // 2, 214, "PLUS 12 PRINTABLE PAGES", font=fs(30), pad_x=48, pad_y=20)
    tc(d, (SIZE // 2, 300), "Print, Fill & Keep at the Station", fserif(42), WHITE)
    gold_divider(img, SIZE // 2, 362, width=480)
    tc(d, (SIZE // 2, 404), "A matching print-ready pack \u2014 consultation form, colour formula card, day sheet, client card & more",
       fs(21, bold=False), (226, 214, 190))
    labels = ["Service Pricing", "Chair & Break-Even", "Menu by the Hour", "New Client Consult",
              "Colour Formula Card", "Client Card", "Day Sheet", "Rebooking Tracker",
              "Retail Sales Log", "Inventory & Reorder", "Income, Tips & Tax", "Monthly Review"]
