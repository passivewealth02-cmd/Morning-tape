"""Detailed marketing image set for Salon, Barber & Booth Renter Command Center™ (07-10)."""
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR THE CHAIR", "Price It, Book It, Keep It",
                "Four ways it stops you working a full week for someone else's rent")
    feats = [
        ("$", "Live Chair Score", "donut", (90, "90%", "healthy"),
         "True margin, rebooking, retail attach, break-even cover, no-shows and new clients roll into one number \u2014 the health of your whole chair at a glance."),
        ("%", "The True-Ticket Engine", "table", None,
         "Price minus product, minus card fees, minus the rent your chair charges by the hour. Most stylists have never subtracted that last one, and it is the biggest."),
        ("box", "Run the Whole Chair", "stock", None,
         "Your menu costed by the hour, client book, appointments, retail attach, tips, expenses, rebooking and product inventory \u2014 every part of the business in one place."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack \u2014 consultation form, colour formula card, client card, day sheet, rebooking tracker and a tax set-aside page for the station."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("The ticket says", "$65.00"), ("You actually keep", "$47.32"), ("Your real $/hour", "$37.86")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["THE TRUE TICKET", "VALUE"], [0.0, 0.62]):
            ...
            od = ImageDraw.Draw(img)
            items = [("True margin", "72.8%", MINT_BG, PRIMARY), ("Rebooking rate", "78%", MINT_BG, PRIMARY),
                     ("Retail attach", "11.2%", MINT_BG, PRIMARY), ("New clients / mo", "4", SURFACE, (110, 88, 58))]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Price List",
                "Everything a basic salon tracker leaves out \u2014 built in")
    rows = [
        ("A list of your services & prices", True, True),
        ("Product & card fees subtracted", False, True),
        ("Chair rent charged BY THE HOUR", False, True),
        ("Break-even client count", False, True),
        ("Whole menu costed per hour", False, True),
        ("Retail attach rate tracked", False, True),
        ("Rebooking & no-show rates", False, True),
        ("Tips logged & tax set-aside", False, True),
        ("Product inventory & reorder", False, True),
        ("Live Chair Score dashboard", False, True),
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "SET UP IN AN AFTERNOON", "Price Your Chair in 4 Steps",
                "No spreadsheet skills needed \u2014 if you can type, you can run it")
    steps = [
        ("1", "List what the chair costs", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack, then list your booth rent, supplies, insurance and software in one place."),
        ("2", "Split it by the hour", "Your fixed costs are divided across the hours the chair is open, so every service knows what it owes in rent \u2014 booked or empty, the chair charges the same."),
        ("3", "Price a service properly", "Enter a price, the product it uses and how long it takes. Product, card fees and chair rent come out, and you see what you actually keep and your real rate per hour."),
        ("4", "Run the whole book", "Cost your entire menu, track rebooking, retail attach, no-shows, tips and inventory, and let the Chair Score tell you whether you are priced right or just busy."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "EVERYTHING YOU GET", "One System, Whole Chair",
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + Chair Score",
        "True-ticket pricing engine", "Chair rent & break-even math",
        "Whole menu costed by the hour", "Client book & appointments",
        "Retail attach, tips & inventory", "Start-Here quick-start guide",
    ]
    ...
    who = ["Booth & suite renters", "Hairstylists and colourists", "Barbers & barbershop owners",
           "Lash, brow, nail & esthetics techs", "Salon owners with a team", "Anyone cutting from a home studio"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "PRICE IT \u00b7 BOOK IT \u00b7 KEEP IT", font=fs(27), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Price the chair, not just the cut \u2014 and keep what you earn.",
       fs(23, bold=False), TEXT_MUTED)
    square_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "SALON & BARBER", 58, max_w=1150)
    img.convert("RGB").save(out, "PNG", optimize=True)
