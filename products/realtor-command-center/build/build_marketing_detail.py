"""Detailed marketing image set for Real Estate Agent Command Center™ (07-10)."""
...
from build_marketing import (
    ...
    ROW_ALT, fs, fserif, premium_bg, pill, wordmark, gold_divider, sign_crest,
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR AGENTS", "Net It, Track It, Hit the Goal",
                "Four ways it turns a real-estate business into a number you can hit")
    feats = [
        ("$", "Live Agent Score", "donut", (90, "90%", "healthy"),
         "Net margin, GCI per deal, pace, pipeline, profit and database roll into one number — so you know your business health at a glance."),
        ("%", "Commission Engine", "table", None,
         "Sale price to GCI to your split to net — the engine shows what a deal really pays after the brokerage split and every cost."),
        ("box", "Run the Business", "stock", None,
         "A pipeline, closings, buyers & sellers, listings, lead-source ROI, a database plan and a GCI goal — the whole business in one place."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack — commission worksheet, pipeline, closing checklist, database plan & more for the file."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("GCI per deal", "$13,500"), ("Agent commission", "$9,450"), ("Net per deal", "$8,000")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["$450K DEAL · 3.0%", "VALUE"], [0.0, 0.62]):
            ...
            od = ImageDraw.Draw(img)
            items = [("GCI YTD", "$75,600", MINT_BG, PRIMARY), ("Net income", "$50,000", MINT_BG, PRIMARY),
                     ("Pipeline GCI", "$30,000", MINT_BG, PRIMARY), ("Goal progress", "76%", SURFACE, (110, 88, 58))]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Calculator",
                "Everything a basic commission sheet leaves out — built in")
    rows = [
        ("A commission calculator", True, True),
        ("Net per deal after split & costs", False, True),
        ("A full deal pipeline", False, True),
        ("Closings → GCI & volume", False, True),
        ("Lead-source ROI", False, True),
        ("Database / sphere plan", False, True),
        ("GCI goal & pace tracker", False, True),
        ("Business expenses for taxes", False, True),
        ("Mileage log (deductible)", False, True),
        ("Live Agent Score dashboard", False, True),
...
def render_howitworks(out):
    ...
    steps = [
        ("1", "Net every deal", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack, then run any deal through the commission calculator."),
        ("2", "Fill the pipeline", "Add your active deals by stage with the estimated commission, and log every closing as it happens."),
        ("3", "Work the database", "Track buyers, sellers and listings, log lead-source ROI, and touch your sphere on a plan — the database is the business."),
        ("4", "Hit the goal", "Watch your GCI, net income and pace against your annual goal, keep your expenses and mileage, and grow on real numbers."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + Agent Score",
        "Commission calculator (net/deal)", "Deal pipeline & closings log",
        "Lead-source ROI & database plan", "GCI goal & pace tracker",
        "Business expenses & mileage log", "Start-Here quick-start guide",
    ]
    ...
    who = ["New & experienced agents", "Realtors & brokers", "Buyer's & listing agents",
           "Small teams & solo agents", "Referral & relocation agents", "Anyone tracking GCI & a goal"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "NET IT · TRACK IT · HIT THE GOAL", font=fs(27), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Net every deal, fill the pipeline, and hit your GCI goal.",
       fs(23, bold=False), TEXT_MUTED)
    sign_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "REAL ESTATE AGENT", 48, max_w=1150)
    img.convert("RGB").save(out, "PNG", optimize=True)
