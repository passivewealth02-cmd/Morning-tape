"""Marketing image set for Real Estate Agent Command Center™ (6 images, 2000x2000)."""
...
TABS = ["Dashboard", "Commission Calculator", "Pipeline", "Closings", "Buyers & Sellers", "Listings",
        "Lead Sources", "Database", "Goals & GCI", "Business Expenses",
        "Mileage & Auto", "Monthly Summary", "Settings"]

FILE_LABEL = "Realtor_Command_Center.xlsx — Taylor Brooks · Summit Realty"
...
def sign_crest(c, cx, cy, r=56, glow=True):
    """Brand crest — a real-estate yard sign with a SOLD rider."""
    if glow:
    ...
    ov = Image.new("RGBA", c.size, (0, 0, 0, 0)); d = ImageDraw.Draw(ov)
    # post
    d.rectangle((cx + r * 0.30, cy - r * 0.30, cx + r * 0.37, cy + r * 0.44), fill=WHITE, outline=(150, 120, 70))
    d.rectangle((cx + r * 0.22, cy - r * 0.34, cx + r * 0.45, cy - r * 0.26), fill=WHITE, outline=(150, 120, 70))
    # SOLD rider on top
    d.rounded_rectangle((cx - r * 0.36, cy - r * 0.40, cx + r * 0.24, cy - r * 0.22), radius=4, fill=DANGER, outline=(150, 120, 70))
    d.text((cx - r * 0.06, cy - r * 0.31), "SOLD", font=fs(int(r * 0.20)), fill=WHITE, anchor="mm")
    # hanging board
    d.rounded_rectangle((cx - r * 0.40, cy - r * 0.16, cx + r * 0.28, cy + r * 0.26), radius=6, fill=GOLD_HI, outline=(150, 120, 70))
    for k in range(3):
        d.line((cx - r * 0.30, cy - r * 0.05 + k * r * 0.11, cx + r * 0.16, cy - r * 0.05 + k * r * 0.11), fill=PRIMARY, width=3)
    c.alpha_composite(ov)


# aliases (shared render helpers may reference these names across products)
spray_crest = sign_crest
briefcase_crest = sign_crest
house_crest = sign_crest
tent_crest = sign_crest
toque_crest = sign_crest
bento_crest = sign_crest
cupcake_crest = sign_crest
clock_crest = sign_crest
clipboard_crest = sign_crest
cloche_crest = sign_crest
ghost_crest = sign_crest
mug_crest = sign_crest
book_crest = sign_crest
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "REAL ESTATE", font=fs(15), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("AVG SALE PRICE", "$450K", "per closing"),
    ("COMMISSION", "3.0%", "your side"),
    ("GCI / DEAL", "$13,500", "gross"),
    ("NET / DEAL", "$8,000", "in pocket"),
    ("CLOSINGS YTD", "8", "this year"),
    ("VOLUME YTD", "$3.6M", "sold"),
    ("GCI YTD", "$75,600", "earned"),
    ("NET INCOME", "$50,000", "after costs"),
    ("PIPELINE", "8", "deals"),
    ("PIPELINE GCI", "$30,000", "coming"),
    ("GOAL PROGRESS", "76%", "of GCI goal"),
    ("AGENT SCORE", "90%", "healthy"),
]

GCI_MONTHS = [("Jan", 8400), ("Feb", 10500), ("Mar", 14700), ("Apr", 11550), ("May", 16800), ("Jun", 13650)]


def content_dashboard(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Real Estate Dashboard", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 64), "Taylor Brooks · Summit Realty  ·  know your GCI, your net, and your real business", font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 210, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 105, y0 + 44), "● $75,600 GCI YTD", font=fs(14), fill=PRIMARY, anchor="mm")
    ...
    d.text((gx, cy_top), "AGENT HEALTH · WHERE THE COMMISSION GOES · GCI BY MONTH", font=fs(19), fill=ACCENT, anchor="lt")
    ...
    d.text((px + 16, panels_y + 14), "Agent Health", font=fs(17), fill=ACCENT, anchor="lt")
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("Net margin healthy", 1.0, "100%"), ("GCI/deal on target", 1.0, "100%"), ("Closings on pace", 1.0, "100%"),
           ("Pipeline full", 1.0, "100%"), ("Profitable", 1.0, "100%"), ("Database nurtured", 0.40, "40%")],
          color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Where the Commission Goes", font=fs(14), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.44, panels_y + panel_h * 0.50, min(panel_h * 0.27, pw * 0.27),
          [(59, PRIMARY), (30, GOLD_LT), (11, ACCENT)], "59%", "your net")
    legend(d, px + pw * 0.05, panels_y + panel_h - 108, [(PRIMARY, "Your net 59%"), (GOLD_LT, "Brokerage 30%"), (ACCENT, "Costs 11%")], 14, 30)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "GCI by Month", font=fs(17), fill=ACCENT, anchor="lt")
    vbars(img, d, (px + 18, panels_y + 60, px + pw - 14, panels_y + panel_h - 10), GCI_MONTHS, 16800)
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Agent Score", font=fs(17), fill=ACCENT, anchor="lt")
    ...
    for i, row in enumerate(rows):
        ...
        for ci, val in enumerate(row):
            ...
            if status_map is not None and ci == status_col:
                ...
                d.rounded_rectangle((hx - 88, ry + rh / 2 - 15, hx + 88, ry + rh / 2 + 15), radius=14, fill=bg)
                d.text((hx, ry + rh / 2), sval, font=fs(13), fill=fg, anchor="mm")
            else:
...
def content_pipeline(img, cbox):
    rows = [
        ("Ford — buyer", "Under contract", "$470,000", "$9,870"),
        ("Nguyen — listing", "Offer", "$520,000", "$10,920"),
        ("Whit — listing", "Lead", "$610,000", "$12,810"),
        ("Ivy Duplex", "Showing", "$540,000", "$11,340"),
        ("Osei — buyer", "Offer", "$415,000", "$8,715"),
        ("Vance — listing", "Lead", "$495,000", "$10,395"),
    ]
    smap = {"Under contract": (MINT_BG, PRIMARY), "Offer": (WARN_BG, (150, 110, 60)),
            "Showing": (SURFACE, (110, 88, 58)), "Lead": (RED_BG, DANGER)}
    _table(img, cbox, "Deal Pipeline",
           "Stage, price and estimated agent commission — see the goal from here.",
           ["DEAL", "STAGE", "PRICE", "EST. GCI"],
           [0.0, 0.36, 0.64, 0.86], rows, total_row=("PIPELINE  ·  8 deals", "", "", "$30,000"),
           status_col=1, status_map=smap)


def content_closings(img, cbox):
    rows = [
        ("102 Elm St", "$400,000", "Listing", "$8,400"),
        ("55 Oak Ave", "$450,000", "Buyer", "$9,450"),
        ("8 Birch Ln", "$500,000", "Listing", "$10,500"),
        ("77 Maple Dr", "$550,000", "Listing", "$11,550"),
        ("301 Ash Blvd", "$475,000", "Both", "$9,975"),
        ("9 Willow Way", "$450,000", "Listing", "$9,450"),
    ]
    _table(img, cbox, "Closings — YTD",
           "Every closed deal — sale price and agent commission roll up to your GCI and volume.",
           ["ADDRESS", "PRICE", "SIDE", "AGENT GCI"],
           [0.0, 0.38, 0.62, 0.84], rows, total_row=("YTD  ·  8 closings  ·  $3.6M volume", "", "", "$75,600"))


def content_commission(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Commission — What You Net", font=fs(30), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 60), "Sale price → GCI → your split → net per deal — know a deal before you take it.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("Gross commission · $450k × 3.0%", "$13,500", PRIMARY), ("Agent commission · 70% split", "$9,450", PRIMARY),
            ("− Transaction costs", "−$1,450", DANGER)]
    ...
    for i, (label, val, col) in enumerate(rows):
        ...
        d.text((x0 + pad + 14, ry + rh / 2), label, font=fs(17, bold=True), fill=PRIMARY, anchor="lm")
    ...
    d.text((x0 + pad + 14, ry + rh / 2), "= NET PER DEAL", font=fs(18), fill=PRIMARY, anchor="lm")
    d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.80, ry + rh / 2), "$8,000", font=fs(18), fill=PRIMARY, anchor="mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2), "$75,600 GCI YTD  ·  $50,000 net  ·  76% to goal", font=fs(22), fill=WHITE, anchor="mm")
    img.alpha_composite(ov)
...
def render_hero(out):
    ...
    sign_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE COMPLETE REALTOR SYSTEM", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "REAL ESTATE AGENT", 66, max_w=1650)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 546), "A commission calculator, pipeline, closings, GCI goal, lead ROI & database — one system.",
       fs(20, bold=False), (224, 213, 190))
    chips = [("14", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("GCI", "CALCULATOR")]
    cw = 440
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not a commission calculator — a complete net-it, track-it, hit-the-goal system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Start Here", "how it all works"), ("Dashboard", "GCI & score"),
        ("Commission Calc", "net any deal"), ("Pipeline", "what's coming"),
        ("Closings", "GCI & volume"), ("Buyers & Sellers", "your clients"),
        ("Listings", "days on market"), ("Lead Sources", "ROI by source"),
        ("Database", "touch the sphere"), ("Goals & GCI", "hit the number"),
        ("Business Expenses", "for tax time"), ("Mileage & Auto", "deductible drives"),
        ("Monthly Summary", "watch the year build"), ("Settings", "set it once"),
        ("+ 12 Printable Pages", "worksheets & checklists"), ("Live Agent Score", "it all rolls up"),
    ]
    cols = 4
    margin = 88
    gx, gy = 22, 20
...
def render_commission(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "NET ANY DEAL", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "The Commission Calculator", fserif(42), WHITE)
    tc(d, (SIZE // 2, 300), "Sale price to GCI to your split to net per deal — know what a deal really pays before you take it",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 1, content_commission)
    img.convert("RGB").save(out, "PNG", optimize=True)


def render_pipeline(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "SEE THE GOAL FROM HERE", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "Your Whole Pipeline", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Every deal by stage, with the estimated commission — so you always know if you'll hit the number",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 2, content_pipeline)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_engine(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 110, "NET IT · TRACK IT", font=fs(32), pad_x=52, pad_y=22)
    tc(d, (SIZE // 2, 224), "The Real Estate Engine", fserif(44), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 1, content_commission)
    app_window(img, (60, 1180, SIZE - 60, SIZE - 60), 3, content_closings)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_printables(out):
    ...
    d = ImageDraw.Draw(img)
    sign_crest(img, SIZE // 2, 110, r=48)
    pill(img, SIZE // 2, 214, "PLUS 12 PRINTABLE PAGES", font=fs(30), pad_x=48, pad_y=20)
    tc(d, (SIZE // 2, 300), "Print, Fill & Keep — For the File", fserif(44), WHITE)
    gold_divider(img, SIZE // 2, 362, width=480)
    tc(d, (SIZE // 2, 404), "A matching print-ready pack — commission worksheet, pipeline, closing checklist & more",
       fs(21, bold=False), (226, 214, 190))
    labels = ["Commission Worksheet", "Deal Pipeline", "Closings Log", "Buyers & Sellers",
              "Listings", "Lead Source ROI", "Database Plan", "GCI Goal Tracker",
              "Business Expenses", "Mileage Log", "Monthly Summary", "Closing Checklist"]
    ...
    for i in range(12):
        ...
        d.text((x + cw // 2, y + ch + 30), labels[i], font=fs(16), fill=PRIMARY, anchor="mm")
...
def main():
    ...
    targets = [
        ...
        ("03_commission.png", render_commission),
        ("04_pipeline.png", render_pipeline),
