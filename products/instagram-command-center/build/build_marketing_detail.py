"""Detailed marketing image set for Instagram Command Center™ (4 images, 2000x2000).
...
from build_marketing import (
    ...
    ROW_ALT, fs, fserif, premium_bg, pill, wordmark, gold_divider, ig_crest,
...
def render_features(out):
    ...
    header_band(img, "BUILT TO GROW & MONETIZE", "Where Creators Win",
                "Four engines that run the business behind your grid")
    feats = [
        ("$", "Revenue Engine", "donut", (77, "77%", "margin"),
         "7 income streams — brand deals, IG Shop, affiliate, digital products & coaching — roll into one live net-profit number."),
        ("%", "Captions & Reels", "table", None,
         "Bank captions that earn saves & shares and track reel retention — the two signals that decide your reach."),
        ("box", "Shop & Affiliate", "stock", None,
         "Turn saves into sales — units, GMV and commission per product, plus every affiliate link's clicks and payout."),
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("'Save this for later 🌱'", "9.2"), ("'You're overwatering'", "9.0"), ("'Comment PLANT'", "8.8")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["CAPTION / HOOK", "SCORE"], [0.0, 0.82]):
            ...
            od = ImageDraw.Draw(img)
            items = [("Planter set ×40", "$1.9K GMV", MINT_BG, PRIMARY), ("Grow light ×18", "$1.2K GMV", MINT_BG, PRIMARY),
                     ("LTK Storefront", "9.8K clicks", SURFACE, (110, 88, 58)), ("Macrame ×55", "$1.5K GMV", MINT_BG, PRIMARY)]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Planner",
                "Everything a basic content calendar leaves out — built in")
    rows = [
        ("Plan & schedule your posts", True, True),
        ("Caption & hook bank", False, True),
        ("Grid planner + feed preview", False, True),
        ("Per-video analytics + health score", False, True),
        ("IG Shop GMV & commission", False, True),
...
def render_howitworks(out):
    ...
    steps = [
        ...
        ("3", "Plan, bank & track", "Plan your grid & calendar, bank captions & ideas, and map your hashtags. Everything cross-links."),
...
def render_value(out):
    ...
    includes = [
        ...
        "12-KPI creator dashboard", "Caption bank + grid planner",
        "IG Shop + affiliate trackers", "Brand-deal CRM (lead → paid)",
        "7-stream finance + net profit", "Repurposing & stories planners",
    ...
    who = ["Instagram creators & influencers", "UGC creators", "IG Shop sellers",
           "Niche & aesthetic pages", "Coaches & educators", "Agencies managing creators"]
    ...
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "One point of control for the grid AND the business behind it.",
       fs(24, bold=False), TEXT_MUTED)
    ig_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "INSTAGRAM COMMAND CENTER", 56, max_w=1560)
    img.convert("RGB").save(out, "PNG", optimize=True)
