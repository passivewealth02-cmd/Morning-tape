"""Detailed marketing image set for IEP Command Center™ (4 images, 2000x2000).
...
def header_band(img, tagpill, title, subtitle, band_h=430):
    ...
    tc(d, (SIZE // 2, 300), title, fserif(52), WHITE)
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR PARENTS WHO ADVOCATE", "Where Parents Feel Prepared",
                "Four ways it turns a stack of papers into one clear picture")
    feats = [
        ("$", "Live Progress Score", "donut", (80, "80%", "on track"),
         "Goal progress, on-pace goals, accommodations in place, services delivered & data collection roll into one number — so you always know where things stand."),
        ("%", "Goals That Trend Themselves", "table", None,
         "Enter baseline, target & current for each IEP goal — progress calculates automatically, and you walk into the meeting with the data in hand."),
        ("box", "Services, Accommodations & Data", "stock", None,
         "Track minutes owed vs delivered, check that every accommodation is in place, and log the data points that show whether the plan is working."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready advocacy binder — profile, goals, monitoring, services, accommodations, meeting prep, communication log & wins."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        od.text((x + 124, by - 16), title, font=fs(24), fill=PRIMARY, anchor="lm")
        ...
        if kind == "donut":
            ...
            for ri, (a, b) in enumerate(rows):
                ...
                od.text((tx0 + 14 + (tx1 - tx0) * 0.60, ry + 21), b, font=fs(16, bold=False), fill=TEXT, anchor="lm")
        elif kind == "stock":
            od = ImageDraw.Draw(img)
            items = [("Avg goal progress", "64%", MINT_BG, PRIMARY), ("Goals on pace", "4/5", MINT_BG, PRIMARY),
                     ("Services delivered", "94%", SURFACE, (110, 88, 58)), ("Accommodations", "9/10", MINT_BG, PRIMARY)]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Binder of Papers",
                "Everything a folder of IEP paperwork can't do — built in")
    rows = [
        ("A place to store IEP documents", True, True),
        ("Goals that calculate progress", False, True),
        ("Data & progress monitoring", False, True),
        ("Services: minutes owed vs delivered", False, True),
        ("Accommodations 'is it in place?' check", False, True),
        ("Therapy & behavior logs", False, True),
        ("Meeting notes & prep questions", False, True),
        ("Dated communication log", False, True),
        ("Strengths-based profile", False, True),
        ("Live progress dashboard", False, True),
    ...
    d.text((c1, top + hh // 2), "A Paper\nFolder", font=fs(18), fill=(210, 210, 205), anchor="mm")
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "GET ORGANIZED IN AN EVENING", "Ready to Advocate in 4 Steps",
                "No spreadsheet skills needed — if you can type, you can run it")
    steps = [
        ("1", "Download & open", "Buy once and instantly download the Google Sheets system plus the 12-page printable advocacy binder."),
        ("2", "Add your child & plan", "In Settings, add your child, plan type, case manager & next review date — then the whole-child profile."),
        ("3", "Enter goals & supports", "List each IEP goal with baseline / target / current, the services & minutes, and the accommodations."),
        ("4", "Log, track & walk in ready", "Add data points, therapy notes & communications as they happen — the dashboard shows progress at a glance."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "EVERYTHING YOU GET", "One Calm System, Meeting-Ready",
    ...
    includes = [
        ...
        "12-page printable advocacy binder", "Live progress dashboard + score",
        "Goals that calculate progress", "Services, minutes & accommodations",
        "Progress-monitoring data log", "Therapy, behavior & meeting logs",
        "Dated communication log & records", "Start-Here guide + free updates",
    ]
    ...
    who = ["Parents of kids with an IEP or 504", "Newly-diagnosed & first-IEP families", "Foster & kinship caregivers",
           "Advocates & parent mentors", "Anyone prepping for an IEP meeting", "Families managing therapies & services"]
    for i, w in enumerate(who):
        yy = 604 + i * 44
        od.text((rx0 + 44, yy), "•", font=fs(24), fill=GOLD_LT, anchor="lm")
        od.text((rx0 + 72, yy), w, font=fs(19, bold=False), fill=TEXT, anchor="lm")
    img.alpha_composite(ov)
    card(img, (rx0, 916, rx1, 1300))
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text((rx0 + 40, 956), "GOOD TO KNOW", font=fs(26), fill=ACCENT, anchor="lt")
    for i, t in enumerate(["✓ Google Sheets, Excel & print at home", "✓ Keep your file private & secure",
                            "✓ An organizing tool — not legal advice"]):
        od.text((rx0 + 44, 1020 + i * 44), t, font=fs(20, bold=False), fill=TEXT, anchor="lm")
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "WALK IN PREPARED · ADVOCATE WITH CONFIDENCE", font=fs(25), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Your child is so much more than a plan — and you're their best advocate.",
       fs(23, bold=False), TEXT_MUTED)
    book_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "IEP COMMAND CENTER", 54, max_w=1200)
    img.convert("RGB").save(out, "PNG", optimize=True)
