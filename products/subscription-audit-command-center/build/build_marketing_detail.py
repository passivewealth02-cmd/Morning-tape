"""Detailed marketing image set for Subscription & Bills Audit Command Center™ (07-10)."""
...
from build_marketing import (
    ...
    ROW_ALT, fs, fserif, premium_bg, pill, wordmark, gold_divider, magnify_crest,
...
def render_features(out):
    ...
    header_band(img, "BUILT TO PLUG LEAKS", "Find It, Cut It, Keep It Cut",
                "Four ways it turns forgotten charges into money back")
    feats = [
        ("$", "Live Audit Score", "donut", (90, "90%", "healthy"),
         "Everything reviewed, cutting waste, all categorized, under budget, trials tracked and billed-annually roll into one number — your audit's health at a glance."),
        ("%", "Cancel-Savings Finder", "table", None,
         "Flag the subscriptions to cut and the engine totals your monthly and annual savings instantly — so you see the money before you cancel."),
        ("box", "Track the Whole Leak", "stock", None,
         "Subscriptions, bills, renewals, free trials, price hikes and a negotiation list — every recurring charge in one place."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack — subscription audit, cancel list, renewal calendar, negotiation & more, for a paper sweep."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("Monthly subs", "$216.77"), ("− Cancel 5", "$105.92"), ("Annual save", "$1,271")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["CANCEL FINDER", "VALUE"], [0.0, 0.62]):
            ...
            od = ImageDraw.Draw(img)
            items = [("Monthly savings", "$106", MINT_BG, PRIMARY), ("Trials ending", "3", MINT_BG, PRIMARY),
                     ("Hikes add / yr", "$132", MINT_BG, PRIMARY), ("Billed annually", "40%", SURFACE, (110, 88, 58))]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Subscription List",
                "Everything a basic tracker leaves out — built in")
    rows = [
        ("A list of subscriptions", True, True),
        ("Monthly + annualized totals", False, True),
        ("A cancel-savings finder", False, True),
        ("Bills alongside subscriptions", False, True),
        ("Spend by category", False, True),
        ("A renewal calendar", False, True),
        ("Free-trial cancel reminders", False, True),
        ("Price-hike tracking", False, True),
        ("A bill-negotiation list", False, True),
        ("Live Audit Score dashboard", False, True),
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "DONE IN AN AFTERNOON", "Audit Your Spend in 4 Steps",
                "No spreadsheet skills needed — if you can type, you can run it")
    steps = [
        ("1", "List every charge", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack, then list every subscription and bill you pay."),
        ("2", "Flag what to cut", "Mark each one Keep or Cancel; the cancel-savings finder instantly totals your monthly and annual savings from cutting them."),
        ("3", "Cancel & catch", "Work the cancel list from the top, log price hikes, and set the free-trial reminders so nothing charges you by surprise."),
        ("4", "Keep it cut", "Negotiate the keepers down, watch your recurring spend drop month over month, and let the Audit Score keep you honest."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "EVERYTHING YOU GET", "One System, Every Charge",
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + Audit Score",
        "Cancel-savings finder engine", "Subscriptions & bills lists",
        "Renewals & free-trial tracking", "Price-hike & category views",
        "Bill-negotiation & savings log", "Start-Here quick-start guide",
    ]
    ...
    who = ["Anyone with too many subscriptions", "Budgeters cutting waste", "Couples sharing bills",
           "Frugal & FIRE-minded savers", "Families trimming expenses", "Anyone doing a spending reset"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "FIND IT · CUT IT · KEEP IT CUT", font=fs(27), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "See every recurring charge, and cut the ones you forgot.",
       fs(23, bold=False), TEXT_MUTED)
    magnify_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "SUBSCRIPTION AUDIT", 50, max_w=1200)
    img.convert("RGB").save(out, "PNG", optimize=True)
