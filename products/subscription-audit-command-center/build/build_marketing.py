"""Marketing image set for Subscription & Bills Audit Command Center™ (6 images, 2000x2000)."""
...
TABS = ["Dashboard", "Subscription Audit", "Subscriptions", "Bills", "Cancel Finder", "Categories", "Renewals",
        "Free Trials", "Price Hikes", "Savings", "Negotiation", "Monthly Summary", "Settings"]

FILE_LABEL = "Subscription_Audit_Command_Center.xlsx — Clearing House · Devon"
...
def magnify_crest(c, cx, cy, r=56, glow=True):
    """Brand crest — a magnifying glass over a dollar: the icon of the spending audit."""
    if glow:
    ...
    ov = Image.new("RGBA", c.size, (0, 0, 0, 0)); d = ImageDraw.Draw(ov)
    # lens ring
    lx, ly, lr = cx - r * 0.10, cy - r * 0.12, r * 0.30
    d.ellipse((lx - lr, ly - lr, lx + lr, ly + lr), outline=GOLD_HI, width=5)
    d.ellipse((lx - lr + 6, ly - lr + 6, lx + lr - 6, ly + lr - 6), fill=(230, 245, 240))
    # dollar sign in lens
    d.text((lx, ly), "$", font=fserif(int(r * 0.42)), fill=PRIMARY, anchor="mm")
    # handle
    d.line((lx + lr * 0.72, ly + lr * 0.72, cx + r * 0.42, cy + r * 0.44), fill=GOLD_HI, width=9)
    c.alpha_composite(ov)


# aliases (shared render helpers may reference these names across products)
hardhat_crest = magnify_crest
basket_crest = magnify_crest
compass_crest = magnify_crest
candle_crest = magnify_crest
camera_crest = magnify_crest
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "AUDIT", font=fs(15), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("MONTHLY SUBS", "$216.77", "recurring"),
    ("ANNUAL SUBS", "$2,601", "per year"),
    ("SUBSCRIPTIONS", "14", "tracked"),
    ("AVG / SUB", "$15.48", "each"),
    ("FLAGGED", "5", "to cancel"),
    ("MONTHLY SAVINGS", "$105.92", "if you cut"),
    ("ANNUAL SAVINGS", "$1,271", "found"),
    ("MONTHLY BILLS", "$468", "utilities"),
    ("KEEP MONTHLY", "$110.85", "after cuts"),
    ("HIKES ADD / YR", "$132", "stealth"),
    ("TRIALS ENDING", "3", "cancel soon"),
    ("AUDIT SCORE", "90%", "healthy"),
]

REC_MONTHS = [("Feb", 322), ("Mar", 315), ("Apr", 300), ("May", 280), ("Jun", 250), ("Jul", 217)]


def content_dashboard(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Audit Dashboard", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 64), "Clearing House · Devon  ·  see every recurring charge, and cut the ones you forgot", font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 210, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 105, y0 + 44), "● $1,271 to save", font=fs(14), fill=PRIMARY, anchor="mm")
    ...
    d.text((gx, cy_top), "AUDIT HEALTH · KEEP VS CANCEL · RECURRING BY MONTH", font=fs(19), fill=ACCENT, anchor="lt")
    ...
    d.text((px + 16, panels_y + 14), "Audit Health", font=fs(17), fill=ACCENT, anchor="lt")
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("Everything reviewed", 1.0, "100%"), ("Cutting waste", 1.0, "100%"), ("All categorized", 1.0, "100%"),
           ("Under budget", 1.0, "100%"), ("Trials tracked", 1.0, "100%"), ("Billed annually", 0.40, "40%")],
          color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Keep vs Cancel", font=fs(15), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.44, panels_y + panel_h * 0.50, min(panel_h * 0.27, pw * 0.27),
          [(51, PRIMARY), (49, DANGER)], "$106", "to cut")
    legend(d, px + pw * 0.06, panels_y + panel_h - 96, [(PRIMARY, "Keep $111"), (DANGER, "Cancel $106")], 15, 32)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Recurring by Month", font=fs(17), fill=ACCENT, anchor="lt")
    vbars(img, d, (px + 18, panels_y + 60, px + pw - 14, panels_y + panel_h - 10), REC_MONTHS, 322)
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Audit Score", font=fs(17), fill=ACCENT, anchor="lt")
...
def _table(img, cbox, title, subtitle, headers, colf, rows, total_row=None):
    ...
    for i, row in enumerate(rows):
        ...
        for ci, val in enumerate(row):
            ...
            col = PRIMARY if ci == 0 else (DANGER if str(val) == "Cancel" else TEXT)
...
def content_subs(img, cbox):
    rows = [
        ("Netflix", "Streaming", "$15.49", "Keep"),
        ("Gym membership", "Fitness", "$40.00", "Cancel"),
        ("Adobe CC", "Software", "$22.99", "Cancel"),
        ("Hulu", "Streaming", "$17.99", "Cancel"),
        ("Spotify", "Music", "$11.99", "Keep"),
        ("Audible", "Books", "$14.95", "Cancel"),
        ("NYT", "News", "$17.00", "Keep"),
    ]
    _table(img, cbox, "Subscriptions",
           "Every recurring subscription — category, monthly cost and keep-or-cancel. The whole leak, listed.",
           ["SUBSCRIPTION", "CATEGORY", "MONTHLY", "ACTION"],
           [0.0, 0.44, 0.66, 0.86], rows,
           total_row=("14 subscriptions", "", "$216.77", "5 to cut"))


def content_audit(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "The Cancel-Savings Finder", font=fs(30), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 60), "Total your recurring spend, flag what to cut, and see exactly what you'd save in a year.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("Monthly subscriptions", "$216.77", PRIMARY),
            ("− Flagged to cancel (5)", "−$105.92", DANGER)]
    ...
    d.text((x0 + pad + 14, ry + rh / 2), "= YOU KEEP (monthly)", font=fs(18), fill=PRIMARY, anchor="lm")
    d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.80, ry + rh / 2), "$110.85", font=fs(18), fill=PRIMARY, anchor="mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2), "cutting 5 subs  =  $1,271 back in your pocket a year", font=fs(22), fill=WHITE, anchor="mm")
    img.alpha_composite(ov)
...
def render_hero(out):
    ...
    magnify_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE COMPLETE SUBSCRIPTION & BILLS AUDIT", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "SUBSCRIPTION AUDIT", 74, max_w=1650)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 546), "An audit engine, a cancel-savings finder, renewals, price hikes & free trials — one system.",
       fs(20, bold=False), (224, 213, 190))
    chips = [("14", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("CANCEL", "FINDER")]
    cw = 440
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not a list — a complete find-it, cut-it, keep-it-cut system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Start Here", "how it all works"), ("Dashboard", "spend & score"),
        ("Subscription Audit", "the savings finder"), ("Subscriptions", "every recurring charge"),
        ("Bills", "utilities & insurance"), ("Cancel Finder", "your hit list"),
        ("Categories", "the biggest bucket"), ("Renewals", "no annual surprise"),
        ("Free Trials", "cancel before charged"), ("Price Hikes", "catch the creep"),
        ("Savings", "proof it pays off"), ("Negotiation", "lower the keepers"),
        ("Monthly Summary", "the line goes down"), ("Settings", "set it once"),
        ("+ 12 Printable Pages", "audit & cancel lists"), ("Live Audit Score", "it all rolls up"),
    ]
    cols = 4
    margin = 88
    gx, gy = 22, 20
...
def render_audit(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "FIND THE SAVINGS", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "The Cancel-Savings Finder", fserif(42), WHITE)
    tc(d, (SIZE // 2, 300), "Total every recurring charge, flag what to cut, and see exactly what you'd get back in a year",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 1, content_audit)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_subs(out):
    ...
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "SEE EVERY CHARGE", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "Every Subscription, Listed", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "The whole leak in one view — category, cost and a keep-or-cancel flag on each recurring charge",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 2, content_subs)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_engine(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 110, "FIND IT · CUT IT", font=fs(32), pad_x=52, pad_y=22)
    tc(d, (SIZE // 2, 224), "The Audit Engine", fserif(44), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 1, content_audit)
    app_window(img, (60, 1180, SIZE - 60, SIZE - 60), 2, content_subs)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_printables(out):
    ...
    d = ImageDraw.Draw(img)
    magnify_crest(img, SIZE // 2, 110, r=48)
    pill(img, SIZE // 2, 214, "PLUS 12 PRINTABLE PAGES", font=fs(30), pad_x=48, pad_y=20)
    tc(d, (SIZE // 2, 300), "Print, Fill & Cancel", fserif(44), WHITE)
    gold_divider(img, SIZE // 2, 362, width=480)
    tc(d, (SIZE // 2, 404), "A matching print-ready pack — subscription audit, cancel list, renewal calendar, negotiation & more",
       fs(21, bold=False), (226, 214, 190))
    labels = ["Subscription Audit", "Cancel List", "Bills List", "Renewal Calendar",
              "Free Trial Tracker", "Price Hike Log", "Spend by Category", "Bill Negotiation",
              "Savings Log", "Monthly Summary", "Audit Worksheet", "Audit Checklist"]
...
def main():
    ...
    targets = [
        ...
        ("03_audit.png", render_audit),
        ("04_subs.png", render_subs),
