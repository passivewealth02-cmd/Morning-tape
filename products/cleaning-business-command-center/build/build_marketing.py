"""Marketing image set for Cleaning & Service Business Command Center™ (6 images, 2000x2000)."""
...
TABS = ["Dashboard", "Job P&L", "Services & Pricing", "Clients", "Schedule", "Leads & Quotes",
        "Supplies", "Team & Labor", "Mileage & Vehicle", "Expenses",
        "Reviews & Referrals", "Monthly Summary", "Settings"]

FILE_LABEL = "Cleaning_Business_Command_Center.xlsx — Bright & Tidy Co. · Ava"
...
def spray_crest(c, cx, cy, r=56, glow=True):
    """Brand crest — a spray bottle: the icon of the cleaning business."""
    if glow:
    ...
    ov = Image.new("RGBA", c.size, (0, 0, 0, 0)); d = ImageDraw.Draw(ov)
    # bottle body
    d.rounded_rectangle((cx - r * 0.20, cy - r * 0.04, cx + r * 0.22, cy + r * 0.42), radius=7, fill=GOLD_HI, outline=(150, 120, 70))
    # label
    d.rectangle((cx - r * 0.20, cy + r * 0.14, cx + r * 0.22, cy + r * 0.28), fill=WHITE)
    # neck
    d.rectangle((cx - r * 0.09, cy - r * 0.14, cx + r * 0.09, cy - r * 0.04), fill=GOLD_HI, outline=(150, 120, 70))
    # trigger head
    d.rounded_rectangle((cx - r * 0.24, cy - r * 0.30, cx + r * 0.10, cy - r * 0.14), radius=4, fill=WHITE, outline=(150, 120, 70))
    # nozzle pointing up-left
    d.polygon([(cx - r * 0.24, cy - r * 0.28), (cx - r * 0.42, cy - r * 0.34),
               (cx - r * 0.38, cy - r * 0.24), (cx - r * 0.24, cy - r * 0.20)], fill=WHITE, outline=(150, 120, 70))
    # spray mist
    for dx, dy in [(-0.52, -0.42), (-0.58, -0.32), (-0.48, -0.30)]:
        x = cx + r * dx; y = cy + r * dy
        d.ellipse((x - r * 0.045, y - r * 0.045, x + r * 0.045, y + r * 0.045), fill=HIGHLIGHT)
    c.alpha_composite(ov)


# aliases (shared render helpers may reference these names across products)
briefcase_crest = spray_crest
house_crest = spray_crest
tent_crest = spray_crest
toque_crest = spray_crest
bento_crest = spray_crest
cupcake_crest = spray_crest
clock_crest = spray_crest
clipboard_crest = spray_crest
cloche_crest = spray_crest
ghost_crest = spray_crest
mug_crest = spray_crest
book_crest = spray_crest
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "SERVICE PRO", font=fs(15), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("JOB PRICE", "$180", "standard clean"),
    ("JOB COSTS", "$80", "supplies+labor"),
    ("JOB PROFIT", "$100", "per job"),
    ("JOB MARGIN", "56%", "of price"),
    ("PROFIT / HOUR", "$40.00", "what you make"),
    ("JOBS / MONTH", "31", "booked"),
    ("MONTHLY REVENUE", "$6,000", "this month"),
    ("RECURRING (MRR)", "$4,500", "the base"),
    ("ACTIVE CLIENTS", "12", "recurring"),
    ("MONTHLY PROFIT", "$2,900", "after costs"),
    ("AVG JOB", "$194", "per job"),
    ("SERVICE SCORE", "90%", "healthy"),
]

PROFIT_MONTHS = [("Jul", 2900), ("Aug", 3100), ("Sep", 3300), ("Oct", 2950), ("Nov", 2700), ("Dec", 3500)]


def content_dashboard(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Service Business Dashboard", font=fs(30), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 62), "Bright & Tidy Co. · Ava  ·  price every job, and grow recurring revenue", font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 210, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 105, y0 + 44), "● $4,500 recurring", font=fs(15), fill=PRIMARY, anchor="mm")
    ...
    d.text((gx, cy_top), "SERVICE HEALTH · WHERE THE JOB DOLLAR GOES · PROFIT BY MONTH", font=fs(19), fill=ACCENT, anchor="lt")
    ...
    d.text((px + 16, panels_y + 14), "Service Health", font=fs(17), fill=ACCENT, anchor="lt")
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("Job margin healthy", 1.0, "100%"), ("Profit/hour on target", 1.0, "100%"), ("Services priced", 1.0, "100%"),
           ("Profitable", 1.0, "100%"), ("Recurring healthy", 1.0, "100%"), ("Client base growing", 0.40, "40%")],
          color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Where the Job $ Goes", font=fs(15), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.44, panels_y + panel_h * 0.50, min(panel_h * 0.27, pw * 0.27),
          [(56, PRIMARY), (28, GOLD_LT), (16, ACCENT)], "56%", "job profit")
    legend(d, px + pw * 0.05, panels_y + panel_h - 108, [(PRIMARY, "Profit 56%"), (GOLD_LT, "Labor 28%"), (ACCENT, "Supplies+travel 16%")], 14, 30)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Profit by Month", font=fs(17), fill=ACCENT, anchor="lt")
    vbars(img, d, (px + 18, panels_y + 60, px + pw - 14, panels_y + panel_h - 10), PROFIT_MONTHS, 3500)
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Service Score", font=fs(17), fill=ACCENT, anchor="lt")
    ...
    for i, row in enumerate(rows):
        ...
        for ci, val in enumerate(row):
            ...
            if status_map is not None and ci == status_col:
                ...
                d.rounded_rectangle((hx - 70, ry + rh / 2 - 15, hx + 70, ry + rh / 2 + 15), radius=14, fill=bg)
...
def content_services(img, cbox):
    rows = [
        ("Standard Home Clean (3BR)", "$180", "2.5", "$72"),
        ("Deep Clean", "$320", "5.0", "$64"),
        ("Move-out Clean", "$380", "6.0", "$63"),
        ("Office Clean (weekly)", "$220", "3.0", "$73"),
        ("Apartment / Studio", "$120", "1.5", "$80"),
        ("Add-on: Inside Oven", "$45", "0.75", "$60"),
    ]
    _table(img, cbox, "Services & Pricing",
           "Price, hours and the rate per hour on every service — quote by the hour of profit.",
           ["SERVICE", "PRICE", "HOURS", "$ / HOUR"],
           [0.0, 0.52, 0.68, 0.86], rows)


def content_jobpnl(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Job P&L — Did It Pay?", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 62), "Price a job to the profit — supplies, labor and travel out, and what you make per hour.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("Job price · standard clean", "$180", PRIMARY), ("− Supplies", "−$18", DANGER),
            ("− Labor (helper)", "−$50", DANGER), ("− Travel & fuel", "−$12", DANGER)]
    callout_h = 150; gap = 40; rh = 104
    ...
    d.text((x0 + pad + 14, ry + rh / 2), "= JOB PROFIT", font=fs(18), fill=PRIMARY, anchor="lm")
    d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.80, ry + rh / 2), "$100", font=fs(18), fill=PRIMARY, anchor="mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2), "÷ 2.5 hours  =  $40.00 an hour — the job paid", font=fs(22), fill=WHITE, anchor="mm")
    img.alpha_composite(ov)


def content_clients(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Clients → Recurring Revenue", font=fs(30), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 60), "Your weekly, biweekly and monthly clients become the recurring base that pays the bills.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("3 weekly  ×  $180", "$2,160", PRIMARY), ("4 biweekly  ×  $180", "$1,440", PRIMARY),
            ("5 monthly  ×  $180", "$900", PRIMARY)]
    ...
    ty = top_region + max(((y1 - pad) - top_region - block_h) / 2, 0)
    grad_round(img, (x0 + pad, ty, x1 - pad, ty + rh), 8, PRIMARY_LT, PRIMARY_DK)
    for h, fx in zip(["RECURRING CLIENTS", "MONTHLY"], [0.0, 0.80]):
        d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * fx, ty + rh / 2), h, font=fs(16), fill=WHITE, anchor="lm" if fx == 0 else "mm")
    for i, (label, val, col) in enumerate(rows):
        ry = ty + rh + i * rh
        if i % 2:
    ...
    ry = ty + rh + len(rows) * rh
    d.rectangle((x0 + pad, ry, x1 - pad, ry + rh), fill=SURFACE)
    d.text((x0 + pad + 14, ry + rh / 2), "= RECURRING (MRR)  ·  12 clients", font=fs(18), fill=PRIMARY, anchor="lm")
    d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.80, ry + rh / 2), "$4,500", font=fs(18), fill=PRIMARY, anchor="mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2), "+ $1,500 one-time  =  $6,000 revenue  ·  $2,900 profit", font=fs(21), fill=WHITE, anchor="mm")
    img.alpha_composite(ov)
...
def render_hero(out):
    ...
    spray_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE COMPLETE SERVICE-BUSINESS SYSTEM", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "CLEANING BUSINESS", 68, max_w=1650)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 546), "A job-P&L engine, service pricing, a recurring client book, schedule, leads & books — one system.",
       fs(20, bold=False), (224, 213, 190))
    chips = [("14", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("JOB", "P&L ENGINE")]
    cw = 440
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not a price list — a complete price-it, book-it, grow-the-recurring system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Start Here", "how it all works"), ("Dashboard", "profit & score"),
        ("Job P&L", "did the job pay?"), ("Services & Pricing", "price for profit"),
        ("Clients", "recurring → MRR"), ("Schedule", "who cleans what"),
        ("Leads & Quotes", "never lose a lead"), ("Supplies", "stock the van"),
        ("Team & Labor", "track the crew"), ("Mileage & Vehicle", "deductible routes"),
        ("Expenses", "monthly costs"), ("Reviews & Referrals", "grow by word of mouth"),
        ("Monthly Summary", "watch it compound"), ("Settings", "set it once"),
        ("+ 12 Printable Pages", "quote & checklists"), ("Live Service Score", "it all rolls up"),
    ]
    cols = 4
    margin = 88
    gx, gy = 22, 20
...
def render_jobpnl(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "DID THE JOB PAY?", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "The Job P&L Engine", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Price a job to the profit — supplies, labor and travel out, and what you truly make per hour",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 1, content_jobpnl)
    img.convert("RGB").save(out, "PNG", optimize=True)


def render_clients(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "THE RECURRING BOOK", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "Clients Become Recurring Revenue", fserif(38), WHITE)
    tc(d, (SIZE // 2, 300), "Weekly, biweekly and monthly clients roll up into the recurring revenue that keeps you steady",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 3, content_clients)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_engine(out):
    ...
    tc(d, (SIZE // 2, 224), "The Service-Business Engine", fserif(42), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 1, content_jobpnl)
    app_window(img, (60, 1180, SIZE - 60, SIZE - 60), 2, content_services)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_printables(out):
    ...
    d = ImageDraw.Draw(img)
    spray_crest(img, SIZE // 2, 110, r=48)
    pill(img, SIZE // 2, 214, "PLUS 12 PRINTABLE PAGES", font=fs(30), pad_x=48, pad_y=20)
    tc(d, (SIZE // 2, 300), "Print, Fill & Keep — For the Van", fserif(44), WHITE)
    gold_divider(img, SIZE // 2, 362, width=480)
    tc(d, (SIZE // 2, 404), "A matching print-ready pack — job quote, price list, schedule, job checklist & more",
       fs(21, bold=False), (226, 214, 190))
    labels = ["Job Quote & P&L", "Service Price List", "Client List", "Weekly Schedule",
              "Leads & Quotes", "Supply List", "Team & Labor", "Mileage Log",
              "Expense Log", "Reviews & Referrals", "Monthly Summary", "Job Checklist"]
...
def main():
    ...
    targets = [
        ...
        ("03_jobpnl.png", render_jobpnl),
        ("04_clients.png", render_clients),
