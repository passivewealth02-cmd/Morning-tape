"""Detailed marketing image set for Cleaning & Service Business Command Center™ (07-10)."""
...
from build_marketing import (
    ...
    ROW_ALT, fs, fserif, premium_bg, pill, wordmark, gold_divider, spray_crest,
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR SERVICE PROS", "Price It, Book It, Grow It",
                "Four ways it turns a service business into steady, profitable work")
    feats = [
        ("$", "Live Service Score", "donut", (90, "90%", "healthy"),
         "Job margin, profit-per-hour, pricing, profit, recurring revenue and client growth roll into one number — your business health at a glance."),
        ("%", "Job P&L Engine", "table", None,
         "Price a job to the profit — supplies, labor and travel out — and the engine shows your job profit and what you truly make per hour."),
        ("box", "Grow Recurring Revenue", "stock", None,
         "A client roster turns weekly, biweekly and monthly clients into recurring revenue — the base that keeps a service business steady."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack — job quote, service price list, schedule, supply list, job checklist & more for the van."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("Job profit", "$100"), ("÷ 2.5 hours", "$40/hr"), ("Job margin", "56%")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["STANDARD CLEAN", "VALUE"], [0.0, 0.62]):
            ...
            od = ImageDraw.Draw(img)
            items = [("Recurring (MRR)", "$4,500", MINT_BG, PRIMARY), ("Monthly revenue", "$6,000", MINT_BG, PRIMARY),
                     ("Monthly profit", "$2,900", MINT_BG, PRIMARY), ("Client base", "40%", SURFACE, (110, 88, 58))]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    ...
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Price List",
                "Everything a basic cleaning sheet leaves out — built in")
    rows = [
        ("A service price list", True, True),
        ("A job P&L — did it pay?", False, True),
        ("Profit per hour on every job", False, True),
        ("Recurring client book → MRR", False, True),
        ("Weekly schedule & cleaners", False, True),
        ("Leads & quotes pipeline", False, True),
        ("Supplies, team & mileage", False, True),
        ("Expenses for tax time", False, True),
        ("Reviews & referral tracking", False, True),
        ("Live Service Score dashboard", False, True),
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "SET UP IN AN AFTERNOON", "Run Your Business in 4 Steps",
                "No spreadsheet skills needed — if you can type, you can run it")
    steps = [
        ("1", "Price your jobs", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack, then price each service by the hour of profit."),
        ("2", "Book your clients", "Add your weekly, biweekly and monthly clients — the system turns them into the recurring revenue that keeps you steady."),
        ("3", "Run the calendar", "Schedule jobs and cleaners, track leads and quotes, and log supplies, team hours and mileage as you go."),
        ("4", "Grow the book", "Keep your expenses for tax time, ask for reviews and referrals, and watch your profit per hour and recurring revenue climb."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + Service Score",
        "Job P&L engine (did it pay?)", "Services & pricing (per hour)",
        "Recurring client book → MRR", "Schedule, leads & quotes",
        "Supplies, team, mileage & expenses", "Start-Here quick-start guide",
    ]
    ...
    who = ["House & office cleaners", "Maid & janitorial services", "Lawn care & landscapers",
           "Window & pressure washers", "Handymen & home-service pros", "Any solo or small service crew"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "PRICE IT · BOOK IT · GROW IT", font=fs(27), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Price every job by the hour of profit, and grow the recurring book.",
       fs(23, bold=False), TEXT_MUTED)
    spray_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "CLEANING BUSINESS", 50, max_w=1150)
    img.convert("RGB").save(out, "PNG", optimize=True)
