"""Detailed marketing image set for Rental Property & Landlord Command Center™ (07-10)."""
...
from build_marketing import (
    ...
    ROW_ALT, fs, fserif, premium_bg, pill, wordmark, gold_divider, house_crest,
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR LANDLORDS", "Analyze It, Run It, Measure It",
                "Four ways it turns a rental into a return you can actually see")
    feats = [
        ("$", "Live Landlord Score", "donut", (90, "90%", "healthy"),
         "Cash flow, cap rate, cash-on-cash, DSCR, expenses & reserves roll into one number — so you know a door's health at a glance."),
        ("%", "Deal Analyzer Engine", "table", None,
         "Rent in, every operating cost and the mortgage out — the engine shows your NOI, cash flow, cap rate, cash-on-cash and DSCR."),
        ("box", "Run the Portfolio", "stock", None,
         "A rent roll, tenants, a rent ledger, expense and maintenance logs, mortgages and reserves — the whole operation in one place."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack — deal analyzer, rent roll, tenant sheet, reserve tracker, move-in/move-out & more."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("NOI (monthly)", "$1,095"), ("Cash flow", "$360"), ("Cap rate / CoC", "6.0% / 8.0%")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["123 MAPLE ST · A", "VALUE"], [0.0, 0.62]):
            ...
            od = ImageDraw.Draw(img)
            items = [("Portfolio rent", "$7,000", MINT_BG, PRIMARY), ("Monthly cash flow", "$360", MINT_BG, PRIMARY),
                     ("Annual cash flow", "$4,320", MINT_BG, PRIMARY), ("Reserves funded", "40%", SURFACE, (110, 88, 58))]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Rent Tracker",
                "Everything a basic rent sheet leaves out — built in")
    rows = [
        ("A rent tracker", True, True),
        ("A deal analyzer (NOI, cash flow)", False, True),
        ("Cap rate, cash-on-cash & DSCR", False, True),
        ("Rent roll across every door", False, True),
        ("Tenant, lease & deposit records", False, True),
        ("Expense & CapEx logs for taxes", False, True),
        ("Mortgage & reserve tracking", False, True),
        ("Mileage log (deductible)", False, True),
        ("Renewals & inspection reminders", False, True),
        ("Live Landlord Score dashboard", False, True),
    ...
    d.text((c1, top + hh // 2), "A Basic\nRent Sheet", font=fs(18), fill=(210, 210, 205), anchor="mm")
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "SET UP IN AN AFTERNOON", "Run Your Rentals in 4 Steps",
                "No spreadsheet skills needed — if you can type, you can run it")
    steps = [
        ("1", "Analyze the deal", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack, then run any rental through the deal analyzer."),
        ("2", "Load your doors", "Build your rent roll, add tenants, lease dates and deposits, and enter each property's mortgage and reserves."),
        ("3", "Log as you go", "Record rent payments, expenses, maintenance and CapEx and mileage — everything lands where tax time needs it."),
        ("4", "Measure & grow", "Watch cash flow, cap rate, cash-on-cash and DSCR per door, keep your reserves funded, and grow the portfolio on real numbers."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "EVERYTHING YOU GET", "One System, Whole Portfolio",
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + Landlord Score",
        "Deal analyzer (NOI, cap, CoC, DSCR)", "Rent roll & tenant records",
        "Rent ledger & expense logs", "Mortgage, reserves & CapEx",
        "Mileage log & renewals tracker", "Start-Here quick-start guide",
    ]
    ...
    who = ["Buy-and-hold landlords", "First-time & aspiring investors", "House-hackers & duplex owners",
           "Small multifamily owners", "Out-of-state investors", "Anyone analyzing a rental"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "ANALYZE · RUN · MEASURE", font=fs(27), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Analyze before you buy, run every door, and know the real return.",
       fs(23, bold=False), TEXT_MUTED)
    house_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "RENTAL PROPERTY", 54, max_w=1100)
    img.convert("RGB").save(out, "PNG", optimize=True)
