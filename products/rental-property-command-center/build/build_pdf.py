"""Printable PDF pack for Rental Property & Landlord Command Center™ (12 pages, US Letter)."""
...
def page(title, subtitle):
    ...
    d.text((M, 90), "RENTAL PROPERTY & LANDLORD COMMAND CENTER™", font=fs(32), fill=GOLD_HI, anchor="lt")
    ...
    d.text((M, H - 108), "Property / Date: ______________", font=fs(24, bold=False), fill=MUTED, anchor="lt")
    d.text((W - M, H - 108), "Landlord Command Center™", font=fs(22, bold=False), fill=ACCENT, anchor="rt")
    return img, d
...
def p01(i):
    img, d = page("Deal Analyzer", "Analyze a rental before you buy")
    field(d, M, TOP, CW / 2 - 30, "Property"); field(d, M + CW / 2 + 30, TOP, CW / 2 - 30, "Purchase price")
    table(d, M, TOP + 150, CW, ["LINE", "MONTHLY"], [0.0, 0.78], 10, 130)
    y = TOP + 150 + 78 + 10 * 130 + 20
    field(d, M, y, CW / 2 - 30, "Cash flow / mo"); field(d, M + CW / 2 + 30, y, CW / 2 - 30, "Cap rate / CoC")
    i.append(img)


def p02(i):
    img, d = page("Rent Roll", "Every door at a glance")
    table(d, M, TOP, CW, ["UNIT", "RENT", "STATUS", "TENANT"], [0.0, 0.42, 0.60, 0.80], 16, 135)
    i.append(img)


def p03(i):
    img, d = page("Tenant Sheet", "Leases & deposits")
    table(d, M, TOP, CW, ["TENANT", "UNIT", "LEASE ENDS", "DEPOSIT"], [0.0, 0.34, 0.58, 0.82], 18, 120)
    i.append(img)


def p04(i):
    img, d = page("Rent Ledger", "Payments received")
    table(d, M, TOP, CW, ["DATE", "UNIT", "AMOUNT", "STATUS"], [0.0, 0.22, 0.56, 0.80], 20, 118)
    i.append(img)


def p05(i):
    img, d = page("Expense Log", "Spending by category")
    table(d, M, TOP, CW, ["DATE", "CATEGORY", "PAYEE", "AMOUNT"], [0.0, 0.18, 0.48, 0.80], 20, 110)
    i.append(img)


def p06(i):
    img, d = page("Maintenance & CapEx", "Repairs & improvements")
    table(d, M, TOP, CW, ["DATE", "UNIT", "ITEM", "COST"], [0.0, 0.16, 0.40, 0.82], 20, 118)
    i.append(img)


def p07(i):
    img, d = page("Mortgage & Loans", "Every loan tracked")
    table(d, M, TOP, CW, ["PROPERTY", "BALANCE", "RATE", "PAYMENT"], [0.0, 0.38, 0.62, 0.80], 16, 140)
    i.append(img)


def p08(i):
    img, d = page("Reserves Tracker", "Balance vs goal")
    table(d, M, TOP, CW, ["FUND", "BALANCE", "GOAL", "FUNDED %"], [0.0, 0.36, 0.58, 0.82], 16, 140)
    i.append(img)


def p09(i):
    img, d = page("Mileage Log", "Deductible drives")
    table(d, M, TOP, CW, ["DATE", "PROPERTY", "MILES", "PURPOSE"], [0.0, 0.20, 0.46, 0.66], 20, 118)
    i.append(img)


def p10(i):
    img, d = page("Renewals & Docs", "Never miss a date")
    table(d, M, TOP, CW, ["ITEM", "UNIT", "DUE", "DONE"], [0.0, 0.38, 0.62, 0.86], 18, 125)
    i.append(img)


def p11(i):
    img, d = page("Monthly Summary", "Income, expenses & cash flow")
    table(d, M, TOP, CW, ["MONTH", "INCOME", "EXPENSES", "CASH FLOW"], [0.0, 0.30, 0.54, 0.78], 14, 155)
    i.append(img)


def p12(i):
    img, d = page("Move-In / Move-Out", "Document every room")
    half = CW / 2 - 30
    for k, st in enumerate(["Move-in condition", "Move-out condition"]):
        x = M + k * (half + 60)
...
def main():
    ...
    pdf_path = os.path.join(out_dir, "Rental_Property_Printables.pdf")
