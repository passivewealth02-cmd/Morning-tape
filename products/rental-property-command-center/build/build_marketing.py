"""Marketing image set for Rental Property & Landlord Command Center™ (6 images, 2000x2000)."""
...
TABS = ["Dashboard", "Deal Analyzer", "Rent Roll", "Tenants", "Rent Ledger", "Expenses Log",
        "Maintenance & CapEx", "Mortgage & Loan", "Reserves & Escrow", "Mileage & Travel",
        "Renewals & Docs", "Monthly Summary", "Settings"]

FILE_LABEL = "Rental_Property_Command_Center.xlsx — Maple Lane Rentals · Jordan"
...
def house_crest(c, cx, cy, r=56, glow=True):
    """Brand crest — a house: the icon of the rental portfolio."""
    if glow:
    ...
    ov = Image.new("RGBA", c.size, (0, 0, 0, 0)); d = ImageDraw.Draw(ov)
    # roof
    d.polygon([(cx, cy - r * 0.42), (cx - r * 0.50, cy - r * 0.02), (cx + r * 0.50, cy - r * 0.02)], fill=GOLD_HI, outline=(150, 120, 70))
    # chimney
    d.rectangle((cx + r * 0.24, cy - r * 0.36, cx + r * 0.34, cy - r * 0.12), fill=(150, 120, 70))
    # body
    d.rectangle((cx - r * 0.38, cy - r * 0.02, cx + r * 0.38, cy + r * 0.42), fill=WHITE, outline=(150, 120, 70))
    # door
    d.rectangle((cx - r * 0.10, cy + r * 0.14, cx + r * 0.10, cy + r * 0.42), fill=PRIMARY)
    d.ellipse((cx + r * 0.03, cy + r * 0.26, cx + r * 0.08, cy + r * 0.31), fill=GOLD_HI)
    # window
    d.rectangle((cx - r * 0.28, cy + r * 0.06, cx - r * 0.15, cy + r * 0.19), fill=HIGHLIGHT, outline=(150, 120, 70))
    c.alpha_composite(ov)


# aliases (shared render helpers may reference these names across products)
tent_crest = house_crest
toque_crest = house_crest
bento_crest = house_crest
cupcake_crest = house_crest
clock_crest = house_crest
clipboard_crest = house_crest
cloche_crest = house_crest
ghost_crest = house_crest
mug_crest = house_crest
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "LANDLORD", font=fs(15), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("MONTHLY RENT", "$1,850", "flagship"),
    ("GROSS INCOME", "$1,925", "+ other"),
    ("OPERATING EXP", "$830", "per month"),
    ("NOI (MONTHLY)", "$1,095", "net op income"),
    ("MORTGAGE P&I", "$735", "debt service"),
    ("CASH FLOW", "$360", "per month"),
    ("CAP RATE", "6.0%", "NOI ÷ price"),
    ("CASH-ON-CASH", "8.0%", "on invested"),
    ("DSCR", "1.49", "coverage"),
    ("1% RULE", "0.84%", "rent ÷ price"),
    ("ANNUAL CASH FLOW", "$4,320", "× 12"),
    ("LANDLORD SCORE", "90%", "healthy"),
]

CF_MONTHS = [("Jul", 2800), ("Aug", 3100), ("Sep", 3050), ("Oct", 1000), ("Nov", 3150), ("Dec", 2900)]


def content_dashboard(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Landlord Dashboard", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 64), "Maple Lane Rentals · Jordan  ·  know the real return on every door", font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 210, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 105, y0 + 44), "● 8.0% cash-on-cash", font=fs(14), fill=PRIMARY, anchor="mm")
    ...
    d.text((gx, cy_top), "LANDLORD HEALTH · WHERE THE RENT GOES · CASH FLOW BY MONTH", font=fs(20), fill=ACCENT, anchor="lt")
    ...
    d.text((px + 16, panels_y + 14), "Landlord Health", font=fs(17), fill=ACCENT, anchor="lt")
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("Cash flow positive", 1.0, "100%"), ("Cap rate on target", 1.0, "100%"), ("Cash-on-cash healthy", 1.0, "100%"),
           ("DSCR safe", 1.0, "100%"), ("Expenses in check", 1.0, "100%"), ("Reserves funded", 0.40, "40%")],
          color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Where the Rent Goes", font=fs(16), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.44, panels_y + panel_h * 0.50, min(panel_h * 0.27, pw * 0.27),
          [(19, PRIMARY), (43, ACCENT), (38, GOLD_LT)], "19%", "cash flow")
    legend(d, px + pw * 0.05, panels_y + panel_h - 108, [(PRIMARY, "Cash flow 19%"), (ACCENT, "Op-ex 43%"), (GOLD_LT, "Mortgage 38%")], 14, 30)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Cash Flow by Month", font=fs(16), fill=ACCENT, anchor="lt")
    vbars(img, d, (px + 18, panels_y + 60, px + pw - 14, panels_y + panel_h - 10), CF_MONTHS, 3150)
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Landlord Score", font=fs(17), fill=ACCENT, anchor="lt")
...
def content_rentroll(img, cbox):
    rows = [
        ("123 Maple St · A", "$1,850", "Occupied", "Ramirez"),
        ("123 Maple St · B", "$1,650", "Occupied", "Kelly"),
        ("88 Oak Ave", "$2,100", "Occupied", "Osei"),
        ("12 Birch Ct", "$1,400", "Turnover", "—"),
    ]
    smap = {"Occupied": (MINT_BG, PRIMARY), "Turnover": (WARN_BG, (150, 110, 60))}
    _table(img, cbox, "Rent Roll",
           "Every door — rent, status and tenant, so your portfolio rent rolls up.",
           ["UNIT", "RENT", "STATUS", "TENANT"],
           [0.0, 0.44, 0.64, 0.86], rows, total_row=("PORTFOLIO RENT", "$7,000", "", ""),
           status_col=2, status_map=smap)


def content_dealanalyzer(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Deal Analyzer — The Real Return", font=fs(30), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 60), "Rent in, every cost and the mortgage out — the number that says buy or pass.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("Gross monthly income", "$1,925", PRIMARY), ("− Operating expenses", "−$830", DANGER),
            ("− Mortgage (P&I)", "−$735", DANGER)]
    ...
    d.text((x0 + pad + 14, ry + rh / 2), "= MONTHLY CASH FLOW", font=fs(18), fill=PRIMARY, anchor="lm")
    d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.80, ry + rh / 2), "$360", font=fs(18), fill=PRIMARY, anchor="mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2), "6.0% cap rate  ·  8.0% cash-on-cash  ·  1.49 DSCR", font=fs(22), fill=WHITE, anchor="mm")
    img.alpha_composite(ov)
...
def render_hero(out):
    ...
    pill(img, SIZE // 2, 256, "THE COMPLETE LANDLORD SYSTEM", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "RENTAL PROPERTY", 76, max_w=1600)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 546), "A deal analyzer, rent roll, tenants, ledgers, reserves & the real return — one system.",
       fs(20, bold=False), (224, 213, 190))
    chips = [("14", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("DEAL", "ANALYZER")]
    cw = 440
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not a rent tracker — a complete analyze-it, run-it, measure-it system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Start Here", "how it all works"), ("Dashboard", "returns & score"),
        ("Deal Analyzer", "buy or pass?"), ("Rent Roll", "every door"),
        ("Tenants", "leases & deposits"), ("Rent Ledger", "payments received"),
        ("Expenses Log", "spending by category"), ("Maintenance & CapEx", "protect the asset"),
        ("Mortgage & Loan", "debt tracked"), ("Reserves & Escrow", "your safety funds"),
        ("Mileage & Travel", "deductible drives"), ("Renewals & Docs", "never miss a date"),
        ("Monthly Summary", "watch it compound"), ("Settings", "set it once"),
        ("+ 12 Printable Pages", "analyzer & checklists"), ("Live Landlord Score", "it all rolls up"),
    ]
    cols = 4
    margin = 88
    gx, gy = 22, 20
...
def render_dealanalyzer(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "BUY OR PASS?", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "The Deal Analyzer", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Rent in, every cost and the mortgage out — the number that tells you if a rental really pays",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 1, content_dealanalyzer)
    img.convert("RGB").save(out, "PNG", optimize=True)


def render_rentroll(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "EVERY DOOR", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "Your Whole Rent Roll", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Rent, status and tenant on every unit — your portfolio's monthly rent, at a glance",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 2, content_rentroll)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_engine(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 110, "ANALYZE IT · RUN IT", font=fs(32), pad_x=52, pad_y=22)
    tc(d, (SIZE // 2, 224), "The Landlord Engine", fserif(44), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 1, content_dealanalyzer)
    app_window(img, (60, 1180, SIZE - 60, SIZE - 60), 2, content_rentroll)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_printables(out):
    ...
    tc(d, (SIZE // 2, 300), "Print, Fill & Keep — For the Files", fserif(44), WHITE)
    gold_divider(img, SIZE // 2, 362, width=480)
    tc(d, (SIZE // 2, 404), "A matching print-ready pack — deal analyzer, rent roll, reserve tracker & more",
       fs(21, bold=False), (226, 214, 190))
    labels = ["Deal Analyzer", "Rent Roll", "Tenant Sheet", "Rent Ledger",
              "Expense Log", "Maintenance & CapEx", "Mortgage & Loans", "Reserves Tracker",
              "Mileage Log", "Renewals & Docs", "Monthly Summary", "Move-In / Move-Out"]
...
def main():
    ...
    targets = [
        ...
        ("03_dealanalyzer.png", render_dealanalyzer),
        ("04_rentroll.png", render_rentroll),
