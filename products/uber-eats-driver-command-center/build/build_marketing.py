"""Marketing image set for Uber Eats Driver Command Center™ (6 images, 2000x2000).
...
all 18 tabs, the REAL computed KPI numbers, and fully populated tables/charts.
  ...
  01_hero.png       - branded hero + live driver dashboard
  02_inside.png     - "everything inside — 18 powerful tabs"
  03_deliverylog.png- delivery log (the earnings engine)
  04_takehome.png   - earnings -> real take-home (+ mileage deduction)
  05_grow.png       - hotspots + promos & quests
...
TABS = ["Dashboard", "Driver Profile", "Delivery Log", "Earnings", "Mileage", "Fuel Log", "Vehicle",
        "Expenses", "Budget", "Tax Center", "Savings", "Promos", "Hotspots", "Ratings", "Analytics",
        "Planner", "Gallery", "Settings"]

FILE_LABEL = "Uber_Eats_Driver_Command_Center.xlsx — Alex · Denver, CO · Full-time"
...
def bag_crest(c, cx, cy, r=56, glow=True):
    """Brand crest with an insulated delivery bag — a nod to food delivery, green accent for Eats."""
    if glow:
        radial_glow(c, cx, cy, int(r * 2.1), GOLD_HI, 90)
        radial_glow(c, cx, cy, int(r * 1.4), EATS_HI, 45)
    ...
    # bag body
    bw, bh = r * 0.92, r * 0.86
    x0, y0 = cx - bw / 2, cy - bh * 0.35
    d.rounded_rectangle((x0, y0, x0 + bw, y0 + bh), radius=10, fill=GOLD_HI, outline=(160, 130, 78), width=2)
    # top flap
    d.rounded_rectangle((x0, y0 - r * 0.18, x0 + bw, y0 + r * 0.14), radius=7, fill=(240, 220, 170), outline=(160, 130, 78), width=2)
    # handle straps
    d.arc((cx - r * 0.28, y0 - r * 0.5, cx + r * 0.28, y0 + r * 0.05), 180, 360, fill=GOLD_HI, width=6)
    # front pocket line
    d.line((x0 + 8, y0 + bh * 0.5, x0 + bw - 8, y0 + bh * 0.5), fill=(120, 95, 55), width=3)
    # green "EATS" chevron badge on the pocket
    d.polygon([(cx - 10, y0 + bh * 0.72), (cx, y0 + bh * 0.62), (cx + 10, y0 + bh * 0.72),
               (cx, y0 + bh * 0.82)], fill=EATS_GREEN)
    # steam wisps
    for dx, a in [(-r * 0.22, 70), (0, 90), (r * 0.22, 70)]:
        d.arc((cx + dx - 8, y0 - r * 0.62, cx + dx + 8, y0 - r * 0.34), 120, 360, fill=(117, 230, 193, a), width=3)
    c.alpha_composite(ov)
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "UBER EATS", font=fs(17), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("NET EARNINGS", "$1,526", "take-home"),
    ("GROSS EARNINGS", "$2,544", "base+tips+promo"),
    ("TIPS", "$1,258", "49% of pay"),
    ("NET $ / HOUR", "$16.96", "after costs"),
    ("NET $ / MILE", "$1.02", "after costs"),
    ("MILES DRIVEN", "1,502", "business"),
    ("ONLINE HOURS", "90.0", "this month"),
    ("DELIVERIES", "321", "completed"),
    ("EXPENSES", "$1,018", "gas + car + more"),
    ("TAX DEDUCTION", "$1,051", "mileage @ $0.70"),
    ("SAVINGS", "65%", "emergency fund"),
    ("DRIVER HEALTH", "87%", "blended"),
]
...
def content_dashboard(img, cbox):
    ...
    d.text((x0 + pad, y0 + 22), "Driver Dashboard", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 64), "Alex · Denver, CO  ·  every dash, mile & dollar, automatically organized", font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    ...
    months = ["Feb", "Mar", "Apr", "May", "Jun", "Jul"]; vals = [1280, 1360, 1440, 1410, 1490, 1526]
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [(m, v / 1526, f"{v/1000:.1f}k") for m, v in zip(months, vals)], color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    ...
    donut(d, px + pw * 0.42, panels_y + panel_h * 0.52, min(panel_h * 0.28, pw * 0.27),
          [(50, HIGHLIGHT), (41, PRIMARY), (9, ACCENT)], "$2.5K", "gross")
    legend(d, px + pw * 0.04, panels_y + panel_h - 104, [(HIGHLIGHT, "Tips 49%"), (PRIMARY, "Base 41%"), (ACCENT, "Promo 9%")], 15, 30)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    ...
    hbars(img, d, (px + 20, panels_y + 56, px + pw - 16, panels_y + panel_h - 20),
          [("Fri dinner", 1.0, "$205"), ("Sat dinner", 0.97, "$198"), ("Sat multi", 0.95, "$195"),
           ("Wed DoorDash", 0.89, "$182"), ("Weeknight", 0.89, "$182")])
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Expense Breakdown", font=fs(17), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.42, panels_y + panel_h * 0.52, min(panel_h * 0.28, pw * 0.27),
          [(31, PRIMARY), (29, ACCENT), (16, HIGHLIGHT), (11, SURFACE), (13, (170, 150, 120))], "$1.0K", "per mo")
    legend(d, px + pw * 0.04, panels_y + panel_h - 104, [(PRIMARY, "Car Pmt 31%"), (ACCENT, "Gas 29%"), (HIGHLIGHT, "Insurance 16%")], 15, 30)
...
PLAT_MAP = {"Uber Eats": (MINT_BG, GREEN), "DoorDash": (RED_BG, DANGER),
            "Grubhub": (WARN_BG, ACCENT), "Multi": (SURFACE, (110, 88, 58))}


def content_deliverylog(img, cbox):
    rows = [
        ("Sat", "DoorDash", "6.5", "25", "$205", "118", "$23"),
        ("Fri", "Multi", "6.5", "24", "$195", "112", "$22"),
        ("Thu", "Uber Eats", "5.5", "20", "$159", "94", "$18"),
        ("Wed", "DoorDash", "6.0", "22", "$182", "102", "$20"),
        ("Tue", "Multi", "6.0", "23", "$182", "108", "$21"),
        ("Sun", "Uber Eats", "5.0", "18", "$136", "84", "$16"),
        ("Sat", "Grubhub", "5.0", "16", "$120", "76", "$15"),
        ("Fri", "Uber Eats", "5.5", "19", "$152", "88", "$17"),
    ]
    _table(img, cbox, "Delivery Log",
           "Log each shift — earnings, miles & gas roll straight into your dashboard",
           ["DAY", "APP", "HOURS", "ORDERS", "EARNINGS", "MILES", "GAS"],
           [0.0, 0.15, 0.42, 0.55, 0.66, 0.83, 0.93], rows,
           total_row=("MONTH TOTAL", "", "90.0", "321", "$2,544", "1,502", "$293"),
           status_col=1, status_map=PLAT_MAP)


def content_takehome(img, cbox):
    rows = [
        ("Base Pay", "$1,045", "41% of gross"),
        ("Tips", "$1,258", "49% of gross"),
        ("Promo / Peak", "$241", "9% of gross"),
        ("− Business expenses", "−$1,018", "40% ratio"),
        ("Mileage tax deduction", "$1,051", "IRS $0.70 / mi"),
    ]
    _table(img, cbox, "Earnings → Real Take-Home",
           "See what you ACTUALLY keep — net after gas & expenses, plus the mileage deduction.  $16.96 net/hr · $1.02 net/mi",
           ["EARNINGS  →  TAKE-HOME", "AMOUNT", "DETAIL"],
           [0.0, 0.52, 0.75], rows,
           total_row=("NET TAKE-HOME", "$1,526", "$16.96 / hour"))


def content_hotspots(img, cbox):
    rows = [
        ("Stadium district", "Event nights", "$34", "High"),
        ("Cherry Creek", "Dinner rush", "$32", "High"),
        ("Downtown / LoDo", "Fri-Sat 6-9pm", "$30", "High"),
        ("University Hill", "11am-2pm, 6-9pm", "$27", "Medium"),
        ("Highlands", "Weekend brunch", "$26", "Medium"),
        ("Suburbs", "Weekday dinner", "$21", "Low"),
    ]
    _table(img, cbox, "Hotspots & Hours",
           "Drive where it pays — the zones & hours that earn the most per hour",
           ["ZONE", "BEST TIME", "$/HR", "DEMAND"],
           [0.0, 0.38, 0.66, 0.85], rows,
           status_col=3, status_map={"High": (MINT_BG, PRIMARY), "Medium": (WARN_BG, ACCENT),
                                     "Low": ((235, 230, 222), TEXT_MUTED)})


def content_promos(img, cbox):
    rows = [
        ("Uber Eats", "Quest · 50 trips", "$90", "Completed"),
        ("DoorDash", "Weekly Guarantee", "$75", "In Progress"),
        ("DoorDash", "Challenge · 25 (Fri-Sun)", "$60", "In Progress"),
        ("DoorDash", "Peak Pay · +$2/order", "$48", "Active"),
        ("Uber Eats", "Consecutive · x5 streaks", "$15", "Completed"),
        ("Grubhub", "Lunch bonus · $5/order", "$0", "Available"),
    ]
    _table(img, cbox, "Promos & Quests",
           "Never miss a promo — every quest, peak pay & guarantee and the extra it pays",
           ["APP", "REQUIREMENT", "REWARD", "STATUS"],
           [0.0, 0.24, 0.62, 0.84], rows,
           status_col=3, status_map={"Completed": (MINT_BG, PRIMARY), "In Progress": (WARN_BG, ACCENT),
                                     "Active": (WARN_BG, ACCENT), "Available": ((235, 230, 222), TEXT_MUTED)})
...
def render_hero(out):
    ...
    bag_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE FOOD-DELIVERY EARNINGS & BUDGET SYSTEM", font=fs(22), pad_x=42, pad_y=20)
    wordmark(img, SIZE // 2, 400, "UBER EATS DRIVER COMMAND CENTER", 78, max_w=1880)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 550), "Dashes, miles, gas, taxes & savings — know your REAL take-home, automatically.",
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not a mileage app — a complete delivery earnings, budget & tax operating system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Driver Dashboard", "12 KPIs + live charts"), ("Driver Profile", "your delivery business"),
        ("Delivery Log", "the earnings engine"), ("Earnings Breakdown", "mix & app split"),
        ...
        ("Savings & Goals", "pay yourself first"), ("Promos & Quests", "never miss a promo"),
        ("Hotspots & Hours", "drive where it pays"), ("Ratings & Feedback", "protect your rating"),
        ("Analytics Center", "driver health score"), ("Weekly Planner", "hit your goal"),
...
def render_deliverylog(out):
    ...
    tc(d, (SIZE // 2, 300), "Hours, orders, base, tips, miles & gas — one row per shift feeds your whole dashboard",
       fs(23, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 2, content_deliverylog)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_takehome(out):
    ...
    tc(d, (SIZE // 2, 300), "After gas & expenses — plus the mileage deduction that saves drivers thousands at tax time",
       fs(23, bold=False), (226, 214, 190))
...
def render_grow(out):
    ...
    tc(d, (SIZE // 2, 224), "Best Zones. Every Promo.", fserif(44), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 12, content_hotspots)
    app_window(img, (60, 1180, SIZE - 60, SIZE - 60), 11, content_promos)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_mobile(out):
    ...
    tc(d, (SIZE // 2, 320), "Log a shift between drop-offs — your whole delivery business in your pocket",
       fs(23, bold=False), (226, 214, 190))
    px, py = SIZE // 2, 1300
    pw, ph = 640, 1230
    ...
    od.text(((sx0 + sx1) // 2, sy0 + 56), "Alex · Denver", font=fserif(30), fill=GOLD_HI, anchor="mm")
    y = sy0 + 150
    cards = [("NET EARNINGS", "$1,526", PRIMARY), ("NET $ / HOUR", "$16.96", ACCENT),
             ("TAX DEDUCTION", "$1,051", PRIMARY), ("DRIVER HEALTH", "87%", PRIMARY)]
    ...
    y += 52
    for lab, state in [("Log Fri dinner shift", True), ("Fill up at Costco", False),
                       ("Bank tax set-aside", True), ("Check DoorDash peak pay", False)]:
...
def main():
    ...
    targets = [
        ...
        ("03_deliverylog.png", render_deliverylog),
