...
Premium pass: gradient backgrounds + deep-green hero band, glowing compass
crest, gold-foil wordmark, gold-capped KPI cards, soft layered shadows, and
a clean stat-chip row (no more badge/laptop collision).
...
def grad_round(c, box, radius, top, bottom, outline=None, width=0):
    """Fill a rounded rectangle with a vertical gradient (composited)."""
...
def premium_bg(c, band_h=0):
    """Cream vertical gradient + faint dot texture; optional green hero band."""
...
def hero_band(c, band_h):
    """Deep-green gradient header band with gold hairline + soft compass watermark."""
    band = vgradient(c.width, band_h, PRIMARY_LT, PRIMARY_DK)
    c.alpha_composite(band, (0, 0))
    # subtle radial lift behind the title
    radial_glow(c, c.width // 2, band_h // 2 - 30, 520, (60, 130, 118), 70)
    # faint watermark compass rings, lower-right
    wm = Image.new("RGBA", c.size, (0, 0, 0, 0))
    wd = ImageDraw.Draw(wm)
    for rr in (300, 230, 160):
        wd.ellipse((c.width - 120 - rr, band_h - 60 - rr,
                    c.width - 120 + rr, band_h - 60 + rr),
...
def wordmark(c, cx, cy, text, size, fill=GOLD_HI, shadow_col=(8, 30, 27)):
    """Serif wordmark with a soft drop shadow + gold sheen for depth."""
    f = fserif(size)
    d = ImageDraw.Draw(c)
    # drop shadow
    ...
    # gradient sheen fill via masked gradient
...
def compass_crest(c, cx, cy, r=58, glow=True):
    if glow:
        radial_glow(c, cx, cy, int(r * 2.1), GOLD_HI, 90)
    ov = Image.new("RGBA", c.size, (0, 0, 0, 0))
    d = ImageDraw.Draw(ov)
    grad = vgradient(2 * r, 2 * r, PRIMARY_LT, PRIMARY_DK)
    mask = Image.new("L", (2 * r, 2 * r), 0)
    ImageDraw.Draw(mask).ellipse((0, 0, 2 * r - 1, 2 * r - 1), fill=255)
    ov.paste(grad, (cx - r, cy - r), mask)
    d.ellipse((cx - r, cy - r, cx + r, cy + r), outline=GOLD_LT, width=4)
    d.ellipse((cx - r + 11, cy - r + 11, cx + r - 11, cy + r - 11), outline=GOLD_HI, width=2)
    L = r * 0.58
    s = r * 0.17
    d.polygon([(cx, cy - L), (cx + s, cy), (cx, cy + L), (cx - s, cy)], fill=HIGHLIGHT)
    d.polygon([(cx - L, cy), (cx, cy - s), (cx + L, cy), (cx, cy + s)], fill=GOLD_HI)
    d.ellipse((cx - 7, cy - 7, cx + 7, cy + 7), fill=WHITE)
    c.alpha_composite(ov)


def stat_chip(c, cx, cy, big, small, w=400, h=150):
    """Gradient stat chip — gold number over white label, gold cap."""
    box = (cx - w // 2, cy - h // 2, cx + w // 2, cy + h // 2)
    shadow(c, box, 20, 24, 80, 16)
    grad_round(c, box, 20, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", c.size, (0, 0, 0, 0))
    d = ImageDraw.Draw(ov)
    d.rounded_rectangle((box[0] + 18, box[1] + 12, box[2] - 18, box[1] + 18),
                        radius=3, fill=GOLD_HI)
...
def feature_card(c, x, y, w, h, title, sub):
    ...
    d.text((x + 106, cyc - 22), title, font=fs(29), fill=PRIMARY, anchor="lm")
    d.text((x + 106, cyc + 24), sub, font=fs(22, bold=False), fill=TEXT_MUTED, anchor="lm")
    c.alpha_composite(ov)


def page_scaffold(img, badge, title, subtitle, band_h=400):
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 132, badge, font=fs(40), pad_x=56, pad_y=22,
         grad=(GOLD_LT, GOLD))
    wordmark(img, SIZE // 2, 250, title, 62, fill=WHITE, shadow_col=(8, 30, 27)) \
        if False else tc(d, (SIZE // 2, 248), title, fserif(64), WHITE)
...
def panel(img, box, fill_color=WHITE, title=None):
    ...
    if title:
        d = ImageDraw.Draw(img)
        d.rounded_rectangle((box[0] + 34, box[1] + 26, box[0] + 42, box[1] + 56),
                            radius=3, fill=GOLD_LT)
...
def draw_table(img, inner, headers, rows, status_map=None, col_aligns=None,
    ...
    grad_round(img, (inner[0], inner[1], inner[2], inner[1] + hdr_h), 12,
               PRIMARY_LT, PRIMARY_DK)
...
def kpi_tile(d, x0, y0, kw, kh, lab, val, col):
    ...
    d.text((x0 + 16, y0 + 20), lab, font=fs(14), fill=ACCENT, anchor="lt")
    d.text((x0 + 16, y0 + 74), val, font=fserif(34), fill=col, anchor="lm")
...
def dashboard_screen(c, screen, charts_note=True):
    ...
    d.text((sx1 - 28, sy0 + head // 2), "Life dashboard", font=fs(22), fill=GOLD_HI, anchor="rm")
...
def laptop(c, cx, cy, w, h, charts_note=True):
    ...
    d.rounded_rectangle((body[0] + 6, body[1] + 6, body[2] - 6, body[1] + 60), radius=20,
                        fill=(58, 60, 64))
    bez = 22
...
def render_hero(out):
    ...
    pill(img, SIZE // 2, 285, "DIVORCE ORGANIZATION & LIFE REBUILD SYSTEM",
         font=fs(31), pad_x=50, pad_y=21, grad=(GOLD_LT, GOLD), fg=WHITE)
    wordmark(img, SIZE // 2, 440, "NEXT CHAPTER", 142)
    ...
    tc(d, (SIZE // 2, 600), "Organize documents, finances & schedules — and move forward with calm",
       fs(28, bold=False), (224, 213, 190))
    # clean stat-chip row (replaces the colliding badges)
    chips = [("20", "ORGANIZED TABS"), ("2-in-1", "EXCEL + SHEETS"),
             ("100%", "FORMULA-DRIVEN")]
    cyr = 745
    cw = 400
    ...
    tc(d, (SIZE // 2, 1430), "EVERYTHING IN ONE COMMAND CENTER", fs(34), ACCENT)
    ...
    pill(img, SIZE // 2, SIZE - 96,
         ...
         font=fs(38), pad_x=60, pad_y=28, star=True, grad=(PRIMARY_LT, PRIMARY_DK))
...
def render_dashboard(out):
    ...
    pill(img, SIZE // 2, SIZE - 110, "ALL LINKED · UPDATES AUTOMATICALLY",
         font=fs(30), pad_x=52, pad_y=24, grad=(PRIMARY_LT, PRIMARY_DK))
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_finances(out):
    ...
    for i, (lab, val) in enumerate(kpis):
        ...
        od.text((x + kw // 2, ky + 56), lab, font=fs(26), fill=ACCENT, anchor="mm")
        od.text((x + kw // 2, ky + 122), val, font=fserif(56), fill=PRIMARY, anchor="mm")
        img.alpha_composite(ov)
    ...
    by0 = bb[1] + 130
    maxv = 329
    for i, (lab, val, top, bot) in enumerate([("Assets", 329, PRIMARY_LT, PRIMARY_DK),
                                              ("Debts", 242, GOLD_LT, ACCENT)]):
        x = bx0 + i * 280
        h = (val / maxv) * (by1 - by0)
        grad_round(img, (x, by1 - h, x + 170, by1), 10, top, bot)
...
def render_analytics(out):
    ...
    for lab, pct in bars:
        ...
        grad_round(img, (bx, y + 48, bx + bw * pct, y + 92), 22, HIGHLIGHT, (70, 200, 165))
...
def render_mobile(out):
    ...
    od.text(((sx0 + sx1) // 2, sy0 + 56), "Next Chapter", font=fserif(38), fill=GOLD_HI, anchor="mm")
    y = sy0 + 150
