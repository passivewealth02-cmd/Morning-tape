"""Marketing image set for Trucking Owner-Operator Command Center™ (6 images, 2000x2000)."""
...
TABS = ["Dashboard", "Cost Per Mile", "Fixed Costs", "Variable Costs", "Loads", "Settlements",
        "Fuel Log", "Maintenance", "Truck & Trailer", "IFTA & Miles", "Reserve Fund",
        "Monthly Summary", "Settings"]

FILE_LABEL = "Trucking_Command_Center.xlsx — Redline Freight Co. · Wes"
...
def road_crest(c, cx, cy, r=56, glow=True):
    """Brand crest — a road running to the horizon: the icon of the owner-operator."""
    if glow:
    ...
    od.polygon([(cx - r * 0.52, cy + r * 0.50), (cx - r * 0.13, cy - r * 0.34),
                (cx + r * 0.13, cy - r * 0.34), (cx + r * 0.52, cy + r * 0.50)],
               fill=GOLD_HI, outline=(150, 120, 70))
    # centre line dashes, shrinking with distance
    segs = [(0.44, 0.30, 0.055), (0.16, 0.06, 0.040), (-0.06, -0.14, 0.028)]
    for y0, y1, w in segs:
        od.polygon([(cx - r * w, cy + r * y0), (cx + r * w, cy + r * y0),
                    (cx + r * w * 0.72, cy + r * y1), (cx - r * w * 0.72, cy + r * y1)], fill=PRIMARY_DK)
    # horizon glow
    od.ellipse((cx - r * 0.30, cy - r * 0.54, cx + r * 0.30, cy - r * 0.28), fill=HIGHLIGHT)
    c.alpha_composite(ov)


square_crest = road_crest
scissors_crest = road_crest
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "THE TRUCK", font=fs(15), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("FIXED / MONTH", "$4,490", "before a wheel turns"),
    ("FIXED COST / MILE", "$0.401", "spread over miles"),
    ("VARIABLE / MILE", "$0.860", "fuel, tires, maint"),
    ("COST PER MILE RUN", "$1.261", "every mile"),
    ("COST / LOADED MILE", "$1.41", "your rate floor"),
    ("RATE / LOADED MILE", "$2.35", "what you booked"),
    ("PROFIT / LOADED MILE", "$0.938", "what's left"),
    ("LOADED MILES", "10,000", "paid miles"),
    ("DEADHEAD", "10.7%", "1,200 empty miles"),
    ("MONTHLY REVENUE", "$23,500", "12 loads"),
    ("MONTHLY PROFIT", "$9,378", "39.9% margin"),
    ("ROAD SCORE", "90%", "healthy"),
]

REV_MONTHS = [("Feb", 19800), ("Mar", 21200), ("Apr", 20400), ("May", 22600), ("Jun", 23100), ("Jul", 23500)]


def content_dashboard(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Road Dashboard", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 64), "Redline Freight Co. \u00b7 Wes  \u00b7  know your cost per mile",
           font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 210, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 105, y0 + 44), "\u25cf floor $1.41/mi", font=fs(14), fill=PRIMARY, anchor="mm")
    ...
    d.text((gx, cy_top), "ROAD HEALTH \u00b7 WHERE THE $2.35 GOES \u00b7 REVENUE BY MONTH", font=fs(19), fill=ACCENT, anchor="lt")
    ...
    d.text((px + 16, panels_y + 14), "Road Health", font=fs(17), fill=ACCENT, anchor="lt")
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("Profit per mile healthy", 1.0, "100%"), ("Net margin healthy", 1.0, "100%"),
           ("Deadhead controlled", 1.0, "100%"), ("Rate above cost", 1.0, "100%"),
           ("Fuel economy on target", 1.0, "100%"), ("Reserve funded", 0.40, "40%")],
          color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Where the $2.35 Goes", font=fs(15), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.44, panels_y + panel_h * 0.46, min(panel_h * 0.25, pw * 0.25),
          [(39.9, PRIMARY), (28.6, ACCENT), (19.1, SURFACE), (12.4, GOLD_LT)], "40%", "you keep")
    legend(d, px + pw * 0.06, panels_y + panel_h - 148,
           [(PRIMARY, "Profit $0.938"), (ACCENT, "Fuel $0.672"),
            (SURFACE, "Fixed $0.449"), (GOLD_LT, "Maint & tires $0.291")], 14, 30)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Revenue by Month", font=fs(17), fill=ACCENT, anchor="lt")
    vbars(img, d, (px + 18, panels_y + 60, px + pw - 14, panels_y + panel_h - 10), REV_MONTHS, 23500)
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Road Score", font=fs(17), fill=ACCENT, anchor="lt")
...
def content_loads(img, cbox):
    rows = [
        ("Oklahoma City OK \u2192 Dallas TX", "500 mi", "50 dh", "$2.65", "$2.41"),
        ("Miami FL \u2192 Tampa FL", "480 mi", "60 dh", "$2.70", "$2.40"),
        ("Memphis TN \u2192 Atlanta GA", "620 mi", "45 dh", "$2.55", "$2.38"),
        ("Birmingham AL \u2192 Denver CO", "1,250 mi", "180 dh", "$2.10", "$1.83"),
        ("Atlanta GA \u2192 Miami FL", "1,100 mi", "160 dh", "$2.15", "$1.88"),
    ]
    _table(img, cbox, "Loads \u2014 What They Really Paid",
           "Book rate vs the all-in rate after deadhead. Anything under $1.41 gets flagged red.",
           ["LANE", "LOADED", "EMPTY", "BOOK RATE", "ALL-IN"],
           [0.0, 0.46, 0.60, 0.75, 0.90], rows,
           total_row=("12 loads", "10,000 mi", "1,200 dh", "$2.35", "$2.10"))


def content_cpm(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "The Cost-Per-Mile Engine", font=fs(30), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 60), "Fixed spread over miles, plus what every mile burns \u2014 then divided by LOADED miles.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("Fixed cost per mile ($4,490 \u00f7 11,200)", "$0.401", PRIMARY),
            ("+ Variable per mile (fuel, tires, maint, tolls)", "$0.860", ACCENT),
            ("= Cost per mile RUN", "$1.261", PRIMARY),
            ("\u00f7 10,000 LOADED miles (1,200 were empty)", "\u00d7 11,200", DANGER)]
    ...
    d.text((x0 + pad + 14, ry + rh / 2), "= COST PER LOADED MILE \u2014 YOUR RATE FLOOR", font=fs(18), fill=PRIMARY, anchor="lm")
    d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.80, ry + rh / 2), "$1.41", font=fs(18), fill=PRIMARY, anchor="mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2 - 20), "$2.35 booked  \u2212  $1.41 cost  =  $0.938 a mile, for real", font=fs(22), fill=WHITE, anchor="mm")
    od.text(((x0 + x1) / 2, by + callout_h / 2 + 24), "YOUR $2.35 LOAD IS REALLY A $2.10 LOAD", font=fs(23), fill=GOLD_HI, anchor="mm")
    img.alpha_composite(ov)
...
def render_hero(out):
    ...
    road_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE COMPLETE OWNER-OPERATOR SYSTEM", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "COST PER MILE", 86, max_w=1550)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 546), "Cost per mile, deadhead, loads, fuel, IFTA & a reserve fund \u2014 one system.",
       fs(20, bold=False), (224, 213, 190))
    chips = [("14", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("RATE", "FLOOR ENGINE")]
    cw = 440
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not a mileage log \u2014 a complete cost-it, book-it, keep-it system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Start Here", "how it all works"), ("Dashboard", "profit & score"),
        ("Cost Per Mile", "your rate floor"), ("Fixed Costs", "standing still"),
        ("Variable Costs", "every mile burns"), ("Loads", "with deadhead"),
        ("Settlements", "check deductions"), ("Fuel Log", "live MPG"),
        ("Maintenance", "by the odometer"), ("Truck & Trailer", "what you own"),
        ("IFTA & Miles", "by state"), ("Reserve Fund", "before it breaks"),
        ("Monthly Summary", "is it working?"), ("Settings", "set it once"),
        ("+ 12 Printable Pages", "trip sheets & logs"), ("Live Road Score", "it all rolls up"),
    ]
    cols = 4
    margin = 88
    gx, gy = 22, 20
...
def render_cpm(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "DEADHEAD ISN'T FREE", font=fs(29), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "The Cost-Per-Mile Engine", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Your costs run on TOTAL miles \u2014 but only the loaded ones ever paid you",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 1, content_cpm)
    img.convert("RGB").save(out, "PNG", optimize=True)


def render_loads(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "EVERY LOAD, SCORED", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "What the Load Really Paid", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Book rate vs all-in rate after the empty miles \u2014 losers flagged automatically",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 4, content_loads)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_engine(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 110, "COST IT \u00b7 BOOK IT", font=fs(32), pad_x=52, pad_y=22)
    tc(d, (SIZE // 2, 224), "The Road Engine", fserif(44), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 1, content_cpm)
    app_window(img, (60, 1180, SIZE - 60, SIZE - 60), 4, content_loads)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_printables(out):
    ...
    d = ImageDraw.Draw(img)
    road_crest(img, SIZE // 2, 110, r=48)
    pill(img, SIZE // 2, 214, "PLUS 12 PRINTABLE PAGES", font=fs(30), pad_x=48, pad_y=20)
    tc(d, (SIZE // 2, 300), "Print, Fill & Keep in the Cab", fserif(42), WHITE)
    gold_divider(img, SIZE // 2, 362, width=480)
    tc(d, (SIZE // 2, 404), "A matching print-ready pack \u2014 trip sheet, fuel log, pre-trip inspection, rate floor card & more",
       fs(21, bold=False), (226, 214, 190))
    labels = ["Cost Per Mile", "Fixed Costs", "Take This Load?", "Trip Sheet",
              "Fuel Log", "PM Schedule", "Settlements", "IFTA by State",
              "Pre-Trip Inspection", "Reserve Fund", "Monthly Summary", "Rate Floor Card"]
...
def main():
    ...
    targets = [
        ...
        ("03_costpermile.png", render_cpm),
        ("04_loads.png", render_loads),
