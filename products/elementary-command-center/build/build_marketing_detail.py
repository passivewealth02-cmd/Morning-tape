"""Detailed marketing image set for Elementary Command Center™ (4 images, 2000x2000).

  07_features.png   - feature spotlights
  08_compare.png    - "basic tracker" vs Elementary Command Center™
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR THE ELEMENTARY YEARS", "Where K–5 Learners Thrive",
                "Four ways it turns a busy homeschool into one clear picture")
    feats = [
        ("$", "Live On-Track Score", "donut", (78, "78%", "on track"),
         "Subject mastery, math facts, sight words, reading & graded work roll into one number — see where each child is thriving and what to work on next."),
        ("%", "A Mastery Tracker, Not Just a Grade", "table", None,
         "Every skill marked Mastered, Practicing or Not Yet — per subject and child — so you always know exactly what to teach next."),
        ("box", "Math Facts, Reading & Sight Words", "stock", None,
         "Watch fluency climb operation by operation, track guided-reading levels & sight-word lists, and celebrate every milestone along the way."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready PDF pack — weekly plan, mastery checklist, math-facts chart, reading log, report card & habits chart for the binder."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("Reading", "Mastered"), ("Math", "Practicing"), ("Spelling", "Practicing")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["SUBJECT", "STATUS"], [0.0, 0.52]):
            ...
            od = ImageDraw.Draw(img)
            items = [("Skills mastered", "16/28", MINT_BG, PRIMARY), ("Math facts fluent", "67%", MINT_BG, PRIMARY),
                     ("Sight words", "210/220", SURFACE, (110, 88, 58)), ("Books read", "52/60", MINT_BG, PRIMARY)]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Grade Book",
                "Everything a basic homeschool tracker leaves out — built in")
    rows = [
        ("Weekly lesson planning", True, True),
        ("Subject-mastery tracker (per skill)", False, True),
        ("Math-facts fluency by operation", False, True),
        ("Sight words & spelling progress", False, True),
        ("Reading log with guided-reading levels", False, True),
        ("Assignments, grades & report card", False, True),
        ("Habits & character streaks", False, True),
        ("Field trips, awards & book list", False, True),
        ("Curriculum & resource tracking", False, True),
        ("Live On-Track Score dashboard", False, True),
...
def render_howitworks(out):
    ...
    steps = [
        ...
        ("2", "Add your students", "In Settings, add your family, students & goals — then a quick profile with each child's grade & reading level."),
        ("3", "Plan & load", "Map the weekly plan, load curriculum, and list the skills that matter this year on the Mastery tab."),
        ("4", "Teach & track", "Mark mastery, math facts, sight words & reading as skills click — the On-Track Score shows exactly where you stand."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "EVERYTHING YOU GET", "One System, Your Whole Year",
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + On-Track Score",
        "Subject-mastery tracker", "Math-facts fluency + sight words",
        "Reading log with guided levels", "Assignments, grades & report card",
        "Habits, field trips & awards", "Start-Here guide + free updates",
    ]
    ...
    who = ["K–5 homeschooling families", "Multi-age & large families", "New & veteran homeschoolers",
           "Classical · Charlotte Mason · eclectic", "After-schoolers & summer-slide parents", "Anyone who keeps state records"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "SEE WHAT'S MASTERED · TEACH WHAT'S NEXT", font=fs(26), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Teach with confidence — and the records to back it up.",
       fs(24, bold=False), TEXT_MUTED)
    book_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "ELEMENTARY COMMAND CENTER", 50, max_w=1560)
    img.convert("RGB").save(out, "PNG", optimize=True)
