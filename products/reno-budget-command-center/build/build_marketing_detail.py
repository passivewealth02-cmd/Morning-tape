"""Detailed marketing image set for Home Renovation & Remodel Budget Command Center™ (07-10)."""
...
from build_marketing import (
    ...
    ROW_ALT, fs, fserif, premium_bg, pill, wordmark, gold_divider, hardhat_crest,
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR YOUR REMODEL", "Plan It, Track It, Finish It",
                "Four ways it keeps a renovation on budget and on schedule")
    feats = [
        ("$", "Live Reno Score", "donut", (90, "90%", "healthy"),
         "Under budget, rooms on budget, contingency healthy, scope defined, payments current and change-orders-in-check roll into one number — your project's health at a glance."),
        ("%", "Budget-vs-Actual Engine", "table", None,
         "Your whole project's budget, spent and remaining — plus a contingency reserve — so an overrun is a number you see, not a surprise."),
        ("box", "Run the Whole Project", "stock", None,
         "Rooms, line items, contractors, payments, change orders, materials and a timeline — the entire remodel in one place."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack — budget worksheet, room budget, contractor list, payment log & more, for the job site."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("Budget", "$60,000"), ("− Spent", "$42,000"), ("Remaining", "$18,000")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["BUDGET VS ACTUAL", "VALUE"], [0.0, 0.62]):
            ...
            od = ImageDraw.Draw(img)
            items = [("Contingency left", "$7,000", MINT_BG, PRIMARY), ("Paid to date", "$38,000", MINT_BG, PRIMARY),
                     ("% complete", "65%", MINT_BG, PRIMARY), ("Change orders", "40%", SURFACE, (110, 88, 58))]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    ...
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Budget Sheet",
                "Everything a basic reno budget leaves out — built in")
    rows = [
        ("A total project budget", True, True),
        ("Budget vs actual by room", False, True),
        ("A contingency reserve", False, True),
        ("Line items by category", False, True),
        ("Contractors & bids", False, True),
        ("A payment log (paid vs owed)", False, True),
        ("Change-order tracking", False, True),
        ("Materials & finishes", False, True),
        ("Timeline & financing", False, True),
        ("Live Reno Score dashboard", False, True),
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "SET UP IN AN AFTERNOON", "Run Your Remodel in 4 Steps",
                "No spreadsheet skills needed — if you can type, you can run it")
    steps = [
        ("1", "Set your budget", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack, then enter each room's budget — your total and contingency appear."),
        ("2", "Track what you spend", "Log line items, contractor payments and every change order; your spent, remaining and variance update live, room by room."),
        ("3", "Watch the reserve", "Keep an eye on your contingency reserve and outstanding payments so a surprise is a number you see coming, not a shock."),
        ("4", "Finish on budget", "Pick materials, run the timeline and financing, and let the Reno Score keep the whole project honest to the last punch-list item."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "EVERYTHING YOU GET", "One System, Whole Remodel",
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + Reno Score",
        "Budget-vs-actual engine", "Room budgets & line items",
        "Contractors, payments & changes", "Materials & finishes",
        "Timeline, financing & decisions", "Start-Here quick-start guide",
    ]
    ...
    who = ["Homeowners renovating", "Kitchen & bath remodels", "Whole-home renovations", "DIY & GC-managed projects",
           "House flippers & investors", "Anyone dreading an overrun"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "BUDGET IT · TRACK IT · FINISH IT", font=fs(27), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Know what it costs, what you've spent, and what's left — before it's a surprise.",
       fs(23, bold=False), TEXT_MUTED)
    hardhat_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "REMODEL BUDGET", 56, max_w=1200)
    img.convert("RGB").save(out, "PNG", optimize=True)
