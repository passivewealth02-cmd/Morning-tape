"""Marketing image set for Home Renovation & Remodel Budget Command Center™ (6 images, 2000x2000)."""
...
TABS = ["Dashboard", "Budget vs Actual", "Rooms", "Line Items", "Contractors", "Payments", "Change Orders",
        "Materials", "Timeline", "Financing", "Decisions", "Monthly Summary", "Settings"]

FILE_LABEL = "Reno_Budget_Command_Center.xlsx — Cedar & Stone · Reese"
...
def hardhat_crest(c, cx, cy, r=56, glow=True):
    """Brand crest — a hard hat: the icon of the renovation project."""
    if glow:
    ...
    ov = Image.new("RGBA", c.size, (0, 0, 0, 0)); d = ImageDraw.Draw(ov)
    # hard hat dome
    d.pieslice((cx - r * 0.40, cy - r * 0.34, cx + r * 0.40, cy + r * 0.30), 180, 360, fill=HAT, outline=(150, 110, 40))
    # brim
    d.rounded_rectangle((cx - r * 0.52, cy - r * 0.04, cx + r * 0.52, cy + r * 0.10), radius=6, fill=HAT, outline=(150, 110, 40))
    # center ridge
    d.rectangle((cx - r * 0.05, cy - r * 0.32, cx + r * 0.05, cy - r * 0.02), fill=(210, 150, 40))
    # side vents
    d.line((cx - r * 0.22, cy - r * 0.18, cx - r * 0.22, cy - r * 0.02), fill=(210, 150, 40), width=3)
    d.line((cx + r * 0.22, cy - r * 0.18, cx + r * 0.22, cy - r * 0.02), fill=(210, 150, 40), width=3)
    c.alpha_composite(ov)


# aliases (shared render helpers may reference these names across products)
basket_crest = hardhat_crest
compass_crest = hardhat_crest
candle_crest = hardhat_crest
camera_crest = hardhat_crest
house_crest = hardhat_crest
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "REMODEL", font=fs(15), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("TOTAL BUDGET", "$60,000", "whole project"),
    ("TOTAL SPENT", "$42,000", "to date"),
    ("REMAINING", "$18,000", "left"),
    ("BUDGET USED", "70%", "of budget"),
    ("CONTINGENCY", "$9,000", "15% reserve"),
    ("CONTINGENCY LEFT", "$7,000", "used $2k"),
    ("ROOMS", "5", "scoped"),
    ("PAID TO DATE", "$38,000", "to contractors"),
    ("OUTSTANDING", "$4,000", "still owed"),
    ("CHANGE ORDERS", "$3,000", "added"),
    ("% COMPLETE", "65%", "of the work"),
    ("RENO SCORE", "90%", "healthy"),
]

SPEND_MONTHS = [("Feb", 4000), ("Mar", 9000), ("Apr", 7000), ("May", 8000), ("Jun", 8000), ("Jul", 6000)]


def content_dashboard(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Remodel Dashboard", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 64), "Cedar & Stone · Reese  ·  know what it costs, what you've spent, what's left", font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 210, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 105, y0 + 44), "● $18k remaining", font=fs(14), fill=PRIMARY, anchor="mm")
    ...
    d.text((gx, cy_top), "RENO HEALTH · WHERE THE BUDGET STANDS · SPENT BY MONTH", font=fs(19), fill=ACCENT, anchor="lt")
    ...
    d.text((px + 16, panels_y + 14), "Reno Health", font=fs(17), fill=ACCENT, anchor="lt")
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("Under budget", 1.0, "100%"), ("Rooms on budget", 1.0, "100%"), ("Contingency healthy", 1.0, "100%"),
           ("Scope defined", 1.0, "100%"), ("Payments current", 1.0, "100%"), ("Change orders in check", 0.40, "40%")],
          color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Budget Used", font=fs(15), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.44, panels_y + panel_h * 0.50, min(panel_h * 0.27, pw * 0.27),
          [(70, PRIMARY), (30, SURFACE)], "70%", "spent")
    legend(d, px + pw * 0.06, panels_y + panel_h - 96, [(PRIMARY, "Spent $42k"), (SURFACE, "Left $18k")], 15, 32)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Spent by Month", font=fs(17), fill=ACCENT, anchor="lt")
    vbars(img, d, (px + 18, panels_y + 60, px + pw - 14, panels_y + panel_h - 10), SPEND_MONTHS, 9000)
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Reno Score", font=fs(17), fill=ACCENT, anchor="lt")
...
def content_rooms(img, cbox):
    rows = [
        ("Kitchen", "$30,000", "$22,000", "$8,000", "73%"),
        ("Primary Bath", "$15,000", "$11,000", "$4,000", "73%"),
        ("Living Room", "$8,000", "$5,000", "$3,000", "63%"),
        ("Exterior & Paint", "$5,000", "$3,000", "$2,000", "60%"),
        ("Landscaping", "$2,000", "$1,000", "$1,000", "50%"),
    ]
    _table(img, cbox, "Rooms — Budget vs Spent",
           "Every room against its budget — spent, remaining and % used. This is where overruns hide.",
           ["ROOM", "BUDGET", "SPENT", "REMAINING", "% USED"],
           [0.0, 0.40, 0.58, 0.76, 0.92], rows,
           total_row=("Total project", "$60,000", "$42,000", "$18,000", "70%"))


def content_budget(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Budget vs Actual", font=fs(30), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 60), "Your whole project — budget out, spent in, and a contingency reserve for the surprises.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("Total project budget", "$60,000", PRIMARY),
            ("− Spent to date", "−$42,000", DANGER)]
    ...
    d.text((x0 + pad + 14, ry + rh / 2), "= REMAINING BUDGET", font=fs(18), fill=PRIMARY, anchor="lm")
    d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.80, ry + rh / 2), "$18,000", font=fs(18), fill=PRIMARY, anchor="mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2), "+ $9,000 contingency reserve · $7,000 still intact", font=fs(22), fill=WHITE, anchor="mm")
    img.alpha_composite(ov)
...
def render_hero(out):
    ...
    hardhat_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE COMPLETE REMODEL-BUDGET SYSTEM", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "REMODEL BUDGET", 82, max_w=1600)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 546), "A budget-vs-actual engine, rooms, contractors, payments & change orders — one system.",
       fs(20, bold=False), (224, 213, 190))
    chips = [("14", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("BUDGET", "ENGINE")]
    cw = 440
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not a budget sheet — a complete plan-it, track-it, finish-it system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Start Here", "how it all works"), ("Dashboard", "spent & score"),
        ("Budget vs Actual", "the whole project"), ("Rooms", "budget by room"),
        ("Line Items", "every line, tracked"), ("Contractors", "who does what"),
        ("Payments", "paid vs owed"), ("Change Orders", "the budget-buster"),
        ("Materials", "every selection"), ("Timeline", "how far along"),
        ("Financing", "how it's paid"), ("Decisions", "lock the choices"),
        ("Monthly Summary", "the burn rate"), ("Settings", "set it once"),
        ("+ 12 Printable Pages", "worksheets & logs"), ("Live Reno Score", "it all rolls up"),
    ]
    cols = 4
    margin = 88
    gx, gy = 22, 20
...
def render_budget(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "KNOW WHAT'S LEFT", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "The Budget-vs-Actual Engine", fserif(42), WHITE)
    tc(d, (SIZE // 2, 300), "Your whole project's budget, spent and remaining — plus a contingency reserve for surprises",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 1, content_budget)
    img.convert("RGB").save(out, "PNG", optimize=True)


def render_rooms(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "ROOM BY ROOM", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "Every Room, On Budget", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Each room against its budget — spent, remaining and % used, so no room quietly creeps",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 2, content_rooms)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_engine(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 110, "BUDGET IT · TRACK IT", font=fs(32), pad_x=52, pad_y=22)
    tc(d, (SIZE // 2, 224), "The Remodel Engine", fserif(44), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 1, content_budget)
    app_window(img, (60, 1180, SIZE - 60, SIZE - 60), 2, content_rooms)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_printables(out):
    ...
    d = ImageDraw.Draw(img)
    hardhat_crest(img, SIZE // 2, 110, r=48)
    pill(img, SIZE // 2, 214, "PLUS 12 PRINTABLE PAGES", font=fs(30), pad_x=48, pad_y=20)
    tc(d, (SIZE // 2, 300), "Print, Fill & Keep On-Site", fserif(44), WHITE)
    gold_divider(img, SIZE // 2, 362, width=480)
    tc(d, (SIZE // 2, 404), "A matching print-ready pack — budget worksheet, room budget, contractor list, payment log & more",
       fs(21, bold=False), (226, 214, 190))
    labels = ["Budget Worksheet", "Room Budget", "Line Items", "Contractor List",
              "Payment Log", "Change Orders", "Materials", "Timeline",
              "Financing Plan", "Decisions Log", "Monthly Summary", "Remodel Checklist"]
