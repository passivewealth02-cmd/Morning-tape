"""Marketing image set for Homeschool Command Center™ (6 images, 2000x2000).

  01_hero.png       - branded hero + live homeschool dashboard
  02_inside.png     - "everything inside — 18 connected tabs"
  03_students.png   - a profile for every student (distinct sheet)
  04_curriculum.png - curriculum & resources (the planning shot)
  05_plan.png       - lesson planner + attendance & hours
...
TABS = ["Homeschool Dashboard", "Student Profiles", "Year Planner", "Curriculum", "Subject Plan",
...
FILE_LABEL = "Homeschool_Command_Center.xlsx — The Bennett Family · 2026–2027 · 4 students"
...
def book_crest(c, cx, cy, r=56, glow=True):
    """Brand crest with an open book & a small apple — the icon of learning at home."""
    if glow:
    ...
    c.alpha_composite(ov)
    ov = Image.new("RGBA", c.size, (0, 0, 0, 0)); d = ImageDraw.Draw(ov)
    # open book: two pages meeting at a center spine, slight upward V
    bw = r * 0.86; bh = r * 0.5
    by = cy + r * 0.26
    spine_top = cy - r * 0.02
    # left page
    d.polygon([(cx, spine_top), (cx - bw, spine_top - r * 0.12), (cx - bw, by), (cx, by + r * 0.06)],
              fill=GOLD_HI, outline=(160, 130, 78))
    # right page
    d.polygon([(cx, spine_top), (cx + bw, spine_top - r * 0.12), (cx + bw, by), (cx, by + r * 0.06)],
              fill=(240, 220, 170), outline=(160, 130, 78))
    # spine
    d.line((cx, spine_top, cx, by + r * 0.06), fill=(150, 120, 70), width=3)
    # text lines
    for k in range(3):
        yy = spine_top + r * 0.08 + k * r * 0.11
        d.line((cx - bw * 0.82, yy + r * 0.02, cx - bw * 0.12, yy - r * 0.03), fill=(150, 120, 70), width=2)
        d.line((cx + bw * 0.12, yy - r * 0.03, cx + bw * 0.82, yy + r * 0.02), fill=(150, 120, 70), width=2)
    # apple above the book
    ax, ay = cx, cy - r * 0.42
    ar = r * 0.2
    d.ellipse((ax - ar, ay - ar, ax + ar, ay + ar), fill=APPLE_RED, outline=(150, 50, 42))
    d.line((ax, ay - ar, ax, ay - ar - r * 0.12), fill=(120, 80, 50), width=3)
    d.polygon([(ax, ay - ar - r * 0.06), (ax + r * 0.16, ay - ar - r * 0.2), (ax + r * 0.02, ay - ar + r * 0.02)], fill=LEAF)
    c.alpha_composite(ov)
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "HOMESCHOOL", font=fs(16), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("STUDENTS", "4", "in the system"),
    ("SCHOOL DAYS", "120", "of 180"),
    ("HOURS LOGGED", "640", "of 900"),
    ("DAYS LEFT", "60", "remaining"),
    ("SUBJECTS", "8", "core"),
    ("LESSONS DONE", "78%", "checked off"),
    ("GRADED", "85%", "assignments"),
    ("BOOKS READ", "46", "this year"),
    ("CURRICULUM", "$1,240", "of $1,500"),
    ("FIELD TRIPS", "7", "this year"),
    ("GOALS", "72%", "avg progress"),
    ("ON-TRACK", "75%", "blended"),
]
...
def content_dashboard(img, cbox):
    ...
    d.text((x0 + pad, y0 + 22), "Homeschool Dashboard", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 64), "The Bennett Family · 4 students · grades K–8  ·  your whole year, automatically organized", font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 170, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 85, y0 + 44), "● 75% on track", font=fs(16), fill=PRIMARY, anchor="mm")
    ...
    d.text((gx, cy_top), "ON TRACK · BUDGET · COMING UP", font=fs(20), fill=ACCENT, anchor="lt")
    ...
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("School days", 0.67, "67%"), ("Hours", 0.71, "71%"), ("Lessons", 0.78, "78%"),
           ("Graded", 0.85, "85%"), ("Goals", 0.72, "72%")], color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    ...
    donut(d, px + pw * 0.42, panels_y + panel_h * 0.52, min(panel_h * 0.28, pw * 0.27),
          [(57, PRIMARY), (13, ACCENT), (10, HIGHLIGHT), (6, SURFACE), (14, (170, 150, 120))], "$2.2K", "spent")
    legend(d, px + pw * 0.04, panels_y + panel_h - 104, [(PRIMARY, "Curriculum 57%"), (ACCENT, "Lessons 13%"), (HIGHLIGHT, "Co-op 10%")], 15, 30)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Coming Up", font=fs(17), fill=ACCENT, anchor="lt")
    tasks = [("State capitol trip", MINT_BG), ("Botanical gardens", MINT_BG), ("Standardized test window", WARN_BG),
             ("Science fair project", WARN_BG), ("Year-end evaluation", WARN_BG), ("Co-op Friday", MINT_BG)]
    ty = panels_y + 58
    ...
    donut(d, px + pw * 0.5, panels_y + panel_h * 0.54, min(panel_h * 0.30, pw * 0.30),
          [(75, PRIMARY), (25, SURFACE)], "75%", "on track")
...
def content_students(img, cbox):
    rows = [
        ("Ella", "Grade 8", "Classical", "Loves history · strong reader"),
        ("Noah", "Grade 6", "Eclectic", "Hands-on · movement breaks"),
        ("Grace", "Grade 3", "Charlotte Mason", "Nature study · early reader"),
        ("Sam", "Grade K", "Play-based", "Letters, numbers & read-alouds"),
    ]
    _table(img, cbox, "Student Profiles",
           "A profile for every student — grade, approach, strengths & goals",
           ["STUDENT", "GRADE", "APPROACH", "STRENGTHS / FOCUS"],
           [0.0, 0.16, 0.32, 0.56], rows)


def content_curriculum(img, cbox):
    rows = [
        ("Math", "Ella", "Saxon 8/7", "$110", "Have"),
        ("Language Arts", "Ella", "IEW writing", "$120", "Have"),
        ("Science", "All", "Apologia + lab kit", "$130", "Have"),
        ("History", "All", "Story of the World", "$95", "Have"),
        ("Math", "Grace", "Math-U-See", "$85", "Have"),
        ("Foreign Language", "Ella/Noah", "Spanish (online)", "$0", "Need"),
        ("Bible / Character", "All", "Devotional set", "$30", "Have"),
    ]
    _table(img, cbox, "Curriculum & Resources",
           "Every subject, every student — what you use, what it cost & what's still needed.  $1,240 acquired",
           ["SUBJECT", "STUDENT", "RESOURCE", "COST", "STATUS"],
           [0.0, 0.26, 0.44, 0.74, 0.87], rows,
           status_col=4, status_map={"Have": (MINT_BG, PRIMARY), "Ordered": (WARN_BG, ACCENT),
                                     "Need": (RED_BG, DANGER)})


def content_lessons(img, cbox):
    rows = [
        ("Math", "Ella", "Percents", "In Progress"),
        ("Science", "All", "Human body", "In Progress"),
        ("History", "All", "Ancient Rome", "In Progress"),
        ("Language Arts", "Grace", "Short vowels", "Done"),
        ("Geography", "All", "US states", "Not Started"),
        ("Foreign Language", "Ella", "Spanish greetings", "Not Started"),
    ]
    _table(img, cbox, "Lesson Planner",
           "Unit by unit, per subject — plan the scope and check it off as you go.  78% done",
           ["SUBJECT", "STUDENT", "UNIT / LESSON", "STATUS"],
           [0.0, 0.26, 0.44, 0.82], rows,
           status_col=3, status_map={"Done": (MINT_BG, PRIMARY), "In Progress": (WARN_BG, ACCENT),
                                     "Not Started": ((235, 230, 222), TEXT_MUTED)})


def content_attendance(img, cbox):
    rows = [
        ("Weeks 1–4", "20", "108", "Term 1 — strong start"),
        ("Weeks 5–8", "20", "107", "Fall labs + field trip"),
        ("Weeks 9–12", "20", "106", "Ancients unit"),
        ("Weeks 13–16", "20", "108", "Winter term"),
        ("Weeks 17–20", "20", "104", "Spring term"),
        ("Weeks 21–24", "20", "107", "Science fair prep"),
    ]
    _table(img, cbox, "Attendance & Hours",
           "Days & instructional hours by week — the records your state may require.  120 days · 640 hrs",
           ["PERIOD", "DAYS", "HOURS", "NOTES"],
           [0.0, 0.30, 0.44, 0.60], rows,
           total_row=("TOTAL", "120", "640", "of 180 days · 900-hr goal"))
...
def render_hero(out):
    ...
    book_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE COMPLETE HOMESCHOOL PLANNING & RECORDS SYSTEM", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "HOMESCHOOL COMMAND CENTER", 84, max_w=1860)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 550), "Curriculum, lessons, attendance, grades & records — one calm system, any method, 1 to many kids.",
       fs(21, bold=False), (224, 213, 190))
    chips = [("18", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("ANY", "METHOD · ANY GRADE")]
    cw = 440
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 238), "18 Connected Tabs", fserif(58), WHITE)
    gold_divider(img, SIZE // 2, 308, width=520)
    tc(d, (SIZE // 2, 352), "Not a lesson-plan template — a complete homeschool planning & records system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Start Here", "how it all works"), ("Homeschool Dashboard", "live On-Track Score"),
        ("Student Profiles", "a profile per student"), ("Year Planner", "terms, breaks & milestones"),
        ("Curriculum & Resources", "what you use & cost"), ("Subject / Course Plan", "scope & cadence"),
        ("Lesson Planner", "unit by unit"), ("Attendance & Hours", "the records you need"),
        ("Assignments & Grades", "graded work"), ("Reading Log", "the heart of it"),
        ("Field Trips", "learning beyond the table"), ("Co-op & Extracurriculars", "the master schedule"),
        ("Portfolio", "work samples"), ("Budget", "curriculum & co-op costs"),
        ("Goals & Milestones", "the wins that matter"), ("Report Card / Transcript", "print-ready"),
        ("Records & Compliance", "the paper trail"), ("+ 12 Printable Pages", "print & keep"),
    ]
    cols = 4
    margin = 88
    gx, gy = 22, 20
...
def render_students(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "A PROFILE FOR EVERY STUDENT", font=fs(28), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "One Method or Many — It Bends", fserif(46), WHITE)
    tc(d, (SIZE // 2, 300), "Classical, Charlotte Mason, unit study or your own mix — every child, their own plan",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 1, content_students)
    img.convert("RGB").save(out, "PNG", optimize=True)


def render_curriculum(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "PLAN & TRACK CURRICULUM", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "What You Use, What It Cost", fserif(48), WHITE)
    tc(d, (SIZE // 2, 300), "Every subject & student, with what's on the shelf and what's still needed — the budget follows",
       fs(21, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 3, content_curriculum)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_plan(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 110, "PLAN IT · LOG IT · PROVE IT", font=fs(32), pad_x=52, pad_y=22)
    tc(d, (SIZE // 2, 224), "Lessons & Hours, Handled", fserif(44), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 5, content_lessons)
    app_window(img, (60, 1180, SIZE - 60, SIZE - 60), 6, content_attendance)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_printables(out):
    ...
    tc(d, (SIZE // 2, 404), "A matching print-ready PDF pack for the binder, the wall & your records",
       fs(22, bold=False), (226, 214, 190))
    labels = ["Student-at-a-Glance", "Weekly Lesson Plan", "Daily Schedule", "Attendance & Hours",
              "Subject Plan", "Reading Log", "Field-Trip Log", "Grade Sheet",
              "Book List", "Goals", "Records Checklist", "Report Card"]
...
def main():
    ...
    targets = [
        ...
        ("03_students.png", render_students),
        ("04_curriculum.png", render_curriculum),
