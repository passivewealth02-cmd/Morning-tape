"""Detailed marketing image set for Budget & Money Command Center™ (4 images, 2000x2000)."""
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR REAL LIFE", "Where Every Dollar Behaves",
                "Four ways it turns money stress into one calm plan")
    feats = [
        ("$", "Live Budget Health Score", "donut", (80, "80%", "on track"),
         "On-budget, savings rate, bills on time, emergency fund, goals & sinking funds roll into one number — so you always know where you stand at a glance."),
        ("%", "A Zero-Based Budget", "table", None,
         "Give every dollar a job until 'Left to Budget' is $0. Planned vs actual flags every category that goes over the day it happens."),
        ("box", "Savings, Debt & Net Worth", "stock", None,
         "Fund goals & sinking funds, snapshot every debt by rate, and watch assets minus liabilities climb — the real scoreboard for your money."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack — monthly budget, bill tracker, savings & debt trackers, net-worth sheet, subscriptions audit & a no-spend challenge."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("Groceries", "Over"), ("Transportation", "Under"), ("Savings", "On track")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["CATEGORY", "STATUS"], [0.0, 0.62]):
            ...
            od = ImageDraw.Draw(img)
            items = [("Saved this month", "$850", MINT_BG, PRIMARY), ("Total debt", "$30,200", RED_BG, DANGER),
                     ("Net worth", "$111,300", MINT_BG, PRIMARY), ("Sinking funds", "$1,650", MINT_BG, PRIMARY)]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            for ri, (nm, st, bg, fg) in enumerate(items):
                ...
                od.rounded_rectangle((tx1 - 138, ry + 6, tx1 - 12, ry + 32), radius=13, fill=bg)
                od.text((tx1 - 75, ry + 19), st, font=fs(15), fill=fg, anchor="mm")
        elif kind == "print":
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Budget Sheet",
                "Everything a basic budget template leaves out — built in")
    rows = [
        ("A simple monthly budget", True, True),
        ("Zero-based, planned vs actual", False, True),
        ("Bill tracker with due days", False, True),
        ("Tagged expense log", False, True),
        ("Savings goals with progress", False, True),
        ("Sinking funds for big costs", False, True),
        ("Debt snapshot by interest rate", False, True),
        ("Net-worth tracker", False, True),
        ("Subscriptions audit", False, True),
        ("Live Budget Health Score", False, True),
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "SET UP IN AN EVENING", "Run Your Money in 4 Steps",
                "No spreadsheet skills needed — if you can type, you can run it")
    steps = [
        ("1", "Add your income", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack, then list every paycheck & source for the month."),
        ("2", "Give every dollar a job", "Build your zero-based budget until 'Left to Budget' hits $0 — every dollar assigned to a category before the month starts."),
        ("3", "Track as you go", "Log expenses and mark bills paid; planned-vs-actual flags any category going over, so nothing sneaks up on you."),
        ("4", "Build wealth", "Fund goals & sinking funds, attack debt by interest rate, and update net worth monthly — watch your Health Score climb."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "EVERYTHING YOU GET", "One System, Whole Money Life",
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + Health Score",
        "Zero-based monthly budget", "Bill tracker & tagged expense log",
        "Savings goals & sinking funds", "Debt snapshot & net-worth tracker",
        "Subscriptions audit & no-spend", "Start-Here guide + free updates",
    ]
    ...
    who = ["Budget beginners & nerds alike", "Couples & families sharing money", "Debt-payoff & FIRE journeys",
           "Anyone living paycheck-to-paycheck", "Freelancers with variable income", "New savers building a first fund"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "EVERY DOLLAR A JOB · BUILD REAL WEALTH", font=fs(27), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Budget with confidence, crush debt & watch your net worth grow — month after month.",
       fs(23, bold=False), TEXT_MUTED)
    book_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "BUDGET COMMAND CENTER", 52, max_w=1300)
    img.convert("RGB").save(out, "PNG", optimize=True)
