"""Marketing image set for Estate & Emergency Organizer Command Center™ (6 images, 2000x2000)."""
...
TABS = ["Dashboard", "Estate Snapshot", "Assets & Accounts", "Debts & Bills", "Beneficiaries",
        "Legal Documents", "Insurance", "Digital Life", "Key Contacts", "Medical & Care",
        "Final Wishes", "Household", "Settings"]

FILE_LABEL = "Estate_Organizer_Command_Center.xlsx — Lantern & Oak · Margot"
...
def lantern_crest(c, cx, cy, r=56, glow=True):
    """Brand crest — a lantern: the light you leave on for the people who come after."""
    if glow:
    ...
    ov = Image.new("RGBA", c.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    # handle
    od.arc((cx - r * 0.26, cy - r * 0.72, cx + r * 0.26, cy - r * 0.34), 180, 360, fill=GOLD_HI, width=max(3, int(r * 0.07)))
    # top cap
    od.polygon([(cx - r * 0.40, cy - r * 0.34), (cx + r * 0.40, cy - r * 0.34),
                (cx + r * 0.28, cy - r * 0.46), (cx - r * 0.28, cy - r * 0.46)], fill=GOLD_HI)
    # glass body
    od.rounded_rectangle((cx - r * 0.32, cy - r * 0.30, cx + r * 0.32, cy + r * 0.34), radius=int(r * 0.08),
                         fill=(238, 248, 245), outline=GOLD_HI, width=3)
    # the flame
    od.ellipse((cx - r * 0.11, cy - r * 0.12, cx + r * 0.11, cy + r * 0.18), fill=HIGHLIGHT)
    od.polygon([(cx, cy - r * 0.26), (cx + r * 0.09, cy - r * 0.04), (cx - r * 0.09, cy - r * 0.04)], fill=HIGHLIGHT)
    # base
    od.polygon([(cx - r * 0.40, cy + r * 0.48), (cx + r * 0.40, cy + r * 0.48),
                (cx + r * 0.30, cy + r * 0.34), (cx - r * 0.30, cy + r * 0.34)], fill=GOLD_HI)
    c.alpha_composite(ov)


square_crest = lantern_crest
acorn_crest = lantern_crest
...
def vbars(img, d, box, items, maxv, color=(PRIMARY_LT, PRIMARY)):
    ...
    for i, (lab, val) in enumerate(items):
        ...
        d.text((bx + bw / 2, y1 - 52 - bh), f"${val/1000:.0f}k", font=fs(13), fill=PRIMARY, anchor="mb")
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "THE ESTATE", font=fs(15), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("TOTAL ASSETS", "$1,499,900", "everything owned"),
    ("TOTAL DEBTS", "$244,200", "everything owed"),
    ("NET ESTATE", "$1,255,700", "what's left"),
    ("EXPOSED TO PROBATE", "$535,500", "no beneficiary named"),
    ("PROBATE SHARE", "35.7%", "of the estate"),
    ("ROUGH PROBATE COST", "$26,775", "at 5%"),
    ("CASH THEY CAN REACH", "$45,400", "in days, not months"),
    ("MONTHS OF RUNWAY", "7.3", "before anything releases"),
    ("LIFE INSURANCE", "$500,000", "covers debts 2.05x"),
    ("DOCUMENTS SIGNED", "4", "of the 10 that matter"),
    ("ACCOUNTS DOCUMENTED", "21", "assets + digital"),
    ("READINESS SCORE", "90%", "healthy"),
]

REV_MONTHS = [("Cash", 45400), ("Retire", 419000), ("Invest", 84000), ("Home", 410000), ("Vehicle", 27500), ("Life", 500000)]


def content_dashboard(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Readiness Dashboard", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 64), "Lantern & Oak \u00b7 Margot  \u00b7  everything they'd need to find",
           font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 240, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=RED_BG)
    d.text((x1 - pad - 120, y0 + 44), "\u25cf $535,500 in probate", font=fs(14), fill=(150, 50, 50), anchor="mm")
    ...
    d.text((gx, cy_top), "READINESS \u00b7 WHAT WOULD GO THROUGH PROBATE \u00b7 WHERE THE ESTATE SITS", font=fs(19), fill=ACCENT, anchor="lt")
    ...
    d.text((px + 16, panels_y + 14), "Readiness", font=fs(17), fill=ACCENT, anchor="lt")
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("Survivors could keep going", 1.0, "100%"), ("Beneficiaries named", 1.0, "100%"),
           ("Insurance covers debts", 1.0, "100%"), ("Key contacts listed", 1.0, "100%"),
           ("Digital life documented", 1.0, "100%"), ("Core documents signed", 0.40, "40%")],
          color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Probate Exposure", font=fs(15), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.44, panels_y + panel_h * 0.46, min(panel_h * 0.25, pw * 0.25),
          [(35.7, DANGER), (64.3, PRIMARY)], "36%", "in probate")
    legend(d, px + pw * 0.06, panels_y + panel_h - 118,
           [(DANGER, "Probate $535,500"), (PRIMARY, "Skips probate $964,400"),
            (SURFACE, "Rough cost $26,775")], 14, 30)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Where the Estate Sits", font=fs(16), fill=ACCENT, anchor="lt")
    vbars(img, d, (px + 18, panels_y + 60, px + pw - 14, panels_y + panel_h - 10), REV_MONTHS, 500000)
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Readiness Score", font=fs(17), fill=ACCENT, anchor="lt")
...
def content_documents(img, cbox):
    rows = [
        ("Will", "Signed", "Fire safe + attorney", "06/2023"),
        ("Healthcare power of attorney", "Signed", "Fire safe + doctor", "06/2023"),
        ("Financial power of attorney", "NOT STARTED", "\u2014", "\u2014"),
        ("Living will / directive", "NOT STARTED", "\u2014", "\u2014"),
        ("Letter of instruction", "NOT STARTED", "\u2014", "\u2014"),
    ]
    _table(img, cbox, "The Ten Documents That Decide Everything",
           "Most people have a will and nothing else \u2014 the other nine get used first.",
           ["DOCUMENT", "STATUS", "WHERE IT'S KEPT", "DATE"],
           [0.0, 0.42, 0.60, 0.86], rows,
           total_row=("4 signed of 10", "40% ready", "6 not started", ""))


def content_snapshot(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "The Estate Snapshot", font=fs(30), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 60), "What you own, what you owe \u2014 and how much of it has to go through probate.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("Total assets", "$1,499,900", PRIMARY),
            ("\u2212 Total debts", "$244,200", ACCENT),
            ("= NET ESTATE", "$1,255,700", PRIMARY),
            ("Assets with no beneficiary or joint owner", "$535,500", DANGER)]
    ...
    d.text((x0 + pad + 14, ry + rh / 2), "= SHARE OF THE ESTATE EXPOSED TO PROBATE", font=fs(18), fill=PRIMARY, anchor="lm")
    d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.80, ry + rh / 2), "35.7%", font=fs(18), fill=PRIMARY, anchor="mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2 - 20), "Roughly $26,775 in cost \u2014 and months before anything is released", font=fs(21), fill=WHITE, anchor="mm")
    od.text(((x0 + x1) / 2, by + callout_h / 2 + 24), "A BENEFICIARY FORM TAKES TEN MINUTES AND SKIPS ALL OF IT", font=fs(21), fill=GOLD_HI, anchor="mm")
    img.alpha_composite(ov)
...
def render_hero(out):
    ...
    lantern_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE COMPLETE ESTATE & EMERGENCY SYSTEM", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "IN CASE", 86, max_w=1550)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 546), "Assets, debts, beneficiaries, documents, contacts & final wishes \u2014 one place.",
       fs(20, bold=False), (224, 213, 190))
    chips = [("14", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("PROBATE", "EXPOSURE ENGINE")]
    cw = 440
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not a binder \u2014 a complete list-it, name-it, tell-them system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Start Here", "how it all works"), ("Dashboard", "readiness at a glance"),
        ("Estate Snapshot", "what's in probate"), ("Assets & Accounts", "everything owned"),
        ("Debts & Bills", "who to call"), ("Beneficiaries", "the forms that win"),
        ("Legal Documents", "the ten that matter"), ("Insurance", "so it gets claimed"),
        ("Digital Life", "where logins live"), ("Key Contacts", "who to reach first"),
        ("Medical & Care", "for the fridge"), ("Final Wishes", "in your words"),
        ("Household", "the shut-off & the dog"), ("Settings", "set it once"),
        ("+ 12 Printable Pages", "forms & checklists"), ("Live Readiness Score", "it all rolls up"),
    ]
    cols = 4
    margin = 88
    gx, gy = 22, 20
...
def render_snapshot(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "35.7% IS HEADING FOR PROBATE", font=fs(28), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "The Estate Snapshot", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Most people have no idea how much of their estate has no beneficiary on it",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 1, content_snapshot)
    img.convert("RGB").save(out, "PNG", optimize=True)


def render_documents(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "A WILL IS NOT ENOUGH", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "The Ten Documents", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Honestly scored \u2014 and the nine most people are missing get used first",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 5, content_documents)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_engine(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 110, "LIST IT \u00b7 NAME IT", font=fs(32), pad_x=52, pad_y=22)
    tc(d, (SIZE // 2, 224), "The Readiness Engine", fserif(44), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 1, content_snapshot)
    app_window(img, (60, 1180, SIZE - 60, SIZE - 60), 5, content_documents)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_printables(out):
    ...
    d = ImageDraw.Draw(img)
    lantern_crest(img, SIZE // 2, 110, r=48)
    pill(img, SIZE // 2, 214, "PLUS 12 PRINTABLE PAGES", font=fs(30), pad_x=48, pad_y=20)
    tc(d, (SIZE // 2, 300), "Print, Fill & Put It Somewhere Safe", fserif(40), WHITE)
    gold_divider(img, SIZE // 2, 362, width=480)
    tc(d, (SIZE // 2, 404), "A matching print-ready pack \u2014 if-something-happens page, document checklist, final wishes & more",
       fs(21, bold=False), (226, 214, 190))
    labels = ["If Something Happens", "Estate Snapshot", "Assets & Accounts", "Debts & Bills",
              "Beneficiary Review", "Document Checklist", "Insurance Policies", "Digital Life",
              "Key Contacts", "Medical & Care", "Final Wishes", "Household"]
...
def main():
    ...
    targets = [
        ...
        ("03_snapshot.png", render_snapshot),
        ("04_documents.png", render_documents),
