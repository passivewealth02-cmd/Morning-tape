"""Detailed marketing image set for Estate & Emergency Organizer Command Center™ (07-10)."""
...
def render_features(out):
    ...
    header_band(img, "FOR THE PEOPLE WHO COME AFTER", "List It, Name It, Tell Them",
                "Four ways it turns a filing cabinet into an answer")
    feats = [
        ("$", "Live Readiness Score", "donut", (90, "90%", "healthy"),
         "Survivor runway, beneficiaries named, insurance cover, key contacts, digital life and signed documents roll into one number \u2014 how ready you actually are, not how ready you feel."),
        ("%", "Probate Exposure Engine", "table", None,
         "Adds up what you own, subtracts what you owe, then shows the share with no beneficiary and no joint owner on it \u2014 the part that has to go through probate."),
        ("box", "Everything in One Place", "stock", None,
         "Assets, debts, beneficiaries, the ten legal documents, insurance, digital logins, key contacts, medical wishes, final wishes and how the house actually works."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack \u2014 an if-something-happens page for the front, a document checklist, a medical page for the fridge and final wishes in your own words."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("Net estate", "$1,255,700"), ("Exposed to probate", "$535,500"), ("Rough probate cost", "$26,775")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["THE SNAPSHOT", "VALUE"], [0.0, 0.62]):
            ...
            od = ImageDraw.Draw(img)
            items = [("Beneficiaries named", "7 of 7", MINT_BG, PRIMARY), ("Key contacts", "12", MINT_BG, PRIMARY),
                     ("Survivor runway", "7.3 mo", MINT_BG, PRIMARY), ("Documents signed", "4 of 10", SURFACE, (110, 88, 58))]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Printable Binder",
                "Everything a fill-in-the-blank estate binder leaves out \u2014 built in")
    rows = [
        ("Somewhere to write it all down", True, True),
        ("Assets and debts actually added up", False, True),
        ("Net estate calculated for you", False, True),
        ("Shows what would go through PROBATE", False, True),
        ("Rough probate cost estimated", False, True),
        ("Cash a survivor can reach in days", False, True),
        ("Months of runway before anything releases", False, True),
        ("Beneficiary gaps flagged in red", False, True),
        ("The ten documents, honestly scored", False, True),
        ("Live Readiness Score dashboard", False, True),
    ...
    d.text((c1, top + hh // 2), "A Printable\nBinder", font=fs(18), fill=(210, 210, 205), anchor="mm")
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "ONE AFTERNOON, ONCE", "Get It Done in 4 Steps",
                "No spreadsheet skills needed \u2014 if you can type, you can finish this")
    steps = [
        ("1", "List what you own and owe", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack, then list every account, policy and debt \u2014 and, for each one, how it transfers and where to find it."),
        ("2", "See what's exposed", "It adds it all up, subtracts the debts, and shows you the share with no beneficiary and no joint owner \u2014 the part that would have to go through probate."),
        ("3", "Fix the gaps it finds", "Beneficiary forms take ten minutes each and skip probate entirely. The document checklist tells you honestly which of the ten you actually have."),
        ("4", "Then tell one person", "Fill in contacts, medical, digital logins, final wishes and how the house works \u2014 print the pack, put it somewhere safe, and tell somebody it exists."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "EVERYTHING YOU GET", "One System, Whole Estate",
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + Readiness Score",
        "Probate exposure engine", "Survivor cash & runway",
        "Beneficiary & document checklists", "Insurance, contacts & digital life",
        "Medical, final wishes & household", "Start-Here quick-start guide",
    ]
    ...
    who = ["Anyone getting their affairs in order", "Retirees and near-retirees",
           "Adult children helping a parent", "Newly widowed, sorting an estate",
           "Anyone with a will and nothing else", "Caregivers & family executors"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "LIST IT \u00b7 NAME IT \u00b7 TELL THEM", font=fs(27), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "An organizing tool, not legal advice \u2014 but the kindest afternoon's work there is.",
       fs(23, bold=False), TEXT_MUTED)
    square_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "IN CASE", 58, max_w=1150)
    img.convert("RGB").save(out, "PNG", optimize=True)
