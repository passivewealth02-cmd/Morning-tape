"""Marketing image set for Relationship & Couples Command Center™ (6 images, 2000x2000)."""
...
TABS = ["Dashboard", "Fair Share", "Shared Bills", "Invisible Labour", "Money Goals", "Savings",
        "Date Nights", "Weekly Check-In", "Big Conversations", "Household Admin",
        "Individual Money", "Monthly Summary", "Settings"]

FILE_LABEL = "Couples_Command_Center.xlsx — Nadia & Sam"
...
def rings_crest(c, cx, cy, r=56, glow=True):
    """Brand crest — two interlocking rings: separate, and joined."""
    if glow:
    ...
    c.alpha_composite(ov)
    ov = Image.new("RGBA", c.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    rr = r * 0.30; off = r * 0.26; w = max(4, int(r * 0.09))
    # left ring, gold
    od.ellipse((cx - off - rr, cy - rr, cx - off + rr, cy + rr), outline=GOLD_HI, width=w)
    # right ring, mint — drawn over, so they read as linked
    od.ellipse((cx + off - rr, cy - rr, cx + off + rr, cy + rr), outline=HIGHLIGHT, width=w)
    # a bright node where the two actually meet
    for sy in (-1, 1):
        ny = cy + sy * (rr ** 2 - off ** 2) ** 0.5 if rr > off else cy
        od.ellipse((cx - r * 0.045, ny - r * 0.045, cx + r * 0.045, ny + r * 0.045), fill=(238, 248, 245))
    c.alpha_composite(ov)


square_crest = rings_crest
spark_crest = rings_crest
...
def vbars(img, d, box, items, maxv, color=(PRIMARY_LT, PRIMARY), fmt=None):
    ...
    for i, (lab, val) in enumerate(items):
        ...
        txt = fmt(val) if fmt else f"${val/1000:.0f}k"
        d.text((bx + bw / 2, y1 - 52 - bh), txt, font=fs(13), fill=PRIMARY, anchor="mb")
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "TOGETHER", font=fs(15), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("SHARED BILLS", "$3,908", "every month"),
    ("50/50 LEAVES ONE OF YOU", "$2,246", "to live on"),
    ("50/50 LEAVES THE OTHER", "$4,346", "to live on"),
    ("THE 50/50 GAP", "$2,100", "a month, quietly"),
    ("FAIR SHARE \u2014 A", "$1,563", "40% of the bills"),
    ("FAIR SHARE \u2014 B", "$2,345", "60% of the bills"),
    ("YOU EACH THEN KEEP", "62.8%", "identical, both of you"),
    ("HOW CLOSE TO FAIR", "100%", "you've adopted it"),
    ("CHORE SPLIT", "2.88\u00d7", "one of you does more"),
    ("EXTRA HOURS A YEAR", "780", "19.5 working weeks"),
    ("SAVED THIS MONTH", "$940", "together"),
    ("TOGETHER SCORE", "90%", "one thing to fix"),
]

REV_MONTHS = [("Feb", 780), ("Mar", 810), ("Apr", 845), ("May", 880), ("Jun", 910), ("Jul", 940)]


def content_dashboard(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "The Two of You", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 64), "Nadia & Sam  \u00b7  money split fairly, hours counted honestly",
           font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 250, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 125, y0 + 44), "\u25cf you each keep 62.8%", font=fs(14), fill=PRIMARY, anchor="mm")
    ...
    for i, (lab, val, sub) in enumerate(KPIS):
        ...
        lf = fit_font(d, lab, kw - 24, 12)
        d.text((kx + 14, ky + 16), lab, font=lf, fill=ACCENT, anchor="lt")
        vf = fit_font(d, val, kw - 28, 30)
        d.text((kx + 14, ky + 58), val, font=vf, fill=PRIMARY, anchor="lm")
        d.text((kx + 14, ky + 96), sub, font=fs(11, bold=False), fill=TEXT_MUTED, anchor="lm")
    cy_top = gy + 2 * (kh + gap) + 18
    d.text((gx, cy_top), "HOW YOU'RE DOING \u00b7 THE HOURS EACH WEEK \u00b7 SAVED BY MONTH", font=fs(19), fill=ACCENT, anchor="lt")
    ...
    d.text((px + 16, panels_y + 14), "How You're Doing", font=fs(17), fill=ACCENT, anchor="lt")
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("Bills split fairly", 1.0, "100%"), ("Weekly check-ins", 1.0, "100%"),
           ("Saving together", 1.0, "100%"), ("Goals on track", 1.0, "100%"),
           ("Date nights happening", 1.0, "100%"), ("Housework shared", 0.40, "40%")],
          color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "The Hours Each Week", font=fs(15), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.44, panels_y + panel_h * 0.46, min(panel_h * 0.25, pw * 0.25),
          [(74.2, DANGER), (25.8, PRIMARY)], "74%", "one of you")
    legend(d, px + pw * 0.06, panels_y + panel_h - 118,
           [(DANGER, "Nadia 23.0 hrs"), (PRIMARY, "Sam 8.0 hrs"),
            (SURFACE, "780 extra hrs a year")], 14, 30)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Saved by Month", font=fs(17), fill=ACCENT, anchor="lt")
    vbars(img, d, (px + 18, panels_y + 60, px + pw - 14, panels_y + panel_h - 10), REV_MONTHS, 940,
          fmt=lambda v: f"${int(v)}")
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Together Score", font=fs(17), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.5, panels_y + panel_h * 0.54, min(panel_h * 0.30, pw * 0.30),
          [(90, PRIMARY), (10, SURFACE)], "90%", "1 to fix")
...
def content_labour(img, cbox):
    rows = [
        ("Cooking", "5.5", "0.5", "5.0", "Nadia"),
        ("Planning & remembering", "4.0", "0.5", "3.5", "Nadia"),
        ("Cleaning", "4.0", "1.0", "3.0", "Nadia"),
        ("Admin, bills & appointments", "3.0", "0.5", "2.5", "Nadia"),
        ("Yard, car & repairs", "0.0", "3.0", "3.0", "Sam"),
    ]
    _table(img, cbox, "Invisible Labour \u2014 Hours a Week",
           "Fill it in separately, then compare. The gap is the conversation, not the verdict.",
           ["TASK", "NADIA", "SAM", "GAP", "FALLS ON"],
           [0.0, 0.44, 0.55, 0.66, 0.80], rows,
           total_row=("HOURS A WEEK", "23.0", "8.0", "15.0", "780 hrs a year"))


def content_fairshare(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "The Fair-Share Engine", font=fs(30), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 60), "Split the bills by what you each earn, not down the middle. Watch what changes.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("Incomes \u2014 $4,200 and $6,300", "40% / 60%", PRIMARY),
            ("50/50 split leaves you with", "$2,246 / $4,346", DANGER),
            ("Proportional split leaves you with", "$2,637 / $3,955", PRIMARY),
            ("\u2026which as a share of your OWN income is", "62.8% / 62.8%", PRIMARY)]
    callout_h = 165; gap = 48; rh = 124
    ...
    for h, fx in zip(["THE SPLIT", "NADIA / SAM"], [0.0, 0.76]):
    ...
    for i, (label, val, col) in enumerate(rows):
        ...
        d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.76, ry + rh / 2), val, font=fs(18, bold=True), fill=col, anchor="mm")
    ry = ty + rh + len(rows) * rh
    d.rectangle((x0 + pad, ry, x1 - pad, ry + rh), fill=SURFACE)
    d.text((x0 + pad + 14, ry + rh / 2), "= THE GAP 50/50 QUIETLY CREATES", font=fs(18), fill=PRIMARY, anchor="lm")
    d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.76, ry + rh / 2), "$2,100 a month", font=fs(18), fill=DANGER, anchor="mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2 - 20), "Same house. Same bills. Completely different life.", font=fs(22), fill=WHITE, anchor="mm")
    od.text(((x0 + x1) / 2, by + callout_h / 2 + 24), "50/50 IS NOT THE SAME AS FAIR", font=fs(24), fill=GOLD_HI, anchor="mm")
    img.alpha_composite(ov)
...
def render_hero(out):
    ...
    rings_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE COMPLETE SHARED-LIFE SYSTEM", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "BOTH OF YOU", 86, max_w=1550)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 546), "Fair-share bills, invisible labour, goals, check-ins & the big conversations.",
       fs(20, bold=False), (224, 213, 190))
    chips = [("14", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("FAIR", "SHARE ENGINE")]
    cw = 440
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not a budget \u2014 a whole shared life, split fairly",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Start Here", "how to use it well"), ("Dashboard", "the two of you"),
        ("Fair Share", "50/50 vs proportional"), ("Shared Bills", "everything, listed"),
        ("Invisible Labour", "the hours, honestly"), ("Money Goals", "what you're building"),
        ("Savings", "named pots"), ("Date Nights", "who plans them"),
        ("Weekly Check-In", "fifteen minutes"), ("Big Conversations", "the avoided ones"),
        ("Household Admin", "whose job is it"), ("Individual Money", "what stays yours"),
        ("Monthly Summary", "money & hours"), ("Settings", "set it once"),
        ("+ 12 Printable Pages", "worksheets & prompts"), ("Live Together Score", "it all rolls up"),
    ]
    cols = 4
    margin = 88
    gx, gy = 22, 20
    ...
    for i, (title, sub) in enumerate(cards):
        ...
        sf = fit_font(od, sub, cw - 84, 13, serif=False)
        od.text((x + 76, cyc + 18), sub, font=sf, fill=TEXT_MUTED, anchor="lm")
        img.alpha_composite(ov)
    img.convert("RGB").save(out, "PNG", optimize=True)


def render_fairshare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "50/50 IS NOT THE SAME AS FAIR", font=fs(27), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "The Fair-Share Engine", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Splitting the bills down the middle quietly leaves a $2,100 a month gap",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 1, content_fairshare)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_labour(out):
    ...
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "780 EXTRA HOURS A YEAR", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "The Invisible Labour Split", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Both of you fill it in separately \u2014 the gap is the conversation",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 3, content_labour)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_engine(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 110, "SPLIT IT \u00b7 SHARE IT", font=fs(32), pad_x=52, pad_y=22)
    tc(d, (SIZE // 2, 224), "The Fairness Engine", fserif(44), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 1, content_fairshare)
    app_window(img, (60, 1180, SIZE - 60, SIZE - 60), 3, content_labour)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_printables(out):
    ...
    d = ImageDraw.Draw(img)
    rings_crest(img, SIZE // 2, 110, r=48)
    pill(img, SIZE // 2, 214, "PLUS 12 PRINTABLE PAGES", font=fs(30), pad_x=48, pad_y=20)
    tc(d, (SIZE // 2, 300), "Print, Fill In Separately, Compare", fserif(40), WHITE)
    gold_divider(img, SIZE // 2, 362, width=480)
    tc(d, (SIZE // 2, 404), "A matching print-ready pack \u2014 fair-share worksheet, the labour split, check-in & big conversations",
       fs(21, bold=False), (226, 214, 190))
    labels = ["Fair Share", "Shared Bills", "Invisible Labour", "The Conversation",
              "Money Goals", "Weekly Check-In", "Big Conversations", "Date Nights",
              "Household Admin", "Individual Money", "Month in Review", "If Incomes Change"]
...
def main():
    ...
    targets = [
        ...
        ("03_fairshare.png", render_fairshare),
        ("04_labour.png", render_labour),
