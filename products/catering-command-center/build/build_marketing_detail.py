"""Detailed marketing image set for Catering Command Center™ (07-10)."""
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR CATERERS", "Where Catering Makes Money",
                "Four ways it turns every event into real profit")
    feats = [
        ("$", "Live Catering Score", "donut", (90, "90%", "healthy"),
         "Food cost, event margin, packages costed, labor, bookings & gross margin roll into one number — so you know your catering business's financial health at a glance."),
        ("%", "Cost by the Head", "table", None,
         "Cost each plate by the head — protein, sides, dessert & overhead — then price packages with a healthy margin. The secret to real catering pricing."),
        ("box", "Every Quote Is a P&L", "stock", None,
         "Guests × package, plus service, staff & rentals — every quote becomes a full event P&L, so you know the profit on a booking before you commit the kitchen."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack — plate cost card, package price list, event quote & run sheet, staffing sheet & a client contact sheet."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("Cost per head", "$14.00"), ("Price per head", "$48.00"), ("Margin / head", "$34.00")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["PLATED DINNER", "VALUE"], [0.0, 0.62]):
            ...
            od = ImageDraw.Draw(img)
            items = [("Total booked revenue", "$21,560", MINT_BG, PRIMARY), ("Avg event value", "$3,593", MINT_BG, PRIMARY),
                     ("Avg event margin", "32%", SURFACE, (110, 88, 58)), ("Events booked", "6", MINT_BG, PRIMARY)]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    ...
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Price List",
                "Everything a basic catering sheet leaves out — built in")
    rows = [
        ("A package price list", True, True),
        ("Plate cost-per-head engine", False, True),
        ("Margin & food-cost % per package", False, True),
        ("Event quote = full event P&L", False, True),
        ("Staffing & labor rate card", False, True),
        ("Rentals & equipment tracker", False, True),
        ("Bookings calendar & deposits", False, True),
        ("Inventory, waste & ordering", False, True),
        ("Cash, deposits & client CRM", False, True),
        ("Live Catering Score dashboard", False, True),
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "SET UP IN AN AFTERNOON", "Price Your Catering in 4 Steps",
                "No spreadsheet skills needed — if you can type, you can run it")
    steps = [
        ("1", "Cost your plates", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack, then cost each plate by the head — protein, sides, dessert & overhead."),
        ("2", "Price your packages", "See margin and food-cost % on every package — set a price per head with a healthy margin instead of guessing at a number."),
        ("3", "Quote every event", "Build a quote that becomes a full event P&L — guests × package, plus service, staff & rentals — so you know the profit before you say yes."),
        ("4", "Run the business", "Track staffing, rentals, bookings, inventory, waste, ordering & deposits, and watch your Catering Score climb."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + Catering Score",
        "Plate cost-per-head engine", "Package pricing & margins",
        "Event quote & P&L builder", "Staffing, rentals & bookings",
        "Inventory, waste, cash & clients", "Start-Here guide + free updates",
    ]
    ...
    who = ["Full-service caterers", "Private & personal chefs", "Wedding & event caterers",
           "Corporate & drop-off catering", "Food trucks adding catering", "Anyone catering for profit"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "COST THE HEAD · BOOK MORE PROFIT", font=fs(27), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Cost every head, quote with confidence, and keep more of every dollar you book.",
       fs(23, bold=False), TEXT_MUTED)
    book_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "CATERING COMMAND CENTER", 52, max_w=1200)
    img.convert("RGB").save(out, "PNG", optimize=True)
