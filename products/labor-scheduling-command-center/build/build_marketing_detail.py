"""Detailed marketing image set for Restaurant Labor & Scheduling Command Center™ (07-10)."""
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR OPERATORS", "Where Labor Is Won",
                "Four ways it puts your biggest controllable cost back under control")
    feats = [
        ("$", "Live Labor Score", "donut", (90, "90%", "healthy"),
         "Labor %, sales per labor hour, overtime, prime cost, coverage & wage roll into one number — so you know your labor health at a glance."),
        ("%", "Labor % Live", "table", None,
         "Build the weekly schedule and every shift's hours and cost calculate as you type; labor ÷ sales gives a true labor %, instantly."),
        ("box", "Schedule to Your Sales", "stock", None,
         "Forecast sales to set a labor target, track sales per labor hour, watch overtime, and roll food + labor into a prime-cost number."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack — blank weekly schedule, roster, sales forecast, overtime log, time-off request & a tip sheet."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("Scheduled labor", "$4,234"), ("÷ Weekly sales", "$15,000"), ("Labor %", "28.2%")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["THE WEEK", "VALUE"], [0.0, 0.62]):
            ...
            od = ImageDraw.Draw(img)
            items = [("Sales per labor hour", "$53.57", MINT_BG, PRIMARY), ("Labor target (30%)", "$4,500", MINT_BG, PRIMARY),
                     ("Under budget", "$266", MINT_BG, PRIMARY), ("Prime cost", "58.2%", SURFACE, (110, 88, 58))]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Blank Schedule",
                "Everything a basic schedule template leaves out — built in")
    rows = [
        ("A blank weekly schedule", True, True),
        ("Live hours & cost per employee", False, True),
        ("A true labor-% engine", False, True),
        ("Sales forecast → labor target", False, True),
        ("Sales per labor hour", False, True),
        ("Overtime watch list", False, True),
        ("Prime cost (food + labor)", False, True),
        ("Roles, rates & availability", False, True),
        ("Tips & tip-out tracker", False, True),
        ("Live Labor Score dashboard", False, True),
    ...
    d.text((c1, top + hh // 2), "A Blank\nSchedule", font=fs(18), fill=(210, 210, 205), anchor="mm")
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "SET UP IN AN AFTERNOON", "Control Labor in 4 Steps",
                "No spreadsheet skills needed — if you can type, you can run it")
    steps = [
        ("1", "Add your team", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack, then add your employees, roles and wages."),
        ("2", "Build the schedule", "Type each shift's hours in the weekly grid — every employee's hours and labor cost calculate live, and so does your labor %."),
        ("3", "Forecast your sales", "Enter a sales forecast and the system sets a labor target at your target % — so you know what you can spend before you publish."),
        ("4", "Protect your prime", "Watch sales per labor hour and overtime, roll food + labor into prime cost, and watch your Labor Score climb."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + Labor Score",
        "Live weekly schedule grid", "True labor-% engine",
        "Sales forecast → labor target", "Sales per labor hour & overtime",
        "Prime cost, roles, tips & more", "Start-Here guide + free updates",
    ]
    ...
    who = ["Restaurants & cafés", "Bars & pubs", "Quick-service & fast-casual",
           "Coffee shops & bakeries", "Managers & shift leads", "Anyone who builds a schedule"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "SCHEDULE TO YOUR SALES", font=fs(27), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Build a smarter schedule, protect your prime cost, and never over-spend on labor again.",
       fs(23, bold=False), TEXT_MUTED)
    book_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "LABOR & SCHEDULING", 60, max_w=1280)
    img.convert("RGB").save(out, "PNG", optimize=True)
