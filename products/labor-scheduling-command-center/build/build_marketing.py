"""Marketing image set for Restaurant Labor & Scheduling Command Center™ (6 images, 2000x2000)."""
...
TABS = ["Dashboard", "Labor Cost Calc", "Weekly Schedule", "Employees", "Sales Forecast", "Sales per Labor Hr",
        "Overtime", "Roles & Rates", "Availability", "Prime Cost", "Tips", "Labor by Day", "Settings"]

FILE_LABEL = "Labor_Scheduling_Command_Center.xlsx — Maple & Ash · Jordan"
...
def clock_crest(c, cx, cy, r=56, glow=True):
    """Brand crest — a clock face: the icon of the shift & the schedule."""
    if glow:
    ...
    # clock face
    fr = r * 0.42
    d.ellipse((cx - fr, cy - fr, cx + fr, cy + fr), fill=GOLD_HI, outline=(150, 120, 70), width=3)
    # tick marks at 12/3/6/9
    import math
    for ang in (0, 90, 180, 270):
        a = math.radians(ang)
        x1 = cx + (fr - r * 0.06) * math.sin(a); y1 = cy - (fr - r * 0.06) * math.cos(a)
        x2 = cx + fr * 0.92 * math.sin(a); y2 = cy - fr * 0.92 * math.cos(a)
        d.line((x1, y1, x2, y2), fill=PRIMARY_DK, width=3)
    # hands — pointing to ~10:10
    d.line((cx, cy, cx - fr * 0.42, cy - fr * 0.30), fill=PRIMARY, width=6)   # hour hand
    d.line((cx, cy, cx + fr * 0.30, cy - fr * 0.60), fill=PRIMARY, width=5)   # minute hand
    d.ellipse((cx - r * 0.05, cy - r * 0.05, cx + r * 0.05, cy + r * 0.05), fill=PRIMARY_DK)
    c.alpha_composite(ov)


clipboard_crest = clock_crest
cloche_crest = clock_crest
ghost_crest = clock_crest
mug_crest = clock_crest
book_crest = clock_crest
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "LABOR", font=fs(16), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("LABOR COST", "$4,234", "this week"),
    ("LABOR %", "28.2%", "of sales"),
    ("WEEKLY SALES", "$15,000", "forecast"),
    ("TOTAL HOURS", "280", "scheduled"),
    ("SALES / LABOR HR", "$53.57", "productivity"),
    ("EMPLOYEES", "8", "on the team"),
    ("LABOR TARGET", "$4,500", "at 30%"),
    ("VS TARGET", "$266", "under budget"),
    ("AVG WAGE", "$15.12", "per hour"),
    ("PRIME COST", "58.2%", "food + labor"),
    ("OT HOURS", "6", "watch list"),
    ("LABOR SCORE", "90%", "healthy"),
]

WEEK = [("Mon", 520), ("Tue", 664), ("Wed", 664), ("Thu", 724), ("Fri", 796), ("Sat", 644), ("Sun", 222)]


def content_dashboard(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Labor Dashboard", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 64), "Maple & Ash · Jordan  ·  schedule to your sales, protect your prime cost", font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 190, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 95, y0 + 44), "● 28.2% labor", font=fs(15), fill=PRIMARY, anchor="mm")
    ...
    d.text((gx, cy_top), "LABOR HEALTH · PRIME COST · LABOR BY DAY", font=fs(20), fill=ACCENT, anchor="lt")
    ...
    d.text((px + 16, panels_y + 14), "Labor Health", font=fs(17), fill=ACCENT, anchor="lt")
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("Labor on target", 1.0, "100%"), ("Sales/labor hr", 1.0, "100%"), ("Overtime low", 0.40, "40%"),
           ("Prime in check", 1.0, "100%"), ("Schedule covered", 1.0, "100%"), ("Wage in budget", 1.0, "100%")],
          color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    ...
    donut(d, px + pw * 0.44, panels_y + panel_h * 0.50, min(panel_h * 0.27, pw * 0.27),
          [(30, ACCENT), (28, GOLD_LT), (42, PRIMARY)], "58%", "prime")
    legend(d, px + pw * 0.06, panels_y + panel_h - 96, [(ACCENT, "Food 30%"), (GOLD_LT, "Labor 28%")], 15, 32)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Labor by Day", font=fs(17), fill=ACCENT, anchor="lt")
    vbars(img, d, (px + 18, panels_y + 60, px + pw - 14, panels_y + panel_h - 10), WEEK, 796)
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Labor Score", font=fs(17), fill=ACCENT, anchor="lt")
...
def content_menu(img, cbox):
    rows = [
        ("Alex", "Line Cook", "40", "$760"),
        ("Bailey", "Line Cook", "40", "$720"),
        ("Casey", "Server", "40", "$480"),
        ("Harper", "Prep Cook", "32", "$544"),
        ("Emerson", "Dishwasher", "34", "$510"),
        ("Gray", "Bartender", "28", "$364"),
    ]
    _table(img, cbox, "Weekly Schedule",
           "Type each shift and every employee's weekly hours & labor cost calculate live — 8 on the team.",
           ["EMPLOYEE", "ROLE", "HOURS", "COST"],
           [0.0, 0.40, 0.66, 0.86], rows,
           total_row=("WEEK TOTAL  ·  280 hrs", "", "280", "$4,234"))


def content_itemmargin(img, cbox):
    rows = [
        ("Scheduled labor cost", "$4,234"),
        ("÷ Weekly sales (forecast)", "$15,000"),
        ("Total scheduled hours", "280"),
        ("Sales per labor hour", "$53.57"),
    ]
    _table(img, cbox, "Labor Cost Calc",
           "Scheduled labor ÷ sales = your true labor %. Change one shift and it updates instantly.",
           ["LINE", "AMOUNT"],
           [0.0, 0.72], rows,
           total_row=("= LABOR %  ·  $15.12 avg wage", "28.2%"))


def content_pnl(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Sales per Labor Hour", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 62), "Each day's sales ÷ labor hours — the productivity number. Higher is a leaner shift.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("Thursday", "$2,100", "47", "$44.68"), ("Friday", "$2,900", "54", "$53.70"),
            ("Saturday", "$3,200", "46", "$69.57"), ("Sunday", "$1,800", "18", "$100.00")]
    ...
    for h, fx in zip(["DAY", "SALES", "LABOR HRS", "SPLH"], fxs):
    ...
    for i, row in enumerate(rows):
        ...
        for ci, (val, fx) in enumerate(zip(row, fxs)):
            col = PRIMARY if ci in (0, 3) else TEXT
    ...
    for ci, (val, fx) in enumerate(zip(["WEEK", "$15,000", "280", "$53.57"], fxs)):
        d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * fx, ry + rh / 2), val, font=fs(18), fill=PRIMARY, anchor="lm" if fx == 0 else "mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2), "Schedule to the forecast — every hour you add is a dollar you spend", font=fs(21), fill=WHITE, anchor="mm")
    img.alpha_composite(ov)
...
def render_hero(out):
    ...
    clock_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE COMPLETE LABOR-COST SYSTEM", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "LABOR & SCHEDULING", 88, max_w=1720)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 546), "Weekly schedule, labor %, sales per labor hour, overtime & prime cost — one system.",
       fs(20, bold=False), (224, 213, 190))
    chips = [("14", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("LABOR %", "ENGINE")]
    cw = 440
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not a blank schedule — a complete schedule-to-your-sales system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Start Here", "how it all works"), ("Dashboard", "labor %, SPLH & score"),
        ("Labor Cost Calc", "your true labor %"), ("Weekly Schedule", "hours & cost, live"),
        ("Employees", "roster, roles & wages"), ("Sales Forecast", "forecast → labor target"),
        ("Sales / Labor Hr", "the productivity number"), ("Overtime", "watch the OT"),
        ("Roles & Rates", "one wage per role"), ("Availability", "who can't work when"),
        ("Prime Cost", "food % + labor %"), ("Tips", "tip-out, split fair"),
        ("Labor by Day", "spot the heavy days"), ("Settings", "set it once"),
        ("+ 12 Printable Pages", "for the wall"), ("Live Labor Score", "it all rolls up"),
    ]
    cols = 4
    margin = 88
    gx, gy = 22, 20
...
def render_menu(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "HOURS & COST, LIVE", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "The Weekly Schedule", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Type each shift and every employee's weekly hours and labor cost calculate as you go",
...
def render_variance(out):
    ...
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "SCHEDULE TO YOUR SALES", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "Sales Per Labor Hour", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Each day's sales ÷ labor hours — the productivity number that keeps every shift lean",
       fs(22, bold=False), (226, 214, 190))
...
def render_engine(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 110, "SCHEDULE IT · COST IT", font=fs(32), pad_x=52, pad_y=22)
    tc(d, (SIZE // 2, 224), "The Labor-Cost Engine", fserif(44), WHITE)
...
def render_printables(out):
    ...
    d = ImageDraw.Draw(img)
    clock_crest(img, SIZE // 2, 110, r=48)
    pill(img, SIZE // 2, 214, "PLUS 12 PRINTABLE PAGES", font=fs(30), pad_x=48, pad_y=20)
    tc(d, (SIZE // 2, 300), "Print, Fill & Post — For the Wall", fserif(44), WHITE)
    gold_divider(img, SIZE // 2, 362, width=480)
    tc(d, (SIZE // 2, 404), "A matching print-ready pack — blank weekly schedule, roster, time-off request & tip sheet",
       fs(21, bold=False), (226, 214, 190))
    labels = ["Weekly Schedule", "Labor Worksheet", "Employee Roster", "Sales Forecast",
              "Sales / Labor Hr", "Overtime Log", "Roles & Rates", "Time-Off Request",
              "Prime Cost Sheet", "Tip Sheet", "Labor by Day", "Shift Swaps"]
