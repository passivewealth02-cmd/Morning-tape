"""Detailed marketing image set for Freelancer Cashflow & Tax Command Center™ (07-10)."""
...
from build_marketing import (
    ...
    ROW_ALT, fs, fserif, premium_bg, pill, wordmark, gold_divider, briefcase_crest,
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR FREELANCERS", "Get Paid, Set Aside, Keep It",
                "Four ways it turns freelance income into money you actually keep")
    feats = [
        ("$", "Live Freelance Score", "donut", (90, "90%", "healthy"),
         "Income, margin, effective rate, take-home, tax set-aside and runway roll into one number — so you know your business health at a glance."),
        ("%", "Cashflow & Tax Engine", "table", None,
         "Income in, expenses and tax out — the engine sets aside your tax automatically and shows the take-home that's really yours to spend."),
        ("box", "Run the Business", "stock", None,
         "Invoices, clients, time and rates, a tax vault, mileage and home-office deductions, a pipeline and a runway — all in one place."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack — invoice, tax set-aside worksheet, mileage log, expense sheet, pipeline & more for your books."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("Net income", "$6,800"), ("− Tax set-aside", "−$2,040"), ("Your take-home", "$4,760")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["THIS MONTH", "VALUE"], [0.0, 0.62]):
            ...
            od = ImageDraw.Draw(img)
            items = [("Take-home", "$4,760", MINT_BG, PRIMARY), ("Effective rate", "$68/hr", MINT_BG, PRIMARY),
                     ("Year pace", "$96,000", MINT_BG, PRIMARY), ("Runway built", "40%", SURFACE, (110, 88, 58))]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than an Invoice",
                "Everything a basic invoice template leaves out — built in")
    rows = [
        ("An invoice template", True, True),
        ("A cashflow & tax engine", False, True),
        ("Auto tax set-aside & take-home", False, True),
        ("Your real effective hourly rate", False, True),
        ("Paid vs outstanding at a glance", False, True),
        ("Tax vault & quarterly estimates", False, True),
        ("Mileage & home-office deductions", False, True),
        ("Pipeline so work never dries up", False, True),
        ("Savings & runway tracker", False, True),
        ("Live Freelance Score dashboard", False, True),
    ...
    d.text((c1, top + hh // 2), "A Basic\nInvoice", font=fs(18), fill=(210, 210, 205), anchor="mm")
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "SET UP IN AN AFTERNOON", "Run Your Freelance in 4 Steps",
                "No spreadsheet skills needed — if you can type, you can run it")
    steps = [
        ("1", "Bill your clients", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack, then log every invoice you send."),
        ("2", "Log your expenses", "Enter your deductible business costs and mileage — every one lowers your net and the tax you'll owe."),
        ("3", "Set aside the tax", "The engine sets aside your tax the moment you're paid and shows your take-home — the money that's actually yours."),
        ("4", "Grow with confidence", "Track your effective rate, keep the pipeline full, build a runway, and watch your after-tax take-home climb."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + Freelance Score",
        "Cashflow & auto tax set-aside", "Invoices, paid & outstanding",
        "Time, rates & effective-rate", "Tax vault & quarterly estimates",
        "Mileage, pipeline & runway", "Start-Here quick-start guide",
    ]
    ...
    who = ["Freelancers & the self-employed", "Designers, writers & developers", "Consultants & coaches",
           "Virtual assistants & marketers", "1099 contractors & side-hustlers", "Anyone who invoices clients"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "GET PAID · SET ASIDE · KEEP IT", font=fs(27), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Get paid, set aside the tax, and finally know what's really yours.",
       fs(23, bold=False), TEXT_MUTED)
    briefcase_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "FREELANCER", 60, max_w=1100)
    img.convert("RGB").save(out, "PNG", optimize=True)
