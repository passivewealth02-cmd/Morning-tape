"""Marketing image set for Freelancer Cashflow & Tax Command Center™ (6 images, 2000x2000)."""
...
TABS = ["Dashboard", "Cashflow & Tax", "Invoices", "Clients", "Time & Rates", "Business Expenses",
        "Tax Vault", "Mileage & Home Office", "Pipeline", "Savings & Runway",
        "Subscriptions", "Monthly Summary", "Settings"]

FILE_LABEL = "Freelancer_Command_Center.xlsx — Studio Fern · Sasha"
...
def briefcase_crest(c, cx, cy, r=56, glow=True):
    """Brand crest — a briefcase: the icon of the self-employed."""
    if glow:
    ...
    ov = Image.new("RGBA", c.size, (0, 0, 0, 0)); d = ImageDraw.Draw(ov)
    # handle
    d.line((cx - r * 0.16, cy - r * 0.30, cx - r * 0.16, cy - r * 0.14), fill=WHITE, width=4)
    d.line((cx + r * 0.16, cy - r * 0.30, cx + r * 0.16, cy - r * 0.14), fill=WHITE, width=4)
    d.line((cx - r * 0.16, cy - r * 0.30, cx + r * 0.16, cy - r * 0.30), fill=WHITE, width=4)
    # body
    d.rounded_rectangle((cx - r * 0.46, cy - r * 0.14, cx + r * 0.46, cy + r * 0.40), radius=8, fill=GOLD_HI, outline=(150, 120, 70))
    # divider band
    d.rectangle((cx - r * 0.46, cy + r * 0.06, cx + r * 0.46, cy + r * 0.16), fill=PRIMARY)
    # clasp
    d.rounded_rectangle((cx - r * 0.08, cy + r * 0.03, cx + r * 0.08, cy + r * 0.19), radius=3, fill=WHITE, outline=(150, 120, 70))
    c.alpha_composite(ov)


# aliases (shared render helpers may reference these names across products)
house_crest = briefcase_crest
tent_crest = briefcase_crest
toque_crest = briefcase_crest
bento_crest = briefcase_crest
cupcake_crest = briefcase_crest
clock_crest = briefcase_crest
clipboard_crest = briefcase_crest
cloche_crest = briefcase_crest
ghost_crest = briefcase_crest
mug_crest = briefcase_crest
book_crest = briefcase_crest
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "FREELANCE", font=fs(15), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("MONTHLY INCOME", "$8,000", "paid this mo"),
    ("EXPENSES", "$1,200", "deductible"),
    ("NET INCOME", "$6,800", "after costs"),
    ("TAX SET-ASIDE", "$2,040", "30% vaulted"),
    ("TAKE-HOME", "$4,760", "really yours"),
    ("EFFECTIVE RATE", "$68.00", "per hour"),
    ("BILLABLE HOURS", "100", "this month"),
    ("UTILIZATION", "63%", "of available"),
    ("INVOICES", "6", "this month"),
    ("OUTSTANDING", "$2,400", "unpaid"),
    ("YEAR PACE", "$96,000", "× 12"),
    ("FREELANCE SCORE", "90%", "healthy"),
]

TAKE_MONTHS = [("Jul", 4760), ("Aug", 4400), ("Sep", 5460), ("Oct", 4050), ("Nov", 5100), ("Dec", 6050)]


def content_dashboard(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Freelance Dashboard", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 64), "Studio Fern · Sasha  ·  get paid, set aside tax, keep what's yours", font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 210, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 105, y0 + 44), "● $68/hr effective", font=fs(15), fill=PRIMARY, anchor="mm")
    ...
    d.text((gx, cy_top), "FREELANCE HEALTH · WHERE THE INCOME GOES · TAKE-HOME BY MONTH", font=fs(19), fill=ACCENT, anchor="lt")
    ...
    d.text((px + 16, panels_y + 14), "Freelance Health", font=fs(17), fill=ACCENT, anchor="lt")
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("Income vs goal", 1.0, "100%"), ("Margin healthy", 1.0, "100%"), ("Rate on target", 1.0, "100%"),
           ("Take-home vs goal", 1.0, "100%"), ("Tax set aside", 1.0, "100%"), ("Runway built", 0.40, "40%")],
          color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Where the Income Goes", font=fs(15), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.44, panels_y + panel_h * 0.50, min(panel_h * 0.27, pw * 0.27),
          [(59, PRIMARY), (26, GOLD_LT), (15, ACCENT)], "59%", "take-home")
    legend(d, px + pw * 0.05, panels_y + panel_h - 108, [(PRIMARY, "Take-home 59%"), (GOLD_LT, "Tax 26%"), (ACCENT, "Expenses 15%")], 14, 30)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Take-home by Month", font=fs(16), fill=ACCENT, anchor="lt")
    vbars(img, d, (px + 18, panels_y + 60, px + pw - 14, panels_y + panel_h - 10), TAKE_MONTHS, 6050)
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Freelance Score", font=fs(17), fill=ACCENT, anchor="lt")
...
def content_invoices(img, cbox):
    rows = [
        ("Northwind Co.", "INV-101", "$1,800", "Paid"),
        ("Acme Studio", "INV-102", "$1,500", "Paid"),
        ("Lumen Labs", "INV-103", "$2,200", "Paid"),
        ("Bright Agency", "INV-104", "$2,500", "Paid"),
        ("Cobalt Inc.", "INV-105", "$1,400", "Unpaid"),
        ("Vertex Media", "INV-106", "$1,000", "Sent"),
    ]
    smap = {"Paid": (MINT_BG, PRIMARY), "Unpaid": (RED_BG, DANGER), "Sent": (WARN_BG, (150, 110, 60))}
    _table(img, cbox, "Invoices",
           "Paid invoices become income, unpaid become outstanding — always know what's owed.",
           ["CLIENT", "INVOICE #", "AMOUNT", "STATUS"],
           [0.0, 0.44, 0.64, 0.86], rows, total_row=("INCOME (paid)  ·  $2,400 outstanding", "", "$8,000", ""),
           status_col=3, status_map=smap)


def content_cashflow(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Cashflow & Tax — What's Yours", font=fs(30), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 60), "Income in, expenses and tax out — the take-home number freelancers forget to protect.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("Income (paid invoices)", "$8,000", PRIMARY), ("− Business expenses", "−$1,200", DANGER),
            ("− Tax set-aside (30%)", "−$2,040", DANGER)]
    ...
    d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.80, ry + rh / 2), "$4,760", font=fs(18), fill=PRIMARY, anchor="mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2), "$68.00 an hour effective  ·  tax already set aside", font=fs(22), fill=WHITE, anchor="mm")
    img.alpha_composite(ov)
...
def render_hero(out):
    ...
    briefcase_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE COMPLETE FREELANCE SYSTEM", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "FREELANCER", 92, max_w=1500)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 546), "Invoices, a cashflow & tax engine, time, a tax vault, a pipeline & runway — one system.",
       fs(20, bold=False), (224, 213, 190))
    chips = [("14", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("TAX", "VAULT ENGINE")]
    cw = 440
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not an invoice template — a complete get-paid, set-aside-tax, keep-it system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Start Here", "how it all works"), ("Dashboard", "take-home & score"),
        ("Cashflow & Tax", "what's really yours"), ("Invoices", "paid vs outstanding"),
        ("Clients", "your roster"), ("Time & Rates", "your effective rate"),
        ("Business Expenses", "every deduction"), ("Tax Vault", "quarterly estimates"),
        ("Mileage & Home Office", "the forgotten write-offs"), ("Pipeline", "keep work coming"),
        ("Savings & Runway", "your cushion"), ("Subscriptions", "the quiet drain"),
        ("Monthly Summary", "watch take-home grow"), ("Settings", "set it once"),
        ("+ 12 Printable Pages", "invoice & worksheets"), ("Live Freelance Score", "it all rolls up"),
    ]
    cols = 4
    margin = 88
    gx, gy = 22, 20
...
def render_cashflow(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "SET ASIDE THE TAX", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "What's Really Yours", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Income in, expenses and tax out — the take-home number that keeps the self-employed safe",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 1, content_cashflow)
    img.convert("RGB").save(out, "PNG", optimize=True)


def render_invoices(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "GET PAID", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "Invoices, Paid & Outstanding", fserif(40), WHITE)
    tc(d, (SIZE // 2, 300), "See what's been paid and what's still owed — income and outstanding, both at a glance",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 2, content_invoices)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_engine(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 110, "BILL IT · KEEP IT", font=fs(32), pad_x=52, pad_y=22)
    tc(d, (SIZE // 2, 224), "The Freelance Engine", fserif(44), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 1, content_cashflow)
    app_window(img, (60, 1180, SIZE - 60, SIZE - 60), 2, content_invoices)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_printables(out):
    ...
    d = ImageDraw.Draw(img)
    briefcase_crest(img, SIZE // 2, 110, r=48)
    pill(img, SIZE // 2, 214, "PLUS 12 PRINTABLE PAGES", font=fs(30), pad_x=48, pad_y=20)
    tc(d, (SIZE // 2, 300), "Print, Fill & Keep — For the Books", fserif(44), WHITE)
    gold_divider(img, SIZE // 2, 362, width=480)
    tc(d, (SIZE // 2, 404), "A matching print-ready pack — invoice, tax set-aside worksheet, mileage log & more",
       fs(21, bold=False), (226, 214, 190))
    labels = ["Invoice", "Cashflow & Tax", "Client List", "Rate Card",
              "Business Expenses", "Tax Vault", "Mileage & Office", "Pipeline",
              "Savings & Runway", "Subscriptions", "Monthly Summary", "Weekly Checklist"]
...
def main():
    ...
    targets = [
        ...
        ("03_cashflow.png", render_cashflow),
        ("04_invoices.png", render_invoices),
