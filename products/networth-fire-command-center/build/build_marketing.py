"""Marketing image set for Net Worth & FIRE Command Center™ (6 images, 2000x2000)."""
...
TABS = ["Dashboard", "Net Worth", "FIRE Number", "Assets", "Liabilities", "Accounts", "Contributions",
        "Income & Expenses", "Savings Rate", "Coast & Projection", "Milestones",
        "Net Worth Trend", "Settings"]

FILE_LABEL = "Networth_FIRE_Command_Center.xlsx — North Star Finance · Sage"
...
def compass_crest(c, cx, cy, r=56, glow=True):
    """Brand crest — a compass rose: the icon of finding your financial north."""
    if glow:
    ...
    ov = Image.new("RGBA", c.size, (0, 0, 0, 0)); d = ImageDraw.Draw(ov)
    rr = r * 0.52
    d.ellipse((cx - rr, cy - rr, cx + rr, cy + rr), outline=GOLD_HI, width=3)
    # compass needle — north (highlight) & south (gold), a 4-point star
    d.polygon([(cx, cy - rr * 0.92), (cx + rr * 0.22, cy), (cx, cy + rr * 0.05), (cx - rr * 0.22, cy)], fill=HIGHLIGHT)
    d.polygon([(cx, cy + rr * 0.92), (cx + rr * 0.22, cy), (cx, cy - rr * 0.05), (cx - rr * 0.22, cy)], fill=GOLD_HI)
    d.polygon([(cx - rr * 0.92, cy), (cx, cy - rr * 0.22), (cx + rr * 0.05, cy), (cx, cy + rr * 0.22)], fill=SURFACE)
    d.polygon([(cx + rr * 0.92, cy), (cx, cy - rr * 0.22), (cx - rr * 0.05, cy), (cx, cy + rr * 0.22)], fill=SURFACE)
    d.ellipse((cx - 5, cy - 5, cx + 5, cy + 5), fill=PRIMARY_DK, outline=GOLD_HI)
    c.alpha_composite(ov)


# aliases (shared render helpers may reference these names across products)
candle_crest = compass_crest
camera_crest = compass_crest
sign_crest = compass_crest
house_crest = compass_crest
...
def vbars(img, d, box, items, maxv, color=(PRIMARY_LT, PRIMARY)):
    ...
    for i, (lab, val) in enumerate(items):
        ...
        vtxt = f"${val/1000:.0f}k" if maxv >= 2000 else f"${val:,.0f}"
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "FIRE PLAN", font=fs(15), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("NET WORTH", "$250,000", "you own − owe"),
    ("TOTAL ASSETS", "$530,000", "everything"),
    ("TOTAL DEBT", "$280,000", "what you owe"),
    ("FIRE NUMBER", "$1,000,000", "your goal"),
    ("FIRE PROGRESS", "25%", "of the way"),
    ("ANNUAL SPEND", "$40,000", "your life costs"),
    ("SAVINGS RATE", "50%", "of income"),
    ("ANNUAL SAVINGS", "$40,000", "invested/yr"),
    ("COAST NUMBER", "$131,367", "coast to 65"),
    ("YEARS TO FI", "10.5", "at this pace"),
    ("INVESTED", "$200,000", "your engine"),
    ("FIRE SCORE", "90%", "healthy"),
]

NW_MONTHS = [("Feb", 214000), ("Mar", 222000), ("Apr", 231000), ("May", 238000), ("Jun", 244000), ("Jul", 250000)]


def content_dashboard(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "FIRE Dashboard", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 64), "North Star Finance · Sage  ·  know your number, and your progress to it", font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 210, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 105, y0 + 44), "● 25% to FIRE", font=fs(14), fill=PRIMARY, anchor="mm")
    ...
    d.text((gx, cy_top), "FIRE HEALTH · WHERE YOU ARE TO FIRE · NET WORTH BY MONTH", font=fs(19), fill=ACCENT, anchor="lt")
    ...
    d.text((px + 16, panels_y + 14), "FIRE Health", font=fs(17), fill=ACCENT, anchor="lt")
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("Saving enough", 1.0, "100%"), ("Emergency fund", 1.0, "100%"), ("Positive net worth", 1.0, "100%"),
           ("Coast FIRE reached", 1.0, "100%"), ("Investing to goal", 1.0, "100%"), ("Halfway to FI", 0.40, "40%")],
          color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Progress to FIRE", font=fs(15), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.44, panels_y + panel_h * 0.50, min(panel_h * 0.27, pw * 0.27),
          [(25, PRIMARY), (75, SURFACE)], "25%", "of $1M")
    legend(d, px + pw * 0.06, panels_y + panel_h - 96, [(PRIMARY, "Net worth 25%"), (SURFACE, "To go 75%")], 15, 32)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Net Worth by Month", font=fs(17), fill=ACCENT, anchor="lt")
    vbars(img, d, (px + 18, panels_y + 60, px + pw - 14, panels_y + panel_h - 10), NW_MONTHS, 250000)
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "FIRE Score", font=fs(17), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.5, panels_y + panel_h * 0.54, min(panel_h * 0.30, pw * 0.30),
          [(90, PRIMARY), (10, SURFACE)], "90%", "healthy")


def content_fire(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "FIRE Number — Your Goal", font=fs(30), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 60), "Your annual spending, over a safe 4% withdrawal rate, is the amount that sets you free.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("Annual spending (what your life costs)", "$40,000", PRIMARY),
            ("÷ Safe withdrawal rate", "4.0%", PRIMARY)]
    ...
    d.text((x0 + pad + 14, ry + rh / 2), "= YOUR FIRE NUMBER", font=fs(18), fill=PRIMARY, anchor="lm")
    d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.80, ry + rh / 2), "$1,000,000", font=fs(18), fill=PRIMARY, anchor="mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2), "$250,000 net worth  =  25% of the way there", font=fs(23), fill=WHITE, anchor="mm")
    img.alpha_composite(ov)


def content_coast(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Coast FIRE — Invest No More", font=fs(30), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 60), "Enough invested today that, with no new money, it grows into your FIRE number by 65.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("FIRE number", "$1,000,000", PRIMARY),
            ("÷ growth over 30 yrs at 7%", "7.6×", PRIMARY)]
    callout_h = 150; gap = 44; rh = 118
    n_units = 1 + len(rows) + 1
    top_region = y0 + 108
    block_h = rh * n_units + gap + callout_h
    ...
    ry = ty + rh + len(rows) * rh
    d.rectangle((x0 + pad, ry, x1 - pad, ry + rh), fill=SURFACE)
    d.text((x0 + pad + 14, ry + rh / 2), "= COAST FIRE NUMBER", font=fs(18), fill=PRIMARY, anchor="lm")
    d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.80, ry + rh / 2), "$131,367", font=fs(18), fill=PRIMARY, anchor="mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2), "$200,000 invested  >  coast number  =  YOU CAN COAST", font=fs(21), fill=WHITE, anchor="mm")
    img.alpha_composite(ov)
...
def render_hero(out):
    ...
    pill(img, SIZE // 2, 256, "THE COMPLETE NET WORTH & FIRE SYSTEM", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "NET WORTH & FIRE", 82, max_w=1600)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 546), "A FIRE-number engine, net-worth tracker, coast projection & milestones — one system.",
       fs(20, bold=False), (224, 213, 190))
    chips = [("14", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("FIRE", "ENGINE")]
    cw = 440
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not a net-worth sheet — a complete know-your-number, get-there system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Start Here", "how it all works"), ("Dashboard", "net worth & score"),
        ("Net Worth", "own minus owe"), ("FIRE Number", "your goal"),
        ("Assets", "everything you own"), ("Liabilities", "everything you owe"),
        ("Accounts", "one glance"), ("Contributions", "what you invest"),
        ("Income & Expenses", "your savings rate"), ("Savings Rate", "watch it climb"),
        ("Coast & Projection", "when you're free"), ("Milestones", "markers to freedom"),
        ("Net Worth Trend", "the line goes up"), ("Settings", "set it once"),
        ("+ 12 Printable Pages", "worksheets & lists"), ("Live FIRE Score", "it all rolls up"),
    ]
    cols = 4
    margin = 88
    gx, gy = 22, 20
...
def render_fire(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "KNOW YOUR NUMBER", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "The FIRE Number Engine", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Turn your annual spending into the exact amount you need to make work optional forever",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 2, content_fire)
    img.convert("RGB").save(out, "PNG", optimize=True)


def render_coast(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "COAST TO FREEDOM", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "The Coast FIRE Engine", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "The moment you've invested enough that compounding alone carries you to your number",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 9, content_coast)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_engine(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 110, "NUMBER · PROGRESS", font=fs(32), pad_x=52, pad_y=22)
    tc(d, (SIZE // 2, 224), "The FIRE Engine", fserif(44), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 2, content_fire)
    app_window(img, (60, 1180, SIZE - 60, SIZE - 60), 9, content_coast)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_printables(out):
    ...
    d = ImageDraw.Draw(img)
    compass_crest(img, SIZE // 2, 110, r=48)
    pill(img, SIZE // 2, 214, "PLUS 12 PRINTABLE PAGES", font=fs(30), pad_x=48, pad_y=20)
    tc(d, (SIZE // 2, 300), "Print, Fill & Keep — On Paper", fserif(44), WHITE)
    gold_divider(img, SIZE // 2, 362, width=480)
    tc(d, (SIZE // 2, 404), "A matching print-ready pack — net-worth worksheet, FIRE-number worksheet, asset & debt lists & more",
       fs(21, bold=False), (226, 214, 190))
    labels = ["Net Worth Worksheet", "FIRE Number Worksheet", "Asset List", "Liability List",
              "Account List", "Contributions Log", "Income & Expenses", "Savings Rate Log",
              "Coast & Projection", "Milestones", "Net Worth Trend", "Money Checklist"]
...
def main():
    ...
    targets = [
        ...
        ("03_fire.png", render_fire),
        ("04_coast.png", render_coast),
