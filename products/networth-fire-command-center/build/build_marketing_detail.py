"""Detailed marketing image set for Net Worth & FIRE Command Center™ (07-10)."""
...
from build_marketing import (
    ...
    ROW_ALT, fs, fserif, premium_bg, pill, wordmark, gold_divider, compass_crest,
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR THE FI JOURNEY", "Know Your Number, Get There",
                "Four ways it turns saving into a plan with a finish line")
    feats = [
        ("$", "Live FIRE Score", "donut", (90, "90%", "healthy"),
         "Saving enough, emergency fund, positive net worth, coast FIRE, investing to goal and halfway-to-FI roll into one number — your plan's health at a glance."),
        ("%", "FIRE Number Engine", "table", None,
         "Your annual spending over a safe 4% withdrawal rate gives the exact amount you need to make work optional forever."),
        ("box", "Track the Whole Picture", "stock", None,
         "Net worth, assets, liabilities, accounts, contributions, savings rate and a coast projection — your entire financial life in one place."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack — net-worth worksheet, FIRE-number worksheet, asset & debt lists, milestones & more."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("Annual spending", "$40,000"), ("÷ 4% rule", "×25"), ("FIRE number", "$1,000,000")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["KNOW YOUR NUMBER", "VALUE"], [0.0, 0.62]):
            ...
            od = ImageDraw.Draw(img)
            items = [("Net worth", "$250,000", MINT_BG, PRIMARY), ("Savings rate", "50%", MINT_BG, PRIMARY),
                     ("Years to FI", "10.5", MINT_BG, PRIMARY), ("Halfway to FI", "40%", SURFACE, (110, 88, 58))]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Net-Worth Sheet",
                "Everything a basic tracker leaves out — built in")
    rows = [
        ("A net-worth total", True, True),
        ("A FIRE-number engine", False, True),
        ("Your progress to FI", False, True),
        ("Coast FIRE & years to FI", False, True),
        ("Assets & liabilities detail", False, True),
        ("Accounts & contributions", False, True),
        ("Savings-rate tracking", False, True),
        ("A coast & growth projection", False, True),
        ("Milestones to freedom", False, True),
        ("Live FIRE Score dashboard", False, True),
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "SET UP IN AN AFTERNOON", "Find Your Number in 4 Steps",
                "No spreadsheet skills needed — if you can type, you can run it")
    steps = [
        ("1", "Enter what you own & owe", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack, then list your assets and liabilities — your net worth appears."),
        ("2", "Set your FIRE number", "Enter what your life costs in a year; the 4% rule gives the exact amount you need to make work optional forever."),
        ("3", "Track your progress", "Log your accounts, contributions and savings rate, and watch your progress, coast FIRE and years to FI update live."),
        ("4", "Follow the plan", "Check off milestones as your net worth climbs, read your projection to each age, and let the FIRE Score keep you honest."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + FIRE Score",
        "FIRE-number & coast engine", "Net worth, assets & liabilities",
        "Accounts & contributions", "Savings-rate tracking",
        "Coast projection & milestones", "Start-Here quick-start guide",
    ]
    ...
    who = ["FIRE & early-retirement planners", "New investors & savers", "Debt-payoff & net-worth trackers",
           "Couples planning together", "Anyone chasing financial freedom", "Anyone who wants to know their number"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "KNOW IT · TRACK IT · REACH IT", font=fs(27), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Know your number, track your net worth, and reach financial independence.",
       fs(23, bold=False), TEXT_MUTED)
    compass_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "NET WORTH & FIRE", 54, max_w=1200)
    img.convert("RGB").save(out, "PNG", optimize=True)
