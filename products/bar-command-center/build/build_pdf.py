"""Printable PDF pack for Bar & Pub Command Center™ (12 pages, US Letter)."""
...
def page(title, subtitle):
    ...
    d.text((M, 90), "BAR & PUB COMMAND CENTER™", font=fs(34), fill=GOLD_HI, anchor="lt")
    ...
    d.text((M, H - 108), "Bar / Date: ______________", font=fs(24, bold=False), fill=MUTED, anchor="lt")
    d.text((W - M, H - 108), "Bar & Pub Command Center™", font=fs(24, bold=False), fill=ACCENT, anchor="rt")
    return img, d
...
def p01(i):
    img, d = page("Pour Cost Card", "Cost a drink to the pour")
    field(d, M, TOP, CW / 2 - 30, "Drink"); field(d, M + CW / 2 + 30, TOP, CW / 2 - 30, "Glass / size")
    table(d, M, TOP + 130, CW, ["COMPONENT", "QTY", "UNIT", "COST/UNIT", "EXT."], [0.0, 0.44, 0.58, 0.72, 0.86], 11, 140)
    y = TOP + 130 + 78 + 11 * 140 + 30
    field(d, M, y, CW / 2 - 30, "Pour cost"); field(d, M + CW / 2 + 30, y, CW / 2 - 30, "Menu price")
    i.append(img)


def p02(i):
    img, d = page("Drink Menu", "Pour cost, price, pour-cost % & margin")
    table(d, M, TOP, CW, ["DRINK", "POUR COST", "PRICE", "POUR %", "MARGIN"], [0.0, 0.40, 0.56, 0.72, 0.86], 18, 120)
    i.append(img)


def p03(i):
    img, d = page("Keg & Draft Log", "Cost per pint & profit per keg")
    table(d, M, TOP, CW, ["KEG", "KEG COST", "PINTS", "COST/PT", "PRICE/PT", "PROFIT/KEG"],
          [0.0, 0.30, 0.46, 0.58, 0.72, 0.86], 14, 150)
    i.append(img)


def p04(i):
    img, d = page("Inventory Variance", "Theoretical vs actual = shrinkage")
    d.text((M, TOP + 10), "VARIANCE %  =  (ACTUAL − THEORETICAL)  ÷  THEORETICAL", font=fs(30), fill=PRIMARY, anchor="lt")
    table(d, M, TOP + 100, CW, ["CATEGORY", "THEORETICAL", "ACTUAL", "VARIANCE %"], [0.0, 0.36, 0.58, 0.80], 12, 150)
    i.append(img)


def p05(i):
    img, d = page("Liquor Count Sheet", "Count par vs on hand")
    table(d, M, TOP, CW, ["ITEM", "PAR", "ON HAND", "UNIT", "TO ORDER"], [0.0, 0.42, 0.56, 0.72, 0.86], 20, 110)
    i.append(img)


def p06(i):
    img, d = page("Happy Hour Planner", "Discount that still profits")
    table(d, M, TOP, CW, ["DRINK", "REGULAR", "HH PRICE", "POUR COST", "HH MARGIN"], [0.0, 0.34, 0.52, 0.70, 0.86], 14, 155)
    i.append(img)


def p07(i):
    img, d = page("Weekly Sales Log", "Sales & covers by day")
    table(d, M, TOP, CW, ["DAY", "SALES", "COVERS", "AVG CHECK"], [0.0, 0.30, 0.56, 0.80], 8, 170, filled_rows=[[x] for x in DAYS])
    i.append(img)


def p08(i):
    img, d = page("Waste & Spill Log", "The quiet leaks behind variance")
    table(d, M, TOP, CW, ["DATE", "ITEM", "REASON", "COST"], [0.0, 0.16, 0.44, 0.82], 20, 118)
    i.append(img)


def p09(i):
    img, d = page("Ordering Sheet", "Your standing distributor order")
    table(d, M, TOP, CW, ["ITEM", "SUPPLIER", "PAR ORDER", "COST"], [0.0, 0.38, 0.62, 0.82], 18, 120)
    i.append(img)


def p10(i):
    img, d = page("Cash & Tips", "Balance the drawer every night")
    table(d, M, TOP, CW, ["DAY", "CASH", "CARD", "TIPS", "TOTAL"], [0.0, 0.24, 0.42, 0.60, 0.80], 8, 170, filled_rows=[[x] for x in DAYS])
    i.append(img)


def p11(i):
    img, d = page("Events & Big Tabs", "Bookings that pack slow nights")
    for k, lab in enumerate(["Event & date", "Host / contact", "Party size", "Deposit / min spend",
                             "Package / drinks", "Notes", "Total spend"]):
...
def p12(i):
    ...
    half = CW / 2 - 30
    for k, st in enumerate(["Opening", "Closing"]):
        x = M + k * (half + 60)
        section(d, x, TOP, half, st)
        for j in range(12):
            checkbox(d, x, TOP + 96 + j * 110)
            d.line((x + 72, TOP + 96 + j * 110 + 48, x + half, TOP + 96 + j * 110 + 48), fill=LINE, width=2)
    i.append(img)
...
def main():
    ...
    pdf_path = os.path.join(out_dir, "Bar_Printables.pdf")
