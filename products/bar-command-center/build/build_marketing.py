"""Marketing image set for Bar & Pub Command Center™ (6 images, 2000x2000)."""
...
TABS = ["Dashboard", "Pour Cost", "Drink Menu", "Keg & Draft", "Inventory Variance", "Liquor Inventory",
        "Happy Hour", "Weekly Sales", "Waste & Spill", "Ordering", "Cash & Tips", "Events & Tabs", "Settings"]

FILE_LABEL = "Bar_Command_Center.xlsx — The Oak & Iron · Reed"
...
def mug_crest(c, cx, cy, r=56, glow=True):
    """Brand crest — a frothy beer mug: the icon of the bar."""
    if glow:
    ...
    ov = Image.new("RGBA", c.size, (0, 0, 0, 0)); d = ImageDraw.Draw(ov)
    # mug body
    bx0, by0, bx1, by1 = cx - r * 0.34, cy - r * 0.24, cx + r * 0.20, cy + r * 0.50
    d.rounded_rectangle((bx0, by0, bx1, by1), radius=8, fill=GOLD_HI, outline=(150, 120, 70))
    # beer fill line
    d.line((bx0 + 4, cy - r * 0.06, bx1 - 4, cy - r * 0.06), fill=(150, 120, 70), width=2)
    # handle
    d.arc((cx + r * 0.10, by0 + r * 0.06, cx + r * 0.50, by1 - r * 0.10), start=300, end=60, fill=GOLD_HI, width=int(r * 0.10))
    # froth (three bumps)
    for fx in (bx0 + r * 0.10, cx - r * 0.06, bx1 - r * 0.12):
        d.ellipse((fx - r * 0.11, by0 - r * 0.16, fx + r * 0.11, by0 + r * 0.06), fill=WHITE)
    c.alpha_composite(ov)


book_crest = mug_crest
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "BAR", font=fs(16), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("DRINKS", "8", "on the menu"),
    ("AVG POUR COST", "$1.61", "per drink"),
    ("AVG PRICE", "$9.62", "per drink"),
    ("AVG MARGIN", "$8.02", "per drink"),
    ("POUR COST", "16%", "6 pts under target"),
    ("TOP SELLER", "Draft Beer", "$2,940/wk"),
    ("WEEKLY SALES", "$16,940", "drink sales"),
    ("WEEKLY UNITS", "1,870", "poured"),
    ("AVG PROFIT/KEG", "$571", "per keg"),
    ("INV VARIANCE", "6.1%", "shrinkage"),
    ("HAPPY HR MARGIN", "79%", "still profits"),
    ("BAR SCORE", "90%", "healthy"),
]

WEEK = [("Mon", 1450), ("Tue", 1680), ("Wed", 2100), ("Thu", 2680), ("Fri", 3850), ("Sat", 4180)]


def content_dashboard(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Bar Dashboard", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 64), "The Oak & Iron · Reed  ·  cost every pour, kill your shrinkage, pour more profit", font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 190, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 95, y0 + 44), "● 16% pour cost", font=fs(15), fill=PRIMARY, anchor="mm")
    ...
    d.text((gx, cy_top), "BAR HEALTH · VARIANCE · SALES BY DAY", font=fs(20), fill=ACCENT, anchor="lt")
    ...
    d.text((px + 16, panels_y + 14), "Bar Health", font=fs(17), fill=ACCENT, anchor="lt")
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("Pour cost on target", 1.0, "100%"), ("Margin per drink", 1.0, "100%"), ("Fully costed", 1.0, "100%"),
           ("Variance low", 0.39, "39%"), ("Happy-hour margin", 1.0, "100%"), ("Gross margin", 1.0, "100%")],
          color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Inventory Variance", font=fs(17), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.44, panels_y + panel_h * 0.50, min(panel_h * 0.27, pw * 0.27),
          [(6.1, DANGER), (93.9, PRIMARY)], "6.1%", "shrink")
    legend(d, px + pw * 0.06, panels_y + panel_h - 96, [(PRIMARY, "On the books 94%"), (DANGER, "Shrinkage 6%")], 15, 32)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Sales This Week", font=fs(17), fill=ACCENT, anchor="lt")
    vbars(img, d, (px + 18, panels_y + 60, px + pw - 14, panels_y + panel_h - 10), WEEK, 4180)
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Bar Score", font=fs(17), fill=ACCENT, anchor="lt")
...
def content_menu(img, cbox):
    rows = [
        ("Draft Beer (pint)", "$0.95", "$7.00", "14%", "$6.05"),
        ("Craft Draft", "$1.30", "$8.00", "16%", "$6.70"),
        ("Well Cocktail", "$1.40", "$9.00", "16%", "$7.60"),
        ("Margarita", "$1.90", "$11.00", "17%", "$9.10"),
        ("Old Fashioned", "$2.20", "$13.00", "17%", "$10.80"),
        ("Premium Cocktail", "$2.80", "$14.00", "20%", "$11.20"),
    ]
    _table(img, cbox, "Drink Menu",
           "Pour cost, price, pour-cost % & margin on every drink — price for a healthy pour",
           ["DRINK", "POUR COST", "PRICE", "POUR %", "MARGIN"],
           [0.0, 0.38, 0.55, 0.72, 0.87], rows)


def content_pourcost(img, cbox):
    rows = [
        ("Bourbon", "2 oz", "$0.90", "$1.80"),
        ("Simple syrup", "0.25 oz", "$0.10", "$0.03"),
        ("Bitters", "3 dash", "$0.03", "$0.09"),
        ("Orange garnish", "1 ea", "$0.18", "$0.18"),
        ("Ice / napkin", "1 ea", "$0.105", "$0.11"),
    ]
    _table(img, cbox, "Pour Cost — Old Fashioned",
           ...
           total_row=("POUR COST  ·  17% at $13", "", "", "$2.20"))


def content_variance(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Inventory Variance", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 62), "Theoretical usage vs actual — the gap is shrinkage: over-pours, comps & walk-outs.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("Beer / draft", "$2,400", "$2,520", "+5.0%"), ("Liquor / spirits", "$1,800", "$1,940", "+7.8%"),
            ("Wine", "$900", "$950", "+5.6%")]
    ty = y0 + 130; rh = 96
    grad_round(img, (x0 + pad, ty, x1 - pad, ty + rh), 8, PRIMARY_LT, PRIMARY_DK)
    for h, fx in zip(["CATEGORY", "THEORETICAL", "ACTUAL", "VARIANCE"], [0.0, 0.40, 0.60, 0.82]):
    ...
    for i, row in enumerate(rows):
        ...
        for ci, (val, fx) in enumerate(zip(row, [0.0, 0.40, 0.60, 0.82])):
            d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * fx, ry + rh / 2), val, font=fs(18, bold=(ci == 0 or ci == 3)),
                   fill=DANGER if ci == 3 else (PRIMARY if ci == 0 else TEXT), anchor="lm" if fx == 0 else "mm")
    ry = ty + rh + 3 * rh
    d.rectangle((x0 + pad, ry, x1 - pad, ry + rh), fill=SURFACE)
    for ci, (val, fx) in enumerate(zip(["TOTAL", "$5,100", "$5,410", "+6.1%"], [0.0, 0.40, 0.60, 0.82])):
        d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * fx, ry + rh / 2), val, font=fs(18), fill=DANGER if ci == 3 else PRIMARY, anchor="lm" if fx == 0 else "mm")
    by = ry + rh + 40
    grad_round(img, (x0 + pad, by, x1 - pad, y1 - pad), 14, (200, 90, 90), DANGER, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text((x0 + pad + 28, (by + y1 - pad) / 2), "Every point of variance over 2-3% is profit walking out the door", font=fs(20), fill=WHITE, anchor="lm")
    img.alpha_composite(ov)
...
def render_hero(out):
    ...
    mug_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE COMPLETE BAR BUSINESS SYSTEM", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "BAR & PUB COMMAND CENTER", 76, max_w=1780)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 546), "Pour cost, keg profit, inventory variance, happy hour & events — one system.",
       fs(20, bold=False), (224, 213, 190))
    chips = [("14", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("POUR", "COST ENGINE")]
    cw = 440
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not a sales log — a complete cost-the-pour, kill-the-shrinkage system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Start Here", "how it all works"), ("Bar Dashboard", "pour cost, variance & score"),
        ("Pour Cost", "cost a drink to the pour"), ("Drink Menu", "pour-cost % & margin"),
        ("Keg & Draft", "profit per keg"), ("Inventory Variance", "catch the shrinkage"),
        ("Liquor Inventory", "par vs on hand"), ("Happy Hour", "discount that still profits"),
        ("Weekly Sales", "the weekend carries it"), ("Waste & Spill", "the quiet leaks"),
        ("Ordering", "your standing order"), ("Cash & Tips", "balance the drawer"),
        ("Events & Tabs", "pack the slow nights"), ("Settings", "set it once"),
        ("+ 12 Printable Pages", "behind the bar"), ("Live Bar Score", "it all rolls up"),
    ]
    cols = 4
    margin = 88
    gx, gy = 22, 20
...
def render_menu(out):
    ...
    tc(d, (SIZE // 2, 232), "Pour-Cost % on Every Drink", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Pour cost & price side by side — with each drink's pour-cost % and margin",
...
def render_variance(out):
    ...
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "KILL THE SHRINKAGE", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "Theoretical vs Actual Usage", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "The gap between what you should have used and what you did — that's profit walking out",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 4, content_variance)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_engine(out):
    ...
    tc(d, (SIZE // 2, 224), "The Pour-Cost Engine", fserif(44), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 1, content_pourcost)
...
def render_printables(out):
    ...
    d = ImageDraw.Draw(img)
    mug_crest(img, SIZE // 2, 110, r=48)
    pill(img, SIZE // 2, 214, "PLUS 12 PRINTABLE PAGES", font=fs(30), pad_x=48, pad_y=20)
    tc(d, (SIZE // 2, 300), "Print, Fill & Keep — Behind the Bar", fserif(44), WHITE)
    gold_divider(img, SIZE // 2, 362, width=480)
    tc(d, (SIZE // 2, 404), "A matching print-ready pack — pour-cost card, keg log, variance sheet & count sheet",
       fs(21, bold=False), (226, 214, 190))
    labels = ["Pour Cost Card", "Drink Menu", "Keg & Draft Log", "Inventory Variance",
              "Liquor Count", "Happy Hour", "Weekly Sales", "Waste & Spill",
              "Ordering Sheet", "Cash & Tips", "Events & Tabs", "Open / Close"]
