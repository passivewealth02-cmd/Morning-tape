"""Detailed marketing image set for Preschool Command Center™ (4 images, 2000x2000).

  07_features.png   - feature spotlights (what it does + why it matters)
  08_compare.png    - "basic printable pack" vs Preschool Command Center™
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR REAL PRESCHOOL DAYS", "Where Little Learners Bloom",
                "Four ways it turns everyday play into visible progress")
    feats = [
        ("$", "Live Ready-for-K Score", "donut", (70, "70%", "ready"),
         "Letters, skills, themes, activities & the readiness checklist roll into one number — see at a glance how ready they are for kindergarten."),
        ("%", "A Whole-Child Skills Tracker", "table", None,
         "Motor, language, pre-math, pre-reading, social & self-help — mark each milestone Mastered, Emerging or Not Yet, at their own pace."),
        ("box", "ABC, 123 & Weekly Themes", "stock", None,
         "Track every letter (sees, says, writes), counting, and a gentle theme a week — the learning made visible, with zero pressure."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready PDF pack — weekly theme plan, skills checklist, ABC & 123 chart, readiness list, portfolio & more."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        od.text((x + 124, by - 16), title, font=fs(27), fill=PRIMARY, anchor="lm")
        ...
        if kind == "donut":
            ...
            rows = [("Fine Motor", "Mastered"), ("Language", "Mastered"), ("Pre-Math", "Emerging")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["DOMAIN", "STATUS"], [0.0, 0.52]):
            ...
            for ri, (a, b) in enumerate(rows):
                ...
                od.text((tx0 + 14 + (tx1 - tx0) * 0.52, ry + 21), b, font=fs(16, bold=False), fill=TEXT, anchor="lm")
        elif kind == "stock":
            od = ImageDraw.Draw(img)
            items = [("Letters known", "24/26", MINT_BG, PRIMARY), ("Skills mastered", "25/35", MINT_BG, PRIMARY),
                     ("Themes done", "7/14", SURFACE, (110, 88, 58)), ("Books read", "38", MINT_BG, PRIMARY)]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Printable Pack",
                "Everything a basic preschool printable leaves out — built in")
    rows = [
        ("A weekly theme & activity plan", True, True),
        ("Whole-child skills & milestones tracker", False, True),
        ("ABC & 123 mastery (sees / says / writes)", False, True),
        ("Read-aloud log & book basket", False, True),
        ("Arts, crafts & sensory idea bank", False, True),
        ("Nature walks & field trips", False, True),
        ("Portfolio & keepsakes", False, True),
        ("Supplies budget", False, True),
        ("Kindergarten-readiness checklist", False, True),
        ("Live Ready-for-K Score", False, True),
    ...
    d.text((c1, top + hh // 2), "Basic\nPrintable", font=fs(18), fill=(210, 210, 205), anchor="mm")
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "SET UP DURING NAPTIME", "Up & Running in 4 Steps",
                "No spreadsheet skills needed — if you can type, you can run it")
    steps = [
        ("1", "Download & open", "Buy once and instantly download the Google Sheets system plus the 12-page printable PDF pack."),
        ("2", "Add your little ones", "In Settings, add your family & children, then fill a quick profile for each — age, interests & what they're working on."),
        ("3", "Pick a theme & plan play", "Choose the week's theme and pull a few activities from the play bank. Themes, activities & books all roll into one dashboard."),
        ("4", "Play & watch them grow", "Mark skills, letters & activities as they go — the Ready-for-K Score shows exactly how they're blossoming."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "EVERYTHING YOU GET", "One System, Their Whole Year",
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + Ready-for-K Score",
        "Whole-child skills & milestones tracker", "ABC & 123 + weekly themes",
        "Play-based activity planner", "Read-aloud log & book basket",
        "Portfolio, nature walks & supplies", "Start-Here guide + free updates",
    ]
    ...
    who = ["At-home preschool & pre-K families", "Homeschool & tot-school parents", "First-time & experienced moms",
           "Play-based · Montessori-inspired · eclectic", "Nannies & in-home care providers", "Anyone prepping for kindergarten"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "PLAY WITH A PLAN · WATCH THEM GROW", font=fs(28), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Give them a joyful start — and a keepsake of the year.",
       fs(24, bold=False), TEXT_MUTED)
    book_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "PRESCHOOL COMMAND CENTER", 50, max_w=1560)
    img.convert("RGB").save(out, "PNG", optimize=True)
