"""Marketing image set for College Command Center™ (6 images, 2000x2000)."""
...
TABS = ["Dashboard", "Applicant Profile", "College List", "Essays", "Recommendations",
        "Test Scores", "Activities", "Scholarships", "Net Price", "Visits", "Decisions",
        "To-Do", "Deadlines", "Settings"]

FILE_LABEL = "College_Command_Center.xlsx — Ella Bennett · Class of 2027"
...
def gradcap_crest(c, cx, cy, r=56, glow=True):
    """Brand crest — a graduation mortarboard: the icon of college admissions."""
    if glow:
    ...
    # cap base (head band)
    by = cy + r * 0.14
    d.polygon([(cx - r * 0.34, by), (cx + r * 0.34, by), (cx + r * 0.28, by + r * 0.34), (cx - r * 0.28, by + r * 0.34)],
              fill=(150, 120, 70))
    # mortarboard (diamond)
    bw = r * 0.62
    top = cy - r * 0.30
    d.polygon([(cx, top - bw * 0.5), (cx + bw, top), (cx, top + bw * 0.5), (cx - bw, top)],
              fill=GOLD_HI, outline=(150, 120, 70))
    # button
    d.ellipse((cx - r * 0.06, top - r * 0.06, cx + r * 0.06, top + r * 0.06), fill=PRIMARY_DK)
    # tassel
    d.line((cx, top, cx + bw * 0.82, top + r * 0.06), fill=(150, 120, 70), width=3)
    d.line((cx + bw * 0.82, top + r * 0.06, cx + bw * 0.82, top + r * 0.5), fill=(150, 120, 70), width=3)
    d.ellipse((cx + bw * 0.82 - r * 0.08, top + r * 0.5, cx + bw * 0.82 + r * 0.08, top + r * 0.5 + r * 0.16), fill=PRIMARY_DK)
    c.alpha_composite(ov)


book_crest = gradcap_crest
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "ADMISSIONS", font=fs(16), fill=GOLD_HI, anchor="lt")
    od.text((bx, sb[1] + 49), "15-tab system", font=fs(14, bold=False), fill=(170, 200, 192), anchor="lt")
...
KPIS = [
    ("COLLEGES", "8", "on the list"),
    ("APPS SUBMITTED", "4", "of 8"),
    ("AVG PROGRESS", "69%", "per school"),
    ("ESSAYS DONE", "50%", "final drafts"),
    ("RECS SECURED", "3", "of 4"),
    ("SCHOLARSHIPS", "6", "applied"),
    ("AID AWARDED", "$12,000", "so far"),
    ("NEXT DEADLINE", "20 days", "Hillcrest"),
    ("ACCEPTANCES", "2", "in hand"),
    ("BEST NET PRICE", "$2,000", "per year"),
    ("TASKS DONE", "60%", "of the list"),
    ("READY SCORE", "63%", "blended"),
]
...
def content_dashboard(img, cbox):
    ...
    d.text((x0 + pad, y0 + 22), "Application Dashboard", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 64), "Ella Bennett · Class of 2027  ·  apply smart, hit every deadline, compare offers", font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 180, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 90, y0 + 44), "● 2 acceptances", font=fs(15), fill=PRIMARY, anchor="mm")
    ...
    d.text((gx, cy_top), "ON TRACK · LIST BALANCE · DEADLINES", font=fs(20), fill=ACCENT, anchor="lt")
    ...
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("Progress", 0.69, "69%"), ("Submitted", 0.50, "50%"), ("Essays", 0.50, "50%"),
           ("Recs", 0.75, "75%"), ("Tasks", 0.60, "60%")], color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Reach · Match · Safety", font=fs(17), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.42, panels_y + panel_h * 0.52, min(panel_h * 0.28, pw * 0.27),
          [(37.5, DANGER), (37.5, PRIMARY), (25, SURFACE)], "8", "colleges")
    legend(d, px + pw * 0.04, panels_y + panel_h - 104, [(DANGER, "Reach 3"), (PRIMARY, "Match 3"), (SURFACE, "Safety 2")], 15, 30)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Next Deadlines", font=fs(17), fill=ACCENT, anchor="lt")
    tasks = [("Hillcrest — 20 days", WARN_BG), ("Coastal U — 24 days", WARN_BG), ("Rotary schol. — 22d", MINT_BG),
             ("Northwood — 30 days", WARN_BG), ("Valley Tech — 30d", WARN_BG), ("Community (rolling)", MINT_BG)]
    ty = panels_y + 58
    ...
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Ready Score", font=fs(17), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.5, panels_y + panel_h * 0.54, min(panel_h * 0.30, pw * 0.30),
          [(63, PRIMARY), (37, SURFACE)], "63%", "on track")
...
def content_collegelist(img, cbox):
    rows = [
        ("State Flagship (EA)", "Match", "100%"),
        ("Riverside State", "Safety", "100%"),
        ("Lakeside College", "Match", "100%"),
        ("Hillcrest University", "Match", "75%"),
        ("Coastal U", "Reach", "50%"),
        ("Northwood University", "Reach", "25%"),
    ]
    _table(img, cbox, "College List",
           "A balanced list — reach, match & safety — with per-school progress.  4 of 8 submitted",
           ["COLLEGE", "TYPE", "PROGRESS"],
           [0.0, 0.60, 0.84], rows,
           status_col=1, status_map={"Reach": (WARN_BG, ACCENT), "Match": (MINT_BG, PRIMARY),
                                     "Safety": (SURFACE, (110, 88, 58))})


def content_netprice(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Financial Aid — Net Price", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 62), "Sticker price is a myth — compare what each school actually costs after aid.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 240, y0 + 24, x1 - pad, y0 + 60), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 120, y0 + 42), "● best: $2,000/yr", font=fs(15), fill=PRIMARY, anchor="mm")
    items = [
        ("Community Honors", 2000, "$2,000"),
        ("State Flagship", 18500, "$18,500"),
        ("Riverside State", 12000, "$12,000"),
        ("Hillcrest University", 22000, "$22,000"),
        ("Lakeside College", 20000, "$20,000"),
        ("Coastal U", 32000, "$32,000"),
    ]
    mx = max(v for _, v, _ in items)
    gx = x0 + pad; gy = y0 + 130; gw = x1 - x0 - 2 * pad
    rowh = (y1 - gy - pad) / len(items)
    bw_max = gw - 260
    best = min(v for _, v, _ in items)
    for i, (lab, val, vlab) in enumerate(items):
        yy = gy + i * rowh
        d.text((gx, yy + rowh * 0.26), lab, font=fs(22, bold=False), fill=TEXT, anchor="lt")
        d.rounded_rectangle((gx, yy + rowh * 0.52, gx + bw_max, yy + rowh * 0.52 + 34), radius=17, fill=(236, 230, 220))
        col = (HIGHLIGHT, (70, 200, 165)) if val == best else (GOLD_HI, GOLD)
        grad_round(img, (gx, yy + rowh * 0.52, gx + bw_max * (val / mx), yy + rowh * 0.52 + 34), 17, col[0], col[1])
        d.text((gx + bw_max + 20, yy + rowh * 0.52 + 17), vlab, font=fs(24), fill=PRIMARY, anchor="lm")


def content_essays(img, cbox):
    rows = [
        ("Common App", "Personal statement", "Final"),
        ("State Flagship", "Why us?", "Final"),
        ("Coastal U", "Why this major?", "Revising"),
        ("Northwood University", "Intellectual curiosity", "Revising"),
        ("Northwood University", "Community contribution", "Drafting"),
        ("Valley Tech", "Engineering interest", "Drafting"),
    ]
    _table(img, cbox, "Essays & Supplements",
           "Every prompt & where the draft stands — 5 of 10 final",
           ["SCHOOL", "PROMPT", "STATUS"],
           [0.0, 0.34, 0.80], rows,
           status_col=2, status_map={"Final": (MINT_BG, PRIMARY), "Revising": (WARN_BG, ACCENT),
                                     "Drafting": (SURFACE, (110, 88, 58))})


def content_scholarships(img, cbox):
    rows = [
        ("State Merit Scholarship", "$8,000", "Awarded"),
        ("Green Future Environmental", "$4,000", "Awarded"),
        ("First-Gen STEM Grant", "$5,000", "Applied"),
        ("Community Foundation", "$2,500", "Applied"),
        ("Rotary Club Local", "$1,500", "Applied"),
        ("National Merit", "$2,500", "Researching"),
    ]
    _table(img, cbox, "Scholarships & Aid",
           "Free money is worth the extra essays — $12,000 awarded so far",
           ["SCHOLARSHIP", "AMOUNT", "STATUS"],
           [0.0, 0.56, 0.82], rows,
           status_col=2, status_map={"Awarded": (MINT_BG, PRIMARY), "Applied": (WARN_BG, ACCENT),
                                     "Researching": (SURFACE, (110, 88, 58))})
...
def render_hero(out):
    ...
    pill(img, SIZE // 2, 256, "THE COMPLETE COLLEGE-ADMISSIONS SYSTEM", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "COLLEGE COMMAND CENTER", 84, max_w=1780)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 550), "College list, essays, recs, scholarships & offers — the whole admissions season, one system.",
       fs(20, bold=False), (224, 213, 190))
    chips = [("15", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("REACH", "MATCH · SAFETY")]
    cw = 440
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 238), "15 Connected Tabs", fserif(58), WHITE)
    gold_divider(img, SIZE // 2, 308, width=520)
    tc(d, (SIZE // 2, 352), "Not a checklist — a complete apply-smart, compare-offers admissions system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Start Here", "how it all works"), ("Application Dashboard", "apps, deadlines & Ready Score"),
        ("Applicant Profile", "your stats snapshot"), ("College List", "reach / match / safety + progress"),
        ("Essays", "every prompt & draft"), ("Recommendations", "asked & submitted"),
        ("Test Scores", "SAT / ACT / AP"), ("Activities", "your résumé"),
        ("Scholarships", "chase the free money"), ("Net Price", "true cost after aid"),
        ("Visits", "tours & interviews"), ("Decisions", "compare & choose"),
        ("To-Do", "the master list"), ("Deadlines", "in order, never missed"),
...
def render_list(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "THE COLLEGE LIST", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "A Balanced List That Tracks Itself", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Reach, match & safety — check off each piece and watch every application fill up",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 2, content_collegelist)
    img.convert("RGB").save(out, "PNG", optimize=True)


def render_netprice(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "COMPARE OFFERS", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "See What College Really Costs", fserif(46), WHITE)
    tc(d, (SIZE // 2, 300), "Net price after aid — not the sticker — so you compare offers apples-to-apples",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 8, content_netprice)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_plan(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 110, "WRITE IT · WIN IT", font=fs(32), pad_x=52, pad_y=22)
    tc(d, (SIZE // 2, 224), "Essays & Scholarships, Handled", fserif(44), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 3, content_essays)
    app_window(img, (60, 1180, SIZE - 60, SIZE - 60), 7, content_scholarships)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_printables(out):
    ...
    tc(d, (SIZE // 2, 404), "A matching print-ready PDF pack for the counselor meeting & the fridge",
       fs(22, bold=False), (226, 214, 190))
    labels = ["Applicant Profile", "College List", "Application Tracker", "Essay Tracker",
              "Recommendation Tracker", "Test Scores", "Activities Résumé", "Scholarship Log",
              "Net-Price Compare", "Visits & Interviews", "Decisions & Compare", "To-Do & Deadlines"]
...
def main():
    ...
    targets = [
        ...
        ("03_list.png", render_list),
        ("04_netprice.png", render_netprice),
