"""Detailed marketing image set for Personal & Private Chef Command Center™ (07-10)."""
...
from build_marketing import (
    ...
    ROW_ALT, fs, fserif, premium_bg, pill, wordmark, gold_divider, toque_crest,
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR PRIVATE CHEFS", "Cost It, Price It, Pay Yourself",
                "Four ways it turns private cheffing into a business that pays")
    feats = [
        ("$", "Live Chef Score", "donut", (90, "90%", "healthy"),
         "Food cost, margin, menu pricing, clients, profit & waste roll into one number — so you know your business health at a glance."),
        ("%", "Pay-Yourself Engine", "table", None,
         "Cost a dinner per guest, set your per-guest price, and the engine shows your take-home and your real hourly rate — the number chefs forget."),
        ("box", "Run the Business", "stock", None,
         "A service menu, dish costing, a client roster that becomes monthly revenue, a booking calendar and mileage for taxes — all in one place."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack — event quote, dish cost card, prep & shopping lists, event run sheet, mileage log & more for the kitchen."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("Event food + travel", "$162"), ("Your take-home", "$378"), ("Your hourly rate", "$54.00")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["PRIVATE DINNER · 6 GUESTS", "VALUE"], [0.0, 0.70]):
            ...
            for ri, (a, b) in enumerate(rows):
                ...
                od.text((tx0 + 14 + (tx1 - tx0) * 0.70, ry + 21), b, font=fs(16, bold=False), fill=TEXT, anchor="lm")
        elif kind == "stock":
            od = ImageDraw.Draw(img)
            items = [("Monthly revenue", "$5,950", MINT_BG, PRIMARY), ("Monthly profit", "$3,340", MINT_BG, PRIMARY),
                     ("Active clients", "5", MINT_BG, PRIMARY), ("Events / month", "13", SURFACE, (110, 88, 58))]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    ...
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Price List",
                "Everything a basic chef price list leaves out — built in")
    rows = [
        ("A per-event price list", True, True),
        ("A pay-yourself pricing engine", False, True),
        ("Your real hourly rate", False, True),
        ("Service menu & dish costing", False, True),
        ("Client roster → monthly revenue", False, True),
        ("Booking calendar", False, True),
        ("Grocery & kitchen-kit lists", False, True),
        ("Mileage log for taxes", False, True),
        ("Income & expenses", False, True),
        ("Live Chef Score dashboard", False, True),
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "SET UP IN AN AFTERNOON", "Run Your Chef Business in 4 Steps",
                "No spreadsheet skills needed — if you can type, you can run it")
    steps = [
        ("1", "Cost your dinners", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack, then cost a dinner per guest."),
        ("2", "Price to pay yourself", "Set your per-guest price and the engine shows your take-home and your real hourly rate after food and travel — live."),
        ("3", "Build menu & book clients", "Build your service menu, cost your signature dishes, and add clients and bookings — monthly revenue rolls up on its own."),
        ("4", "Run & grow", "Track mileage for taxes, keep your income and expenses, log waste, and watch your profit and your Chef Score climb."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + Chef Score",
        "Pay-yourself pricing engine", "Service menu & dish costing",
        "Client roster → monthly revenue", "Booking calendar & run sheets",
        "Mileage, grocery & waste logs", "Start-Here quick-start guide",
    ]
    ...
    who = ["Personal & private chefs", "In-home dinner-party chefs", "Meal-prep & drop-off chefs",
           "Yacht & villa chefs", "Caterers going solo", "Anyone cooking for private clients"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "COST IT · PRICE IT · PAY YOURSELF", font=fs(27), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Cost every dinner, price to pay yourself, and run a chef business that pays.",
       fs(23, bold=False), TEXT_MUTED)
    toque_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "PRIVATE CHEF", 64, max_w=1100)
    img.convert("RGB").save(out, "PNG", optimize=True)
