"""Marketing image set for Personal & Private Chef Command Center™ (6 images, 2000x2000)."""
...
TABS = ["Dashboard", "Event Pricing", "Service Menu", "Dish Costing", "Clients", "Booking Calendar",
        "Grocery List", "Kitchen Kit", "Mileage & Travel", "Waste Log",
        "Income & Expenses", "Monthly Summary", "Settings"]

FILE_LABEL = "Private_Chef_Command_Center.xlsx — Chef's Table Co. · Rowan"
...
def toque_crest(c, cx, cy, r=56, glow=True):
    """Brand crest — a chef's toque: the icon of the private chef."""
    if glow:
    ...
    c.alpha_composite(ov)
    ov = Image.new("RGBA", c.size, (0, 0, 0, 0)); d = ImageDraw.Draw(ov)
    # puffs (three white clouds at the top of the toque)
    for dx, rr in [(-0.28, 0.22), (0.0, 0.28), (0.28, 0.22)]:
        px = cx + r * dx; py = cy - r * 0.22
        d.ellipse((px - r * rr, py - r * rr, px + r * rr, py + r * rr), fill=WHITE)
    # hat body between puffs and band
    d.rectangle((cx - r * 0.36, cy - r * 0.22, cx + r * 0.36, cy + r * 0.16), fill=WHITE)
    # band
    band = (cx - r * 0.42, cy + r * 0.14, cx + r * 0.42, cy + r * 0.42)
    d.rounded_rectangle(band, radius=6, fill=GOLD_HI, outline=(150, 120, 70))
    for fx in (-0.22, 0.0, 0.22):
        x = cx + r * fx
        d.line((x, band[1] + 3, x, band[3] - 3), fill=(150, 120, 70), width=2)
    c.alpha_composite(ov)


# aliases (shared render helpers may reference these names across products)
bento_crest = toque_crest
cupcake_crest = toque_crest
clock_crest = toque_crest
clipboard_crest = toque_crest
cloche_crest = toque_crest
ghost_crest = toque_crest
mug_crest = toque_crest
book_crest = toque_crest
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "PRIVATE CHEF", font=fs(15), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("FOOD / GUEST", "$22.00", "per guest"),
    ("PRICE / GUEST", "$90", "you charge"),
    ("FOOD COST", "24%", "of price"),
    ("MARGIN/EVENT", "70%", "your take"),
    ("YOUR HOURLY", "$54.00", "real rate"),
    ("TOP CLIENT", "Anderson", "$2,160/mo"),
    ("MONTHLY REV", "$5,950", "this month"),
    ("ACTIVE CLIENTS", "5", "on roster"),
    ("EVENTS / MONTH", "13", "booked"),
    ("MONTHLY PROFIT", "$3,340", "after costs"),
    ("WASTE", "2.4%", "of revenue"),
    ("CHEF SCORE", "90%", "healthy"),
]

REV_MONTHS = [("Jul", 4800), ("Aug", 5200), ("Sep", 5950), ("Oct", 6400), ("Nov", 7100), ("Dec", 8200)]


def content_dashboard(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Private Chef Dashboard", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 64), "Chef's Table Co. · Rowan  ·  cost every dinner, and pay yourself properly", font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 210, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 105, y0 + 44), "● $54.00/hr take-home", font=fs(14), fill=PRIMARY, anchor="mm")
    ...
    d.text((gx, cy_top), "CHEF HEALTH · WHERE THE PRICE GOES · REVENUE BY MONTH", font=fs(20), fill=ACCENT, anchor="lt")
    ...
    d.text((px + 16, panels_y + 14), "Chef Health", font=fs(17), fill=ACCENT, anchor="lt")
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("Food cost on target", 1.0, "100%"), ("Margin healthy", 1.0, "100%"), ("Menu priced", 1.0, "100%"),
           ("Clients vs goal", 1.0, "100%"), ("Profitable", 1.0, "100%"), ("Waste low", 0.40, "40%")],
          color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Where the Price Goes", font=fs(16), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.44, panels_y + panel_h * 0.50, min(panel_h * 0.27, pw * 0.27),
          [(70, PRIMARY), (24, ACCENT), (6, GOLD_LT)], "70%", "your take")
    legend(d, px + pw * 0.05, panels_y + panel_h - 108, [(PRIMARY, "Take-home 70%"), (ACCENT, "Food 24%"), (GOLD_LT, "Travel 6%")], 14, 30)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Revenue by Month", font=fs(17), fill=ACCENT, anchor="lt")
    vbars(img, d, (px + 18, panels_y + 60, px + pw - 14, panels_y + panel_h - 10), REV_MONTHS, 8200)
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Chef Score", font=fs(17), fill=ACCENT, anchor="lt")
...
def content_menu(img, cbox):
    ...
    _table(img, cbox, "Service Menu",
           "Your packages, guests and rates — simple to quote, priced for profit.",
           ["PACKAGE", "GUESTS", "BASIS", "PRICE"],
           [0.0, 0.52, 0.68, 0.88], rows)


def content_dishes(img, cbox):
    rows = [
        ("Filet Mignon", "Main", "$12.50", "$42", "30%"),
        ("Pan-Seared Salmon", "Main", "$9.40", "$34", "28%"),
        ("Herb-Roast Chicken", "Main", "$7.20", "$30", "24%"),
        ("Truffle Risotto", "Main", "$5.60", "$26", "22%"),
        ("Seared Scallops", "Appetizer", "$5.20", "$18", "29%"),
        ("Chocolate Torte", "Dessert", "$2.90", "$12", "24%"),
    ]
    _table(img, cbox, "Dish Costing",
           "Cost each signature dish per guest — cost, menu price and food-cost %.",
           ["DISH", "COURSE", "COST", "PRICE", "FOOD %"],
           [0.0, 0.42, 0.60, 0.74, 0.88], rows)


def content_eventprice(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Event Pricing — Pay Yourself", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 62), "The number chefs forget: your real hourly rate after food and travel.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("Event price  ·  6 guests × $90", "$540", PRIMARY), ("− Event food  ·  $22/guest", "−$132", DANGER),
            ("− Travel / mileage", "−$30", DANGER)]
    ...
    d.text((x0 + pad + 14, ry + rh / 2), "= YOUR TAKE-HOME", font=fs(18), fill=PRIMARY, anchor="lm")
    d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.80, ry + rh / 2), "$378", font=fs(18), fill=PRIMARY, anchor="mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2), "÷ 7 hours worked  =  $54.00 an hour, in your pocket", font=fs(22), fill=WHITE, anchor="mm")
    img.alpha_composite(ov)
...
def content_clients(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Clients → Monthly Revenue", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 62), "Your roster — service, events a month and rate — rolls up to monthly revenue.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("Anderson  ·  weekly dinner ×4", "$2,160", PRIMARY), ("Chen  ·  meal prep ×4", "$1,120", PRIMARY),
            ("Bello  ·  dinner ×2", "$1,080", PRIMARY), ("Ruiz  ·  dinner party", "$990", PRIMARY),
            ("Okoro  ·  date night ×2", "$600", PRIMARY)]
    ...
    ty = top_region + max(((y1 - pad) - top_region - block_h) / 2, 0)
    grad_round(img, (x0 + pad, ty, x1 - pad, ty + rh), 8, PRIMARY_LT, PRIMARY_DK)
    for h, fx in zip(["CLIENT  ·  SERVICE", "MONTHLY"], [0.0, 0.80]):
        d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * fx, ty + rh / 2), h, font=fs(16), fill=WHITE, anchor="lm" if fx == 0 else "mm")
    for i, (label, val, col) in enumerate(rows):
        ry = ty + rh + i * rh
        if i % 2:
        ...
        d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.80, ry + rh / 2), val, font=fs(17, bold=True), fill=col, anchor="mm")
    ry = ty + rh + len(rows) * rh
    d.rectangle((x0 + pad, ry, x1 - pad, ry + rh), fill=SURFACE)
    d.text((x0 + pad + 14, ry + rh / 2), "= MONTHLY REVENUE  ·  5 clients", font=fs(17), fill=PRIMARY, anchor="lm")
    d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.80, ry + rh / 2), "$5,950", font=fs(17), fill=PRIMARY, anchor="mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2), "13 events booked this month  ·  $3,340 profit after costs", font=fs(21), fill=WHITE, anchor="mm")
    img.alpha_composite(ov)
...
def render_hero(out):
    ...
    toque_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE COMPLETE PRIVATE-CHEF SYSTEM", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "PRIVATE CHEF", 92, max_w=1500)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 546), "Event pricing, your real hourly rate, a service menu, clients, bookings & books — one system.",
       fs(20, bold=False), (224, 213, 190))
    chips = [("14", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("PAY-YOU", "ENGINE")]
    cw = 440
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not a price list — a complete cost-it, price-it, pay-yourself system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Start Here", "how it all works"), ("Dashboard", "revenue, profit & score"),
        ("Event Pricing", "your real hourly rate"), ("Service Menu", "packages & rates"),
        ("Dish Costing", "cost every dish"), ("Clients", "roster → monthly rev"),
        ("Booking Calendar", "who's on the books"), ("Grocery List", "shop by vendor"),
        ("Kitchen Kit", "pack the kit"), ("Mileage & Travel", "every drive, for taxes"),
        ("Waste Log", "the quiet leaks"), ("Income & Expenses", "your monthly books"),
        ("Monthly Summary", "watch the season build"), ("Settings", "set it once"),
        ("+ 12 Printable Pages", "quotes & run sheets"), ("Live Chef Score", "it all rolls up"),
    ]
    cols = 4
    margin = 88
    gx, gy = 22, 20
...
def render_eventprice(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "PAY YOURSELF", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "Your Real Hourly Rate", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "The number private chefs forget — what you truly earn per hour after food and travel",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 1, content_eventprice)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_clients(out):
    ...
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "YOUR ROSTER", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "Clients Become Revenue", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Track who's on the books — service, events a month and rate roll up to your monthly revenue",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 4, content_clients)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_engine(out):
    ...
    tc(d, (SIZE // 2, 224), "The Private-Chef Pricing Engine", fserif(42), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 1, content_eventprice)
...
def render_printables(out):
    ...
    d = ImageDraw.Draw(img)
    toque_crest(img, SIZE // 2, 110, r=48)
    ...
    tc(d, (SIZE // 2, 404), "A matching print-ready pack — event quote, dish cost card, run sheet & more",
       fs(21, bold=False), (226, 214, 190))
    labels = ["Event Quote", "Service Menu", "Dish Cost Card", "Client Roster",
              "Prep List", "Shopping List", "Kitchen Kit", "Event Run Sheet",
              "Mileage & Travel", "Income & Expenses", "Monthly Summary", "Event Day Checklist"]
...
def main():
    ...
    targets = [
        ...
        ("03_clients.png", render_clients),
        ("04_pricing.png", render_eventprice),
