"""Printable PDF pack for Personal & Private Chef Command Center™ (12 pages, US Letter)."""
...
def page(title, subtitle):
    ...
    d.text((M, 90), "PERSONAL & PRIVATE CHEF COMMAND CENTER™", font=fs(32), fill=GOLD_HI, anchor="lt")
    ...
    d.text((W - M, H - 108), "Private Chef Command Center™", font=fs(22, bold=False), fill=ACCENT, anchor="rt")
    return img, d
...
def p01(i):
    img, d = page("Event Quote", "Cost a dinner, per guest — then price it")
    field(d, M, TOP, CW / 2 - 30, "Client / event"); field(d, M + CW / 2 + 30, TOP, CW / 2 - 30, "Guests")
    table(d, M, TOP + 150, CW, ["COURSE / COMPONENT", "COST / GUEST"], [0.0, 0.78], 9, 150)
    y = TOP + 150 + 78 + 9 * 150 + 20
    field(d, M, y, CW / 2 - 30, "Price / guest"); field(d, M + CW / 2 + 30, y, CW / 2 - 30, "Your take-home")
    i.append(img)


def p02(i):
    img, d = page("Service Menu", "Your packages & rates")
    table(d, M, TOP, CW, ["PACKAGE", "GUESTS", "BASIS", "PRICE"], [0.0, 0.46, 0.64, 0.84], 12, 175)
    i.append(img)


def p03(i):
    img, d = page("Dish Cost Card", "Cost each signature dish")
    table(d, M, TOP, CW, ["DISH", "COURSE", "COST", "PRICE", "FOOD %"], [0.0, 0.40, 0.58, 0.72, 0.86], 18, 120)
    i.append(img)


def p04(i):
    img, d = page("Client Roster", "Who's on the books")
    table(d, M, TOP, CW, ["CLIENT", "SERVICE", "EVENTS/MO", "RATE", "MONTHLY"], [0.0, 0.30, 0.54, 0.70, 0.86], 18, 120)
    i.append(img)


def p05(i):
    img, d = page("Prep List", "The mise en place")
    table(d, M, TOP, CW, ["DISH / COMPONENT", "QTY", "PREP", "DONE"], [0.0, 0.52, 0.70, 0.88], 18, 120)
    i.append(img)


def p06(i):
    img, d = page("Shopping List", "By vendor")
    table(d, M, TOP, CW, ["ITEM / CATEGORY", "VENDOR", "QTY", "COST"], [0.0, 0.40, 0.66, 0.84], 18, 120)
    i.append(img)


def p07(i):
    img, d = page("Kitchen Kit", "Pack the travelling kitchen")
    half = CW / 2 - 30
    for k, st in enumerate(["Tools & equipment", "Serviceware & extras"]):
        x = M + k * (half + 60)
...
def p08(i):
    img, d = page("Event Run Sheet", "The timeline to service")
    table(d, M, TOP, CW, ["TIME", "TASK", "DONE"], [0.0, 0.22, 0.88], 18, 130)
    i.append(img)


def p09(i):
    img, d = page("Mileage & Travel", "Every drive, for taxes")
    table(d, M, TOP, CW, ["DATE", "CLIENT", "MILES", "COST"], [0.0, 0.22, 0.62, 0.82], 20, 118)
    i.append(img)
...
def p11(i):
    img, d = page("Monthly Summary", "Revenue, expenses & profit")
    table(d, M, TOP, CW, ["MONTH", "REVENUE", "EXPENSES", "PROFIT"], [0.0, 0.30, 0.54, 0.78], 14, 155)
    i.append(img)


def p12(i):
    img, d = page("Event Day Checklist", "Same standard, every event")
    half = CW / 2 - 30
    for k, st in enumerate(["Before you leave", "At the client"]):
        x = M + k * (half + 60)
        section(d, x, TOP, half, st)
        for j in range(14):
            checkbox(d, x, TOP + 96 + j * 96)
...
def main():
    ...
    pdf_path = os.path.join(out_dir, "Private_Chef_Printables.pdf")
