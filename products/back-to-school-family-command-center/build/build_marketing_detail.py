"""Detailed marketing image set for Back-to-School Command Center™ (4 images, 2000x2000).

  07_features.png   - feature spotlights (what it does + why it matters)
  08_compare.png    - "basic checklist" vs Back-to-School Command Center™
...
from build_marketing import (
    ...
    ROW_ALT, fs, fserif, premium_bg, pill, wordmark, gold_divider, backpack_crest,
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR A FULL HOUSE", "Where Parents Win",
                "Four ways it turns August chaos into one calm system")
    feats = [
        ("$", "Live Family Dashboard", "donut", (82, "82%", "ready"),
         "Supplies, fees, uniforms & forms roll into one Readiness Score — see at a glance how close to first-day-ready you really are."),
        ("%", "A Profile for Every Child", "table", None,
         "One organized profile per child — grade, teacher, sizes, allergies & emergency contacts. Scales from 1 kid to a full house of 8."),
        ("box", "The Budget That Adds Up", "stock", None,
         "Set a number and track every dollar — supplies, clothes, fees & tech, planned vs actual, with exactly what's left."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready PDF pack — child info, backpack & supply checklists, weekly schedule, lunchbox planner & keepsake memory pages."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        od.text((x + 124, by - 16), title, font=fs(28), fill=PRIMARY, anchor="lm")
        ...
        if kind == "donut":
            ...
            rows = [("Mateo — Grade 9", "Lincoln High"), ("Ava — Grade 3", "Oakwood"), ("Emma — PreK", "Little Sprouts")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["CHILD", "SCHOOL"], [0.0, 0.60]):
            ...
            od = ImageDraw.Draw(img)
            items = [("Clothing", "$415", MINT_BG, PRIMARY), ("School Fees", "$300", MINT_BG, PRIMARY),
                     ("Supplies", "$300", SURFACE, (110, 88, 58)), ("Devices (to buy)", "$0", WARN_BG, ACCENT)]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            ...
            # mini row of 5 printable thumbnails
            tw = cw - 88; th = int((tw / 5) * 11 / 8.5)
            pw = (tw - 4 * 16) / 5
            ph = int(pw * 11 / 8.5)
            px0 = x + 44; py0 = vy - 40
            for k in range(5):
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Checklist",
                "Everything a basic back-to-school printable leaves out")
    rows = [
        ("First-day & supply checklists", True, True),
        ("A profile for every child", False, True),
        ("All schools & teachers in one place", False, True),
        ("Budget vs actual spending", False, True),
        ("Fees & payment tracking", False, True),
        ("Live back-to-school Readiness Score", False, True),
        ("Lunch, homework & reading logs", False, True),
        ("Parent-teacher comms & absences", False, True),
        ("Grades & report-card overview", False, True),
        ("Scales from 1 to 8 kids", False, True),
...
def render_howitworks(out):
    ...
    steps = [
        ...
        ("2", "Set up your family", "In Settings, add your family, school year, first day & budget — then fill a quick profile for each child."),
        ("3", "Load your season", "Add schools, the calendar, supplies, fees & uniforms. It all rolls into one live Family Dashboard."),
        ("4", "Stay ahead all year", "Check the dashboard for what's due, print the pages for the fridge & backpacks, and breathe."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "EVERYTHING YOU GET", "One System, Your Whole Season",
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live Family Dashboard (12 KPIs)",
        "A profile for every child (1–8)", "Budget, fees & supply trackers",
        "Lunch, homework & comms logs", "First-day & memory keepsakes",
        "Start-Here quick-start guide", "Free lifetime updates",
    ]
    ...
    who = ["Busy parents of 1–8 kids", "Large & blended families", "Multiple schools & schedules",
           "Grandparents & caregivers", "Foster & kinship families", "ADHD-household organizers"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "BUILT BY A MOM OF SIX — TESTED ON A FULL HOUSE", font=fs(28), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "The calm, coordinated system I wished existed every August.",
       fs(24, bold=False), TEXT_MUTED)
    backpack_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "BACK-TO-SCHOOL COMMAND CENTER", 48, max_w=1560)
    img.convert("RGB").save(out, "PNG", optimize=True)
