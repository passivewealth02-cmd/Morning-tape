"""Marketing image set for Handmade & Maker Pricing Command Center™ (6 images, 2000x2000)."""
...
TABS = ["Dashboard", "Pricing Calculator", "Product Line", "Supplies & Inventory", "Orders", "Overhead & Fees",
        "Sales Log", "Time & Labor", "Expenses", "Reviews & Repeat", "Channels & Markets",
        "Monthly Summary", "Settings"]

FILE_LABEL = "Maker_Pricing_Command_Center.xlsx — Ember & Oak · Wren"
...
def candle_crest(c, cx, cy, r=56, glow=True):
    """Brand crest — a lit candle: the icon of the handmade / maker shop."""
    if glow:
    ...
    ov = Image.new("RGBA", c.size, (0, 0, 0, 0)); d = ImageDraw.Draw(ov)
    # candle jar / body
    d.rounded_rectangle((cx - r * 0.32, cy - r * 0.04, cx + r * 0.32, cy + r * 0.44), radius=7, fill=GOLD_HI, outline=(150, 120, 70))
    # wax top rim
    d.rounded_rectangle((cx - r * 0.32, cy - r * 0.08, cx + r * 0.32, cy + r * 0.04), radius=4, fill=SURFACE, outline=(150, 120, 70))
    # wick
    d.line((cx, cy - r * 0.08, cx, cy - r * 0.24), fill=PRIMARY_DK, width=3)
    # flame (teardrop)
    d.ellipse((cx - r * 0.11, cy - r * 0.40, cx + r * 0.11, cy - r * 0.16), fill=FLAME)
    d.polygon([(cx, cy - r * 0.54), (cx - r * 0.11, cy - r * 0.28), (cx + r * 0.11, cy - r * 0.28)], fill=FLAME)
    # flame inner glow
    d.ellipse((cx - r * 0.05, cy - r * 0.34, cx + r * 0.05, cy - r * 0.20), fill=HIGHLIGHT)
    # label band on jar
    d.rectangle((cx - r * 0.32, cy + r * 0.14, cx + r * 0.32, cy + r * 0.26), fill=PRIMARY)
    c.alpha_composite(ov)


# aliases (shared render helpers may reference these names across products)
camera_crest = candle_crest
sign_crest = candle_crest
spray_crest = candle_crest
briefcase_crest = candle_crest
house_crest = candle_crest
tent_crest = candle_crest
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "MAKER SHOP", font=fs(15), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("MATERIALS", "$6.00", "per item"),
    ("YOUR LABOR", "$5.00", "0.25 hr"),
    ("BASE COST", "$12.00", "all-in"),
    ("WHOLESALE", "$15.00", "×0.5"),
    ("RETAIL", "$30.00", "×2.5"),
    ("RETAIL MARGIN", "60%", "of price"),
    ("PROFIT / ITEM", "$18.00", "retail"),
    ("YOUR HOURLY", "$92.00", "true wage"),
    ("MONTHLY REVENUE", "$2,700", "this month"),
    ("UNITS SOLD", "90", "this month"),
    ("MONTHLY PROFIT", "$1,350", "paid you first"),
    ("MAKER SCORE", "90%", "healthy"),
]

REV_MONTHS = [("Feb", 1800), ("Mar", 2100), ("Apr", 2400), ("May", 2600), ("Jun", 2500), ("Jul", 2700)]


def content_dashboard(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Maker Dashboard", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 64), "Ember & Oak · Wren  ·  price for profit, and pay yourself for your time", font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 210, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 105, y0 + 44), "● $92/hr true wage", font=fs(14), fill=PRIMARY, anchor="mm")
    ...
    d.text((gx, cy_top), "MAKER HEALTH · WHERE THE RETAIL $ GOES · REVENUE BY MONTH", font=fs(19), fill=ACCENT, anchor="lt")
    ...
    d.text((px + 16, panels_y + 14), "Maker Health", font=fs(17), fill=ACCENT, anchor="lt")
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("Retail margin healthy", 1.0, "100%"), ("Markup healthy", 1.0, "100%"), ("Paying yourself", 1.0, "100%"),
           ("Products priced", 1.0, "100%"), ("Profitable", 1.0, "100%"), ("Wholesale margin", 0.40, "40%")],
          color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Where the Retail $ Goes", font=fs(15), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.44, panels_y + panel_h * 0.50, min(panel_h * 0.27, pw * 0.27),
          [(60, PRIMARY), (40, ACCENT)], "60%", "your profit")
    legend(d, px + pw * 0.06, panels_y + panel_h - 96, [(PRIMARY, "Your profit 60%"), (ACCENT, "Base cost 40%")], 15, 32)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Revenue by Month", font=fs(17), fill=ACCENT, anchor="lt")
    vbars(img, d, (px + 18, panels_y + 60, px + pw - 14, panels_y + panel_h - 10), REV_MONTHS, 2700)
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Maker Score", font=fs(17), fill=ACCENT, anchor="lt")
...
def content_productline(img, cbox):
    rows = [
        ("Soy Candle 8oz", "$12.00", "$15", "$30", "60%"),
        ("Soy Candle 4oz", "$7.50", "$9", "$18", "58%"),
        ("Wax Melts (6)", "$5.00", "$7", "$14", "64%"),
        ("Reed Diffuser", "$9.00", "$12", "$24", "63%"),
        ("Candle Trio Set", "$30.00", "$36", "$72", "58%"),
        ("Room Spray", "$4.50", "$6", "$12", "63%"),
    ]
    _table(img, cbox, "Product Line",
           "Every product — base cost, wholesale, retail and margin. Price the whole line for profit.",
           ["PRODUCT", "BASE COST", "WHOLESALE", "RETAIL", "MARGIN"],
           [0.0, 0.42, 0.60, 0.76, 0.90], rows,
           total_row=("6 products", "", "", "avg $30", "60%"))


def content_pricing(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Pricing Calculator — Base Cost", font=fs(30), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 60), "Materials + your labor + overhead = base cost, then a markup gives retail & wholesale.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("Materials · 6 items (wax, oil, jar…)", "$6.00", PRIMARY),
            ("+ Your labor · 0.25 hr × $20", "$5.00", PRIMARY),
            ("+ Overhead per item", "$1.00", PRIMARY)]
    ...
    d.text((x0 + pad + 14, ry + rh / 2), "= BASE COST", font=fs(18), fill=PRIMARY, anchor="lm")
    d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.80, ry + rh / 2), "$12.00", font=fs(18), fill=PRIMARY, anchor="mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2), "× 2.5 markup  =  $30.00 retail  ·  $15.00 wholesale", font=fs(22), fill=WHITE, anchor="mm")
    img.alpha_composite(ov)


def content_wage(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Pay Yourself — Your True Wage", font=fs(30), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 60), "Take retail, subtract materials and overhead — what's left pays you for your time.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("Retail price · Soy Candle 8oz", "$30.00", PRIMARY),
            ("− Materials", "−$6.00", DANGER),
            ("− Overhead per item", "−$1.00", DANGER)]
    callout_h = 150; gap = 44; rh = 108
    n_units = 1 + len(rows) + 1
    top_region = y0 + 108
    block_h = rh * n_units + gap + callout_h
    ...
    ry = ty + rh + len(rows) * rh
    d.rectangle((x0 + pad, ry, x1 - pad, ry + rh), fill=SURFACE)
    d.text((x0 + pad + 14, ry + rh / 2), "= FOR YOUR TIME", font=fs(18), fill=PRIMARY, anchor="lm")
    d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.80, ry + rh / 2), "$23.00", font=fs(18), fill=PRIMARY, anchor="mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2), "÷ 0.25 hr  =  $92.00 an hour  ·  your true wage", font=fs(23), fill=WHITE, anchor="mm")
    img.alpha_composite(ov)
...
def render_hero(out):
    ...
    candle_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE COMPLETE HANDMADE PRICING SYSTEM", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "MAKER PRICING", 88, max_w=1550)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 546), "A price-for-profit engine, a product line, orders, supplies & books — one system.",
       fs(20, bold=False), (224, 213, 190))
    chips = [("14", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("PRICE", "ENGINE")]
    cw = 440
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not a price list — a complete price-for-profit, pay-yourself system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Start Here", "how it all works"), ("Dashboard", "profit & score"),
        ("Pricing Calculator", "price for profit"), ("Product Line", "your whole menu"),
        ("Supplies & Inventory", "never run out"), ("Orders", "nothing ships late"),
        ("Overhead & Fees", "the real costs"), ("Sales Log", "revenue & units"),
        ("Time & Labor", "pay for your time"), ("Expenses", "for tax time"),
        ("Reviews & Repeat", "your best buyers"), ("Channels & Markets", "where sales come from"),
        ("Monthly Summary", "watch profit grow"), ("Settings", "set it once"),
        ("+ 12 Printable Pages", "worksheets & lists"), ("Live Maker Score", "it all rolls up"),
    ]
    cols = 4
    margin = 88
    gx, gy = 22, 20
...
def render_pricing(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "PRICE FOR PROFIT", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "The Pricing Engine", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Add your materials, your labor and your overhead — get a wholesale and a retail price that pays",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 1, content_pricing)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_wage(out):
    ...
    tc(d, (SIZE // 2, 232), "Your True Hourly Wage", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "A handmade product that doesn't pay you for your time isn't priced right — this shows what you earn",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 1, content_wage)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_engine(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 110, "PRICE IT · SELL IT", font=fs(32), pad_x=52, pad_y=22)
    tc(d, (SIZE // 2, 224), "The Maker Engine", fserif(44), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 1, content_pricing)
    app_window(img, (60, 1180, SIZE - 60, SIZE - 60), 2, content_productline)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_printables(out):
    ...
    d = ImageDraw.Draw(img)
    candle_crest(img, SIZE // 2, 110, r=48)
    pill(img, SIZE // 2, 214, "PLUS 12 PRINTABLE PAGES", font=fs(30), pad_x=48, pad_y=20)
    tc(d, (SIZE // 2, 300), "Print, Fill & Keep — At the Bench", fserif(44), WHITE)
    gold_divider(img, SIZE // 2, 362, width=480)
    tc(d, (SIZE // 2, 404), "A matching print-ready pack — pricing worksheet, product price list, order form, supply list & more",
       fs(21, bold=False), (226, 214, 190))
    labels = ["Pricing Worksheet", "Product Price List", "Order Form", "Supply List",
              "Overhead & Fees", "Sales Log", "Time & Labor", "Expense Log",
              "Channel Tracker", "Monthly Summary", "Craft Show List", "Making Checklist"]
...
def main():
    ...
    targets = [
        ...
        ("03_pricing.png", render_pricing),
        ("04_wage.png", render_wage),
