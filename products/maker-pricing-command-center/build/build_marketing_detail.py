"""Detailed marketing image set for Handmade & Maker Pricing Command Center™ (07-10)."""
...
from build_marketing import (
    ...
    ROW_ALT, fs, fserif, premium_bg, pill, wordmark, gold_divider, candle_crest,
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR MAKERS", "Price for Profit, Pay Yourself",
                "Four ways it turns a craft into a business that pays")
    feats = [
        ("$", "Live Maker Score", "donut", (90, "90%", "healthy"),
         "Retail margin, markup, paying yourself, products priced, profit and wholesale margin roll into one number — your shop health at a glance."),
        ("%", "Price-for-Profit Engine", "table", None,
         "Your materials, your labor and your overhead give a base cost, then a markup gives a wholesale and a retail price that pays."),
        ("box", "Run the Shop", "stock", None,
         "A product line, supplies, orders, a sales log, overhead, expenses and channels — the whole handmade shop in one place."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack — pricing worksheet, product price list, order form, supply list & more."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("Base cost", "$12.00"), ("× 2.5 markup", "$30.00"), ("Retail price", "$30.00")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["PRICE FOR PROFIT", "VALUE"], [0.0, 0.66]):
            ...
            od = ImageDraw.Draw(img)
            items = [("Profit / item", "$18.00", MINT_BG, PRIMARY), ("Your true wage", "$92/hr", MINT_BG, PRIMARY),
                     ("Monthly profit", "$1,350", MINT_BG, PRIMARY), ("Wholesale margin", "40%", SURFACE, (110, 88, 58))]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Pricing Sheet",
                "Everything a basic price calculator leaves out — built in")
    rows = [
        ("A retail price calculator", True, True),
        ("Labor & overhead in the price", False, True),
        ("Your true hourly wage", False, True),
        ("Wholesale AND retail pricing", False, True),
        ("A full product line & margins", False, True),
        ("Supplies & reorder tracking", False, True),
        ("Orders, sales & channels", False, True),
        ("Overhead, fees & expenses", False, True),
        ("Revenue & profit by month", False, True),
        ("Live Maker Score dashboard", False, True),
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "SET UP IN AN AFTERNOON", "Run Your Shop in 4 Steps",
                "No spreadsheet skills needed — if you can type, you can run it")
    steps = [
        ("1", "Price a product", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack, then price a product from its materials, labor and overhead."),
        ("2", "Read your numbers", "See the base cost, the retail and wholesale price, the profit per item and your true hourly wage — before you ever list it."),
        ("3", "Build & sell", "Build your product line, run orders and supplies, log sales, and track overhead, fees and expenses across every channel."),
        ("4", "Grow the shop", "Watch your revenue and profit by month, keep an eye on your repeat customers, and let the Maker Score keep the whole shop honest."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "EVERYTHING YOU GET", "One System, Whole Shop",
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + Maker Score",
        "Price-for-profit engine", "Wholesale & retail + true wage",
        "Product line, supplies & orders", "Sales, overhead & expenses",
        "Channels & monthly summary", "Start-Here quick-start guide",
    ]
    ...
    who = ["Candle & soap makers", "Jewelry & accessory makers", "Ceramic, resin & woodworkers",
           "Knit, crochet & textile makers", "New & part-time makers", "Anyone selling handmade goods"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "PRICE IT · SELL IT · PROFIT", font=fs(27), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Price for real profit, and finally pay yourself for your time.",
       fs(23, bold=False), TEXT_MUTED)
    candle_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "MAKER PRICING", 58, max_w=1150)
    img.convert("RGB").save(out, "PNG", optimize=True)
