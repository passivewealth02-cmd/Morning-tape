"""Detailed marketing image set for Recipe Costing & Menu Engineering Command Center™ (07-10)."""
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR PROFIT", "Where a Menu Makes Money",
                "Four ways it turns guesswork into a profitable menu")
    feats = [
        ("$", "Live Menu Score", "donut", (92, "92%", "menu health"),
         "Food cost on target, margins vs goal, contribution & menu balance roll into one number — so you know your menu's financial health at a glance."),
        ("%", "Cost Every Plate", "table", None,
         "An ingredient library gives cost per unit; recipes cost themselves line by line. Change a supplier price and every affected plate re-costs instantly."),
        ("box", "Engineer the Menu", "stock", None,
         "Every item is classed Star, Plowhorse, Puzzle or Dog by popularity × profit — so you know exactly what to feature, reprice, rework or cut."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack — recipe cost cards, a menu-engineering worksheet, prep lists, a pricing guide & a weekly food-cost tracker for the line."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "table":
            ...
            od.rectangle((bx - 22, by - 22, bx - 2, by - 2), fill=GOLD_HI)
            od.rectangle((bx + 2, by - 22, bx + 22, by - 2), outline=GOLD_HI, width=3)
            od.rectangle((bx - 22, by + 2, bx - 2, by + 22), outline=GOLD_HI, width=3)
            od.rectangle((bx + 2, by + 2, bx + 22, by + 22), fill=GOLD_HI)
        elif kind == "print":
        ...
        if kind == "donut":
            ...
            rows = [("Ground beef", "$0.28/oz"), ("Brioche bun", "$0.55/ea"), ("Burger plate", "$2.92")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["INGREDIENT", "COST"], [0.0, 0.62]):
            ...
            od = ImageDraw.Draw(img)
            items = [("Stars", "1", MINT_BG, PRIMARY), ("Plowhorses", "4", WARN_BG, (110, 88, 58)),
                     ("Puzzles", "2", SURFACE, (110, 88, 58)), ("Dogs", "1", RED_BG, DANGER)]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            for ri, (nm, st, bg, fg) in enumerate(items):
                ...
                od.rounded_rectangle((tx1 - 90, ry + 6, tx1 - 12, ry + 32), radius=13, fill=bg)
                od.text((tx1 - 51, ry + 19), st, font=fs(15), fill=fg, anchor="mm")
        elif kind == "print":
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Costing Sheet",
                "Everything a basic recipe calculator leaves out — built in")
    rows = [
        ("A recipe cost calculator", True, True),
        ("Ingredient price library", False, True),
        ("Food-cost % on every item", False, True),
        ("Menu-engineering matrix", False, True),
        ("Target-margin price calculator", False, True),
        ("Sales-mix profit contribution", False, True),
        ("Portion & yield (true cost)", False, True),
        ("Specials, batch & vendor tools", False, True),
        ("Waste & food-cost tracking", False, True),
        ("Live Menu Score dashboard", False, True),
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "SET UP IN AN AFTERNOON", "Profit Your Menu in 4 Steps",
                "No spreadsheet skills needed — if you can type, you can run it")
    steps = [
        ("1", "Build the price library", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack, then enter each ingredient's pack size & price."),
        ("2", "Cost every plate", "Cost each recipe line by line — quantity × unit cost. Change a supplier price and every affected plate re-costs itself."),
        ("3", "See the numbers", "Add each dish to Menu Items — food-cost % and margin calculate live, and every item lands in its engineering quadrant."),
        ("4", "Engineer the menu", "Feature your Stars, reprice Puzzles, trim Plowhorse costs and cut Dogs — then watch your Menu Score climb."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "EVERYTHING YOU GET", "One System, Whole Menu",
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + Menu Score",
        "Ingredient price library", "Recipe-costing engine",
        "Menu-engineering matrix", "Target-margin price calculator",
        "Sales-mix, yield, vendor & waste tools", "Start-Here guide + free updates",
    ]
    ...
    who = ["Restaurant & café owners", "Chefs & kitchen managers", "Food trucks & caterers",
           "Bakeries & bars", "Ghost kitchens & pop-ups", "Anyone pricing a menu"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "COST · PRICE · ENGINEER · PROFIT", font=fs(27), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Cost every plate, price with intent & engineer a menu that actually makes money.",
       fs(23, bold=False), TEXT_MUTED)
    book_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "RECIPE COSTING", 52, max_w=1000)
    img.convert("RGB").save(out, "PNG", optimize=True)
