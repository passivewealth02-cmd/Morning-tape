"""Marketing image set for Recipe Costing & Menu Engineering Command Center™ (6 images)."""
...
TABS = ["Dashboard", "Ingredients", "Recipe Costing", "Menu Items", "Menu Engineering",
        "Price Calculator", "Sales Mix", "Portion & Yield", "Specials & LTO", "Batch & Prep",
        "Vendor Prices", "Waste Log", "Settings"]

FILE_LABEL = "Recipe_Costing_Command_Center.xlsx — The Copper Skillet · Marco"
...
def plate_crest(c, cx, cy, r=56, glow=True):
    """Brand crest — a plate with crossed fork & knife: the icon of the menu."""
    if glow:
    ...
    # plate
    pr = r * 0.52
    d.ellipse((cx - pr, cy - pr, cx + pr, cy + pr), outline=GOLD_HI, width=4)
    d.ellipse((cx - pr * 0.66, cy - pr * 0.66, cx + pr * 0.66, cy + pr * 0.66), outline=(150, 120, 70), width=2)
    # knife (left)
    kx = cx - r * 0.66
    d.line((kx, cy - r * 0.5, kx, cy + r * 0.5), fill=GOLD_HI, width=int(r * 0.09))
    d.line((kx, cy - r * 0.5, kx + r * 0.06, cy - r * 0.2), fill=GOLD_HI, width=int(r * 0.05))
    # fork (right)
    fx = cx + r * 0.66
    d.line((fx, cy - r * 0.12, fx, cy + r * 0.5), fill=GOLD_HI, width=int(r * 0.09))
    for off in (-0.08, 0, 0.08):
        d.line((fx + off * r, cy - r * 0.5, fx + off * r, cy - r * 0.12), fill=GOLD_HI, width=int(r * 0.045))
    c.alpha_composite(ov)


book_crest = plate_crest
...
KPIS = [
    ...
    ("AVG FOOD COST", "24%", "6 pts under target"),
    ("TARGET FOOD COST", "30%", "your line"),
    ("AVG PLATE COST", "$4.75", "per dish"),
    ("AVG MENU PRICE", "$18.00", "per dish"),
    ("AVG MARGIN", "$13.25", "per plate"),
    ("STARS", "1", "feature these"),
    ("PLOWHORSES", "4", "fix the cost"),
    ("PUZZLES", "2", "reprice up"),
    ("DOGS", "1", "cut or rework"),
    ("TOP MARGIN", "Ribeye", "$21.50/plate"),
    ("MENU SCORE", "92%", "menu health"),
]
...
def content_dashboard(img, cbox):
    ...
    d.text((x0 + pad, y0 + 22), "Menu Profit Dashboard", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 64), "The Copper Skillet · Marco  ·  cost every plate, price with intent, engineer the menu", font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 200, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 100, y0 + 44), "● 24% food cost", font=fs(15), fill=PRIMARY, anchor="mm")
    ...
    d.text((gx, cy_top), "MENU HEALTH · MENU MIX · TOP PROFIT", font=fs(20), fill=ACCENT, anchor="lt")
    ...
    d.text((px + 16, panels_y + 14), "Menu Health", font=fs(17), fill=ACCENT, anchor="lt")
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("Food cost on target", 1.0, "100%"), ("Margin vs goal", 1.0, "100%"), ("Fully costed", 1.0, "100%"),
           ("Above margin goal", 0.62, "62%"), ("Menu balance", 0.88, "88%"), ("Contribution", 1.0, "100%")],
          color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Menu Mix by Class", font=fs(17), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.44, panels_y + panel_h * 0.50, min(panel_h * 0.27, pw * 0.27),
          [(12.5, PRIMARY), (50, GOLD_LT), (25, SURFACE), (12.5, DANGER)], "8", "items")
    legend(d, px + pw * 0.04, panels_y + panel_h - 118, [(PRIMARY, "Stars 1"), (GOLD_LT, "Plowhorses 4"),
           (SURFACE, "Puzzles 2"), (DANGER, "Dogs 1")], 15, 28)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Top Profit / Month", font=fs(17), fill=ACCENT, anchor="lt")
    rows = [("Pasta Carbonara — $3,600", MINT_BG), ("Classic Burger — $4,107", MINT_BG),
            ("Salmon Fillet — $3,760", MINT_BG), ("Ribeye Steak — $2,365", WARN_BG),
            ("Truffle Fries — $2,304", WARN_BG), ("Fish Tacos — $2,875", WARN_BG)]
    ty = panels_y + 58
    for lab, dot in rows:
    ...
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Menu Score", font=fs(17), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.5, panels_y + panel_h * 0.54, min(panel_h * 0.30, pw * 0.30),
          [(92, PRIMARY), (8, SURFACE)], "92%", "healthy")
...
def content_menu(img, cbox):
    rows = [
        ("Classic Burger", "$2.92", "$15.00", "19%", "Plowhorse"),
        ("Margherita Pizza", "$3.10", "$14.00", "22%", "Plowhorse"),
        ("Ribeye Steak", "$11.50", "$33.00", "35%", "Puzzle"),
        ("Salmon Fillet", "$8.20", "$27.00", "30%", "Puzzle"),
        ("Pasta Carbonara", "$3.60", "$18.00", "20%", "Star"),
        ("Caesar Salad", "$2.40", "$11.00", "22%", "Dog"),
    ]
    _table(img, cbox, "Menu Items",
           "Every plate's cost, price, food-cost % & menu-engineering class — live",
           ["ITEM", "COST", "PRICE", "FOOD %", "CLASS"],
           [0.0, 0.36, 0.52, 0.70, 0.85], rows,
           status_col=4, status_map={"Star": (MINT_BG, PRIMARY), "Plowhorse": (WARN_BG, ACCENT),
                                     "Puzzle": (SURFACE, (110, 88, 58)), "Dog": (RED_BG, DANGER)})


def content_engineering(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Menu Engineering", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 62), "Every item by popularity × profitability — the map to a more profitable menu.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    gx0 = x0 + pad + 44; gy0 = y0 + 120; gx1 = x1 - pad; gy1 = y1 - pad - 40
    midx = (gx0 + gx1) / 2; midy = (gy0 + gy1) / 2
    quads = [("PUZZLES", "reprice up", SURFACE, gx0, gy0, ["Ribeye Steak", "Salmon Fillet"]),
             ("STARS", "feature & protect", MINT_BG, midx, gy0, ["Pasta Carbonara"]),
             ("DOGS", "cut or rework", RED_BG, gx0, midy, ["Caesar Salad"]),
             ("PLOWHORSES", "fix the cost", WARN_BG, midx, midy, ["Classic Burger", "Marg. Pizza", "Truffle Fries", "Fish Tacos"])]
    qw = (gx1 - gx0) / 2; qh = (gy1 - gy0) / 2
    for name, act, col, qx, qy, items in quads:
        d.rectangle((qx, qy, qx + qw, qy + qh), fill=col, outline=PRIMARY, width=2)
        d.text((qx + 18, qy + 14), name, font=fs(20), fill=PRIMARY, anchor="lt")
        d.text((qx + 18, qy + 44), act, font=fs(14, bold=False), fill=TEXT, anchor="lt")
        for k, it in enumerate(items):
            cy = qy + 84 + k * 44
            d.rounded_rectangle((qx + 18, cy, qx + qw - 18, cy + 34), radius=10, fill=WHITE, outline=GRID, width=2)
            d.text((qx + 32, cy + 17), it, font=fs(15), fill=PRIMARY, anchor="lm")
    d.rectangle((gx0, gy0, gx1, gy1), outline=PRIMARY, width=3)
    d.line((midx, gy0, midx, gy1), fill=PRIMARY, width=3)
    d.line((gx0, midy, gx1, midy), fill=PRIMARY, width=3)
    d.text((x0 + pad + 18, midy), "PROFIT", font=fs(16), fill=ACCENT, anchor="mm")
    d.text((midx, gy1 + 22), "POPULARITY  →", font=fs(16), fill=ACCENT, anchor="mm")


def content_recipecost(img, cbox):
    rows = [
        ("Ground beef 80/20", "6 oz", "$0.28", "$1.68"),
        ("Brioche bun", "1 ea", "$0.55", "$0.55"),
        ("Cheddar cheese", "1 oz", "$0.35", "$0.35"),
        ("Special sauce", "1 oz", "$0.12", "$0.12"),
        ("Tomato / lettuce / onion", "—", "—", "$0.17"),
        ("Pickles", "0.5 oz", "$0.09", "$0.05"),
    ]
    _table(img, cbox, "Recipe Costing — Classic Burger",
           "Cost a plate line by line: quantity × unit cost. Change a price, it re-costs itself.",
           ["INGREDIENT", "QTY", "COST/UNIT", "EXT."],
           [0.0, 0.48, 0.66, 0.85], rows,
           total_row=("PLATE COST (yield 1)", "", "", "$2.92"))


def content_pricecalc(img, cbox):
    rows = [
        ("New Wings App", "$2.80", "30%", "$9.33"),
        ("Veggie Bowl", "$3.20", "28%", "$11.43"),
        ("Steak Sandwich", "$5.40", "32%", "$16.88"),
        ("Dessert Special", "$1.90", "25%", "$7.60"),
    ]
    _table(img, cbox, "Price Calculator",
           "Suggested price = plate cost ÷ target food-cost % — no more pricing by gut feel",
           ["ITEM", "PLATE COST", "TARGET %", "SUGGESTED PRICE"],
           [0.0, 0.40, 0.62, 0.84], rows)
...
def render_hero(out):
    ...
    plate_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE COMPLETE MENU-PROFIT SYSTEM", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "RECIPE COSTING", 92, max_w=1700)
    tc(d, (SIZE // 2, 476), "& MENU ENGINEERING COMMAND CENTER", fs(26), GOLD_HI)
    gold_divider(img, SIZE // 2, 522, width=520)
    tc(d, (SIZE // 2, 566), "Cost every plate, price with intent & engineer a more profitable menu — one system.",
       fs(20, bold=False), (224, 213, 190))
    chips = [("14", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("STAR", "PLOWHORSE · DOG")]
    cw = 440
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not a costing sheet — a complete cost-price-engineer profit system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Start Here", "how it all works"), ("Menu Profit Dashboard", "food cost, margins & score"),
        ("Ingredient Library", "pack price → cost per unit"), ("Recipe Costing", "cost every plate"),
        ("Menu Items", "food-cost % & margin, live"), ("Menu Engineering", "star / plowhorse / puzzle / dog"),
        ("Price Calculator", "target-margin pricing"), ("Sales Mix", "where the money comes from"),
        ("Portion & Yield", "true edible-portion cost"), ("Specials & LTO", "cost it before you run it"),
        ("Batch & Prep", "cost your prep batches"), ("Vendor Prices", "catch price creep"),
        ("Waste Log", "plug the profit leaks"), ("Settings", "set it once"),
        ("+ 12 Printable Pages", "print for the line"), ("Live Menu Score", "it all rolls up"),
    ]
    cols = 4
    margin = 88
    gx, gy = 22, 20
...
def render_menu(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "COST EVERY PLATE", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "Food-Cost % on Every Dish", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Cost & price side by side, with each item's food-cost % and menu-engineering class",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 3, content_menu)
    img.convert("RGB").save(out, "PNG", optimize=True)


def render_engineering(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "ENGINEER THE MENU", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "Star, Plowhorse, Puzzle, Dog", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "The matrix shows exactly what to promote, reprice, rework or cut — automatically",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 4, content_engineering)
    img.convert("RGB").save(out, "PNG", optimize=True)


def render_tools(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=300)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 110, "COST IT · PRICE IT", font=fs(32), pad_x=52, pad_y=22)
    tc(d, (SIZE // 2, 224), "The Costing & Pricing Engine", fserif(44), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 2, content_recipecost)
    app_window(img, (60, 1180, SIZE - 60, SIZE - 60), 5, content_pricecalc)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_printables(out):
    ...
    d = ImageDraw.Draw(img)
    plate_crest(img, SIZE // 2, 110, r=48)
    ...
    tc(d, (SIZE // 2, 404), "A matching print-ready pack — recipe cards, a menu-engineering worksheet & more",
       fs(22, bold=False), (226, 214, 190))
    labels = ["Recipe Cost Card", "Prep List", "Menu Engineering", "Pricing Guide",
              "Ingredient Price Log", "Portion & Yield", "Menu Item P&L", "Specials Planner",
              "Batch Scaler", "Vendor Tracker", "Waste Log", "Food-Cost Tracker"]
...
def main():
    ...
    targets = [
        ...
        ("04_engineering.png", render_engineering),
        ("05_tools.png", render_tools),
