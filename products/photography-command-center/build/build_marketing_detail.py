"""Detailed marketing image set for Photography Business Command Center™ (07-10)."""
...
from build_marketing import (
    ...
    ROW_ALT, fs, fserif, premium_bg, pill, wordmark, gold_divider, camera_crest,
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR PHOTOGRAPHERS", "Price It Right, Book It, Profit",
                "Four ways it turns a camera into a business that pays")
    feats = [
        ("$", "Live Studio Score", "donut", (90, "90%", "healthy"),
         "Shoot margin, break-even, rate, bookings, profit and booked-ahead roll into one number — your business health at a glance."),
        ("%", "CODB Break-Even Engine", "table", None,
         "Your overhead plus the salary you want, over the shoots you'll book, gives the price you must charge to pay yourself."),
        ("box", "Run the Studio", "stock", None,
         "A per-shoot P&L, packages, bookings, clients & leads, an editing queue, gear and expenses — the whole studio in one place."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack — CODB worksheet, shoot P&L, package menu, shot list, session checklist & more."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("You need (year)", "$72,000"), ("÷ 48 shoots", "$1,500"), ("Break-even/shoot", "$1,500")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["COST OF DOING BUSINESS", "VALUE"], [0.0, 0.66]):
            ...
            od = ImageDraw.Draw(img)
            items = [("Net per shoot", "$2,200", MINT_BG, PRIMARY), ("Revenue YTD", "$45,000", MINT_BG, PRIMARY),
                     ("Net profit", "$21,000", MINT_BG, PRIMARY), ("Booked ahead", "40%", SURFACE, (110, 88, 58))]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    ...
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Price List",
                "Everything a basic pricing sheet leaves out — built in")
    rows = [
        ("A package price list", True, True),
        ("A CODB / break-even engine", False, True),
        ("Profit & hourly rate per shoot", False, True),
        ("Bookings & booked-ahead revenue", False, True),
        ("Clients, leads & editing queue", False, True),
        ("Gear inventory for insurance", False, True),
        ("Expenses & mileage for taxes", False, True),
        ("Reviews & referral tracking", False, True),
        ("Revenue & bookings by month", False, True),
        ("Live Studio Score dashboard", False, True),
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "SET UP IN AN AFTERNOON", "Run Your Studio in 4 Steps",
                "No spreadsheet skills needed — if you can type, you can run it")
    steps = [
        ("1", "Find your break-even", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack, then find the price you must charge to pay yourself."),
        ("2", "Price your packages", "Set every package above your break-even and see the profit and hourly rate on each one before you quote it."),
        ("3", "Book & shoot", "Run your bookings and calendar, track clients and leads, and move every shoot through the editing queue on time."),
        ("4", "Grow the studio", "Keep your gear, expenses and mileage for taxes, ask for reviews and referrals, and watch your profit and booked-ahead revenue climb."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "EVERYTHING YOU GET", "One System, Whole Studio",
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + Studio Score",
        "CODB / break-even engine", "Per-shoot P&L & effective rate",
        "Packages, bookings & editing queue", "Clients, gear & expenses",
        "Mileage log & monthly summary", "Start-Here quick-start guide",
    ]
    ...
    who = ["Wedding & portrait photographers", "Family & newborn photographers", "Brand & product photographers",
           "Event & real-estate photographers", "New & part-time photographers", "Anyone selling photography"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "PRICE IT · BOOK IT · PROFIT", font=fs(27), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Know your break-even, price above it, and profit on every shoot.",
       fs(23, bold=False), TEXT_MUTED)
    camera_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "PHOTOGRAPHY", 58, max_w=1150)
    img.convert("RGB").save(out, "PNG", optimize=True)
