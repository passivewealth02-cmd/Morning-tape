"""Printable PDF pack for Photography Business Command Center™ (12 pages, US Letter)."""
...
def page(title, subtitle):
    ...
    d.text((M, 90), "PHOTOGRAPHY BUSINESS COMMAND CENTER™", font=fs(32), fill=GOLD_HI, anchor="lt")
    ...
    d.text((M, H - 108), "Shoot / Date: ______________", font=fs(24, bold=False), fill=MUTED, anchor="lt")
    d.text((W - M, H - 108), "Photography Command Center™", font=fs(22, bold=False), fill=ACCENT, anchor="rt")
    return img, d
...
def p01(i):
    img, d = page("CODB Worksheet", "Your break-even price per shoot")
    field(d, M, TOP, CW / 2 - 30, "Annual overhead"); field(d, M + CW / 2 + 30, TOP, CW / 2 - 30, "Desired salary")
    table(d, M, TOP + 150, CW, ["LINE", "AMOUNT"], [0.0, 0.78], 8, 150)
    y = TOP + 150 + 78 + 8 * 150 + 24
    field(d, M, y, CW / 2 - 30, "Shoots / year"); field(d, M + CW / 2 + 30, y, CW / 2 - 30, "Break-even / shoot")
    i.append(img)


def p02(i):
    img, d = page("Shoot P&L", "Profit on any shoot")
    field(d, M, TOP, CW, "Shoot")
    table(d, M, TOP + 130, CW, ["LINE", "AMOUNT"], [0.0, 0.78], 9, 150)
    y = TOP + 130 + 78 + 9 * 150 + 20
    field(d, M, y, CW / 2 - 30, "Net per shoot"); field(d, M + CW / 2 + 30, y, CW / 2 - 30, "$ / hour")
    i.append(img)


def p03(i):
    img, d = page("Package Menu", "Your prices & inclusions")
    table(d, M, TOP, CW, ["PACKAGE", "PRICE", "HOURS", "INCLUDES"], [0.0, 0.36, 0.52, 0.68], 14, 155)
    i.append(img)


def p04(i):
    img, d = page("Bookings Calendar", "Every shoot on the books")
    table(d, M, TOP, CW, ["DATE", "CLIENT / SHOOT", "TYPE", "PRICE"], [0.0, 0.20, 0.56, 0.78], 18, 120)
    i.append(img)


def p05(i):
    img, d = page("Clients & Leads", "No lead left behind")
    table(d, M, TOP, CW, ["CLIENT", "TYPE", "SOURCE", "STAGE"], [0.0, 0.34, 0.54, 0.76], 18, 120)
    i.append(img)


def p06(i):
    img, d = page("Editing Queue", "Galleries on time")
    table(d, M, TOP, CW, ["CLIENT", "SHOOT DATE", "IMAGES", "DUE"], [0.0, 0.34, 0.56, 0.78], 18, 120)
    i.append(img)


def p07(i):
    img, d = page("Shot List", "Never miss the shot")
    half = CW / 2 - 30
    for k, st in enumerate(["Must-have shots", "Detail & candid"]):
        x = M + k * (half + 60)
...
def p08(i):
    img, d = page("Gear Inventory", "Insure & track your kit")
    table(d, M, TOP, CW, ["ITEM", "VALUE", "INSURED", "SERIAL"], [0.0, 0.38, 0.56, 0.74], 16, 140)
    i.append(img)
...
def p10(i):
    ...
    table(d, M, TOP, CW, ["DATE", "SHOOT", "MILES", "COST"], [0.0, 0.18, 0.60, 0.82], 20, 118)
    i.append(img)


def p11(i):
    img, d = page("Monthly Summary", "Revenue & bookings")
    table(d, M, TOP, CW, ["MONTH", "REVENUE", "BOOKINGS", "AVG"], [0.0, 0.30, 0.54, 0.78], 14, 155)
    i.append(img)


def p12(i):
    img, d = page("Session Checklist", "Every shoot, dialed in")
    half = CW / 2 - 30
    for k, st in enumerate(["Before the shoot", "After the shoot"]):
        x = M + k * (half + 60)
        section(d, x, TOP, half, st)
        for j in range(14):
            checkbox(d, x, TOP + 96 + j * 96)
...
def main():
    ...
    pdf_path = os.path.join(out_dir, "Photography_Printables.pdf")
