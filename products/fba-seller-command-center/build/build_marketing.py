"""Marketing image set for Amazon FBA & Online Seller Profit Command Center™ (6 images, 2000x2000)."""
...
TABS = ["Dashboard", "Profit Calculator", "Product Catalog", "Fee Breakdown", "Inventory", "Sales",
        "PPC & ACoS", "Returns", "Suppliers & POs", "Reviews", "Expenses",
        "Monthly Summary", "Settings"]

FILE_LABEL = "FBA_Seller_Command_Center.xlsx — Northport Goods · Sam"
...
def box_crest(c, cx, cy, r=56, glow=True):
    """Brand crest — a shipping box: the icon of the product seller."""
    if glow:
    ...
    ov = Image.new("RGBA", c.size, (0, 0, 0, 0)); d = ImageDraw.Draw(ov)
    # box front
    d.polygon([(cx - r * 0.38, cy - r * 0.12), (cx + r * 0.38, cy - r * 0.12),
               (cx + r * 0.38, cy + r * 0.40), (cx - r * 0.38, cy + r * 0.40)], fill=GOLD_HI, outline=(150, 120, 70))
    # box lid (perspective top)
    d.polygon([(cx - r * 0.38, cy - r * 0.12), (cx - r * 0.22, cy - r * 0.34),
               (cx + r * 0.52, cy - r * 0.34), (cx + r * 0.38, cy - r * 0.12)], fill=SURFACE, outline=(150, 120, 70))
    # tape down the middle
    d.rectangle((cx - r * 0.06, cy - r * 0.12, cx + r * 0.06, cy + r * 0.40), fill=(150, 120, 70))
    # smile-style arrow on the front
    d.arc((cx - r * 0.26, cy + r * 0.02, cx + r * 0.26, cy + r * 0.32), 15, 165, fill=PRIMARY, width=5)
    c.alpha_composite(ov)


key_crest = box_crest
ledger_crest = box_crest
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "FBA SELLER", font=fs(15), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("SALE PRICE", "$29.99", "flagship SKU"),
    ("AMAZON FEES", "$10.25", "referral + FBA"),
    ("LANDED COST", "$8.70", "COGS + inbound"),
    ("NET / UNIT", "$11.04", "you keep"),
    ("NET MARGIN", "36.8%", "of price"),
    ("ROI", "126.9%", "on your cash"),
    ("UNITS / MONTH", "420", "this SKU"),
    ("MONTHLY REVENUE", "$12,596", "this SKU"),
    ("MONTHLY PROFIT", "$4,637", "after fees"),
    ("ACOS", "20.0%", "goal 25%"),
    ("DAYS OF COVER", "60", "no stockout"),
    ("SELLER SCORE", "90%", "healthy"),
]

PROFIT_MONTHS = [("Feb", 3312), ("Mar", 3643), ("Apr", 3974), ("May", 4195), ("Jun", 4416), ("Jul", 4637)]


def content_dashboard(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Seller Dashboard", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 64), "Northport Goods \u00b7 Sam  \u00b7  know your real profit per unit, after every fee", font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 210, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 105, y0 + 44), "\u25cf $11.04 net / unit", font=fs(14), fill=PRIMARY, anchor="mm")
    ...
    d.text((gx, cy_top), "SELLER HEALTH \u00b7 WHERE THE $29.99 GOES \u00b7 NET PROFIT BY MONTH", font=fs(19), fill=ACCENT, anchor="lt")
    ...
    d.text((px + 16, panels_y + 14), "Seller Health", font=fs(17), fill=ACCENT, anchor="lt")
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("Margin healthy", 1.0, "100%"), ("ROI healthy", 1.0, "100%"), ("ACoS in check", 1.0, "100%"),
           ("Inventory covered", 1.0, "100%"), ("Catalog built out", 1.0, "100%"), ("Reviews per SKU", 0.40, "40%")],
          color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Where the $29.99 Goes", font=fs(15), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.44, panels_y + panel_h * 0.50, min(panel_h * 0.27, pw * 0.27),
          [(37, PRIMARY), (34, DANGER), (29, ACCENT)], "37%", "your net")
    legend(d, px + pw * 0.06, panels_y + panel_h - 118, [(PRIMARY, "Your net $11.04"), (DANGER, "Amazon $10.25"), (ACCENT, "Product $8.70")], 14, 30)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Net Profit by Month", font=fs(17), fill=ACCENT, anchor="lt")
    vbars(img, d, (px + 18, panels_y + 60, px + pw - 14, panels_y + panel_h - 10), PROFIT_MONTHS, 4637)
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Seller Score", font=fs(17), fill=ACCENT, anchor="lt")
...
def content_catalog(img, cbox):
    rows = [
        ("NP-CAN-08", "Soy Candle 8oz", "$29.99", "$7.50", "420", "$11.04"),
        ("NP-DIF-01", "Reed Diffuser", "$34.99", "$9.00", "180", "$12.69"),
        ("NP-SET-03", "Gift Set Trio", "$69.99", "$21.00", "95", "$31.44"),
        ("NP-MLT-06", "Wax Melts 6pk", "$16.99", "$4.20", "260", "$3.29"),
        ("NP-SPR-04", "Room Spray 4oz", "$14.99", "$3.60", "210", "$2.19"),
    ]
    _table(img, cbox, "Product Catalog",
           "Every SKU with its price, cost and volume \u2014 and the true net each one throws off after fees.",
           ["SKU", "PRODUCT", "PRICE", "COGS", "UNITS/MO", "NET/UNIT"],
           [0.0, 0.22, 0.48, 0.62, 0.76, 0.90], rows,
           total_row=("8 SKUs", "", "", "", "", "$9,842/mo"))


def content_profit(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "True Profit Per Unit", font=fs(30), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 60), "A $29.99 sale is not $29.99 of revenue. Here is what actually reaches you.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("Sale price", "$29.99", PRIMARY),
            ("\u2212 Amazon fees (referral + FBA + storage)", "\u2212$10.25", DANGER),
            ("\u2212 Cost of goods", "\u2212$7.50", DANGER),
            ("\u2212 Inbound shipping", "\u2212$1.20", DANGER)]
    ...
    d.text((x0 + pad + 14, ry + rh / 2), "= NET PROFIT PER UNIT", font=fs(18), fill=PRIMARY, anchor="lm")
    d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.80, ry + rh / 2), "$11.04", font=fs(18), fill=PRIMARY, anchor="mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2 - 20), "36.8% net margin  \u00b7  126.9% ROI", font=fs(23), fill=WHITE, anchor="mm")
    od.text(((x0 + x1) / 2, by + callout_h / 2 + 24), "\u00d7 420 units = $4,637 A MONTH", font=fs(24), fill=GOLD_HI, anchor="mm")
    img.alpha_composite(ov)
...
def render_hero(out):
    ...
    box_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE COMPLETE FBA SELLER SYSTEM", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "FBA SELLER", 86, max_w=1550)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 546), "A true-profit engine, catalog, inventory, PPC & ACoS, returns & reviews \u2014 one system.",
       fs(20, bold=False), (224, 213, 190))
    chips = [("14", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("PROFIT", "ENGINE")]
    cw = 440
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not a fee calculator \u2014 a complete price-it, stock-it, scale-it system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Start Here", "how it all works"), ("Dashboard", "profit & score"),
        ("Profit Calculator", "net per unit"), ("Product Catalog", "every SKU costed"),
        ("Fee Breakdown", "every Amazon fee"), ("Inventory", "days of cover"),
        ("Sales", "units & revenue"), ("PPC & ACoS", "do ads pay?"),
        ("Returns", "the warning sign"), ("Suppliers & POs", "what's on order"),
        ("Reviews", "velocity matters"), ("Expenses", "outside the unit"),
        ("Monthly Summary", "net by month"), ("Settings", "set it once"),
        ("+ 12 Printable Pages", "worksheets & logs"), ("Live Seller Score", "it all rolls up"),
    ]
    cols = 4
    margin = 88
    gx, gy = 22, 20
...
def render_profit(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "YOUR REAL PROFIT", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "The True-Profit Engine", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Referral, FBA, storage, cost of goods and inbound shipping \u2014 stripped out to the dollar",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 1, content_profit)
    img.convert("RGB").save(out, "PNG", optimize=True)


def render_catalog(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "EVERY SKU COSTED", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "Your Whole Catalog", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Which SKUs actually make money \u2014 and which are quietly costing you every sale",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 2, content_catalog)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_engine(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 110, "PRICE IT \u00b7 SCALE IT", font=fs(32), pad_x=52, pad_y=22)
    tc(d, (SIZE // 2, 224), "The Seller Engine", fserif(44), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 1, content_profit)
    app_window(img, (60, 1180, SIZE - 60, SIZE - 60), 2, content_catalog)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_printables(out):
    ...
    d = ImageDraw.Draw(img)
    box_crest(img, SIZE // 2, 110, r=48)
    pill(img, SIZE // 2, 214, "PLUS 12 PRINTABLE PAGES", font=fs(30), pad_x=48, pad_y=20)
    tc(d, (SIZE // 2, 300), "Print, Fill & Run the Business", fserif(42), WHITE)
    gold_divider(img, SIZE // 2, 362, width=480)
    tc(d, (SIZE // 2, 404), "A matching print-ready pack \u2014 profit worksheet, SKU sheet, reorder planner, PPC log & more",
       fs(21, bold=False), (226, 214, 190))
    labels = ["Profit Per Unit", "Product Catalog", "Fee Worksheet", "Inventory & Reorder",
              "PPC & ACoS Log", "Sales Log", "Supplier & PO Log", "Returns Log",
              "Reviews Tracker", "Product Research", "Monthly Summary", "Launch Checklist"]
