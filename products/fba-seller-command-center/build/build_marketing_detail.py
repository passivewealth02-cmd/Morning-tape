"""Detailed marketing image set for Amazon FBA & Online Seller Profit Command Center™ (07-10)."""
...
from build_marketing import (
    ...
    ROW_ALT, fs, fserif, premium_bg, pill, wordmark, gold_divider, box_crest,
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR PRODUCT SELLERS", "Price It, Stock It, Scale It",
                "Four ways it turns guesswork into a profitable catalog")
    feats = [
        ("$", "Live Seller Score", "donut", (90, "90%", "healthy"),
         "Margin, ROI, ACoS, inventory cover, catalog depth and review velocity roll into one number — the health of your whole brand at a glance."),
        ("%", "True-Profit Engine", "table", None,
         "Sale price minus the referral fee, FBA fee, storage, your cost of goods and inbound shipping — the net per unit, margin and ROI that decide if a product is worth selling."),
        ("box", "Run the Whole Brand", "stock", None,
         "Catalog, inventory and reorder points, PPC and ACoS, returns, suppliers, reviews and expenses — every moving part of an FBA business in one place."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack — profit worksheet, SKU sheet, reorder planner, PPC log, product-research scorer and a launch checklist."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("Sale price", "$29.99"), ("Net per unit", "$11.04"), ("ROI", "126.9%")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["TRUE-PROFIT ENGINE", "VALUE"], [0.0, 0.66]):
            ...
            od = ImageDraw.Draw(img)
            items = [("Monthly profit", "$4,637", MINT_BG, PRIMARY), ("ACoS", "20.0%", MINT_BG, PRIMARY),
                     ("Days of cover", "60", MINT_BG, PRIMARY), ("Reviews per SKU", "40%", SURFACE, (110, 88, 58))]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than an FBA Fee Calculator",
                "Everything a free fee calculator leaves out — built in")
    rows = [
        ("A single-SKU fee estimate", True, True),
        ("Net per unit, margin AND ROI", False, True),
        ("Your whole catalog costed", False, True),
        ("Inventory days-of-cover", False, True),
        ("Reorder point per SKU", False, True),
        ("PPC campaigns & ACoS", False, True),
        ("TACoS against total revenue", False, True),
        ("Returns rate tracking", False, True),
        ("Suppliers, POs & reviews", False, True),
        ("Live Seller Score dashboard", False, True),
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "SET UP IN AN AFTERNOON", "Run Your Brand in 4 Steps",
                "No spreadsheet skills needed — if you can type, you can run it")
    steps = [
        ("1", "Cost your best SKU", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack, then enter a sale price, your fees and your landed cost."),
        ("2", "See the real number", "The engine strips out the referral fee, FBA fee, storage, cost of goods and inbound shipping to show your net per unit, margin and ROI — the truth, not the estimate."),
        ("3", "Load the catalog", "Cost every SKU the same way, set reorder points from your days-of-cover, and find the products quietly losing money on every single sale."),
        ("4", "Scale what works", "Track PPC and ACoS, returns, suppliers and review velocity, and let the Seller Score tell you where the brand is leaking."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "EVERYTHING YOU GET", "One System, Whole Brand",
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + Seller Score",
        "True-profit engine (net, margin, ROI)", "Product catalog & fee breakdown",
        "Inventory, reorder & days-of-cover", "PPC, ACoS & TACoS tracking",
        "Returns, suppliers & reviews", "Start-Here quick-start guide",
    ]
    ...
    who = ["Amazon FBA sellers", "Private-label brand owners", "Shopify & Walmart sellers",
           "New sellers costing their first SKU", "Sellers scaling past 6 figures", "Anyone tired of guessing at fees"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "PRICE IT · STOCK IT · SCALE IT", font=fs(27), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Know your real profit per unit — after every fee Amazon takes.",
       fs(23, bold=False), TEXT_MUTED)
    box_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "FBA SELLER", 58, max_w=1150)
    img.convert("RGB").save(out, "PNG", optimize=True)
