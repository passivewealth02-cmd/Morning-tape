"""Marketing image set for Notary & Loan Signing Agent Command Center™ (6 images, 2000x2000)."""
...
TABS = ["Dashboard", "Signing Profit", "Signings Log", "Fee Schedule", "Mileage Log",
        "Printing & Supplies", "Invoices", "Signing Companies", "Notarial Journal", "Expenses",
        "Tax Set-Aside", "Monthly Summary", "Settings"]

FILE_LABEL = "Notary_Command_Center.xlsx — Quill & Seal Notary · Sloane"
...
def seal_crest(c, cx, cy, r=56, glow=True):
    """Brand crest — a notary seal: the embossed ring and star."""
    if glow:
    ...
    c.alpha_composite(ov)
    ov = Image.new("RGBA", c.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    # embossed outer and inner rings
    od.ellipse((cx - r * 0.62, cy - r * 0.62, cx + r * 0.62, cy + r * 0.62), outline=GOLD_HI, width=max(3, int(r * 0.07)))
    od.ellipse((cx - r * 0.44, cy - r * 0.44, cx + r * 0.44, cy + r * 0.44), outline=GOLD_HI, width=max(2, int(r * 0.04)))
    # serration around the rim
    import math as _m
    for k in range(24):
        a = _m.radians(k * 15)
        x1 = cx + _m.cos(a) * r * 0.62; y1 = cy + _m.sin(a) * r * 0.62
        x2 = cx + _m.cos(a) * r * 0.72; y2 = cy + _m.sin(a) * r * 0.72
        od.line((x1, y1, x2, y2), fill=GOLD_LT, width=2)
    # five-pointed star at the centre
    pts = []
    for k in range(10):
        a = _m.radians(-90 + k * 36)
        rad = r * 0.30 if k % 2 == 0 else r * 0.13
        pts.append((cx + _m.cos(a) * rad, cy + _m.sin(a) * rad))
    od.polygon(pts, fill=HIGHLIGHT, outline=(150, 120, 70))
    c.alpha_composite(ov)


square_crest = seal_crest
lantern_crest = seal_crest
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "THE BUSINESS", font=fs(15), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("AVERAGE FEE", "$125.00", "per signing"),
    ("LOOKS LIKE / HOUR", "$138.89", "45-min appointment"),
    ("PRINTING", "$6.30", "180 pages"),
    ("DRIVING", "$8.36", "38 miles round trip"),
    ("NET PER SIGNING", "$110.34", "after both"),
    ("REALLY EARNS / HOUR", "$44.14", "over 2.5 hrs door to door"),
    ("SIGNINGS / MONTH", "52", "booked"),
    ("MONTHLY REVENUE", "$6,500", "before costs"),
    ("MONTHLY PROFIT", "$5,449", "83.8% margin"),
    ("BREAK-EVEN", "3", "signings cover overhead"),
    ("MILEAGE DEDUCTION", "$1,383", "log every mile"),
    ("SIGNING SCORE", "90%", "healthy"),
]

REV_MONTHS = [("Feb", 5125), ("Mar", 5500), ("Apr", 5875), ("May", 6125), ("Jun", 6375), ("Jul", 6500)]


def content_dashboard(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Signing Dashboard", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 64), "Quill & Seal Notary \u00b7 Sloane  \u00b7  count the drive and the printer",
           font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 230, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 115, y0 + 44), "\u25cf really $44.14/hr", font=fs(14), fill=PRIMARY, anchor="mm")
    ...
    d.text((gx, cy_top), "BUSINESS HEALTH \u00b7 WHERE THE 2.5 HOURS GO \u00b7 REVENUE BY MONTH", font=fs(19), fill=ACCENT, anchor="lt")
    ...
    d.text((px + 16, panels_y + 14), "Business Health", font=fs(17), fill=ACCENT, anchor="lt")
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("Real hourly on target", 1.0, "100%"), ("Margin healthy", 1.0, "100%"),
           ("Overhead covered", 1.0, "100%"), ("Actually getting paid", 1.0, "100%"),
           ("Enough signings booked", 1.0, "100%"), ("Tax reserve funded", 0.40, "40%")],
          color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Where the 2.5 Hours Go", font=fs(15), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.44, panels_y + panel_h * 0.46, min(panel_h * 0.25, pw * 0.25),
          [(44, ACCENT), (36, PRIMARY), (20, SURFACE)], "36%", "at the table")
    legend(d, px + pw * 0.06, panels_y + panel_h - 118,
           [(ACCENT, "Driving 1.1 hrs"), (PRIMARY, "Appointment 0.9 hrs"),
            (SURFACE, "Prep & printing 0.5 hrs")], 14, 30)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Revenue by Month", font=fs(17), fill=ACCENT, anchor="lt")
    vbars(img, d, (px + 18, panels_y + 60, px + pw - 14, panels_y + panel_h - 10), REV_MONTHS, 6500)
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Signing Score", font=fs(17), fill=ACCENT, anchor="lt")
...
def content_fees(img, cbox):
    rows = [
        ("Reverse mortgage", "$175", "246 pages", "$154.95", "Best net"),
        ("Purchase / seller package", "$150", "212 pages", "$133.56", "Two packages"),
        ("Refinance signing", "$125", "180 pages", "$110.34", "Your bread & butter"),
        ("HELOC", "$100", "96 pages", "$86.08", "Short package"),
        ("Trip fee \u2014 cancelled at door", "$60", "already printed", "$53.70", "BILL IT"),
    ]
    _table(img, cbox, "Fee Schedule \u2014 Net of Printing & Driving",
           "What each job actually leaves you, once the printer and the car take their cut.",
           ["SERVICE", "FEE", "PAGES", "NET", "NOTE"],
           [0.0, 0.40, 0.53, 0.67, 0.80], rows,
           total_row=("52 signings", "$6,500", "avg 180pp", "$5,738", "before overhead"))


def content_profit(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "The Signing Profit Engine", font=fs(30), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 60), "Fee minus printing minus driving, over the hours it ACTUALLY takes door to door.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("The fee \u00f7 a 0.9 hr appointment", "$138.89/hr", ACCENT),
            ("\u2212 Printing (180 pages)", "$6.30", DANGER),
            ("\u2212 Driving (38 miles round trip)", "$8.36", DANGER),
            ("= Net per signing, over 2.5 hrs door to door", "$110.34", PRIMARY)]
    ...
    d.text((x0 + pad + 14, ry + rh / 2), "= WHAT YOU ACTUALLY EARN PER HOUR", font=fs(18), fill=PRIMARY, anchor="lm")
    d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.80, ry + rh / 2), "$44.14", font=fs(18), fill=PRIMARY, anchor="mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2 - 20), "Still a good business \u2014 just a different one from the one in your head", font=fs(21), fill=WHITE, anchor="mm")
    od.text(((x0 + x1) / 2, by + callout_h / 2 + 24), "$138.89 AN HOUR IS REALLY $44.14 AN HOUR", font=fs(23), fill=GOLD_HI, anchor="mm")
    img.alpha_composite(ov)
...
def render_hero(out):
    ...
    seal_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE COMPLETE SIGNING AGENT SYSTEM", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "NOTARY", 86, max_w=1550)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 546), "Signing profit, fees, mileage, invoices, journal & tax \u2014 one system.",
       fs(20, bold=False), (224, 213, 190))
    chips = [("14", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("REAL", "HOURLY ENGINE")]
    cw = 440
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not a mileage log \u2014 a complete price-it, run-it, keep-it system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Start Here", "how it all works"), ("Dashboard", "profit & score"),
        ("Signing Profit", "your real hourly"), ("Signings Log", "every job netted"),
        ("Fee Schedule", "stop undercharging"), ("Mileage Log", "the deduction"),
        ("Printing & Supplies", "cost per page"), ("Invoices", "who owes you"),
        ("Signing Companies", "who pays late"), ("Notarial Journal", "every act"),
        ("Expenses", "fixed & running"), ("Tax Set-Aside", "nobody withholds"),
        ("Monthly Summary", "is it paying?"), ("Settings", "set it once"),
        ("+ 12 Printable Pages", "logs & checklists"), ("Live Signing Score", "it all rolls up"),
    ]
    cols = 4
    margin = 88
    gx, gy = 22, 20
...
def render_profit(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "$138.89/HR IS REALLY $44.14/HR", font=fs(26), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "The Signing Profit Engine", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "You printed 180 pages and drove 38 miles before the appointment even started",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 1, content_profit)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_fees(out):
    ...
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "STOP QUOTING FROM MEMORY", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "Your Fee Schedule", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Every service priced, and what each one nets after the printer and the car",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 3, content_fees)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_engine(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 110, "PRICE IT \u00b7 RUN IT", font=fs(32), pad_x=52, pad_y=22)
    tc(d, (SIZE // 2, 224), "The Signing Engine", fserif(44), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 1, content_profit)
    app_window(img, (60, 1180, SIZE - 60, SIZE - 60), 3, content_fees)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_printables(out):
    ...
    d = ImageDraw.Draw(img)
    seal_crest(img, SIZE // 2, 110, r=48)
    pill(img, SIZE // 2, 214, "PLUS 12 PRINTABLE PAGES", font=fs(30), pad_x=48, pad_y=20)
    tc(d, (SIZE // 2, 300), "Print, Fill & Keep in the Bag", fserif(42), WHITE)
    gold_divider(img, SIZE // 2, 362, width=480)
    tc(d, (SIZE // 2, 404), "A matching print-ready pack \u2014 journal sheet, mileage log, signing-day checklist, invoice & more",
       fs(21, bold=False), (226, 214, 190))
    labels = ["What It Really Pays", "Fee Schedule", "Signings Log", "Mileage Log",
              "Journal Sheet", "Signing Day Checklist", "Invoice", "Who Owes You",
              "Signing Companies", "Printing & Supplies", "Tax Set-Aside", "Monthly Review"]
...
def main():
    ...
    targets = [
        ...
        ("03_profit.png", render_profit),
        ("04_fees.png", render_fees),
