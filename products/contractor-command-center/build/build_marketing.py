"""Marketing image set for Contractor Job Costing & Bidding Command Center™ (6 images, 2000x2000)."""
...
TABS = ["Dashboard", "Bid Builder", "Job Costing", "Jobs & Pipeline", "Labor & Crew", "Materials",
        "Subcontractors", "Change Orders", "Equipment", "Invoices", "Bid Log",
        "Monthly Summary", "Settings"]

FILE_LABEL = "Contractor_Command_Center.xlsx — Ironwood Builders · Cal"
...
def square_crest(c, cx, cy, r=56, glow=True):
    """Brand crest — a carpenter's square and level bubble: the icon of building it right."""
    if glow:
    ...
    c.alpha_composite(ov)
    ov = Image.new("RGBA", c.size, (0, 0, 0, 0)); d = ImageDraw.Draw(ov)
    # carpenter's square (L shape)
    d.polygon([(cx - r * 0.40, cy - r * 0.34), (cx - r * 0.18, cy - r * 0.34),
               (cx - r * 0.18, cy + r * 0.16), (cx + r * 0.40, cy + r * 0.16),
               (cx + r * 0.40, cy + r * 0.38), (cx - r * 0.40, cy + r * 0.38)],
              fill=GOLD_HI, outline=(150, 120, 70))
    # tick marks along the blade
    for k in range(1, 5):
        xx = cx - r * 0.10 + k * r * 0.11
        d.line((xx, cy + r * 0.16, xx, cy + r * 0.25), fill=(150, 120, 70), width=2)
    # spirit level vial above
    d.rounded_rectangle((cx - r * 0.16, cy - r * 0.54, cx + r * 0.34, cy - r * 0.40), radius=6,
                        fill=(238, 248, 245), outline=GOLD_HI, width=2)
    d.ellipse((cx + r * 0.02, cy - r * 0.52, cx + r * 0.14, cy - r * 0.42), fill=HIGHLIGHT)
    c.alpha_composite(ov)


box_crest = square_crest
key_crest = square_crest
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "THE BUILD", font=fs(15), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("DIRECT COST", "$38,100", "materials + labor"),
    ("OVERHEAD", "$4,572", "12% applied"),
    ("TOTAL COST", "$42,672", "true cost"),
    ("BID THIS JOB AT", "$53,340", "quote it"),
    ("PLANNED PROFIT", "$10,668", "20% margin"),
    ("ACTUAL MARGIN", "22.8%", "came in under"),
    ("ACTUAL PROFIT", "$12,162", "on this job"),
    ("JOBS THIS YEAR", "9", "booked"),
    ("BACKLOG", "$186,000", "won + running"),
    ("RECEIVABLES", "$31,000", "still owed"),
    ("WIN RATE", "40.9%", "9 of 22 bids"),
    ("BUILDER SCORE", "90%", "healthy"),
]

REV_MONTHS = [("Feb", 52000), ("Mar", 61000), ("Apr", 68000), ("May", 74000), ("Jun", 81000), ("Jul", 92000)]


def content_dashboard(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Builder Dashboard", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 64), "Ironwood Builders \u00b7 Cal  \u00b7  bid it right, cost it honestly, finish in profit", font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 210, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 105, y0 + 44), "\u25cf bid at $53,340", font=fs(14), fill=PRIMARY, anchor="mm")
    ...
    d.text((gx, cy_top), "BUILDER HEALTH \u00b7 WHERE THE BID GOES \u00b7 REVENUE BY MONTH", font=fs(19), fill=ACCENT, anchor="lt")
    ...
    d.text((px + 16, panels_y + 14), "Builder Health", font=fs(17), fill=ACCENT, anchor="lt")
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("Job margin healthy", 1.0, "100%"), ("Backlog full", 1.0, "100%"), ("Net margin healthy", 1.0, "100%"),
           ("Getting paid on time", 1.0, "100%"), ("Jobs booked", 1.0, "100%"), ("Bid win rate", 0.41, "41%")],
          color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Where the Bid Goes", font=fs(15), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.44, panels_y + panel_h * 0.50, min(panel_h * 0.27, pw * 0.27),
          [(20, PRIMARY), (71, ACCENT), (9, SURFACE)], "20%", "your margin")
    legend(d, px + pw * 0.06, panels_y + panel_h - 118, [(PRIMARY, "Profit $10,668"), (ACCENT, "Direct $38,100"), (SURFACE, "Overhead $4,572")], 14, 30)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Revenue by Month", font=fs(17), fill=ACCENT, anchor="lt")
    vbars(img, d, (px + 18, panels_y + 60, px + pw - 14, panels_y + panel_h - 10), REV_MONTHS, 92000)
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Builder Score", font=fs(17), fill=ACCENT, anchor="lt")
...
def content_jobs(img, cbox):
    rows = [
        ("Kitchen remodel \u2014 Hale", "In progress", "$53,340", "$42,672", "$10,668"),
        ("Basement finish \u2014 Park", "In progress", "$61,200", "$48,900", "$12,300"),
        ("Garage conversion \u2014 Reid", "Won", "$44,500", "$35,800", "$8,700"),
        ("Bath gut \u2014 Ortiz", "Won", "$28,500", "$22,400", "$6,100"),
        ("Addition \u2014 Marsh", "Bidding", "$96,000", "$76,500", "$19,500"),
    ]
    _table(img, cbox, "Jobs & Pipeline",
           "Every job \u2014 what you bid, what it should cost, and the profit riding on it.",
           ["JOB", "STATUS", "BID", "EST. COST", "EST. PROFIT"],
           [0.0, 0.38, 0.56, 0.73, 0.90], rows,
           total_row=("9 jobs", "backlog", "$186,000", "", "$79,000"))


def content_bid(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "The Bid Builder", font=fs(30), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 60), "Cost \u00f7 (1 \u2212 margin) is the price to quote. Multiply instead and you lose thousands.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("Materials + labor + subs + equipment", "$38,100", PRIMARY),
            ("+ Overhead (12% of direct cost)", "$4,572", PRIMARY),
            ("= Total cost", "$42,672", PRIMARY),
            ("\u00f7 (1 \u2212 20% target margin)", "0.80", ACCENT)]
    ...
    d.text((x0 + pad + 14, ry + rh / 2), "= BID THIS JOB AT", font=fs(18), fill=PRIMARY, anchor="lm")
    d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.80, ry + rh / 2), "$53,340", font=fs(18), fill=PRIMARY, anchor="mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2 - 20), "$10,668 planned profit  \u00b7  a real 20% margin", font=fs(22), fill=WHITE, anchor="mm")
    od.text(((x0 + x1) / 2, by + callout_h / 2 + 24), "MULTIPLYING WOULD LOSE YOU $2,134", font=fs(23), fill=GOLD_HI, anchor="mm")
    img.alpha_composite(ov)
...
def render_hero(out):
    ...
    square_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE COMPLETE CONTRACTOR SYSTEM", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "CONTRACTOR", 86, max_w=1550)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 546), "A bid builder, job costing, crew, subs, change orders & win rate \u2014 one system.",
       fs(20, bold=False), (224, 213, 190))
    chips = [("14", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("BID", "ENGINE")]
    cw = 440
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not a bid sheet \u2014 a complete bid-it, cost-it, profit-from-it system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Start Here", "how it all works"), ("Dashboard", "margin & score"),
        ("Bid Builder", "the price to quote"), ("Job Costing", "estimate vs actual"),
        ("Jobs & Pipeline", "every job"), ("Labor & Crew", "hours & cost"),
        ("Materials", "the takeoff"), ("Subcontractors", "quoted vs invoiced"),
        ("Change Orders", "get it signed"), ("Equipment", "rentals & machines"),
        ("Invoices", "cash flow"), ("Bid Log", "your win rate"),
        ("Monthly Summary", "is it working?"), ("Settings", "set it once"),
        ("+ 12 Printable Pages", "forms & logs"), ("Live Builder Score", "it all rolls up"),
    ]
    cols = 4
    margin = 88
    gx, gy = 22, 20
...
def render_bid(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "MARKUP IS NOT MARGIN", font=fs(29), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "The Bid Builder", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Real overhead applied, then divided by your margin \u2014 the price you must actually quote",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 1, content_bid)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_jobs(out):
    ...
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "EVERY JOB TRACKED", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "Your Whole Pipeline", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Bid, estimated cost and the profit riding on every job you have in the book",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 3, content_jobs)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_engine(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 110, "BID IT \u00b7 COST IT", font=fs(32), pad_x=52, pad_y=22)
    tc(d, (SIZE // 2, 224), "The Builder Engine", fserif(44), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 1, content_bid)
    app_window(img, (60, 1180, SIZE - 60, SIZE - 60), 3, content_jobs)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_printables(out):
    ...
    tc(d, (SIZE // 2, 300), "Print, Fill & Take to the Site", fserif(42), WHITE)
    gold_divider(img, SIZE // 2, 362, width=480)
    tc(d, (SIZE // 2, 404), "A matching print-ready pack \u2014 bid worksheet, job cost sheet, change order form, daily log & more",
       fs(21, bold=False), (226, 214, 190))
    labels = ["Bid Worksheet", "Job Cost Sheet", "Materials Takeoff", "Labor & Crew Log",
              "Subcontractor Log", "Change Order Form", "Daily Job Log", "Bid Log",
              "Invoice Schedule", "Punch List", "Monthly Summary", "Job Closeout"]
...
def main():
    ...
    targets = [
        ...
        ("03_bid.png", render_bid),
        ("04_jobs.png", render_jobs),
