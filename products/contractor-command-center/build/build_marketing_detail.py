"""Detailed marketing image set for Contractor Job Costing & Bidding Command Center™ (07-10)."""
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR CONTRACTORS", "Bid It, Cost It, Profit From It",
                "Four ways it stops jobs finishing at break-even")
    feats = [
        ("$", "Live Builder Score", "donut", (90, "90%", "healthy"),
         "Job margin, backlog, net margin, receivables, jobs booked and bid win rate roll into one number — the health of the whole operation at a glance."),
        ("%", "The Bid Builder", "table", None,
         "Materials, labor, subs and equipment, plus your real overhead rate, divided by your target margin — because markup is not margin and the difference is thousands."),
        ("box", "Run the Whole Job", "stock", None,
         "Job costing estimate-vs-actual, crew hours, materials takeoff, subs, change orders, equipment and receivables — every moving part of a build in one place."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack — bid worksheet, job cost sheet, change order form, daily job log, punch list and a closeout checklist for the truck."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("Total cost", "$42,672"), ("Bid at", "$53,340"), ("Planned profit", "$10,668")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["THE BID BUILDER", "VALUE"], [0.0, 0.62]):
            ...
            od = ImageDraw.Draw(img)
            items = [("Actual margin", "22.8%", MINT_BG, PRIMARY), ("Backlog", "$186,000", MINT_BG, PRIMARY),
                     ("Jobs booked", "9", MINT_BG, PRIMARY), ("Bid win rate", "41%", SURFACE, (110, 88, 58))]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Bid Sheet",
                "Everything a basic estimate template leaves out — built in")
    rows = [
        ("A materials & labor estimate", True, True),
        ("Real overhead applied to the bid", False, True),
        ("Margin as a divisor, not a markup", False, True),
        ("Estimate vs actual job costing", False, True),
        ("Crew hours costed per job", False, True),
        ("Subs: quoted vs invoiced", False, True),
        ("Change orders, priced & signed", False, True),
        ("Backlog & receivables", False, True),
        ("Bid log with your win rate", False, True),
        ("Live Builder Score dashboard", False, True),
    ...
    d.text((c1, top + hh // 2), "A Basic\nEstimate", font=fs(18), fill=(210, 210, 205), anchor="mm")
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "SET UP IN AN AFTERNOON", "Bid Your Next Job in 4 Steps",
                "No spreadsheet skills needed — if you can type, you can run it")
    steps = [
        ("1", "Build the bid", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack, then enter materials, labor hours, subs and equipment for the job."),
        ("2", "Get the real price", "Your overhead rate is applied to the direct cost, then the total is divided by your target margin — that is the price to quote, with the profit actually in it."),
        ("3", "Cost it as you build", "Log real materials, crew hours, sub invoices and change orders, and watch estimate-versus-actual move live so nothing surprises you at the end."),
        ("4", "Learn from every job", "Track backlog, receivables and your bid win rate, and let the Builder Score tell you whether you are priced right or working for free."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "EVERYTHING YOU GET", "One System, Whole Operation",
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + Builder Score",
        "Bid builder with real overhead", "Estimate vs actual job costing",
        "Crew, materials & subcontractors", "Change orders & equipment",
        "Invoices, backlog & bid win rate", "Start-Here quick-start guide",
    ]
    ...
    who = ["General contractors & builders", "Remodelers & renovation crews", "Electricians, plumbers & HVAC",
           "Painters, roofers & landscapers", "Handyman & small-crew businesses", "Anyone bidding jobs on a napkin"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "BID IT · COST IT · PROFIT", font=fs(27), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Bid it right, cost it honestly, and finish every job in profit.",
       fs(23, bold=False), TEXT_MUTED)
    square_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "CONTRACTOR", 58, max_w=1150)
    img.convert("RGB").save(out, "PNG", optimize=True)
