"""Detailed marketing image set for Ghost Kitchen Command Center™ (07-10)."""
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR GHOST KITCHENS", "Where Delivery Makes Money",
                "Four ways it turns every order into real profit")
    feats = [
        ("$", "Live Kitchen Score", "donut", (90, "90%", "healthy"),
         "Food cost, net margin, commission, direct-order mix & gross margin roll into one number — so you know your delivery kitchen's financial health at a glance."),
        ("%", "Margin After the App", "table", None,
         "Start from the app price, subtract the commission, food & packaging — see the true net you keep. The number the menu price hides from you."),
        ("box", "A P&L Per Platform", "stock", None,
         "Every app's orders, commission & net payout side by side — see what DoorDash, Uber Eats & Grubhub really pay you, and why direct orders win."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack — item-margin card, platform P&L, packaging sheet, prep list, inventory & a promotions tracker."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("App price", "$13.95"), ("App + food + pkg", "−$7.94"), ("Net you keep", "$6.01")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["SIGNATURE BURRITO", "VALUE"], [0.0, 0.62]):
            ...
            od = ImageDraw.Draw(img)
            items = [("Weekly gross revenue", "$17,360", MINT_BG, PRIMARY), ("Net payout (after apps)", "$13,117", MINT_BG, PRIMARY),
                     ("Apps take weekly", "$4,243", RED_BG, (150, 60, 60)), ("Blended commission", "24%", SURFACE, (110, 88, 58))]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Menu",
                "Everything a basic delivery menu leaves out — built in")
    rows = [
        ("A menu with prices", True, True),
        ("Item margin AFTER commission", False, True),
        ("Net margin % on every item", False, True),
        ("A full P&L per platform", False, True),
        ("Virtual brands from one kitchen", False, True),
        ("Packaging cost per order", False, True),
        ("Order volume & AOV by day", False, True),
        ("Inventory, waste & ordering", False, True),
        ("Payouts & promo ROI tracker", False, True),
    ...
    d.text((c1, top + hh // 2), "A Basic\nMenu", font=fs(18), fill=(210, 210, 205), anchor="mm")
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "SET UP IN AN AFTERNOON", "Beat the Apps in 4 Steps",
                "No spreadsheet skills needed — if you can type, you can run it")
    steps = [
        ("1", "Cost your items", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack, then cost each item — app price, food & packaging."),
        ("2", "See your real margin", "The item-margin engine subtracts the app's commission first — so you see the true net you keep, not the menu price that hides it."),
        ("3", "Read each platform", "Enter orders & commission for every app — a full P&L shows what DoorDash, Uber Eats & Grubhub really pay you, and why direct orders win."),
        ("4", "Run the kitchen", "Track virtual brands, packaging, order volume, inventory, waste, payouts & promos, and watch your Kitchen Score climb."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    ...
    includes = [
        ...
        "Item-margin-after-app engine", "Net margin on every item",
        "Full P&L per platform", "Virtual brands & packaging",
        "Order volume, payouts & promos", "Start-Here guide + free updates",
    ]
    ...
    who = ["Ghost & cloud kitchens", "Delivery-only restaurants", "Virtual-brand operators",
           "Restaurants adding delivery", "Home cooks going delivery", "Anyone selling on the apps"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "BEAT THE APPS · KEEP MORE MARGIN", font=fs(27), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Know your real margin after commission, and keep more of every order you cook.",
       fs(23, bold=False), TEXT_MUTED)
    book_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "GHOST KITCHEN COMMAND CENTER", 48, max_w=1280)
    img.convert("RGB").save(out, "PNG", optimize=True)
