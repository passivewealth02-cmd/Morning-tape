"""Detailed marketing image set for Bakery Command Center™ (07-10)."""
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR BAKERS", "Where a Bakery Makes Money",
                "Four ways it turns flour & butter into real profit")
    feats = [
        ("$", "Live Bakery Score", "donut", (88, "88%", "healthy"),
         "Food cost, margins, pre-orders, waste & gross margin roll into one number — so you know your bakery's financial health at a glance."),
        ("%", "Cost by the Batch", "table", None,
         "Cost each recipe by the batch, then divide by yield for a true cost per loaf, cookie or croissant — the secret to real bakery pricing."),
        ("box", "Retail AND Wholesale", "stock", None,
         "Every product carries both a retail and a wholesale price and margin — so cafés, grocers and your own counter all make money."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack — recipe cost card, product price list, pre-order form, production plan, waste log & a bake-day checklist."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("Batch cost", "$12.80"), ("Yield", "8 loaves"), ("Cost per loaf", "$1.60")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["SOURDOUGH", "VALUE"], [0.0, 0.62]):
            ...
            od = ImageDraw.Draw(img)
            items = [("Retail revenue / wk", "$6,495", MINT_BG, PRIMARY), ("Wholesale revenue / wk", "$2,182", MINT_BG, PRIMARY),
                     ("Pre-order value", "$678", SURFACE, (110, 88, 58)), ("Weekly units baked", "1,480", MINT_BG, PRIMARY)]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    ...
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Price List",
                "Everything a basic bakery sheet leaves out — built in")
    rows = [
        ("A product price list", True, True),
        ("Recipe cost-per-batch engine", False, True),
        ("Cost per unit by yield", False, True),
        ("Retail AND wholesale margins", False, True),
        ("Pre-orders / custom board", False, True),
        ("Wholesale accounts tracker", False, True),
        ("Production plan & inventory", False, True),
        ("Waste & day-old log", False, True),
        ("Sales, cash & market days", False, True),
        ("Live Bakery Score dashboard", False, True),
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "SET UP IN AN AFTERNOON", "Price Your Bakery in 4 Steps",
                "No spreadsheet skills needed — if you can type, you can run it")
    steps = [
        ("1", "Cost your recipes", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack, then cost each recipe by the batch."),
        ("2", "Build your price list", "Divide batch cost by yield for a true cost per unit, then set retail and wholesale prices — margins calculate live."),
        ("3", "Book the orders", "Keep the pre-order board and wholesale accounts current — the steady orders that fill your ovens between rushes."),
        ("4", "Run the bakery", "Plan production, track inventory, waste, sales & market days, and watch your Bakery Score climb."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "EVERYTHING YOU GET", "One System, Whole Bakery",
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + Bakery Score",
        "Recipe cost-per-batch engine", "Retail & wholesale price list",
        "Pre-orders & wholesale accounts", "Production plan & inventory",
        "Waste, sales, cash & market days", "Start-Here guide + free updates",
    ]
    ...
    who = ["Bakeries & bakery cafés", "Home & cottage bakers", "Bread & pastry shops",
           "Cake & cookie makers", "Wholesale & farmers-market bakers", "Anyone pricing baked goods"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "COST THE BATCH · BAKE MORE PROFIT", font=fs(27), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Cost every batch, price retail & wholesale, and keep more of every dollar you bake.",
       fs(23, bold=False), TEXT_MUTED)
    book_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "BAKERY COMMAND CENTER", 52, max_w=1200)
    img.convert("RGB").save(out, "PNG", optimize=True)
