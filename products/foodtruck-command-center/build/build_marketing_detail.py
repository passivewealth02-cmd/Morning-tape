"""Detailed marketing image set for Food Truck Command Center™ (07-10)."""
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR THE ROAD", "Where a Truck Makes Money",
                "Four ways it turns busy gigs into real profit")
    feats = [
        ("$", "Live Truck Score", "donut", (93, "93%", "on track"),
         "Profit margin, food cost, events booked, permits & overhead coverage roll into one number — so you know your truck's health at a glance."),
        ("%", "Profit Per Event", "stock", None,
         "Log every gig's sales and costs and see the net profit of each — so you chase the events that pay and drop the ones that don't."),
        ("box", "Know Your Break-Even", "be", None,
         "The break-even calculator tells you exactly how many gigs cover your commissary rent, insurance & loan. Every event after that is profit."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack — event P&L sheet, prep & par list, fuel log, permit tracker, cash-&-tips sheet & a monthly P&L for the truck."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "stock":
            ...
            od.line((bx - 22, by + 16, bx - 6, by - 8), fill=GOLD_HI, width=4)
            od.line((bx - 6, by - 8, bx + 8, by + 6), fill=GOLD_HI, width=4)
            od.line((bx + 8, by + 6, bx + 22, by - 18), fill=GOLD_HI, width=4)
        elif kind == "print":
        ...
        if kind == "donut":
            ...
            items = [("Corporate Catering", "$1,695", MINT_BG, PRIMARY), ("Music Festival", "$1,480", MINT_BG, PRIMARY),
                     ("Brewery Friday", "$1,095", MINT_BG, PRIMARY), ("Farmers Market", "$421", WARN_BG, (110, 88, 58))]
            ...
            od = ImageDraw.Draw(img)
            bx0 = x + 44; bx1 = x + cw - 44; byy = vy - 10
            od.rounded_rectangle((bx0, byy, bx1, byy + 150), radius=12, fill=MINT_BG)
            od.text(((bx0 + bx1) / 2, byy + 46), "2.3 events", font=fserif(44), fill=PRIMARY, anchor="mm")
            od.text(((bx0 + bx1) / 2, byy + 108), "to cover $2,140 overhead / month", font=fs(18, bold=False), fill=TEXT, anchor="mm")
        elif kind == "print":
...
def render_compare(out):
    ...
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Sales Log",
                "Everything a basic tracker leaves out — built for mobile food")
    rows = [
        ("A daily sales tracker", True, True),
        ("Net profit per event", False, True),
        ("Break-even calculator", False, True),
        ("Menu costing & food-cost %", False, True),
        ("Commissary & overhead tracking", False, True),
        ("Inventory & par levels", False, True),
        ("Fuel & mileage log", False, True),
        ("Permit & license tracker", False, True),
        ("Bookings calendar & deposits", False, True),
        ("Live Truck Score dashboard", False, True),
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "SET UP IN AN AFTERNOON", "Run the Numbers in 4 Steps",
                "No spreadsheet skills needed — if you can type, you can run it")
    steps = [
        ("1", "Set up your truck", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack, then add your menu costs & fixed overhead."),
        ("2", "Log every gig", "After each event enter sales, food, fuel, fees & staff — the sheet shows the net profit of that gig instantly."),
        ("3", "Know your break-even", "See exactly how many events cover your fixed costs — then chase high-sales, low-fee gigs that clear it fast."),
        ("4", "Book & keep rolling", "Track bookings, inventory, fuel & permits, and watch your Truck Score climb as the profitable gigs stack up."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + Truck Score",
        "Event P&L engine", "Break-even calculator",
        "Menu costing & daily sales", "Commissary, inventory & fuel",
        "Permits, bookings & cash-&-tips", "Start-Here guide + free updates",
    ]
    ...
    who = ["Food truck & trailer owners", "Concession & fair vendors", "Pop-ups & street food",
           "Coffee & dessert carts", "New trucks finding their numbers", "Anyone selling food on wheels"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "KNOW YOUR NUMBERS · KEEP ROLLING", font=fs(27), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Know the profit of every gig, cover your overhead & book the events that pay.",
       fs(23, bold=False), TEXT_MUTED)
    book_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "FOOD TRUCK COMMAND CENTER", 50, max_w=1300)
    img.convert("RGB").save(out, "PNG", optimize=True)
