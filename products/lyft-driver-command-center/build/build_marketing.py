"""Marketing image set for Lyft Driver Command Center™ (6 images, 2000x2000).
...
all 18 tabs, the REAL computed KPI numbers, and fully populated tables/charts.
  ...
  03_shiftlog.png   - shift log (the earnings engine)
  04_takehome.png   - earnings -> real take-home (+ mileage deduction)
  05_grow.png       - hot zones + bonuses & quests
...
TABS = ["Dashboard", "Driver Profile", "Shift Log", "Earnings", "Mileage", "Fuel Log", "Vehicle",
        "Expenses", "Budget", "Tax Center", "Savings", "Bonuses", "Hot Zones", "Ratings", "Analytics",
        "Planner", "Gallery", "Settings"]

FILE_LABEL = "Lyft_Driver_Command_Center.xlsx — Jordan · Austin, TX · Full-time"
...
def wheel_crest(c, cx, cy, r=56, glow=True):
    """Brand crest with a steering wheel — a nod to the driver's seat, pink hub for Lyft."""
    if glow:
        radial_glow(c, cx, cy, int(r * 2.1), GOLD_HI, 90)
        radial_glow(c, cx, cy, int(r * 1.4), PINK_HI, 55)
    ...
    rim = r * 0.62
    # wheel rim
    d.ellipse((cx - rim, cy - rim, cx + rim, cy + rim), outline=GOLD_HI, width=8)
    d.ellipse((cx - rim + 8, cy - rim + 8, cx + rim - 8, cy + rim - 8), outline=(160, 130, 78), width=2)
    # hub (pink accent)
    hub = r * 0.2
    d.ellipse((cx - hub, cy - hub, cx + hub, cy + hub), fill=LYFT_PINK, outline=GOLD_HI, width=3)
    # three spokes
    d.line((cx, cy, cx, cy - rim + 6), fill=GOLD_HI, width=8)
    for ang in (210, 330):
        import math
        ex = cx + (rim - 6) * math.cos(math.radians(ang))
        ey = cy - (rim - 6) * math.sin(math.radians(ang))
        d.line((cx, cy, ex, ey), fill=GOLD_HI, width=8)
    c.alpha_composite(ov)
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "LYFT DRIVER", font=fs(17), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("NET EARNINGS", "$2,786", "take-home"),
    ("GROSS EARNINGS", "$4,226", "fares+tips+bonus"),
    ("TIPS", "$652", "this month"),
    ("NET $ / HOUR", "$21.68", "after costs"),
    ("NET $ / MILE", "$0.97", "after costs"),
    ("MILES DRIVEN", "2,871", "business"),
    ("ONLINE HOURS", "128.5", "this month"),
    ("TRIPS", "316", "completed"),
    ("EXPENSES", "$1,440", "fuel + car + more"),
    ("TAX DEDUCTION", "$2,010", "mileage @ $0.70"),
    ("SAVINGS", "70%", "emergency fund"),
    ("DRIVER HEALTH", "90%", "blended"),
]
...
def content_dashboard(img, cbox):
    ...
    d.text((x0 + pad, y0 + 64), "Jordan · Austin, TX  ·  every shift, mile & dollar, automatically organized", font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    ...
    months = ["Feb", "Mar", "Apr", "May", "Jun", "Jul"]; vals = [2380, 2510, 2690, 2620, 2740, 2786]
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [(m, v / 2786, f"{v/1000:.1f}k") for m, v in zip(months, vals)], color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    ...
    donut(d, px + pw * 0.42, panels_y + panel_h * 0.52, min(panel_h * 0.28, pw * 0.27),
          [(76, PRIMARY), (15, HIGHLIGHT), (9, ACCENT)], "$4.2K", "gross")
    legend(d, px + pw * 0.04, panels_y + panel_h - 104, [(PRIMARY, "Fares 76%"), (HIGHLIGHT, "Tips 15%"), (ACCENT, "Bonus 9%")], 15, 30)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    ...
    hbars(img, d, (px + 20, panels_y + 56, px + pw - 16, panels_y + panel_h - 20),
          [("Concert surge", 1.0, "$340"), ("Long day", 0.96, "$326"), ("Airport day", 0.94, "$318"),
           ("Weekend rush", 0.91, "$310"), ("Mixed day", 0.87, "$297")])
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Expense Breakdown", font=fs(17), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.42, panels_y + panel_h * 0.52, min(panel_h * 0.28, pw * 0.27),
          [(36, PRIMARY), (29, ACCENT), (13, HIGHLIGHT), (10, SURFACE), (12, (170, 150, 120))], "$1.4K", "per mo")
    legend(d, px + pw * 0.04, panels_y + panel_h - 104, [(PRIMARY, "Fuel 36%"), (ACCENT, "Car Pmt 29%"), (HIGHLIGHT, "Insurance 13%")], 15, 30)
...
PLAT_MAP = {"Lyft": (RED_BG, LYFT_PINK), "Uber": ((225, 235, 232), (30, 90, 80)),
            "Both": (WARN_BG, ACCENT), "Delivery": (SURFACE, (110, 88, 58))}


def content_shiftlog(img, cbox):
    rows = [
        ("Sat", "Lyft", "9.5", "24", "$340", "214", "$39"),
        ("Fri", "Both", "8.5", "22", "$297", "198", "$35"),
        ("Thu", "Uber", "8.0", "19", "$246", "176", "$32"),
        ("Wed", "Lyft", "9.0", "23", "$326", "208", "$38"),
        ("Tue", "Both", "8.0", "20", "$273", "186", "$34"),
        ("Sun", "Lyft", "9.0", "22", "$310", "200", "$36"),
        ("Sat", "Uber", "8.5", "21", "$274", "190", "$34"),
        ("Fri", "Both", "8.0", "19", "$261", "178", "$33"),
    ]
    _table(img, cbox, "Shift Log",
           "Log each shift — earnings, miles & fuel roll straight into your dashboard",
           ["DAY", "PLATFORM", "HOURS", "TRIPS", "EARNINGS", "MILES", "FUEL"],
           [0.0, 0.16, 0.40, 0.53, 0.65, 0.82, 0.92], rows,
           total_row=("MONTH TOTAL", "", "128.5", "316", "$4,226", "2,871", "$522"),
           status_col=1, status_map=PLAT_MAP)


def content_takehome(img, cbox):
    rows = [
        ("Fares", "$3,208", "76% of gross"),
        ("Tips", "$652", "15% of gross"),
        ("Bonus / Surge", "$366", "9% of gross"),
        ("− Business expenses", "−$1,440", "34% ratio"),
        ("Mileage tax deduction", "$2,010", "IRS $0.70 / mi"),
    ]
    _table(img, cbox, "Earnings → Real Take-Home",
           "See what you ACTUALLY keep — net after fuel & expenses, plus the mileage deduction.  $21.68 net/hr · $0.97 net/mi",
           ["EARNINGS  →  TAKE-HOME", "AMOUNT", "DETAIL"],
           [0.0, 0.52, 0.75], rows,
           total_row=("NET TAKE-HOME", "$2,786", "$21.68 / hour"))


def content_zones(img, cbox):
    rows = [
        ("Convention Center", "Event nights", "$45", "High"),
        ("Downtown / 6th St", "Fri-Sat 9pm-2am", "$41", "High"),
        ("Campus (UT)", "Game days", "$38", "High"),
        ("Airport (AUS)", "6-9am, 4-7pm", "$34", "Medium"),
        ("Domain / North", "Weekday PM", "$28", "Low"),
        ("Suburbs", "Weekday AM", "$22", "Low"),
    ]
    _table(img, cbox, "Hot Zones & Hours",
           "Drive where it pays — the zones & hours that earn the most per hour",
           ["ZONE", "BEST TIME", "$/HR", "SURGE"],
           [0.0, 0.38, 0.66, 0.85], rows,
           status_col=3, status_map={"High": (MINT_BG, PRIMARY), "Medium": (WARN_BG, ACCENT),
                                     "Low": ((235, 230, 222), TEXT_MUTED)})


def content_bonuses(img, cbox):
    rows = [
        ("Lyft", "Ride Challenge · 50 rides", "$120", "Completed"),
        ("Lyft", "Peak Bonus · downtown", "$64", "Completed"),
        ("Uber", "Quest · 40 trips", "$90", "In Progress"),
        ("Lyft", "Weekly Guarantee", "$75", "In Progress"),
        ("Uber", "Consecutive · 6 in a row", "$45", "Available"),
        ("Lyft", "Streak Bonus · x4", "$18", "Completed"),
    ]
    _table(img, cbox, "Bonuses & Quests",
           "Never miss a promo — every quest, streak & guarantee and the extra it pays",
           ["PLATFORM", "REQUIREMENT", "REWARD", "STATUS"],
           [0.0, 0.22, 0.62, 0.84], rows,
           status_col=3, status_map={"Completed": (MINT_BG, PRIMARY), "In Progress": (WARN_BG, ACCENT),
                                     "Available": ((235, 230, 222), TEXT_MUTED)})
...
def render_hero(out):
    ...
    wheel_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE RIDESHARE EARNINGS & BUDGET SYSTEM", font=fs(23), pad_x=42, pad_y=20)
    wordmark(img, SIZE // 2, 400, "LYFT DRIVER COMMAND CENTER", 92, max_w=1860)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 550), "Shifts, miles, fuel, taxes & savings — know your REAL take-home, automatically.",
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not a mileage app — a complete rideshare earnings, budget & tax operating system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Driver Dashboard", "12 KPIs + live charts"), ("Driver Profile", "your driving business"),
        ("Shift Log", "the earnings engine"), ("Earnings Breakdown", "mix & platform split"),
        ("Mileage Tracker", "the IRS deduction"), ("Fuel Log", "real fuel spend"),
        ...
        ("Savings & Goals", "pay yourself first"), ("Bonuses & Quests", "never miss a promo"),
        ("Hot Zones & Hours", "drive where it pays"), ("Ratings & Feedback", "protect your rating"),
...
def render_shiftlog(out):
    ...
    tc(d, (SIZE // 2, 300), "Hours, trips, fares, tips, miles & fuel — one row per shift feeds your whole dashboard",
       fs(23, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 2, content_shiftlog)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_takehome(out):
    ...
    tc(d, (SIZE // 2, 300), "After fuel & expenses — plus the mileage deduction that saves drivers thousands at tax time",
       fs(23, bold=False), (226, 214, 190))
...
def render_grow(out):
    ...
    tc(d, (SIZE // 2, 224), "Best Zones. Every Bonus.", fserif(44), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 12, content_zones)
    app_window(img, (60, 1180, SIZE - 60, SIZE - 60), 11, content_bonuses)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_mobile(out):
    ...
    tc(d, (SIZE // 2, 320), "Log a shift from the driver's seat — your whole driving business in your pocket",
       fs(23, bold=False), (226, 214, 190))
    px, py = SIZE // 2, 1300
    pw, ph = 640, 1230
    ...
    od.text(((sx0 + sx1) // 2, sy0 + 56), "Jordan · Austin", font=fserif(30), fill=GOLD_HI, anchor="mm")
    y = sy0 + 150
    cards = [("NET EARNINGS", "$2,786", PRIMARY), ("NET $ / HOUR", "$21.68", ACCENT),
             ("TAX DEDUCTION", "$2,010", PRIMARY), ("DRIVER HEALTH", "90%", PRIMARY)]
    ...
    y += 52
    for lab, state in [("Log Fri night shift", True), ("Fill up at Costco", False),
                       ("Bank tax set-aside", True), ("Check surge downtown", False)]:
...
def main():
    ...
    targets = [
        ...
        ("03_shiftlog.png", render_shiftlog),
