"""Detailed marketing image set for Savings Challenge & Sinking Funds Command Center™ (07-10)."""
...
from build_marketing import (
    ...
    ROW_ALT, fs, fserif, premium_bg, pill, wordmark, gold_divider, jar_crest,
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR REAL SAVERS", "Name It, Save It, Celebrate It",
                "Four ways it turns saving from a chore into a game you win")
    feats = [
        ("$", "Live Savings Score", "donut", (90, "90%", "healthy"),
         "Funds on pace, emergency fund, challenges running, monthly goal, every bill covered and your streak roll into one number — your savings health at a glance."),
        ("%", "Sinking Funds Engine", "table", None,
         "Every yearly bill divided by twelve becomes one monthly number to set aside — so Christmas, tyres and insurance stop being emergencies."),
        ("box", "Challenges That Actually Work", "stock", None,
         "The 100-Envelope ($5,050), the 52-Week ($1,378), cash stuffing, no-spend months and a savings streak — the games that make saving stick."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack — the 100-envelope grid, 52-week tracker, savings thermometer and cash-envelope sheets, ready to colour in."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("Yearly targets", "$9,000"), ("÷ 12 months", "$750"), ("On pace", "100%")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["SINKING FUNDS ENGINE", "VALUE"], [0.0, 0.66]):
            ...
            od = ImageDraw.Draw(img)
            items = [("100 Envelope", "$5,050", MINT_BG, PRIMARY), ("52-Week", "$1,378", MINT_BG, PRIMARY),
                     ("Emergency fund", "$6,000", MINT_BG, PRIMARY), ("Savings streak", "40%", SURFACE, (110, 88, 58))]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Savings Tracker",
                "Everything a basic savings sheet leaves out — built in")
    rows = [
        ("A savings goal total", True, True),
        ("Sinking funds by category", False, True),
        ("Monthly set-aside per fund", False, True),
        ("On-pace check vs the calendar", False, True),
        ("100-Envelope Challenge built in", False, True),
        ("52-Week Challenge built in", False, True),
        ("Cash-envelope stuffing tracker", False, True),
        ("No-spend & savings streak", False, True),
        ("Emergency fund kept separate", False, True),
        ("Live Savings Score dashboard", False, True),
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "SET UP IN ONE SITTING", "Start Saving On Purpose in 4 Steps",
                "No spreadsheet skills needed — if you can type, you can run it")
    steps = [
        ("1", "List the bills that aren't monthly", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack, then list every yearly cost — Christmas, insurance, tyres, vet."),
        ("2", "Get your monthly number", "The engine divides each target by twelve and totals them, so you know the single amount to set aside every month to never be caught out again."),
        ("3", "Pick a challenge", "Run the 100-Envelope for $5,050, the 52-Week for $1,378, or stuff cash envelopes — the printables are ready to colour in as you go."),
        ("4", "Keep the streak alive", "Log every deposit, track no-spend days, watch each fund fill up, and let the Savings Score show you exactly where you stand."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "EVERYTHING YOU GET", "One System, Every Dollar Named",
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + Savings Score",
        "Sinking funds engine", "100-Envelope & 52-Week challenges",
        "Cash-envelope stuffing tracker", "Emergency fund & goal countdown",
        "Deposit log & savings streak", "Start-Here quick-start guide",
    ]
    ...
    who = ["Cash stuffers & envelope savers", "Anyone doing a savings challenge", "Budgeters tired of surprise bills",
           "Families saving for Christmas", "Anyone building an emergency fund", "Anyone who wants saving to be fun"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "NAME IT · SAVE IT · CELEBRATE IT", font=fs(27), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Give every saved dollar a name — and never be blindsided by a bill again.",
       fs(23, bold=False), TEXT_MUTED)
    jar_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "SAVINGS CHALLENGE", 52, max_w=1200)
    img.convert("RGB").save(out, "PNG", optimize=True)
