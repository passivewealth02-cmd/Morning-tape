"""Marketing image set for Savings Challenge & Sinking Funds Command Center™ (6 images, 2000x2000)."""
...
TABS = ["Dashboard", "Sinking Funds", "100 Envelope", "52-Week", "Challenges", "Goal Countdown",
        "Cash Envelopes", "Savings Accounts", "Emergency Fund", "Deposit Log", "Savings Streak",
        "Monthly Summary", "Settings"]

FILE_LABEL = "Savings_Challenge_Command_Center.xlsx — Copper & Clover · June"
...
def jar_crest(c, cx, cy, r=56, glow=True):
    """Brand crest — a savings jar: the icon of money set aside on purpose."""
    if glow:
    ...
    ov = Image.new("RGBA", c.size, (0, 0, 0, 0)); d = ImageDraw.Draw(ov)
    # jar lid
    d.rounded_rectangle((cx - r * 0.30, cy - r * 0.42, cx + r * 0.30, cy - r * 0.28), radius=5, fill=GOLD_HI, outline=(150, 120, 70))
    # jar body
    d.rounded_rectangle((cx - r * 0.36, cy - r * 0.26, cx + r * 0.36, cy + r * 0.42), radius=10, fill=(238, 248, 245), outline=GOLD_HI, width=3)
    # coins filling the jar
    d.rounded_rectangle((cx - r * 0.30, cy + r * 0.06, cx + r * 0.30, cy + r * 0.38), radius=7, fill=HIGHLIGHT)
    for k, yy in enumerate((0.14, 0.26)):
        d.line((cx - r * 0.24, cy + r * yy, cx + r * 0.24, cy + r * yy), fill=(60, 160, 135), width=2)
    # coin dropping in
    d.ellipse((cx - r * 0.09, cy - r * 0.20, cx + r * 0.09, cy - r * 0.02), fill=GOLD_HI, outline=(150, 120, 70))
    d.text((cx, cy - r * 0.11), "$", font=fserif(int(r * 0.20)), fill=PRIMARY, anchor="mm")
    c.alpha_composite(ov)


key_crest = jar_crest
ledger_crest = jar_crest
...
def vbars(img, d, box, items, maxv, color=(PRIMARY_LT, PRIMARY)):
    ...
    for i, (lab, val) in enumerate(items):
        ...
        d.text((bx + bw / 2, y1 - 52 - bh), f"${val:,.0f}", font=fs(13), fill=PRIMARY, anchor="mb")
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "SAVINGS", font=fs(15), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("FUNDS TARGET", "$9,000", "for the year"),
    ("FUNDS SAVED", "$6,000", "so far"),
    ("MONTHLY SET-ASIDE", "$750", "every month"),
    ("ON PACE", "100%", "right on track"),
    ("EMERGENCY FUND", "$6,000", "fully funded"),
    ("TOTAL SAVED", "$12,274", "everywhere"),
    ("100 ENVELOPE", "$5,050", "challenge total"),
    ("52-WEEK", "$1,378", "challenge total"),
    ("ACTIVE CHALLENGES", "3", "running now"),
    ("SAVED THIS MONTH", "$520", "goal $500"),
    ("SAVINGS STREAK", "24", "days"),
    ("SAVINGS SCORE", "90%", "healthy"),
]

SAVE_MONTHS = [("Feb", 420), ("Mar", 450), ("Apr", 480), ("May", 500), ("Jun", 510), ("Jul", 520)]


def content_dashboard(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Savings Dashboard", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 64), "Copper & Clover \u00b7 June  \u00b7  give every saved dollar a name", font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 210, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 105, y0 + 44), "\u25cf $750 a month", font=fs(14), fill=PRIMARY, anchor="mm")
    ...
    d.text((gx, cy_top), "SAVINGS HEALTH \u00b7 WHERE THE FUNDS ARE \u00b7 SAVED BY MONTH", font=fs(19), fill=ACCENT, anchor="lt")
    ...
    d.text((px + 16, panels_y + 14), "Savings Health", font=fs(17), fill=ACCENT, anchor="lt")
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("Funds on pace", 1.0, "100%"), ("Emergency fund", 1.0, "100%"), ("Challenges running", 1.0, "100%"),
           ("Hitting monthly goal", 1.0, "100%"), ("Every bill has a fund", 1.0, "100%"), ("Savings streak", 0.40, "40%")],
          color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Sinking Funds Filled", font=fs(15), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.44, panels_y + panel_h * 0.50, min(panel_h * 0.27, pw * 0.27),
          [(67, PRIMARY), (33, SURFACE)], "67%", "of $9,000")
    legend(d, px + pw * 0.06, panels_y + panel_h - 96, [(PRIMARY, "Saved $6,000"), (SURFACE, "To go $3,000")], 15, 32)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Saved by Month", font=fs(17), fill=ACCENT, anchor="lt")
    vbars(img, d, (px + 18, panels_y + 60, px + pw - 14, panels_y + panel_h - 10), SAVE_MONTHS, 520)
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Savings Score", font=fs(17), fill=ACCENT, anchor="lt")
...
def content_funds(img, cbox):
    rows = [
        ("Vacation", "Travel", "$2,400", "$1,600", "67%", "$200"),
        ("Home repairs", "Home", "$1,500", "$1,000", "67%", "$125"),
        ("Christmas & holidays", "Holiday", "$1,200", "$800", "67%", "$100"),
        ("Insurance premiums", "Annual bill", "$1,200", "$800", "67%", "$100"),
        ("Car maintenance", "Car", "$900", "$600", "67%", "$75"),
        ("Medical & dental", "Health", "$800", "$533", "67%", "$67"),
    ]
    _table(img, cbox, "Sinking Funds",
           "Every non-monthly bill, divided by the months you have \u2014 the engine of a calm budget.",
           ["FUND", "CATEGORY", "TARGET", "SAVED", "% FULL", "MONTHLY"],
           [0.0, 0.34, 0.50, 0.64, 0.77, 0.90], rows,
           total_row=("8 funds", "", "$9,000", "$6,000", "67%", "$750"))


def content_sinking(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "The Sinking Funds Engine", font=fs(30), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 60), "A yearly bill divided by twelve months is a monthly number you can actually plan for.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("All your yearly targets", "$9,000", PRIMARY),
            ("\u00f7 12 months", "12", PRIMARY)]
    ...
    d.text((x0 + pad + 14, ry + rh / 2), "= SET ASIDE EVERY MONTH", font=fs(18), fill=PRIMARY, anchor="lm")
    d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.80, ry + rh / 2), "$750", font=fs(18), fill=PRIMARY, anchor="mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2), "$6,000 saved of $9,000  \u00b7  100% ON PACE", font=fs(23), fill=WHITE, anchor="mm")
    img.alpha_composite(ov)
...
def render_hero(out):
    ...
    jar_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE COMPLETE SAVINGS CHALLENGE SYSTEM", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "SAVINGS CHALLENGE", 74, max_w=1620)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 546), "A sinking-funds engine, 100 envelopes, 52 weeks, cash stuffing & streaks \u2014 one system.",
       fs(20, bold=False), (224, 213, 190))
    chips = [("14", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("$5,050", "ENVELOPE GAME")]
    cw = 440
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not a tracker \u2014 a complete name-it, save-it, celebrate-it system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Start Here", "how it all works"), ("Dashboard", "saved & score"),
        ("Sinking Funds", "the engine"), ("100 Envelope", "$5,050 game"),
        ("52-Week", "$1,378 game"), ("Challenges", "run several"),
        ("Goal Countdown", "what you want"), ("Cash Envelopes", "cash stuffing"),
        ("Savings Accounts", "where it lives"), ("Emergency Fund", "the real one"),
        ("Deposit Log", "every dollar in"), ("Savings Streak", "keep it alive"),
        ("Monthly Summary", "climbing"), ("Settings", "set it once"),
        ("+ 12 Printable Pages", "grids & trackers"), ("Live Savings Score", "it all rolls up"),
    ]
    cols = 4
    margin = 88
    gx, gy = 22, 20
...
def render_sinking(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "NEVER BE BLINDSIDED", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "The Sinking Funds Engine", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Christmas isn't an emergency \u2014 it's an appointment. This tells you what to set aside monthly",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 1, content_sinking)
    img.convert("RGB").save(out, "PNG", optimize=True)


def render_funds(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "EVERY BILL HAS A FUND", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "All Your Sinking Funds", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Eight funds, each filling up \u2014 with the exact monthly amount every one needs",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 1, content_funds)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_engine(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 110, "NAME IT \u00b7 SAVE IT", font=fs(32), pad_x=52, pad_y=22)
    tc(d, (SIZE // 2, 224), "The Savings Engine", fserif(44), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 1, content_sinking)
    app_window(img, (60, 1180, SIZE - 60, SIZE - 60), 1, content_funds)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_printables(out):
    ...
    d = ImageDraw.Draw(img)
    jar_crest(img, SIZE // 2, 110, r=48)
    pill(img, SIZE // 2, 214, "PLUS 12 PRINTABLE PAGES", font=fs(30), pad_x=48, pad_y=20)
    tc(d, (SIZE // 2, 300), "Print, Colour In & Celebrate", fserif(42), WHITE)
    gold_divider(img, SIZE // 2, 362, width=480)
    tc(d, (SIZE // 2, 404), "A matching print-ready pack \u2014 100-envelope grid, 52-week tracker, savings thermometer & more",
       fs(21, bold=False), (226, 214, 190))
    labels = ["Sinking Funds", "100 Envelope Grid", "52-Week Tracker", "Savings Thermometer",
              "Cash Envelopes", "Deposit Log", "Goal Countdown", "No-Spend Tracker",
...
def main():
    ...
    targets = [
        ...
        ("03_sinkingfunds.png", render_sinking),
        ("04_allfunds.png", render_funds),
