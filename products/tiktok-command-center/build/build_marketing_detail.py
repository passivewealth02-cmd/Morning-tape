"""Detailed marketing image set for TikTok Command Center™ (4 images, 2000x2000).
...
from build_marketing import (
    ...
    ROW_ALT, fs, fserif, premium_bg, pill, wordmark, gold_divider, tiktok_crest,
...
def header_band(img, tagpill, title, subtitle, band_h=430):
    ...
    d = ImageDraw.Draw(img)
    tiktok_crest(img, SIZE // 2, 110, r=48)
...
def render_features(out):
    ...
    header_band(img, "BUILT TO TURN VIEWS INTO INCOME", "Where Creators Win",
                "Four engines that run the business behind your content")
    feats = [
        ("$", "Revenue Engine", "donut", (77, "77%", "margin"),
         "7 income streams — brand deals, TikTok Shop, Creator Rewards, affiliate & LIVE — roll into one live net-profit number."),
        ("%", "Hook & Trend Radar", "table", None,
         "Bank your best hooks and catch sounds while they're still Rising — the two levers that decide if a video pops."),
        ("box", "Shop & Affiliate", "stock", None,
         "Turn views into sales — units, GMV and commission per product, plus every affiliate link's clicks and payout."),
        ("star", "Creator Health", "bars", None,
         "One 0-100% score blends revenue, posting, engagement, retention, brand pipeline & follower growth."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("'I made this for $0'", "9.4"), ("'Do NOT do what I did'", "9.1"), ("'aesthetic morning' — Rising", "▲")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["HOOK / SOUND", "SCORE"], [0.0, 0.82]):
            ...
            od = ImageDraw.Draw(img)
            items = [("Command strips ×130", "$2.4K GMV", MINT_BG, PRIMARY), ("Glue gun ×88", "$2.2K GMV", MINT_BG, PRIMARY),
                     ("Amazon Storefront", "12.4K clicks", SURFACE, (110, 88, 58)), ("Paint sprayer ×19", "$1.7K GMV", MINT_BG, PRIMARY)]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            ...
            hbars(img, ImageDraw.Draw(img), (x + 44, y + 300, x + cw - 44, y + ch - 30),
                  [("Revenue", 0.86, "86%"), ("Posting", 0.92, "92%"), ("Engagement", 0.92, "92%"), ("Growth", 0.92, "92%")])
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Calendar",
                "Everything a basic content planner leaves out — built in")
    rows = [
        ("Plan & schedule your posts", True, True),
        ("Hook & script bank", False, True),
        ("Trend & sound radar", False, True),
        ("Per-video analytics + health score", False, True),
        ("TikTok Shop GMV & commission", False, True),
        ("Affiliate link tracker", False, True),
        ("Brand deal CRM (lead → paid)", False, True),
        ("7-stream finance & net profit", False, True),
        ("Repurposing tracker", False, True),
        ("Goals, audience & collabs", False, True),
        ("One Creator Health Score", False, True),
    ]
    margin = 110; top = 500
    tw = SIZE - 2 * margin
...
def render_howitworks(out):
    ...
    steps = [
        ...
        ("2", "Set your goals", "In Settings, add your handle, niche and monthly goals (followers, revenue, posts). One time only."),
        ("3", "Plan, bank & track", "Fill your calendar, bank hooks & ideas, and log trends while they're rising. Everything cross-links."),
        ("4", "Log & watch it grow", "Add analytics, Shop sales and brand deals — revenue, net profit and your Creator Health Score update automatically."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
    d = ImageDraw.Draw(img)
...
def render_value(out):
    ...
    includes = [
        ...
        "12-KPI creator dashboard", "Hook bank + trend & sound radar",
        "TikTok Shop + affiliate trackers", "Brand-deal CRM (lead → paid)",
        "7-stream finance + net profit", "Repurposing & series planners",
    ...
    who = ["TikTok creators & influencers", "UGC creators", "TikTok Shop sellers",
           "Faceless & niche pages", "Coaches & educators", "Agencies managing creators"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "REPLACES A DOZEN CREATOR APPS & A MANAGER", font=fs(30), pad_x=48, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "One point of control for the content AND the business behind it.",
       fs(24, bold=False), TEXT_MUTED)
    tiktok_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "TIKTOK COMMAND CENTER", 60, max_w=1500)
    img.convert("RGB").save(out, "PNG", optimize=True)
