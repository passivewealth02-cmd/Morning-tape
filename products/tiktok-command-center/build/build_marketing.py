"""Marketing image set for TikTok Command Center™ (6 images, 2000x2000).
...
all 24 tabs, the REAL computed KPI numbers, and fully populated tables/charts.
  ...
  03_analytics.png  - analytics (top videos + engagement)
  04_finance.png    - creator finance center (7 revenue streams)
  05_grow.png       - trends & sounds + brand deals CRM
...
TABS = ["Dashboard", "Profile", "Calendar", "Pipeline", "Ideas", "Hooks", "Trends", "Hashtags",
        "Analytics", "LIVE", "Shop", "Affiliate", "Brand Deals", "Series", "Finance", "Expenses",
        "Equipment", "Repurposing", "Brand Kit", "Gallery", "Goals", "Audience", "Collabs", "Settings"]

FILE_LABEL = "TikTok_Command_Center.xlsx — @vellacreates · DIY & Lifestyle · 215K"
...
def _note(od, cx, cy, r, color):
    # stylized eighth-note: head bottom-left, stem up-right, flag
    hw = r * 0.34
    od.ellipse((cx - r * 0.34 - hw, cy + r * 0.22, cx - r * 0.34 + hw, cy + r * 0.22 + hw * 1.6), fill=color)
    od.rounded_rectangle((cx - r * 0.34 + hw - 6, cy - r * 0.62, cx - r * 0.34 + hw, cy + r * 0.5), radius=3, fill=color)
    # flag / curl
    od.pieslice((cx - r * 0.34 + hw - 4, cy - r * 0.7, cx + r * 0.5, cy + r * 0.05), 250, 20, fill=color)
    od.ellipse((cx + r * 0.02, cy - r * 0.42, cx + r * 0.02 + hw * 1.7, cy - r * 0.42 + hw * 1.5), fill=color)


def tiktok_crest(c, cx, cy, r=56, glow=True):
    """Brand crest with a TikTok-style music note + cyan/magenta glitch."""
    if glow:
    ...
    c.alpha_composite(ov)
    # glitch layers: cyan offset up-left, red offset down-right, white on top
    for dx, dy, col in [(-5, -5, TT_CYAN), (5, 5, TT_RED), (0, 0, WHITE)]:
        ov = Image.new("RGBA", c.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
        _note(od, cx + dx, cy + dy, r * 0.86, col)
        c.alpha_composite(ov)
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "TIKTOK", font=fs(18), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("FOLLOWERS", "215K", "total"),
    ("VIEWS (28D)", "3.2M", "last 28 days"),
    ("ENGAGEMENT", "9.2%", "rate"),
    ("AVG COMPLETION", "48%", "watch-through"),
    ("POSTED (28D)", "22", "of 24 goal"),
    ("MONTHLY REVENUE", "$8,583", "7 streams"),
    ("NET PROFIT", "$6,633", "77% margin"),
    ("SHOP SALES", "$14,233", "GMV"),
    ("BRAND DEALS", "4", "active"),
    ("FOLLOWER GROWTH", "+18.4K", "this month"),
    ("POSTING", "92%", "consistency"),
    ("CREATOR HEALTH", "88%", "blended"),
]
...
def content_dashboard(img, cbox):
    ...
    d.text((x0 + pad, y0 + 64), "@vellacreates · DIY & lifestyle  ·  your whole account, automatically organized", font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    ...
    months = ["Feb", "Mar", "Apr", "May", "Jun", "Jul"]; vals = [128, 149, 168, 184, 201, 215]
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [(m, v / 215, f"{v}") for m, v in zip(months, vals)], color=(GOLD_HI, GOLD))
    # revenue donut
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    ...
    donut(d, px + pw * 0.42, panels_y + panel_h * 0.52, min(panel_h * 0.28, pw * 0.27),
          [(44, PRIMARY), (22, ACCENT), (14, HIGHLIGHT), (9, SURFACE), (11, (170, 150, 120))], "$8.6K", "per mo")
    legend(d, px + pw * 0.04, panels_y + panel_h - 104, [(PRIMARY, "Brand deals 44%"), (ACCENT, "Shop 22%"), (HIGHLIGHT, "Rewards 14%")], 15, 30)
    # top videos bars
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    ...
    hbars(img, d, (px + 20, panels_y + 56, px + pw - 16, panels_y + panel_h - 20),
          [("Thrift flip", 1.0, "3.2M"), ("Amazon finds", 0.59, "1.9M"), ("Renter hacks", 0.45, "1.45M"),
           ("Dresser DIY", 0.31, "980K"), ("Tile test", 0.19, "620K")])
    # expense donut
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Expense Breakdown", font=fs(17), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.42, panels_y + panel_h * 0.52, min(panel_h * 0.28, pw * 0.27),
          [(27, PRIMARY), (26, ACCENT), (21, HIGHLIGHT), (9, SURFACE), (17, (170, 150, 120))], "$1.9K", "per mo")
    legend(d, px + pw * 0.04, panels_y + panel_h - 104, [(PRIMARY, "Editor 26%"), (ACCENT, "Equipment 27%"), (HIGHLIGHT, "Props 21%")], 15, 30)
...
def content_analytics(img, cbox):
    rows = [
        ("$12 thrift flip glow-up", "3.2M", "12.8%", "61%", "+9,200"),
        ("Amazon home finds I use", "1.9M", "10.4%", "54%", "+6,100"),
        ("5 renter kitchen hacks", "1.45M", "11.2%", "58%", "+5,400"),
        ("IKEA dresser makeover", "980K", "9.6%", "49%", "+3,300"),
        ("Peel-and-stick tile test", "620K", "8.1%", "44%", "+1,800"),
        ("Sunday reset routine", "410K", "7.3%", "41%", "+1,100"),
    ]
    _table(img, cbox, "Analytics Command Center",
           "Your account by the numbers — views, engagement, completion & follows per video, ranked",
           ["VIDEO", "VIEWS", "ENG. RATE", "COMPLETION", "FOLLOWS +"],
           [0.0, 0.42, 0.58, 0.74, 0.90], rows)


def content_finance(img, cbox):
    rows = [
        ("Brand Deals", "$3,800", "$45,600", "44%"),
        ("TikTok Shop", "$1,883", "$22,596", "22%"),
        ("Creator Rewards", "$1,200", "$14,400", "14%"),
        ("Affiliate", "$780", "$9,360", "9%"),
        ("LIVE Gifts", "$420", "$5,040", "5%"),
        ("Digital Products", "$400", "$4,800", "5%"),
        ("Consulting", "$100", "$1,200", "1%"),
    ]
    _table(img, cbox, "Creator Finance Center",
           "7 income streams in one place — monthly, annual run-rate & net profit, live.  Net $6,633/mo · 77% margin",
           ["INCOME SOURCE", "THIS MONTH", "ANNUAL (EST.)", "% OF REV"],
           [0.0, 0.42, 0.64, 0.90], rows,
           total_row=("TOTAL REVENUE", "$8,583", "$102,996", "100%"))


def content_trends(img, cbox):
    rows = [
        ("'aesthetic morning' sound", "Sound", "Rising", "No"),
        ("#renterfriendly", "Hashtag", "Peaking", "Yes"),
        ("Slow-zoom reveal", "Effect", "Evergreen", "Yes"),
        ("'oddly satisfying' remix", "Sound", "Peaking", "No"),
        ("#diyonabudget", "Hashtag", "Rising", "Yes"),
        ("Green-screen price tag", "Effect", "Rising", "No"),
        ("'that girl' voiceover", "Sound", "Fading", "No"),
    ]
    _table(img, cbox, "Trends & Sounds",
           "Ride the wave early — track sounds, hashtags & effects while they're still rising",
           ["TREND", "KIND", "STATUS", "USED?"],
           [0.0, 0.44, 0.64, 0.86], rows,
           status_col=2, status_map={"Rising": (MINT_BG, PRIMARY), "Peaking": (WARN_BG, ACCENT),
                                     "Fading": (RED_BG, DANGER), "Evergreen": (SURFACE, (110, 88, 58))})


def content_deals(img, cbox):
    rows = [
        ("Glossier", "Spring GRWM", "$2,800", "Signed"),
        ("Amazon Home", "Prime Day haul", "$3,500", "Negotiation"),
        ("Notion", "Creator template", "$2,200", "Delivered"),
        ("Ruggable", "Rug reveal", "$1,900", "Signed"),
        ("Dyson", "Cleaning day", "$3,200", "Paid"),
        ("Djerf Avenue", "Home edit", "$2,600", "Outreach"),
...
def render_hero(out):
    ...
    tiktok_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE TIKTOK CREATOR BUSINESS SYSTEM", font=fs(25), pad_x=42, pad_y=20)
    wordmark(img, SIZE // 2, 400, "TIKTOK COMMAND CENTER", 104, max_w=1840)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 550), "Ideas, hooks, trends, analytics, Shop & brand deals — turn views into real income, automatically.",
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not a content calendar — a complete TikTok business operating system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Creator Dashboard", "12 KPIs + live charts"), ("Creator Profile", "your account identity"),
        ("Content Calendar", "plan every post"), ("Video Pipeline", "idea → posted"),
        ("Idea Vault", "AI opportunity score"), ("Hook & Script Bank", "the first 2 seconds"),
        ("Trends & Sounds", "ride the wave early"), ("Hashtag & SEO", "get found"),
        ("Analytics Center", "creator health score"), ("LIVE Sessions", "gifts & viewers"),
        ("TikTok Shop", "views → sales"), ("Affiliate Tracker", "links & payouts"),
        ("Brand Deals CRM", "lead → paid"), ("Series & Playlists", "keep them coming back"),
        ("Finance Center", "7 income streams"), ("Expenses", "it's a business"),
        ("Equipment", "your kit"), ("Repurposing", "1 video → 10 posts"),
        ("Brand Kit", "stay recognizable"), ("Content Gallery", "visual archive"),
        ("Goals & OKRs", "measurable results"), ("Audience Insights", "who's watching"),
        ("Collabs & Duets", "grow through others"), ("Settings", "your lists & goals"),
    ]
    cols = 4
    margin = 88
    gx, gy = 22, 18
...
def render_analytics(out):
    ...
    tc(d, (SIZE // 2, 300), "Views, engagement, completion & follows — see exactly what's making you grow",
...
def render_finance(out):
    ...
    tc(d, (SIZE // 2, 300), "Brand deals, Shop, Creator Rewards, affiliate & more — net profit updates automatically",
       fs(23, bold=False), (226, 214, 190))
...
def render_grow(out):
    ...
    tc(d, (SIZE // 2, 224), "Ride Trends. Land Deals.", fserif(46), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 6, content_trends)
    app_window(img, (60, 1180, SIZE - 60, SIZE - 60), 12, content_deals)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_mobile(out):
    ...
    od.text(((sx0 + sx1) // 2, sy0 + 56), "@vellacreates", font=fserif(30), fill=GOLD_HI, anchor="mm")
    y = sy0 + 150
    cards = [("FOLLOWERS", "215K", PRIMARY), ("MONTHLY REVENUE", "$8,583", ACCENT),
             ("SHOP GMV", "$14,233", PRIMARY), ("CREATOR HEALTH", "88%", PRIMARY)]
    ...
    y += 52
    for lab, state in [("Post 'cabinet painting'", False), ("Use rising sound (3 days left)", False),
                       ("Send Amazon deal invoice", True), ("Batch 5 videos", True)]:
