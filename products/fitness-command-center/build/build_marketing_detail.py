"""Detailed marketing image set for Fitness & Meal-Prep Command Center™ (4 images, 2000x2000)."""
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR RESULTS", "Where the Plan Meets the Work",
                "Four ways it turns scattered habits into one system")
    feats = [
        ("$", "Live Fitness Score", "donut", (84, "84%", "on track"),
         "Weight progress, protein, calories, workouts, steps & water roll into one number — so you always know if you're on track at a glance."),
        ("%", "Plan & Prep the Food", "table", None,
         "Build a week of meals from a reusable recipe bank, keep each day near target, and auto-build a grocery list by aisle — meal prep, sorted."),
        ("box", "Log Every Lift & Macro", "stock", None,
         "Track sets × reps × weight for total volume, and calories & protein against your target — the two numbers that actually move the needle."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack — meal planner, grocery list, workout log, macro tracker, weight chart, habit tracker & a meal-prep checklist."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("Chicken & Rice", "520 cal"), ("Salmon & Veg", "540 cal"), ("Protein Smoothie", "280 cal")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["MEAL", "CALORIES"], [0.0, 0.62]):
            ...
            od = ImageDraw.Draw(img)
            items = [("Avg calories", "2,062", MINT_BG, PRIMARY), ("Avg protein", "149 g", MINT_BG, PRIMARY),
                     ("Workouts / wk", "4 / 5", MINT_BG, PRIMARY), ("Total volume", "37,515", SURFACE, (110, 88, 58))]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Fitness Tracker",
                "Everything a basic tracker leaves out — built in")
    rows = [
        ("A workout OR a meal tracker", True, True),
        ("Weekly meal plan + recipe bank", False, True),
        ("Auto grocery list by aisle", False, True),
        ("Macro tracker vs your targets", False, True),
        ("Weekly workout split & log", False, True),
        ("Total-volume progression", False, True),
        ("Body-metrics weight trend", False, True),
        ("Habit tracker (water/sleep/steps)", False, True),
        ("Meal-prep day checklist", False, True),
        ("Live Fitness Score dashboard", False, True),
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "START THIS WEEK", "Get on Track in 4 Steps",
                "No spreadsheet skills needed — if you can type, you can run it")
    steps = [
        ("1", "Set your targets", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack, then add your weight, calorie & protein targets."),
        ("2", "Plan & prep the food", "Build a week of meals from the recipe bank, auto-generate the grocery list, and prep it all on your prep day."),
        ("3", "Train & log it", "Follow the weekly workout split and log every lift; log your macros daily so calories & protein stay on target."),
        ("4", "Track & adjust", "Weigh in weekly, watch the trend line fall, tick off habits, and let your Fitness Score show you what's working."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "EVERYTHING YOU GET", "One System, Whole Routine",
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + Fitness Score",
        "Weekly meal plan & recipe bank", "Auto grocery list by aisle",
        "Macro tracker vs your targets", "Workout plan, log & volume",
        "Body-metrics weight trend & habits", "Start-Here guide + free updates",
    ]
    ...
    who = ["Weight-loss & recomp journeys", "Gym-goers who count macros", "Meal-preppers & batch cookers",
           "Beginners who want structure", "Couples getting fit together", "Anyone building a healthy habit"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "PLAN · TRAIN · TRACK · TRANSFORM", font=fs(27), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Plan the food, log the lifts & track the body — consistency, made simple.",
       fs(23, bold=False), TEXT_MUTED)
    book_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "FITNESS & MEAL-PREP", 52, max_w=1300)
    img.convert("RGB").save(out, "PNG", optimize=True)
