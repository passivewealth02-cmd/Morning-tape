"""Detailed marketing image set for Home Bakery & Cottage Food Command Center™ (07-10)."""
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR HOME BAKERS", "Stop Underpricing",
                "Four ways it turns your baking from a hobby into a business")
    feats = [
        ("$", "Live Bakery Score", "donut", (90, "90%", "healthy"),
         "Food cost, margins, pricing, your hourly wage, profit & waste roll into one number — so you know your bakery's health at a glance."),
        ("%", "Pay Yourself", "table", None,
         "Add ingredients, packaging, YOUR time and overhead — the engine shows your true cost and your real hourly wage, the number bakers forget."),
        ("box", "Run the Business", "stock", None,
         "Manage custom orders, cottage-food labels with allergens, ingredient costs, markets and income & expenses for tax time — all in one place."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack — price-it worksheet, recipe cost card, custom order form, cottage-food label, market checklist & more."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("Ingredients + pkg", "$7.50"), ("+ Your time (45 min)", "$15.00"), ("Your hourly wage", "$32.67")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["CUSTOM COOKIES", "VALUE"], [0.0, 0.62]):
            ...
            od = ImageDraw.Draw(img)
            items = [("Monthly income", "$2,908", MINT_BG, PRIMARY), ("Monthly profit", "$1,658", MINT_BG, PRIMARY),
                     ("Open orders", "5", MINT_BG, PRIMARY), ("Order value", "$416", SURFACE, (110, 88, 58))]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    ...
    rows = [
        ...
        ("A price-it engine that pays you", False, True),
        ("Your true hourly wage", False, True),
        ("Recipe cost-per-batch", False, True),
        ("Custom order tracker", False, True),
        ("Cottage-food labels & allergens", False, True),
        ("Ingredient cost library", False, True),
        ("Markets & booth-cost net", False, True),
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "SET UP IN AN AFTERNOON", "Price Your Bakes in 4 Steps",
    ...
    steps = [
        ...
        ("2", "Price it right", "Add your ingredients, packaging, YOUR time and overhead — the engine shows your true cost and your real hourly wage."),
        ("3", "Book the orders", "Track custom orders, build cottage-food labels with allergens, and keep your ingredient and market costs up to date."),
        ("4", "Run the business", "Keep income & expenses for tax time, log waste, and watch your monthly profit and your Bakery Score climb."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    ...
    includes = [
        ...
        "Price-it-right (pay-yourself) engine", "Recipe costing & product list",
        "Custom order tracker", "Cottage-food labels & allergens",
        "Markets, income & expenses & waste", "Start-Here guide + free updates",
    ]
    ...
    who = ["Home & cottage bakers", "Custom cake & cookie makers", "Farmers-market sellers",
           "Cottage-food businesses", "Side-hustle & hobby bakers", "Anyone selling what they bake"]
    ...
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Cost every recipe, price for profit, and finally pay yourself for your time.",
       fs(23, bold=False), TEXT_MUTED)
    book_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "HOME BAKERY", 64, max_w=1100)
    img.convert("RGB").save(out, "PNG", optimize=True)
