"""Marketing image set for Home Bakery & Cottage Food Command Center™ (6 images, 2000x2000)."""
...
TABS = ["Dashboard", "Price It Right", "Recipe Costing", "Product List", "Custom Orders", "Ingredient Costs",
        "Labeling & Allergens", "Markets & Events", "Income & Expenses", "Waste Log", "Customers", "Monthly Summary", "Settings"]

FILE_LABEL = "Home_Bakery_Command_Center.xlsx — Sugar & Thyme · Mel"
...
def cupcake_crest(c, cx, cy, r=56, glow=True):
    """Brand crest — a cupcake: the icon of the home bakery."""
    if glow:
    ...
    wy0, wy1 = cy + r * 0.02, cy + r * 0.44
    d.polygon([(cx - r * 0.30, wy0), (cx + r * 0.30, wy0), (cx + r * 0.22, wy1), (cx - r * 0.22, wy1)],
              fill=GOLD_HI, outline=(150, 120, 70))
    # wrapper ridges
    for fx in (-0.14, 0.0, 0.14):
        d.line((cx + r * fx, wy0 + 2, cx + r * fx * 0.75, wy1 - 2), fill=(150, 120, 70), width=2)
    # frosting swirl (three stacked bumps)
    fy = wy0
    for k, (rr, dy) in enumerate([(0.32, 0.0), (0.24, -0.18), (0.15, -0.34)]):
        d.ellipse((cx - r * rr, fy - r * 0.20 + r * dy, cx + r * rr, fy + r * 0.16 + r * dy), fill=WHITE)
    # cherry on top
    d.ellipse((cx - r * 0.07, wy0 - r * 0.56, cx + r * 0.07, wy0 - r * 0.42), fill=DANGER)
    c.alpha_composite(ov)


clock_crest = cupcake_crest
clipboard_crest = cupcake_crest
cloche_crest = cupcake_crest
ghost_crest = cupcake_crest
mug_crest = cupcake_crest
book_crest = cupcake_crest
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "BAKERY", font=fs(15), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ...
    ("AVG PRICE", "$28.25", "per product"),
    ("FOOD COST", "18%", "of price"),
    ("AVG MARGIN", "82%", "per product"),
    ("MONTHLY INCOME", "$2,908", "this month"),
    ("TOP SELLER", "Cookies", "$660/mo"),
    ("MONTHLY PROFIT", "$1,658", "after costs"),
    ("YOUR HOURLY", "$32.67", "you earn"),
    ("OPEN ORDERS", "5", "on the books"),
    ("ORDER VALUE", "$416", "coming in"),
    ("MONTHLY UNITS", "113", "baked"),
    ("BAKERY SCORE", "90%", "healthy"),
]

WEEK = [("Jul", 1330), ("Aug", 1500), ("Sep", 1658), ("Oct", 1860), ("Nov", 2330), ("Dec", 2820)]


def content_dashboard(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Home Bakery Dashboard", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 64), "Sugar & Thyme · Mel  ·  cost every recipe, and pay yourself for your time", font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 200, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 100, y0 + 44), "● $32.67/hr for you", font=fs(15), fill=PRIMARY, anchor="mm")
    ...
    d.text((gx, cy_top), "BAKERY HEALTH · FOOD COST · PROFIT BY MONTH", font=fs(20), fill=ACCENT, anchor="lt")
    ...
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("Food cost on target", 1.0, "100%"), ("Margins healthy", 1.0, "100%"), ("Products priced", 1.0, "100%"),
           ("Paying yourself", 1.0, "100%"), ("Profitable", 1.0, "100%"), ("Waste low", 0.38, "38%")],
          color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    ...
    donut(d, px + pw * 0.44, panels_y + panel_h * 0.50, min(panel_h * 0.27, pw * 0.27),
          [(18, ACCENT), (82, PRIMARY)], "18%", "food cost")
    legend(d, px + pw * 0.06, panels_y + panel_h - 96, [(PRIMARY, "Margin 82%"), (ACCENT, "Food cost 18%")], 15, 32)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Profit by Month", font=fs(17), fill=ACCENT, anchor="lt")
    vbars(img, d, (px + 18, panels_y + 60, px + pw - 14, panels_y + panel_h - 10), WEEK, 2820)
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
...
def content_menu(img, cbox):
    rows = [
        ("Custom Cookies (dz)", "$6.00", "$33", "82%", "$660"),
        ("Layer Cake 8\"", "$9.00", "$55", "84%", "$440"),
        ("Cupcakes (dz)", "$5.50", "$30", "82%", "$450"),
        ("Macarons (dz)", "$5.80", "$30", "81%", "$300"),
        ("Brownies (dz)", "$4.10", "$22", "81%", "$308"),
        ("Banana Bread", "$2.40", "$12", "80%", "$300"),
    ]
    _table(img, cbox, "Product List",
           "Ingredient cost, price, margin & monthly revenue on every product — price for profit.",
           ["PRODUCT", "ING. COST", "PRICE", "MARGIN", "REV/MO"],
...
def content_itemmargin(img, cbox):
    rows = [
        ...
        ("+ Packaging", "$1.50"),
        ("+ Your labor (45 min × $20)", "$15.00"),
        ("+ Overhead", "$1.00"),
    ]
    _table(img, cbox, "Price It Right — Custom Cookies",
           "Add your ingredients, packaging, YOUR time & overhead — then see your true hourly wage.",
           ["LINE", "AMOUNT"],
           [0.0, 0.72], rows,
           total_row=("= TRUE COST  ·  price $33", "$23.50"))


def content_pnl(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Price It Right — Pay Yourself", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 62), "The number home bakers forget: your true hourly wage after ingredients & overhead.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("Your price", "$33.00", PRIMARY), ("− Ingredients", "−$6.00", DANGER),
            ("− Packaging", "−$1.50", DANGER), ("− Overhead", "−$1.00", DANGER)]
    fxs = [0.0, 0.80]
    ...
    for h, fx in zip(["LINE", "AMOUNT"], fxs):
    ...
    d.text((x0 + pad + 14, ry + rh / 2), "= YOUR HOURLY WAGE  ·  45 min", font=fs(18), fill=PRIMARY, anchor="lm")
    d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.80, ry + rh / 2), "$32.67", font=fs(18), fill=PRIMARY, anchor="mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2), "If you're not paying yourself, it's a hobby — this makes it a business", font=fs(21), fill=WHITE, anchor="mm")
    img.alpha_composite(ov)
...
def render_hero(out):
    ...
    cupcake_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE COMPLETE HOME-BAKERY SYSTEM", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "HOME BAKERY", 92, max_w=1500)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 546), "Price it right, recipe costing, custom orders, cottage-food labels & income — one system.",
       fs(20, bold=False), (224, 213, 190))
    chips = [("14", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("PRICE-IT", "ENGINE")]
    cw = 440
...
def render_inside(out):
    ...
    cards = [
        ("Start Here", "how it all works"), ("Dashboard", "income, profit & score"),
        ("Price It Right", "pay yourself for time"), ("Recipe Costing", "cost by the batch"),
        ("Product List", "margin on every item"), ("Custom Orders", "nothing gets missed"),
        ("Ingredient Costs", "your pantry prices"), ("Labeling & Allergens", "cottage-food labels"),
        ("Markets & Events", "the booth's real net"), ("Income & Expenses", "your books for taxes"),
        ("Waste Log", "the quiet leaks"), ("Customers", "your regulars"),
        ("Monthly Summary", "watch profit grow"), ("Settings", "set it once"),
        ("+ 12 Printable Pages", "order forms & labels"), ("Live Bakery Score", "it all rolls up"),
    ]
    cols = 4
    margin = 88
    gx, gy = 22, 20
...
def render_menu(out):
    ...
    tc(d, (SIZE // 2, 232), "Margin on Every Product", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Ingredient cost, price, margin & monthly revenue on every product you bake",
...
def render_variance(out):
    ...
    tc(d, (SIZE // 2, 300), "The number home bakers forget — what you really earn per hour after ingredients & overhead",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 1, content_pnl)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_engine(out):
    ...
    tc(d, (SIZE // 2, 224), "The Price-It-Right Engine", fserif(44), WHITE)
...
def render_printables(out):
    ...
    d = ImageDraw.Draw(img)
    cupcake_crest(img, SIZE // 2, 110, r=48)
    ...
    tc(d, (SIZE // 2, 404), "A matching print-ready pack — price-it worksheet, order form, cottage-food label & more",
       fs(21, bold=False), (226, 214, 190))
    labels = ["Price It Right", "Recipe Cost Card", "Product Price List", "Custom Order Form",
              "Ingredient List", "Cottage Food Label", "Market Checklist", "Income & Expenses",
              "Waste Log", "Customer List", "Monthly Summary", "Order Calendar"]
...
def main():
    ...
    targets = [
        ...
        ("04_payyourself.png", render_variance),
