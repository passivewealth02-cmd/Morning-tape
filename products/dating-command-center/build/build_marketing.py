"""Marketing image set for Dating Life Command Center™ (6 images, 2000x2000)."""
...
TABS = ["Dashboard", "Dating Funnel", "Effort & Reciprocity", "Green & Red Flags", "People",
        "Date Log", "Conversations", "Time & Money", "Non-Negotiables", "Safety Plan",
        "Reflection", "Monthly Summary", "Settings"]

FILE_LABEL = "Dating_Command_Center.xlsx — private · yours"
...
def spark_crest(c, cx, cy, r=56, glow=True):
    """Brand crest — a struck match: the spark, before you decide if it's worth keeping lit."""
    if glow:
    ...
    c.alpha_composite(ov)
    ov = Image.new("RGBA", c.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    # the matchstick, angled
    od.line((cx - r * 0.34, cy + r * 0.52, cx + r * 0.12, cy - r * 0.10), fill=GOLD_HI, width=max(4, int(r * 0.10)))
    # the head
    od.ellipse((cx + r * 0.02, cy - r * 0.24, cx + r * 0.24, cy - r * 0.02), fill=(150, 120, 70))
    # the flame
    od.polygon([(cx + r * 0.13, cy - r * 0.62), (cx + r * 0.30, cy - r * 0.22),
                (cx + r * 0.13, cy - r * 0.06), (cx - r * 0.04, cy - r * 0.22)], fill=GOLD_HI)
    od.polygon([(cx + r * 0.13, cy - r * 0.46), (cx + r * 0.22, cy - r * 0.22),
                (cx + r * 0.13, cy - r * 0.12), (cx + r * 0.04, cy - r * 0.22)], fill=HIGHLIGHT)
    # little sparks
    for dx, dy, rad in [(-0.34, -0.44, 0.045), (0.42, -0.40, 0.035), (-0.20, -0.62, 0.030)]:
        od.ellipse((cx + r * dx - r * rad, cy + r * dy - r * rad,
                    cx + r * dx + r * rad, cy + r * dy + r * rad), fill=GOLD_HI)
    c.alpha_composite(ov)


square_crest = spark_crest
seal_crest = spark_crest
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "YOUR MONTH", font=fs(15), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("MATCHES", "240", "this month"),
    ("CONVERSATIONS", "68", "28.3% of matches"),
    ("FIRST DATES", "14", "5.8% of matches"),
    ("SECOND DATES", "5", "35.7% of first dates"),
    ("STILL SEEING", "2", "actually"),
    ("DAYS TO FIRST DATE", "9", "from match"),
    ("HOURS THIS MONTH", "69", "2.9 full days"),
    ("SPENT THIS MONTH", "$497", "apps, dates, getting ready"),
    ("PER SECOND DATE", "$99.40", "and 13.8 hours"),
    ("GREEN MINUS RED", "7", "9 green, 2 red"),
    ("YOU'RE DOING", "3.0\u00d7", "the work"),
    ("DATING SCORE", "90%", "mostly good"),
]

REV_MONTHS = [("Feb", 8), ("Mar", 9), ("Apr", 11), ("May", 12), ("Jun", 13), ("Jul", 14)]


def content_dashboard(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Your Month, Honestly", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 64), "Not to judge anyone \u00b7 to see clearly what you already feel",
           font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 230, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=WARN_BG)
    d.text((x1 - pad - 115, y0 + 44), "\u25cf you're doing 3.0\u00d7", font=fs(14), fill=ACCENT, anchor="mm")
    ...
    d.text((gx, cy_top), "HOW IT'S GOING \u00b7 WHERE THE 69 HOURS WENT \u00b7 FIRST DATES BY MONTH", font=fs(19), fill=ACCENT, anchor="lt")
    ...
    d.text((px + 16, panels_y + 14), "How It's Going", font=fs(17), fill=ACCENT, anchor="lt")
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("Meeting, not just texting", 1.0, "100%"), ("Second dates happening", 1.0, "100%"),
           ("Green outweighs red", 1.0, "100%"), ("Inside your budget", 1.0, "100%"),
           ("Safety steps held", 1.0, "100%"), ("Effort is reciprocal", 0.40, "40%")],
          color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Where the 69 Hours Went", font=fs(14), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.44, panels_y + panel_h * 0.46, min(panel_h * 0.25, pw * 0.25),
          [(50.7, PRIMARY), (20.3, ACCENT), (15.9, SURFACE), (13.0, GOLD_LT)], "51%", "on dates")
    legend(d, px + pw * 0.06, panels_y + panel_h - 148,
           [(PRIMARY, "On dates 35 hrs"), (ACCENT, "Getting ready 14 hrs"),
            (SURFACE, "Messaging 11 hrs"), (GOLD_LT, "Swiping 9 hrs")], 14, 30)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "First Dates by Month", font=fs(16), fill=ACCENT, anchor="lt")
    vbars(img, d, (px + 18, panels_y + 60, px + pw - 14, panels_y + panel_h - 10), REV_MONTHS, 14,
          fmt=lambda v: str(int(v)))
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Dating Score", font=fs(17), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.5, panels_y + panel_h * 0.54, min(panel_h * 0.30, pw * 0.30),
          [(90, PRIMARY), (10, SURFACE)], "90%", "mostly good")
...
def content_effort(img, cbox):
    rows = [
        ("Initiating contact", "9", "2", "7", "You always text first"),
        ("Planning the dates", "9", "2", "7", "You always plan"),
        ("Following through", "9", "3", "6", "They cancel"),
        ("Asking about your life", "7", "2", "5", "They don't ask"),
        ("Paying / splitting fairly", "6", "4", "2", "This one's fine"),
    ]
    _table(img, cbox, "Effort & Reciprocity \u2014 Scored",
           "Out of ten, on what actually happened. Anything with a gap of 5+ turns red.",
           ["WHAT", "YOU", "THEM", "GAP", "WHAT THAT LOOKS LIKE"],
           [0.0, 0.34, 0.43, 0.52, 0.62], rows,
           total_row=("TOTAL", "48", "16", "32", "you're doing 3.0\u00d7 the work"))


def content_funnel(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "The Dating Funnel", font=fs(30), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 60), "Matches to conversations to dates \u2014 and what each one really costs you.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("240 matches", "\u2192 68 conversations", ACCENT),
            ("68 conversations", "\u2192 14 first dates", ACCENT),
            ("14 first dates", "\u2192 5 second dates", PRIMARY),
            ("69 hours and $497 spent", "this month", DANGER)]
    ...
    for h, fx in zip(["THE FUNNEL", "WHAT SURVIVES"], [0.0, 0.72]):
    ...
    for i, (label, val, col) in enumerate(rows):
        ...
        d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.72, ry + rh / 2), val, font=fs(18, bold=True), fill=col, anchor="mm")
    ry = ty + rh + len(rows) * rh
    d.rectangle((x0 + pad, ry, x1 - pad, ry + rh), fill=SURFACE)
    d.text((x0 + pad + 14, ry + rh / 2), "= EVERY SECOND DATE COST YOU", font=fs(18), fill=PRIMARY, anchor="lm")
    d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.72, ry + rh / 2), "$99.40 and 13.8 hrs", font=fs(18), fill=PRIMARY, anchor="mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2 - 20), "Not to make you feel bad about any of it", font=fs(22), fill=WHITE, anchor="mm")
    od.text(((x0 + x1) / 2, by + callout_h / 2 + 24), "TO LET YOU DECIDE, ON PURPOSE, HOW YOU SPEND A TUESDAY", font=fs(20), fill=GOLD_HI, anchor="mm")
    img.alpha_composite(ov)
...
def render_hero(out):
    ...
    spark_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE COMPLETE DATING SYSTEM", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "SEE IT CLEARLY", 86, max_w=1550)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 546), "Your dating funnel, an effort scorecard, flags, patterns & a safety plan.",
       fs(20, bold=False), (224, 213, 190))
    chips = [("14", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("EFFORT", "SCORECARD")]
    cw = 440
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not a cute planner \u2014 a system for seeing what you already feel",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Start Here", "why this exists"), ("Dashboard", "your month, honestly"),
        ("Dating Funnel", "what each date costs"), ("Effort & Reciprocity", "are they meeting you?"),
        ("Green & Red Flags", "what you've SEEN"), ("People", "where it stands"),
        ("Date Log", "and how it felt"), ("Conversations", "dating you or texting you"),
        ("Time & Money", "both currencies"), ("Non-Negotiables", "written down first"),
        ("Safety Plan", "before every first date"), ("Reflection", "the patterns"),
        ("Monthly Summary", "the whole month"), ("Settings", "set it once"),
        ("+ 12 Printable Pages", "flags, logs & prompts"), ("Live Dating Score", "it all rolls up"),
    ]
    cols = 4
    margin = 88
    gx, gy = 22, 20
...
def render_effort(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "YOU'RE DOING 3\u00d7 THE WORK", font=fs(29), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "The Effort Scorecard", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Score yourself, then score them \u2014 and finally see the thing you already felt",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 2, content_effort)
    img.convert("RGB").save(out, "PNG", optimize=True)


def render_funnel(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "$99 AND 14 HOURS PER SECOND DATE", font=fs(25), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "The Dating Funnel", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "240 matches became 5 second dates \u2014 here is exactly where they went",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 1, content_funnel)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_engine(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 110, "COUNT IT \u00b7 SEE IT", font=fs(32), pad_x=52, pad_y=22)
    tc(d, (SIZE // 2, 224), "The Clarity Engine", fserif(44), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 1, content_funnel)
    app_window(img, (60, 1180, SIZE - 60, SIZE - 60), 2, content_effort)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_printables(out):
    ...
    d = ImageDraw.Draw(img)
    spark_crest(img, SIZE // 2, 110, r=48)
    pill(img, SIZE // 2, 214, "PLUS 12 PRINTABLE PAGES", font=fs(30), pad_x=48, pad_y=20)
    tc(d, (SIZE // 2, 300), "Print, Fill & Keep It Private", fserif(42), WHITE)
    gold_divider(img, SIZE // 2, 362, width=480)
    tc(d, (SIZE // 2, 404), "A matching print-ready pack \u2014 non-negotiables, flag lists, safety plan & reflection prompts",
       fs(21, bold=False), (226, 214, 190))
    labels = ["Non-Negotiables", "The Month, Counted", "Effort & Reciprocity", "Green Flags",
              "Red Flags", "Date Log", "People", "Conversations",
              "Time & Money", "Safety Plan", "Reflection", "Month in Review"]
...
def main():
    ...
    targets = [
        ...
        ("03_effort.png", render_effort),
        ("04_funnel.png", render_funnel),
