"""Detailed marketing image set for Dating Life Command Center™ (07-10)."""
...
def render_features(out):
    ...
    header_band(img, "FOR SEEING IT CLEARLY", "Count It, Score It, Decide",
                "Four ways it turns a feeling you can't name into a number you can")
    feats = [
        ("$", "Live Dating Score", "donut", (90, "90%", "mostly good"),
         "Meeting people rather than texting them, second dates, flags, budget, safety and reciprocity roll into one number \u2014 how your dating LIFE is going, not how desirable you are."),
        ("%", "The Effort Scorecard", "table", None,
         "Score yourself out of ten on initiating, planning, follow-through and curiosity. Then score them. The ratio is the thing you've been feeling for weeks and couldn't prove."),
        ("box", "The Whole Picture", "stock", None,
         "A dating funnel with what each date really costs in time and money, green and red flag lists, who you're seeing, conversations that never became dates, and your non-negotiables."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack \u2014 non-negotiables to write before you meet anyone, both flag lists, a full safety plan, and the reflection prompts that break patterns."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("Your effort", "48 / 60"), ("Their effort", "16 / 60"), ("You're doing", "3.0\u00d7 the work")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["THE SCORECARD", "VALUE"], [0.0, 0.62]):
            ...
            od = ImageDraw.Draw(img)
            items = [("Second-date rate", "35.7%", MINT_BG, PRIMARY), ("Green minus red", "7", MINT_BG, PRIMARY),
                     ("Per second date", "$99.40", SURFACE, (110, 88, 58)), ("You're doing", "3.0\u00d7", SURFACE, (110, 88, 58))]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Cute Planner",
                "Everything a dating journal leaves out \u2014 built in")
    rows = [
        ("Somewhere to write about your dates", True, True),
        ("Matches, conversations and dates counted", False, True),
        ("What every second date actually costs", False, True),
        ("Hours spent, not just money", False, True),
        ("An EFFORT SCORECARD with a real ratio", False, True),
        ("Green and red flags you tick as SEEN", False, True),
        ("Who's dating you vs just texting you", False, True),
        ("Non-negotiables written before you meet", False, True),
        ("A full safety plan for first dates", False, True),
        ("Monthly reflection that finds the pattern", False, True),
    ...
    d.text((c1, top + hh // 2), "A Dating\nJournal", font=fs(18), fill=(210, 210, 205), anchor="mm")
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "TEN MINUTES A WEEK", "How It Actually Works",
                "No spreadsheet skills needed \u2014 if you can type, you can use it")
    steps = [
        ("1", "Write your non-negotiables first", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack. Before anything else, write down what you will not bend on \u2014 while nobody's face is attached to it."),
        ("2", "Just count, for one month", "Matches, conversations, first dates, second dates. Log what each date cost and how long it took. No judgement, no story \u2014 only the counts."),
        ("3", "Score the effort, honestly", "Out of ten, on initiating, planning, follow-through and curiosity: you, then them. The ratio it produces is usually the thing you've felt for weeks and couldn't prove."),
        ("4", "Read your own patterns back", "At month end the reflection page asks eight questions. That page is the whole point \u2014 patterns break because you wrote them down twice, not because you noticed once."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "EVERYTHING YOU GET", "One System, Whole Picture",
                "Instant digital download \u00b7 private \u00b7 Google Sheets & Excel")
    d = ImageDraw.Draw(img)
    margin = 84
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + Dating Score",
        "Dating funnel with real costs", "The effort & reciprocity scorecard",
        "Green & red flag checklists", "People, dates & conversations",
        "Non-negotiables, safety plan & reflection", "Start-Here quick-start guide",
    ]
    ...
    who = ["Anyone dating on apps and tired of it", "Back out there after a long relationship",
           "Dating with intention this year", "Anyone who keeps picking the same person",
           "Anyone doing all the texting", "Anyone who wants to stop second-guessing"]
    ...
    od.text((rx0 + 64, 1228), "Private, and entirely yours.", font=fs(19, bold=False), fill=TEXT, anchor="lm")
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "COUNT IT \u00b7 SCORE IT \u00b7 DECIDE", font=fs(27), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "You already know. This just puts a number on the thing you already know.",
       fs(23, bold=False), TEXT_MUTED)
    square_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "SEE IT CLEARLY", 58, max_w=1150)
    img.convert("RGB").save(out, "PNG", optimize=True)
