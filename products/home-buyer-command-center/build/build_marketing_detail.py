"""Detailed marketing image set for First-Time Home Buyer & Mortgage Command Center™ (07-10)."""
...
from build_marketing import (
    ...
    ROW_ALT, fs, fserif, premium_bg, pill, wordmark, gold_divider, key_crest,
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR FIRST-TIME BUYERS", "Afford It, Find It, Close It",
                "Four ways it turns a scary purchase into a plan")
    feats = [
        ("$", "Live Buyer Score", "donut", (90, "90%", "ready"),
         "Cash saved, payment affordable, debt healthy, credit ready, emergency fund and 20%-down roll into one number — how ready you actually are, at a glance."),
        ("%", "Affordability Engine", "table", None,
         "Price, down payment and rate become your true all-in monthly payment — principal, interest, taxes, insurance and PMI — checked against real lender DTI limits."),
        ("box", "Run the Whole Purchase", "stock", None,
         "Down-payment savings, closing costs, home scoring, lender comparison, credit prep and a moving checklist — the entire journey in one place."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack — affordability worksheet, home comparison, closing-cost checklist and a must-have list to take to showings."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("Home price", "$290,000"), ("Monthly payment", "$2,213"), ("Front-end DTI", "28.0%")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["AFFORDABILITY ENGINE", "VALUE"], [0.0, 0.66]):
            ...
            od = ImageDraw.Draw(img)
            items = [("Cash to close", "$31,900", MINT_BG, PRIMARY), ("Saved", "$32,000", MINT_BG, PRIMARY),
                     ("Back-end DTI", "33.6%", MINT_BG, PRIMARY), ("20% down (no PMI)", "40%", SURFACE, (110, 88, 58))]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Mortgage Calculator",
                "Everything a free online calculator leaves out — built in")
    rows = [
        ("A monthly payment estimate", True, True),
        ("Taxes, insurance & PMI included", False, True),
        ("Front & back-end DTI check", False, True),
        ("Down-payment savings tracker", False, True),
        ("Itemized closing costs", False, True),
        ("Home comparison scoring", False, True),
        ("Lender & rate comparison", False, True),
        ("Credit-prep action list", False, True),
        ("Life-after-buying budget", False, True),
        ("Live Buyer Score dashboard", False, True),
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "SET UP IN AN AFTERNOON", "Buy Your First Home in 4 Steps",
                "No spreadsheet skills needed — if you can type, you can run it")
    steps = [
        ("1", "Find your true number", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack, then enter your income and a target price to see your real all-in payment."),
        ("2", "Check the DTI", "The engine checks your payment against the front-end and back-end ratios lenders use, so you know whether it's comfortable or a stretch — before you fall in love with a house."),
        ("3", "Save & shop", "Track your down payment and closing costs to the dollar, compare lenders on rate and fees, and score every home you tour side by side."),
        ("4", "Close with confidence", "Prep your credit, plan the budget you'll live on after closing, work the moving checklist, and let the Buyer Score tell you when you're truly ready."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "EVERYTHING YOU GET", "One System, Whole Purchase",
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + Buyer Score",
        "Affordability & DTI engine", "Down payment & closing costs",
        "Home comparison scorer", "Lender & rate comparison",
        "Credit prep & moving checklist", "Start-Here quick-start guide",
    ]
    ...
    who = ["First-time home buyers", "Couples buying together", "Renters ready to own",
           "Anyone saving a down payment", "Buyers comparing lenders", "Anyone house hunting now"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "AFFORD IT · FIND IT · CLOSE IT", font=fs(27), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Know what you can truly afford — before an agent tells you.",
       fs(23, bold=False), TEXT_MUTED)
    key_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "HOME BUYER", 58, max_w=1150)
    img.convert("RGB").save(out, "PNG", optimize=True)
