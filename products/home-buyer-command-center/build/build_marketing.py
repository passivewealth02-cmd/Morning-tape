"""Marketing image set for First-Time Home Buyer & Mortgage Command Center™ (6 images, 2000x2000)."""
...
TABS = ["Dashboard", "Affordability", "Down Payment", "Closing Costs", "Home Comparison", "Lender Compare",
        "Amortization", "Life After Buying", "Credit Prep", "House Hunting", "Moving Checklist",
        "Monthly Summary", "Settings"]

FILE_LABEL = "Home_Buyer_Command_Center.xlsx — Keystone & Co. · Avery"
...
def key_crest(c, cx, cy, r=56, glow=True):
    """Brand crest — a house key: the icon of getting the keys."""
    if glow:
    ...
    c.alpha_composite(ov)
    ov = Image.new("RGBA", c.size, (0, 0, 0, 0)); d = ImageDraw.Draw(ov)
    # roofline above the key
    d.polygon([(cx, cy - r * 0.46), (cx - r * 0.34, cy - r * 0.20), (cx + r * 0.34, cy - r * 0.20)],
              fill=HIGHLIGHT, outline=(60, 150, 130))
    # key bow (ring)
    bx, by, br = cx - r * 0.18, cy + r * 0.10, r * 0.16
    d.ellipse((bx - br, by - br, bx + br, by + br), outline=GOLD_HI, width=6)
    # key shaft
    d.line((bx + br, by, cx + r * 0.36, by), fill=GOLD_HI, width=7)
    # key teeth
    d.line((cx + r * 0.20, by, cx + r * 0.20, by + r * 0.14), fill=GOLD_HI, width=6)
    d.line((cx + r * 0.32, by, cx + r * 0.32, by + r * 0.11), fill=GOLD_HI, width=6)
    c.alpha_composite(ov)


# aliases (shared render helpers may reference these names across products)
ledger_crest = key_crest
magnify_crest = key_crest
hardhat_crest = key_crest
house_crest = key_crest
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "HOME BUYER", font=fs(15), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("HOME PRICE", "$290,000", "target"),
    ("DOWN PAYMENT", "$23,200", "8% down"),
    ("LOAN AMOUNT", "$266,800", "30 yr @ 6.5%"),
    ("PRINCIPAL & INT", "$1,686", "per month"),
    ("MONTHLY PAYMENT", "$2,213", "all-in PITI"),
    ("FRONT-END DTI", "28.0%", "housing only"),
    ("BACK-END DTI", "33.6%", "all debts"),
    ("CLOSING COSTS", "$8,700", "3% of price"),
    ("CASH TO CLOSE", "$31,900", "at the table"),
    ("SAVED", "$32,000", "fully funded"),
    ("TOTAL INTEREST", "$340,289", "over 30 yrs"),
    ("BUYER SCORE", "90%", "ready"),
]

SAVE_MONTHS = [("Feb", 24000), ("Mar", 25200), ("Apr", 26600), ("May", 28000), ("Jun", 30000), ("Jul", 32000)]


def content_dashboard(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Home Buyer Dashboard", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 64), "Keystone & Co. · Avery  ·  know what you can truly afford, before an agent tells you", font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 210, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 105, y0 + 44), "● $2,213 a month", font=fs(14), fill=PRIMARY, anchor="mm")
    ...
    d.text((gx, cy_top), "BUYER HEALTH · WHERE THE PAYMENT GOES · SAVED BY MONTH", font=fs(19), fill=ACCENT, anchor="lt")
    ...
    d.text((px + 16, panels_y + 14), "Buyer Health", font=fs(17), fill=ACCENT, anchor="lt")
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("Cash to close saved", 1.0, "100%"), ("Payment affordable", 1.0, "100%"), ("Total debt healthy", 1.0, "100%"),
           ("Credit ready", 1.0, "100%"), ("Emergency fund", 1.0, "100%"), ("20% down (no PMI)", 0.40, "40%")],
          color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Where the Payment Goes", font=fs(15), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.44, panels_y + panel_h * 0.50, min(panel_h * 0.27, pw * 0.27),
          [(76, PRIMARY), (12, ACCENT), (7, SURFACE), (5, DANGER)], "76%", "is P&I")
    legend(d, px + pw * 0.06, panels_y + panel_h - 140, [(PRIMARY, "P&I $1,686"), (ACCENT, "Tax $266"), (SURFACE, "Insurance $150"), (DANGER, "PMI $111")], 14, 29)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Saved by Month", font=fs(17), fill=ACCENT, anchor="lt")
    vbars(img, d, (px + 18, panels_y + 60, px + pw - 14, panels_y + panel_h - 10), SAVE_MONTHS, 32000)
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Buyer Score", font=fs(17), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.5, panels_y + panel_h * 0.54, min(panel_h * 0.30, pw * 0.30),
          [(90, PRIMARY), (10, SURFACE)], "90%", "ready")
...
def content_homes(img, cbox):
    rows = [
        ("12 Birch Lane", "$289,000", "3 / 2", "1,580", "$183", "92"),
        ("48 Cedar Ct", "$305,000", "3 / 2", "1,720", "$177", "86"),
        ("7 Maple Row", "$275,000", "3 / 1.5", "1,440", "$191", "78"),
        ("91 Oak Ridge", "$320,000", "4 / 2.5", "1,980", "$162", "74"),
        ("30 Alder St", "$268,000", "2 / 2", "1,290", "$208", "69"),
    ]
    _table(img, cbox, "Home Comparison",
           "Score every house side by side — price, size, cost per square foot and your own rating.",
           ["ADDRESS", "PRICE", "BD / BA", "SQ FT", "$ / SQ FT", "SCORE"],
           [0.0, 0.32, 0.48, 0.62, 0.76, 0.91], rows,
           total_row=("5 homes toured", "", "", "", "best", "92"))


def content_afford(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Affordability — Your True Payment", font=fs(29), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 60), "The all-in monthly number a lender quotes you never includes. This one does.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("Principal & interest", "$1,686", PRIMARY),
            ("+ Property tax", "$266", PRIMARY),
            ("+ Home insurance", "$150", PRIMARY),
            ("+ PMI (under 20% down)", "$111", DANGER)]
    callout_h = 150; gap = 40; rh = 96
    ...
    for h, fx in zip(["LINE", "MONTHLY"], [0.0, 0.80]):
    ...
    d.text((x0 + pad + 14, ry + rh / 2), "= YOUR MONTHLY PAYMENT", font=fs(18), fill=PRIMARY, anchor="lm")
    d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.80, ry + rh / 2), "$2,213", font=fs(18), fill=PRIMARY, anchor="mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2 - 20), "28.0% front-end DTI  ·  33.6% back-end DTI", font=fs(22), fill=WHITE, anchor="mm")
    od.text(((x0 + x1) / 2, by + callout_h / 2 + 24), "VERDICT: COMFORTABLE", font=fs(24), fill=GOLD_HI, anchor="mm")
    img.alpha_composite(ov)
...
def render_hero(out):
    ...
    key_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE COMPLETE HOME-BUYING SYSTEM", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "HOME BUYER", 86, max_w=1550)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 546), "An affordability engine, down-payment tracker, home scorer & lender compare — one system.",
       fs(20, bold=False), (224, 213, 190))
    chips = [("14", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("AFFORD", "ENGINE")]
    cw = 440
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not a calculator — a complete afford-it, find-it, close-it system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Start Here", "how it all works"), ("Dashboard", "payment & score"),
        ("Affordability", "your true PITI"), ("Down Payment", "save the cash"),
        ("Closing Costs", "no surprises"), ("Home Comparison", "score every house"),
        ("Lender Compare", "shop the rate"), ("Amortization", "the balance path"),
        ("Life After Buying", "the real budget"), ("Credit Prep", "raise your score"),
        ("House Hunting", "every showing"), ("Moving Checklist", "offer to keys"),
        ("Monthly Summary", "savings climbing"), ("Settings", "set it once"),
        ("+ 12 Printable Pages", "worksheets & lists"), ("Live Buyer Score", "it all rolls up"),
    ]
    cols = 4
    margin = 88
    gx, gy = 22, 20
...
def render_afford(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "WHAT YOU CAN TRULY AFFORD", font=fs(28), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "The Affordability Engine", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Principal, interest, taxes, insurance and PMI — the all-in payment, checked against your DTI",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 1, content_afford)
    img.convert("RGB").save(out, "PNG", optimize=True)


def render_homes(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "SCORE EVERY HOUSE", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "The Home Comparison", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Five houses blur together after a weekend of showings — this keeps them straight",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 4, content_homes)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_engine(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 110, "AFFORD IT · FIND IT", font=fs(32), pad_x=52, pad_y=22)
    tc(d, (SIZE // 2, 224), "The Home Buyer Engine", fserif(44), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 1, content_afford)
    app_window(img, (60, 1180, SIZE - 60, SIZE - 60), 4, content_homes)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_printables(out):
    ...
    d = ImageDraw.Draw(img)
    key_crest(img, SIZE // 2, 110, r=48)
    pill(img, SIZE // 2, 214, "PLUS 12 PRINTABLE PAGES", font=fs(30), pad_x=48, pad_y=20)
    tc(d, (SIZE // 2, 300), "Print, Fill & Take to Showings", fserif(42), WHITE)
    gold_divider(img, SIZE // 2, 362, width=480)
    tc(d, (SIZE // 2, 404), "A matching print-ready pack — affordability worksheet, home comparison, closing checklist & more",
       fs(21, bold=False), (226, 214, 190))
    labels = ["Affordability Worksheet", "Home Comparison", "Closing Cost List", "Down Payment Tracker",
              "Lender Comparison", "House Hunting Log", "Must-Have List", "Credit Prep",
              "Life After Buying", "Moving Checklist", "Amortization", "Closing Day List"]
...
def main():
    ...
    targets = [
        ...
        ("03_affordability.png", render_afford),
        ("04_homes.png", render_homes),
