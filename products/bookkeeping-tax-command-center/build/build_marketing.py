"""Marketing image set for Small Business Bookkeeping & Tax Command Center™ (6 images, 2000x2000)."""
...
TABS = ["Dashboard", "Schedule C P&L", "Income", "COGS & Inventory", "Expenses", "Mileage", "Sales Tax",
        "Quarterly Taxes", "Schedule C Lines", "Invoices", "Reconciliation", "Monthly Summary", "Settings"]

FILE_LABEL = "Bookkeeping_Tax_Command_Center.xlsx — Quill & Ledger · Morgan"
...
def ledger_crest(c, cx, cy, r=56, glow=True):
    """Brand crest — a ledger book: the icon of clean, tax-ready books."""
    if glow:
    ...
    ov = Image.new("RGBA", c.size, (0, 0, 0, 0)); d = ImageDraw.Draw(ov)
    # ledger cover
    d.rounded_rectangle((cx - r * 0.40, cy - r * 0.40, cx + r * 0.40, cy + r * 0.42), radius=6, fill=GOLD_HI, outline=(150, 120, 70))
    # spine
    d.rectangle((cx - r * 0.40, cy - r * 0.40, cx - r * 0.28, cy + r * 0.42), fill=(150, 120, 70))
    # ruled lines
    for k in range(4):
        yy = cy - r * 0.24 + k * r * 0.15
        d.line((cx - r * 0.18, yy, cx + r * 0.28, yy), fill=PRIMARY, width=3)
    # a green check (books are clean)
    d.line((cx + r * 0.02, cy + r * 0.24, cx + r * 0.12, cy + r * 0.34), fill=GREEN, width=5)
    d.line((cx + r * 0.12, cy + r * 0.34, cx + r * 0.32, cy + r * 0.06), fill=GREEN, width=5)
    c.alpha_composite(ov)


# aliases (shared render helpers may reference these names across products)
magnify_crest = ledger_crest
hardhat_crest = ledger_crest
basket_crest = ledger_crest
compass_crest = ledger_crest
candle_crest = ledger_crest
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "THE BOOKS", font=fs(15), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("GROSS REVENUE", "$96,000", "the year"),
    ("COGS", "$28,800", "cost of sales"),
    ("GROSS PROFIT", "$67,200", "70% margin"),
    ("EXPENSES", "$19,200", "deductible"),
    ("NET PROFIT", "$48,000", "Line 31"),
    ("NET MARGIN", "50%", "of revenue"),
    ("SE TAX", "$6,782", "15.3%"),
    ("INCOME TAX", "$5,353", "estimated"),
    ("TOTAL TAX", "$12,135", "25.3% eff."),
    ("QUARTERLY", "$3,034", "send each Q"),
    ("MILEAGE DEDUCTION", "$2,800", "4,000 mi"),
    ("BOOKS SCORE", "90%", "healthy"),
]

PROFIT_MONTHS = [("Feb", 6000), ("Mar", 7000), ("Apr", 8000), ("May", 8500), ("Jun", 9000), ("Jul", 9500)]


def content_dashboard(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Books Dashboard", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 64), "Quill & Ledger · Morgan  ·  know your profit, never be surprised by a tax bill", font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 210, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 105, y0 + 44), "● $3,034 per quarter", font=fs(14), fill=PRIMARY, anchor="mm")
    ...
    d.text((gx, cy_top), "BOOKS HEALTH · WHERE THE REVENUE GOES · NET PROFIT BY MONTH", font=fs(19), fill=ACCENT, anchor="lt")
    ...
    d.text((px + 16, panels_y + 14), "Books Health", font=fs(17), fill=ACCENT, anchor="lt")
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("Profitable", 1.0, "100%"), ("Healthy margin", 1.0, "100%"), ("Tax set aside", 1.0, "100%"),
           ("Expenses categorized", 1.0, "100%"), ("Books reconciled", 1.0, "100%"), ("Receipts attached", 0.40, "40%")],
          color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Where the Revenue Goes", font=fs(15), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.44, panels_y + panel_h * 0.50, min(panel_h * 0.27, pw * 0.27),
          [(50, PRIMARY), (30, ACCENT), (20, SURFACE)], "50%", "your profit")
    legend(d, px + pw * 0.06, panels_y + panel_h - 118, [(PRIMARY, "Net profit 50%"), (ACCENT, "COGS 30%"), (SURFACE, "Expenses 20%")], 15, 30)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Net Profit by Month", font=fs(17), fill=ACCENT, anchor="lt")
    vbars(img, d, (px + 18, panels_y + 60, px + pw - 14, panels_y + panel_h - 10), PROFIT_MONTHS, 9500)
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Books Score", font=fs(17), fill=ACCENT, anchor="lt")
...
def content_expenses(img, cbox):
    rows = [
        ("Advertising", "Line 8", "$4,200", "4.4%"),
        ("Mileage & travel", "Line 9", "$2,800", "2.9%"),
        ("Rent / studio", "Line 20b", "$3,600", "3.8%"),
        ("Supplies", "Line 22", "$2,400", "2.5%"),
        ("Software", "Line 18", "$1,800", "1.9%"),
        ("Insurance", "Line 15", "$1,500", "1.6%"),
        ("Utilities & phone", "Line 25", "$1,200", "1.3%"),
    ]
    _table(img, cbox, "Expenses → Schedule C",
           "Every deductible cost, mapped to its real Schedule C line — hand this to a CPA and they'll smile.",
           ["CATEGORY", "SCHEDULE C", "AMOUNT", "% OF REV"],
           [0.0, 0.44, 0.66, 0.86], rows,
           total_row=("10 categories", "Line 28", "$19,200", "20.0%"))


def content_schedc(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Schedule C — Your Real Profit", font=fs(30), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 60), "Revenue minus what it cost to make and run — the number the IRS actually taxes.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("Gross revenue (Line 1)", "$96,000", PRIMARY),
            ("− Cost of goods sold (Line 4)", "−$28,800", DANGER),
            ("− Total expenses (Line 28)", "−$19,200", DANGER)]
    callout_h = 150; gap = 44; rh = 108
    ...
    d.text((x0 + pad + 14, ry + rh / 2), "= NET PROFIT (Line 31)", font=fs(18), fill=PRIMARY, anchor="lm")
    d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.80, ry + rh / 2), "$48,000", font=fs(18), fill=PRIMARY, anchor="mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2), "50% net margin  ·  the number the IRS taxes", font=fs(23), fill=WHITE, anchor="mm")
    img.alpha_composite(ov)


def content_tax(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "The Quarterly Tax Engine", font=fs(30), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 60), "Your net profit becomes self-employment tax plus income tax — divided into four payments.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("Net profit", "$48,000", PRIMARY),
            ("Self-employment tax (15.3%)", "$6,782", DANGER),
            ("Income tax (your rate)", "$5,353", DANGER)]
    callout_h = 150; gap = 44; rh = 108
    n_units = 1 + len(rows) + 1
    top_region = y0 + 108
    block_h = rh * n_units + gap + callout_h
    ...
    ry = ty + rh + len(rows) * rh
    d.rectangle((x0 + pad, ry, x1 - pad, ry + rh), fill=SURFACE)
    d.text((x0 + pad + 14, ry + rh / 2), "= TOTAL TAX OWED", font=fs(18), fill=PRIMARY, anchor="lm")
    d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.80, ry + rh / 2), "$12,135", font=fs(18), fill=PRIMARY, anchor="mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2), "÷ 4  =  send $3,034 each quarter  ·  April is a non-event", font=fs(21), fill=WHITE, anchor="mm")
    img.alpha_composite(ov)
...
def render_hero(out):
    ...
    ledger_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE COMPLETE BOOKKEEPING & TAX SYSTEM", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "BOOKKEEPING & TAX", 74, max_w=1650)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 546), "A Schedule C engine, quarterly tax calculator, mileage, sales tax & invoices — one system.",
       fs(20, bold=False), (224, 213, 190))
    chips = [("14", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("SCHED C", "ENGINE")]
    cw = 440
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not a spreadsheet — a complete track-it, total-it, file-it system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Start Here", "how it all works"), ("Dashboard", "profit & score"),
        ("Schedule C P&L", "your real profit"), ("Income", "every dollar in"),
        ("COGS & Inventory", "cost of what sold"), ("Expenses", "every deduction"),
        ("Mileage", "the missed deduction"), ("Sales Tax", "collected vs remitted"),
        ("Quarterly Taxes", "the 4 due dates"), ("Schedule C Lines", "CPA-ready map"),
        ("Invoices", "who owes you"), ("Reconciliation", "audit-proof"),
        ("Monthly Summary", "is it working?"), ("Settings", "set it once"),
        ("+ 12 Printable Pages", "logs & worksheets"), ("Live Books Score", "it all rolls up"),
    ]
    cols = 4
    margin = 88
    gx, gy = 22, 20
...
def render_schedc(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "KNOW YOUR REAL PROFIT", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "The Schedule C Engine", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Revenue minus cost of goods minus expenses — the exact number the IRS taxes, live",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 1, content_schedc)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_tax(out):
    ...
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "NEVER BE SURPRISED", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "The Quarterly Tax Engine", fserif(42), WHITE)
    tc(d, (SIZE // 2, 300), "Self-employment tax plus income tax, divided into four payments you can actually plan for",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 7, content_tax)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_engine(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 110, "PROFIT · THEN TAX", font=fs(32), pad_x=52, pad_y=22)
    tc(d, (SIZE // 2, 224), "The Books Engine", fserif(44), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 1, content_schedc)
    app_window(img, (60, 1180, SIZE - 60, SIZE - 60), 4, content_expenses)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_printables(out):
    ...
    d = ImageDraw.Draw(img)
    ledger_crest(img, SIZE // 2, 110, r=48)
    pill(img, SIZE // 2, 214, "PLUS 12 PRINTABLE PAGES", font=fs(30), pad_x=48, pad_y=20)
    tc(d, (SIZE // 2, 300), "Print, Fill & Hand to Your CPA", fserif(42), WHITE)
    gold_divider(img, SIZE // 2, 362, width=480)
    tc(d, (SIZE // 2, 404), "A matching print-ready pack — P&L worksheet, tax worksheet, mileage log, receipt tracker & more",
       fs(21, bold=False), (226, 214, 190))
    labels = ["Schedule C P&L", "Tax Worksheet", "Income Log", "Expense Log",
              "COGS & Inventory", "Mileage Log", "Sales Tax Log", "Invoice Tracker",
              "Receipt Tracker", "Reconciliation", "Monthly Summary", "Tax-Time Checklist"]
...
def main():
    ...
    targets = [
        ...
        ("03_schedulec.png", render_schedc),
        ("04_tax.png", render_tax),
