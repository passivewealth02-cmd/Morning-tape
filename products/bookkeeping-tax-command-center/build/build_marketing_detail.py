"""Detailed marketing image set for Small Business Bookkeeping & Tax Command Center™ (07-10)."""
...
from build_marketing import (
    ...
    ROW_ALT, fs, fserif, premium_bg, pill, wordmark, gold_divider, ledger_crest,
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR YOUR BOOKS", "Track It, Total It, File It",
                "Four ways it turns messy receipts into a tax-ready year")
    feats = [
        ("$", "Live Books Score", "donut", (90, "90%", "healthy"),
         "Profitable, healthy margin, tax set aside, expenses categorized, books reconciled and receipts attached roll into one number — your books' health at a glance."),
        ("%", "Schedule C Engine", "table", None,
         "Revenue minus cost of goods minus expenses gives your real net profit — then the self-employment and income tax it creates, and the exact quarterly payment."),
        ("box", "Run the Whole Back Office", "stock", None,
         "Income, expenses, COGS, mileage, sales tax, invoices and reconciliation — every number your accountant will ask for, in one place."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack — P&L worksheet, tax worksheet, mileage log, receipt tracker and a tax-time checklist."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("Revenue", "$96,000"), ("Net profit", "$48,000"), ("Quarterly tax", "$3,034")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["SCHEDULE C ENGINE", "VALUE"], [0.0, 0.64]):
            ...
            for ri, (a, b) in enumerate(rows):
                ...
                od.text((tx0 + 14 + (tx1 - tx0) * 0.64, ry + 21), b, font=fs(16, bold=False), fill=TEXT, anchor="lm")
        elif kind == "stock":
            od = ImageDraw.Draw(img)
            items = [("Net margin", "50%", MINT_BG, PRIMARY), ("Mileage deduction", "$2,800", MINT_BG, PRIMARY),
                     ("Total tax owed", "$12,135", MINT_BG, PRIMARY), ("Receipts attached", "40%", SURFACE, (110, 88, 58))]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            for ri, (nm, st, bg, fg) in enumerate(items):
                ...
                od.rounded_rectangle((tx1 - 130, ry + 6, tx1 - 12, ry + 32), radius=13, fill=bg)
                od.text((tx1 - 71, ry + 19), st, font=fs(15), fill=fg, anchor="mm")
        elif kind == "print":
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than an Expense Sheet",
                "Everything a basic tracker leaves out — built in")
    rows = [
        ("An income & expense list", True, True),
        ("A full Schedule C P&L", False, True),
        ("Cost of goods sold", False, True),
        ("Self-employment tax calc", False, True),
        ("Quarterly payment amount", False, True),
        ("IRS mileage deduction", False, True),
        ("Sales tax collected vs remitted", False, True),
        ("Schedule C line mapping", False, True),
        ("Invoices & reconciliation", False, True),
        ("Live Books Score dashboard", False, True),
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "SET UP IN AN AFTERNOON", "Do Your Books in 4 Steps",
                "No accounting degree needed — if you can type, you can run it")
    steps = [
        ("1", "Log income & expenses", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack, then enter your income and your costs by category."),
        ("2", "Read your real profit", "The Schedule C engine subtracts cost of goods and expenses to show the exact net profit the IRS will tax — no guessing."),
        ("3", "Know the tax", "See your self-employment tax, income tax and the precise amount to send each quarter, so April stops being a scary month."),
        ("4", "Stay audit-ready", "Track mileage, sales tax and invoices, reconcile monthly, and let the Books Score tell you what's still missing."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "EVERYTHING YOU GET", "One System, Whole Back Office",
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + Books Score",
        "Schedule C P&L engine", "Quarterly tax calculator",
        "Income, expenses & COGS", "Mileage & sales-tax logs",
        "Invoices & reconciliation", "Start-Here quick-start guide",
    ]
    ...
    who = ["Self-employed & sole proprietors", "Product & handmade sellers", "Service businesses & contractors",
           "Side hustles filing Schedule C", "Anyone dreading tax season", "Anyone quitting QuickBooks"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "TRACK IT · TOTAL IT · FILE IT", font=fs(27), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Know your profit, and never be surprised by a tax bill again.",
       fs(23, bold=False), TEXT_MUTED)
    ledger_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "BOOKKEEPING & TAX", 52, max_w=1200)
    img.convert("RGB").save(out, "PNG", optimize=True)
