"""Printable PDF pack for Meal Prep Business Command Center™ (12 pages, US Letter)."""
...
def page(title, subtitle):
    ...
    d.text((M, 90), "MEAL PREP BUSINESS COMMAND CENTER™", font=fs(32), fill=GOLD_HI, anchor="lt")
    ...
    d.text((W - M, H - 108), "Meal Prep Business Command Center™", font=fs(22, bold=False), fill=ACCENT, anchor="rt")
    return img, d
...
def p01(i):
    img, d = page("Meal Cost Card", "Cost a meal to the component")
    field(d, M, TOP, CW, "Meal")
    table(d, M, TOP + 130, CW, ["COMPONENT", "COST"], [0.0, 0.78], 11, 150)
    y = TOP + 130 + 78 + 11 * 150 + 20
    field(d, M, y, CW / 2 - 30, "Cost per meal"); field(d, M + CW / 2 + 30, y, CW / 2 - 30, "Sell price")
    i.append(img)


def p02(i):
    img, d = page("Meal Plan Pricing", "Meals × cost = plan cost & margin")
    table(d, M, TOP, CW, ["PLAN", "MEALS", "PRICE", "COST", "MARGIN"], [0.0, 0.34, 0.52, 0.70, 0.86], 12, 175)
    i.append(img)


def p03(i):
    img, d = page("Menu", "Every meal, cost & food %")
    table(d, M, TOP, CW, ["MEAL", "PROTEIN", "COST", "PRICE", "FOOD %"], [0.0, 0.40, 0.58, 0.72, 0.86], 18, 120)
    i.append(img)


def p04(i):
    img, d = page("Subscriber List", "Who's on which plan")
    table(d, M, TOP, CW, ["CLIENT", "PLAN", "MEALS/WK", "PRICE/WK", "STATUS"], [0.0, 0.28, 0.52, 0.68, 0.86], 18, 120)
    i.append(img)


def p05(i):
    img, d = page("Prep List", "The weekly batch cook")
    table(d, M, TOP, CW, ["MEAL", "QTY", "PREP TIME", "DONE"], [0.0, 0.46, 0.66, 0.88], 18, 120)
    i.append(img)
...
def p07(i):
    img, d = page("Packaging & Delivery", "The cost to the door")
    table(d, M, TOP, CW, ["ITEM", "COST", "NOTES"], [0.0, 0.32, 0.54], 16, 140)
    i.append(img)


def p08(i):
    img, d = page("Delivery Run Sheet", "Every drop, every day")
    table(d, M, TOP, CW, ["CLIENT", "ADDRESS", "MEALS", "DAY", "DONE"], [0.0, 0.26, 0.62, 0.78, 0.90], 18, 120)
    i.append(img)


def p09(i):
    img, d = page("Waste Log", "The leaks that eat your margin")
...
def p11(i):
    img, d = page("Monthly Summary", "MRR, expenses & profit")
    table(d, M, TOP, CW, ["MONTH", "MRR", "EXPENSES", "PROFIT"], [0.0, 0.30, 0.54, 0.78], 14, 155)
    i.append(img)


def p12(i):
    img, d = page("Prep Day Checklist", "Same standard, every week")
    half = CW / 2 - 30
    for k, st in enumerate(["Cook & portion", "Pack & deliver"]):
        x = M + k * (half + 60)
...
def main():
    ...
    pdf_path = os.path.join(out_dir, "Meal_Prep_Printables.pdf")
