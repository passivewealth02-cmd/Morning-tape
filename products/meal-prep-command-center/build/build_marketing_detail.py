"""Detailed marketing image set for Meal Prep Business Command Center™ (07-10)."""
...
from build_marketing import (
    ...
    ROW_ALT, fs, fserif, premium_bg, pill, wordmark, gold_divider, bento_crest,
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR MEAL-PREP BUSINESSES", "Cost It, Price It, Grow It",
                "Four ways it turns prepped meals into a business that scales")
    feats = [
        ("$", "Live Prep Score", "donut", (90, "90%", "healthy"),
         "Food cost, margin, plan pricing, subscribers, profit & waste roll into one number — so you know your meal-prep health at a glance."),
        ("%", "Per-Meal Cost Engine", "table", None,
         "Cost a meal to the component — protein, carb, veg, sauce, packaging and labor — so every plan you price is built on a real number."),
        ("box", "Subscribers → MRR", "stock", None,
         "Count subscribers by plan and watch weekly revenue and monthly recurring revenue roll up — the number that decides whether you scale."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack — meal cost card, prep list, delivery run sheet, subscriber list, waste log & more for the kitchen."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("Food ingredients", "$3.20"), ("+ Packaging + labor", "$1.80"), ("Cost per meal", "$5.00")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["GRILLED CHICKEN BOWL", "VALUE"], [0.0, 0.66]):
            ...
            od = ImageDraw.Draw(img)
            items = [("Monthly recurring rev", "$17,900", MINT_BG, PRIMARY), ("Monthly profit", "$6,740", MINT_BG, PRIMARY),
                     ("Subscribers", "46", MINT_BG, PRIMARY), ("Weekly revenue", "$4,475", SURFACE, (110, 88, 58))]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Meal Menu",
                "Everything a basic meal list leaves out — built in")
    rows = [
        ("A meal price list", True, True),
        ("A per-meal cost engine", False, True),
        ("Meal-plan pricing 5/10/15/21", False, True),
        ("Subscribers → weekly rev & MRR", False, True),
        ("Production / batch-cook plan", False, True),
        ("Packaging & delivery costing", False, True),
        ("Weekly orders & run sheet", False, True),
        ("Ingredient cost library", False, True),
        ("Income & expenses for taxes", False, True),
        ("Live Prep Score dashboard", False, True),
    ...
    d.text((c1, top + hh // 2), "A Basic\nMeal List", font=fs(18), fill=(210, 210, 205), anchor="mm")
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "SET UP IN AN AFTERNOON", "Launch Your Prep in 4 Steps",
                "No spreadsheet skills needed — if you can type, you can run it")
    steps = [
        ("1", "Cost your meals", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack, then cost each meal to the component."),
        ("2", "Price your plans", "Set your 5, 10, 15 and 21-meal weekly plans — the cost and the margin on every plan calculate live as you type."),
        ("3", "Add your subscribers", "Enter how many subscribers are on each plan and watch weekly revenue and monthly recurring revenue roll up automatically."),
        ("4", "Run & grow", "Plan production, manage packaging and delivery, keep your books for tax time, and watch your profit and Prep Score climb."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + Prep Score",
        "Per-meal cost engine", "Meal-plan pricing & menu",
        "Subscribers → MRR board", "Production & delivery plan",
        "Ingredient, packaging & waste logs", "Start-Here quick-start guide",
    ]
    ...
    who = ["Meal-prep & meal-delivery brands", "Subscription meal makers", "Personal chefs & food entrepreneurs",
           "Fitness / macro meal sellers", "Ghost & commissary kitchens", "Anyone selling prepped meals"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "COST IT · PRICE IT · GROW THE MRR", font=fs(27), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Cost every meal, price your plans, and grow your recurring revenue.",
       fs(23, bold=False), TEXT_MUTED)
    bento_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "MEAL PREP", 64, max_w=1100)
    img.convert("RGB").save(out, "PNG", optimize=True)
