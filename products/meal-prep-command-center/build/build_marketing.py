"""Marketing image set for Meal Prep Business Command Center™ (6 images, 2000x2000)."""
...
TABS = ["Dashboard", "Meal Cost", "Meal Plans", "Menu", "Subscribers", "Production Plan",
        "Ingredient Costs", "Packaging & Delivery", "Weekly Orders", "Waste Log",
        "Income & Expenses", "Monthly Summary", "Settings"]

FILE_LABEL = "Meal_Prep_Command_Center.xlsx — Fresh Fuel Co. · Kai"
...
def bento_crest(c, cx, cy, r=56, glow=True):
    """Brand crest — a bento / meal-prep container: the icon of the meal-prep business."""
    if glow:
    ...
    c.alpha_composite(ov)
    ov = Image.new("RGBA", c.size, (0, 0, 0, 0)); d = ImageDraw.Draw(ov)
    # steam wisps above the container
    for sx in (-0.20, 0.0, 0.20):
        x = cx + r * sx
        d.line((x, cy - r * 0.60, x, cy - r * 0.46), fill=(255, 255, 255, 150), width=3)
    # the container
    cont = (cx - r * 0.56, cy - r * 0.40, cx + r * 0.56, cy + r * 0.50)
    d.rounded_rectangle(cont, radius=9, fill=(247, 242, 232), outline=(150, 120, 70), width=3)
    p = r * 0.09
    midx = cx + r * 0.02
    # left compartment — protein
    d.rounded_rectangle((cont[0] + p, cont[1] + p, midx - p * 0.5, cont[3] - p), radius=6, fill=GOLD_HI)
    # right top — veg
    d.rounded_rectangle((midx + p * 0.5, cont[1] + p, cont[2] - p, cy + r * 0.02), radius=6, fill=HIGHLIGHT)
    # right bottom — carb (rice)
    d.rounded_rectangle((midx + p * 0.5, cy + r * 0.10, cont[2] - p, cont[3] - p), radius=6, fill=WHITE)
    c.alpha_composite(ov)


# aliases (shared render helpers may reference these names across products)
cupcake_crest = bento_crest
clock_crest = bento_crest
clipboard_crest = bento_crest
cloche_crest = bento_crest
ghost_crest = bento_crest
mug_crest = bento_crest
book_crest = bento_crest
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "MEAL PREP", font=fs(15), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("MEAL COST", "$5.00", "per meal"),
    ("AVG PRICE", "$10.33", "per meal"),
    ("FOOD COST", "31%", "of price"),
    ("MARGIN/MEAL", "52%", "per meal"),
    ("SUBSCRIBERS", "46", "active"),
    ("TOP PLAN", "10 meals", "$1,575/wk"),
    ("WEEKLY REV", "$4,475", "this week"),
    ("MRR", "$17,900", "recurring"),
    ("MEALS / WEEK", "433", "prepped"),
    ("MONTHLY PROFIT", "$6,740", "after costs"),
    ("WASTE", "2.4%", "of revenue"),
    ("PREP SCORE", "90%", "healthy"),
]

MRR_MONTHS = [("Jul", 13200), ("Aug", 14800), ("Sep", 17900), ("Oct", 20400), ("Nov", 23600), ("Dec", 26000)]


def content_dashboard(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Meal Prep Dashboard", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 64), "Fresh Fuel Co. · Kai  ·  cost every meal, and grow your recurring revenue", font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 210, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 105, y0 + 44), "● $17,900 MRR", font=fs(15), fill=PRIMARY, anchor="mm")
    ...
    d.text((gx, cy_top), "PREP HEALTH · MARGIN PER MEAL · MRR BY MONTH", font=fs(20), fill=ACCENT, anchor="lt")
    ...
    d.text((px + 16, panels_y + 14), "Prep Health", font=fs(17), fill=ACCENT, anchor="lt")
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("Food cost on target", 1.0, "100%"), ("Margin healthy", 1.0, "100%"), ("Plans costed", 1.0, "100%"),
           ("Subscribers vs goal", 1.0, "100%"), ("Profitable", 1.0, "100%"), ("Waste low", 0.40, "40%")],
          color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Margin / Meal", font=fs(17), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.44, panels_y + panel_h * 0.50, min(panel_h * 0.27, pw * 0.27),
          [(52, PRIMARY), (48, ACCENT)], "52%", "margin")
    legend(d, px + pw * 0.06, panels_y + panel_h - 96, [(PRIMARY, "Margin 52%"), (ACCENT, "Meal cost 48%")], 15, 32)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "MRR by Month", font=fs(17), fill=ACCENT, anchor="lt")
    vbars(img, d, (px + 18, panels_y + 60, px + pw - 14, panels_y + panel_h - 10), MRR_MONTHS, 26000)
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Prep Score", font=fs(17), fill=ACCENT, anchor="lt")
...
def content_plans(img, cbox):
    rows = [
        ("5 meals / week", "5", "$55", "$25", "$30", "55%"),
        ("10 meals / week", "10", "$105", "$50", "$55", "52%"),
        ("15 meals / week", "15", "$150", "$75", "$75", "50%"),
        ("21 meals / week", "21", "$200", "$105", "$95", "48%"),
    ]
    _table(img, cbox, "Meal Plan Pricing",
           "Meals × cost = plan cost, and the margin on every weekly plan calculates live.",
           ["PLAN", "MEALS", "PRICE/WK", "COST/WK", "MARGIN", "MARGIN %"],
           [0.0, 0.40, 0.55, 0.70, 0.83, 0.94], rows)


def content_mealcost(img, cbox):
    rows = [
        ("Protein (chicken 6 oz)", "$1.80"),
        ("Carb (rice / potato)", "$0.45"),
        ("Roasted veg", "$0.60"),
        ("Sauce & seasoning", "$0.35"),
        ("Container + label", "$0.55"),
        ("Kitchen labor / meal", "$1.25"),
    ]
    _table(img, cbox, "Meal Cost — Grilled Chicken Bowl",
           "Cost a meal to the component — protein, carb, veg, sauce, packaging & labor.",
           ["COMPONENT", "COST"],
           [0.0, 0.74], rows,
           total_row=("= COST PER MEAL  ·  sells for $11", "$5.00"))


def content_mrr(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Subscribers → MRR", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 62), "Count the subscribers on each plan — weekly revenue and recurring revenue roll up.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("5 meals/wk  ×  20 subs", "$1,100", PRIMARY), ("10 meals/wk  ×  15 subs", "$1,575", PRIMARY),
            ("15 meals/wk  ×  8 subs", "$1,200", PRIMARY), ("21 meals/wk  ×  3 subs", "$600", PRIMARY)]
    ...
    for h, fx in zip(["PLAN  ·  SUBSCRIBERS", "WEEKLY REV"], [0.0, 0.80]):
    ...
    d.text((x0 + pad + 14, ry + rh / 2), "= WEEKLY REVENUE  ·  46 subscribers", font=fs(18), fill=PRIMARY, anchor="lm")
    d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.80, ry + rh / 2), "$4,475", font=fs(18), fill=PRIMARY, anchor="mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2), "× 4 weeks  =  $17,900 monthly recurring revenue", font=fs(23), fill=WHITE, anchor="mm")
    img.alpha_composite(ov)
...
def render_hero(out):
    ...
    bento_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE COMPLETE MEAL-PREP SYSTEM", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "MEAL PREP", 92, max_w=1500)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 546), "Cost per meal, meal-plan pricing, subscribers, MRR, production & delivery — one system.",
       fs(20, bold=False), (224, 213, 190))
    chips = [("14", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("MRR", "ENGINE")]
    cw = 440
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not a spreadsheet — a complete cost-it, price-it, grow-the-MRR system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Start Here", "how it all works"), ("Dashboard", "MRR, profit & score"),
        ("Meal Cost", "cost to the component"), ("Meal Plans", "price 5/10/15/21"),
        ("Menu", "margin on every meal"), ("Subscribers", "counts become MRR"),
        ("Production Plan", "the weekly batch cook"), ("Ingredient Costs", "your buying list"),
        ("Packaging & Delivery", "cost to the door"), ("Weekly Orders", "who gets what"),
        ("Waste Log", "the quiet leaks"), ("Income & Expenses", "your monthly books"),
        ("Monthly Summary", "watch MRR compound"), ("Settings", "set it once"),
        ("+ 12 Printable Pages", "prep & run sheets"), ("Live Prep Score", "it all rolls up"),
    ]
    cols = 4
    margin = 88
    gx, gy = 22, 20
...
def render_mrr(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "RECURRING REVENUE", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "Subscribers Become MRR", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "The engine of a meal-prep business — count the subscribers, watch the recurring revenue roll up",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 4, content_mrr)
    img.convert("RGB").save(out, "PNG", optimize=True)


def render_mealcost(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "COST EVERY MEAL", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "The Per-Meal Cost Engine", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Protein, carb, veg, sauce, packaging & labor — the true cost of a single meal, to the cent",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 1, content_mealcost)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_engine(out):
    ...
    tc(d, (SIZE // 2, 224), "The Meal-Prep Pricing Engine", fserif(44), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 1, content_mealcost)
    app_window(img, (60, 1180, SIZE - 60, SIZE - 60), 2, content_plans)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_printables(out):
    ...
    d = ImageDraw.Draw(img)
    bento_crest(img, SIZE // 2, 110, r=48)
    ...
    tc(d, (SIZE // 2, 404), "A matching print-ready pack — meal cost card, prep list, delivery run sheet & more",
       fs(21, bold=False), (226, 214, 190))
    labels = ["Meal Cost Card", "Meal Plan Pricing", "Menu", "Subscriber List",
              "Prep List", "Ingredient List", "Packaging & Delivery", "Delivery Run Sheet",
              "Waste Log", "Income & Expenses", "Monthly Summary", "Prep Day Checklist"]
...
def main():
    ...
    targets = [
        ...
        ("03_mrr.png", render_mrr),
        ("04_mealcost.png", render_mealcost),
