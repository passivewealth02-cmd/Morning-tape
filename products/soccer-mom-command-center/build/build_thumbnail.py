"""Premium feature-forward hero thumbnail for Soccer Mom Command Center.
...
def ball_crest(c, cx, cy, r=58):
    """Soccer-ball crest: green roundel with a simple pentagon motif."""
    ov = Image.new("RGBA", c.size, (0, 0, 0, 0))
    d = ImageDraw.Draw(ov)
    ...
    for i in range(5):
        ...
        pts.append((cx + pr * math.cos(a), cy + pr * math.sin(a)))
    d.polygon(pts, fill=GOLD_LT)
    # little stitches radiating
    for i in range(5):
        a = math.radians(-90 + i * 72 + 36)
        x1 = cx + pr * 1.05 * math.cos(a)
        y1 = cy + pr * 1.05 * math.sin(a)
        x2 = cx + (r - 14) * math.cos(a)
        y2 = cy + (r - 14) * math.sin(a)
        d.line((x1, y1, x2, y2), fill=GOLD_LT, width=3)
    c.alpha_composite(ov)
...
def dashboard_screen(c, screen):
    ...
    d.text((sx0 + 28, sy0 + head // 2), "SOCCER MOM COMMAND CENTER", font=fs(26), fill=WHITE, anchor="lm")
    d.text((sx1 - 28, sy0 + head // 2), "Fall Season · U10 Travel", font=fs(22), fill=GOLD_LT, anchor="rm")
    ...
    kpis = [
        ...
        ("REMAINING", "$214", PRIMARY),
        ("TOURNAMENTS", "2", ACCENT),
        ("EQUIP NEEDED", "1", ACCENT),
        ("CONFLICTS", "0", PRIMARY),
    ...
    nav_y = gy + 2 * (kh + gap) + 8
    nav_h = 34
    nav = ["Schedule", "Budget", "Roster", "Carpool", "Tournaments", "Meals"]
    ...
    d.text((sx0 + sw / 2, note_y),
           "Games · Practices · Carpool · Snacks · Tournaments · Mileage — all in one file",
...
def render(out):
    ...
    ball_crest(img, SIZE // 2, 150, r=58)
    pill(img, SIZE // 2, 285, "THE ULTIMATE SPORTS FAMILY PLANNER", font=fs(36), pad_x=56, pad_y=22)

    tc(d, (SIZE // 2, 430), "SOCCER MOM", fserif(150), PRIMARY)
    tc(d, (SIZE // 2, 552), "COMMAND CENTER", fserif(104), PRIMARY)
    gold_divider(img, SIZE // 2, 632, width=560)
    tc(d, (SIZE // 2, 680), "Games, practices & team life — all in ONE organized file",
       fs(26, bold=False), TEXT_MUTED)

    laptop(img, SIZE // 2, 1015, w=1360, h=560)
    benefit_badge(img, 300, 738, "NEXT GAME", "IN 2 DAYS", big_color=GOLD_LT)
    benefit_badge(img, SIZE - 300, 738, "2-in-1", "EXCEL + GOOGLE SHEETS")

    tc(d, (SIZE // 2, 1360), "EVERYTHING A TEAM FAMILY NEEDS", fs(34), ACCENT)
    features = [
        ("Season Schedule", "games + countdown"),
        ("Practice Tracker", "attendance + drills"),
        ("Team Budget", "cost per player"),
        ("Carpool Manager", "driver rotation"),
        ("Snack Roster", "never miss a turn"),
        ("Tournament Planner", "hotels + packing"),
        ("Mileage & Travel", "fuel + tolls"),
        ("Conflict Alerts", "game vs practice"),
    ]
    cols = 4
    margin = 120
    gx, gy = 24, 24
    ...
    pill(img, SIZE // 2, SIZE - 120,
         "14 SHEETS · INSTANT DOWNLOAD · EXCEL + GOOGLE SHEETS",
