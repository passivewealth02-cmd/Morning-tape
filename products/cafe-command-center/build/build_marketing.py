"""Marketing image set for Cafe & Coffee Shop Command Center™ (6 images, 2000x2000)."""
...
TABS = ["Dashboard", "Cup Cost", "Menu Board", "Daypart Sales", "Weekly Sales", "Labor & Prime",
        "Bean & Milk", "Inventory & Par", "Waste Log", "Ordering", "Cash & Tips", "Regulars", "Settings"]

FILE_LABEL = "Cafe_Command_Center.xlsx — Wildroot Coffee · Priya"
...
def cup_crest(c, cx, cy, r=56, glow=True):
    """Brand crest — a to-go coffee cup with a lid & steam."""
    if glow:
    ...
    ov = Image.new("RGBA", c.size, (0, 0, 0, 0)); d = ImageDraw.Draw(ov)
    # steam
    for sx in (cx - r * 0.16, cx + r * 0.16):
        d.arc((sx - r * 0.10, cy - r * 0.52, sx + r * 0.10, cy - r * 0.30), start=120, end=300, fill=GOLD_HI, width=3)
        d.arc((sx - r * 0.10, cy - r * 0.36, sx + r * 0.10, cy - r * 0.14), start=300, end=120, fill=GOLD_HI, width=3)
    # lid
    d.rounded_rectangle((cx - r * 0.40, cy - r * 0.12, cx + r * 0.40, cy - r * 0.02), radius=4, fill=GOLD_HI, outline=(150, 120, 70))
    d.rectangle((cx - r * 0.10, cy - r * 0.20, cx + r * 0.10, cy - r * 0.12), fill=GOLD_HI)
    # cup body (trapezoid)
    d.polygon([(cx - r * 0.34, cy), (cx + r * 0.34, cy), (cx + r * 0.26, cy + r * 0.54), (cx - r * 0.26, cy + r * 0.54)],
              fill=GOLD_HI, outline=(150, 120, 70))
    # sleeve
    d.rectangle((cx - r * 0.31, cy + r * 0.16, cx + r * 0.31, cy + r * 0.34), fill=(150, 120, 70))
    c.alpha_composite(ov)


book_crest = cup_crest
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "CAFE", font=fs(16), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("MENU ITEMS", "10", "on the board"),
    ("AVG CUP COST", "$1.13", "per drink"),
    ("AVG PRICE", "$5.05", "per drink"),
    ("AVG MARGIN", "$3.92", "per cup"),
    ("BEV COST", "22%", "6 pts under target"),
    ("LABOR COST", "28%", "of sales"),
    ("DAILY SALES", "$2,769", "typical day"),
    ("TRANSACTIONS", "352", "per day"),
    ("AVG TICKET", "$7.87", "per order"),
    ("TOP DAYPART", "Morning", "$1,329"),
    ("PRIME COST", "50%", "bev + labor"),
    ("CAFE SCORE", "91%", "healthy"),
]

DAYPARTS = [(50.0, PRIMARY), (30.0, GOLD_LT), (20.0, SURFACE)]
WEEK = [("Mon", 2450), ("Tue", 2680), ("Wed", 2769), ("Thu", 2900), ("Fri", 3350), ("Sat", 3600), ("Sun", 2100)]


def content_dashboard(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Cafe Dashboard", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 64), "Wildroot Coffee · Priya  ·  cost every cup, watch your prime cost & pour more profit", font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 190, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 95, y0 + 44), "● 50% prime cost", font=fs(15), fill=PRIMARY, anchor="mm")
    ...
    d.text((gx, cy_top), "CAFE HEALTH · SALES BY DAYPART · THE WEEK", font=fs(20), fill=ACCENT, anchor="lt")
    ...
    d.text((px + 16, panels_y + 14), "Cafe Health", font=fs(17), fill=ACCENT, anchor="lt")
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("Bev cost on target", 1.0, "100%"), ("Labor on target", 1.0, "100%"), ("Prime cost healthy", 1.0, "100%"),
           ("Avg ticket vs goal", 0.98, "98%"), ("Margin per cup", 0.98, "98%"), ("Low waste", 0.48, "48%")],
          color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Sales by Daypart", font=fs(17), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.44, panels_y + panel_h * 0.50, min(panel_h * 0.27, pw * 0.27), DAYPARTS, "$2,769", "day")
    legend(d, px + pw * 0.04, panels_y + panel_h - 104, [(PRIMARY, "Morning $1,329"), (GOLD_LT, "Midday $831"),
           (SURFACE, "Afternoon $609")], 15, 30)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Sales This Week", font=fs(17), fill=ACCENT, anchor="lt")
    vbars(img, d, (px + 18, panels_y + 60, px + pw - 14, panels_y + panel_h - 10), WEEK, 3600)
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Cafe Score", font=fs(17), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.5, panels_y + panel_h * 0.54, min(panel_h * 0.30, pw * 0.30),
          [(91, PRIMARY), (9, SURFACE)], "91%", "healthy")
...
def content_menu(img, cbox):
    rows = [
        ("Latte", "$1.18", "$5.25", "22%", "$4.07"),
        ("Cappuccino", "$0.98", "$4.75", "21%", "$3.77"),
        ("Drip Coffee", "$0.52", "$3.25", "16%", "$2.73"),
        ("Cold Brew", "$1.10", "$5.00", "22%", "$3.90"),
        ("Mocha", "$1.45", "$5.75", "25%", "$4.30"),
        ("Avocado Toast", "$2.60", "$9.50", "27%", "$6.90"),
    ]
    _table(img, cbox, "Menu Board",
           "Cup cost, price, beverage-cost % & margin on every drink — price for a healthy pour",
           ["ITEM", "CUP COST", "PRICE", "BEV %", "MARGIN"],
           [0.0, 0.36, 0.52, 0.70, 0.85], rows)


def content_cupcost(img, cbox):
    rows = [
        ("Espresso beans", "18 g", "$0.024", "$0.43"),
        ("Whole milk", "12 oz", "$0.048", "$0.58"),
        ("12 oz cup", "1 ea", "$0.12", "$0.12"),
        ("Lid", "1 ea", "$0.03", "$0.03"),
        ("Sleeve", "1 ea", "$0.02", "$0.02"),
    ]
    _table(img, cbox, "Cup Cost — Latte",
           ...
           total_row=("CUP COST", "", "", "$1.18"))


def content_labor(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Labor & Prime Cost", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 62), "The one number that makes or breaks a café — keep prime cost under 60%.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    # prime cost stacked bar
    by = y0 + 130; bx0 = x0 + pad; bx1 = x1 - pad; bh = 90
    d.text((bx0, by - 6), "PRIME COST = BEVERAGE % + LABOR %", font=fs(17), fill=ACCENT, anchor="lt")
    yy = by + 24
    full = bx1 - bx0
    bev_w = full * 0.22; lab_w = full * 0.28
    grad_round(img, (bx0, yy, bx0 + bev_w, yy + bh), 10, GOLD_HI, GOLD)
    grad_round(img, (bx0 + bev_w, yy, bx0 + bev_w + lab_w, yy + bh), 10, PRIMARY_LT, PRIMARY)
    d.rounded_rectangle((bx0 + bev_w + lab_w, yy, bx1, yy + bh), radius=10, fill=(236, 230, 220))
    d.text((bx0 + bev_w / 2, yy + bh / 2), "Bev 22%", font=fs(18), fill=WHITE, anchor="mm")
    d.text((bx0 + bev_w + lab_w / 2, yy + bh / 2), "Labor 28%", font=fs(18), fill=WHITE, anchor="mm")
    d.text((bx0 + bev_w + lab_w + (full - bev_w - lab_w) / 2, yy + bh / 2), "Profit room 50%", font=fs(17, bold=False), fill=TEXT, anchor="mm")
    d.text((bx1, yy + bh + 28), "Prime cost 50% — well under the 60% danger line", font=fs(18), fill=PRIMARY, anchor="rt")
    # week labor table
    rows = [("Friday", "$938", "$3,350", "28%"), ("Saturday", "$1,008", "$3,600", "28%"),
            ("Thursday", "$812", "$2,900", "28%"), ("Wednesday", "$775", "$2,769", "28%"),
            ("Week total", "$5,557", "$19,849", "28%")]
    ty = yy + bh + 70; rh = min((y1 - pad - ty) / (len(rows) + 1), 96)
    grad_round(img, (bx0, ty, bx1, ty + rh), 8, PRIMARY_LT, PRIMARY_DK)
    for h, fx in zip(["DAY", "LABOR $", "SALES", "LABOR %"], [0.0, 0.42, 0.62, 0.84]):
        d.text((bx0 + 14 + (bx1 - bx0) * fx, ty + rh / 2), h, font=fs(15), fill=WHITE, anchor="lm" if fx == 0 else "mm")
    for i, row in enumerate(rows):
        ry = ty + rh + i * rh
        last = i == len(rows) - 1
        if last:
            d.rectangle((bx0, ry, bx1, ry + rh), fill=SURFACE)
        elif i % 2:
            d.rectangle((bx0, ry, bx1, ry + rh), fill=ROW_ALT)
        for ci, (val, fx) in enumerate(zip(row, [0.0, 0.42, 0.62, 0.84])):
            d.text((bx0 + 14 + (bx1 - bx0) * fx, ry + rh / 2), val, font=fs(16 if ci == 0 or last else 15, bold=(ci == 0 or last)),
                   fill=PRIMARY if ci == 0 or last else TEXT, anchor="lm" if fx == 0 else "mm")
...
def render_hero(out):
    ...
    cup_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE COMPLETE COFFEE-SHOP SYSTEM", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "CAFE COMMAND CENTER", 82, max_w=1780)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 546), "Cup cost, beverage & labor %, prime cost, daypart sales & inventory — one system.",
       fs(20, bold=False), (224, 213, 190))
    chips = [("14", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("PRIME", "COST ENGINE")]
    cw = 440
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not a sales tracker — a complete cost-every-cup, watch-your-prime-cost system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Start Here", "how it all works"), ("Cafe Dashboard", "sales, prime cost & score"),
        ("Cup Cost", "cost a drink to the cup"), ("Menu Board", "bev-cost % & margin"),
        ("Daypart Sales", "where the money is"), ("Weekly Sales", "the week at a glance"),
        ("Labor & Prime", "the make-or-break number"), ("Bean & Milk", "your biggest inputs"),
        ("Inventory & Par", "order before you 86 it"), ("Waste Log", "plug the leaks"),
        ("Ordering", "your standing order"), ("Cash & Tips", "balance the drawer"),
        ("Regulars", "the backbone of a cafe"), ("Settings", "set it once"),
        ("+ 12 Printable Pages", "for the bar"), ("Live Cafe Score", "it all rolls up"),
    ]
    cols = 4
    margin = 88
    gx, gy = 22, 20
...
def render_menu(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "PRICE THE POUR", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "Beverage-Cost % on Every Cup", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Cup cost & price side by side — with each drink's beverage-cost % and margin",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 2, content_menu)
    img.convert("RGB").save(out, "PNG", optimize=True)


def render_labor(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "WATCH YOUR PRIME COST", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "The Make-or-Break Number", fserif(46), WHITE)
    tc(d, (SIZE // 2, 300), "Beverage % + labor % = prime cost. Keep it under 60% and the café pours profit.",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 5, content_labor)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_engine(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 110, "COST IT · POUR IT", font=fs(32), pad_x=52, pad_y=22)
    tc(d, (SIZE // 2, 224), "The Cup-Cost Engine", fserif(44), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 1, content_cupcost)
...
def render_printables(out):
    ...
    d = ImageDraw.Draw(img)
    cup_crest(img, SIZE // 2, 110, r=48)
    pill(img, SIZE // 2, 214, "PLUS 12 PRINTABLE PAGES", font=fs(30), pad_x=48, pad_y=20)
    tc(d, (SIZE // 2, 300), "Print, Fill & Keep — For the Bar", fserif(44), WHITE)
    gold_divider(img, SIZE // 2, 362, width=480)
    tc(d, (SIZE // 2, 404), "A matching print-ready pack — cup-cost card, daypart log, waste log & open/close checklist",
       fs(21, bold=False), (226, 214, 190))
    labels = ["Cup Cost Card", "Menu Board", "Daypart Sales", "Weekly Sales",
              "Labor & Prime", "Bean & Milk", "Inventory & Par", "Waste Log",
              "Ordering Sheet", "Cash & Tips", "Open / Close", "Regulars"]
