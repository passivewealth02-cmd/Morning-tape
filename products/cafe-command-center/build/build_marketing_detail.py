"""Detailed marketing image set for Cafe & Coffee Shop Command Center™ (07-10)."""
...
def render_features(out):
    ...
    header_band(img, "BUILT TO POUR PROFIT", "Where a Café Makes Money",
                "Four ways it turns a busy bar into real profit")
    feats = [
        ("$", "Live Café Score", "donut", (91, "91%", "healthy"),
         "Beverage cost, labor, prime cost, avg ticket, margin & waste roll into one number — so you know your café's financial health at a glance."),
        ("%", "Cost Every Cup", "table", None,
         "The cup-cost engine costs each drink to the cup — beans, milk, cup, lid & sleeve — so you price for a healthy pour, not a guess."),
        ("box", "Watch Your Prime Cost", "prime", None,
         "Beverage % + labor % = prime cost, the one number that makes or breaks a café. Keep it under 60% and the money follows."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack — cup-cost card, daypart & weekly logs, labor sheet, waste log, ordering sheet & an open/close checklist."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "table":
            ...
            od.rounded_rectangle((bx - 22, by - 10, bx - 4, by + 10), radius=3, fill=GOLD_HI)
            od.rounded_rectangle((bx - 2, by - 10, bx + 22, by + 10), radius=3, outline=GOLD_HI, width=3)
        elif kind == "print":
        ...
        if kind == "donut":
            ...
            rows = [("Beans + milk", "$1.01"), ("Cup + lid + sleeve", "$0.17"), ("Latte cup cost", "$1.18")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["COMPONENT", "COST"], [0.0, 0.66]):
            ...
            for ri, (a, b) in enumerate(rows):
                ...
                od.text((tx0 + 14 + (tx1 - tx0) * 0.66, ry + 21), b, font=fs(16, bold=False), fill=TEXT, anchor="lm")
        elif kind == "prime":
            od = ImageDraw.Draw(img)
            bx0 = x + 44; bx1 = x + cw - 44; byy = vy + 10; bh = 70
            full = bx1 - bx0
            grad_round(img, (bx0, byy, bx0 + full * 0.22, byy + bh), 8, GOLD_HI, GOLD)
            grad_round(img, (bx0 + full * 0.22, byy, bx0 + full * 0.50, byy + bh), 8, PRIMARY_LT, PRIMARY)
            od.rounded_rectangle((bx0 + full * 0.50, byy, bx1, byy + bh), radius=8, fill=(236, 230, 220))
            od.text((bx0 + full * 0.11, byy + bh / 2), "Bev 22%", font=fs(15), fill=WHITE, anchor="mm")
            od.text((bx0 + full * 0.36, byy + bh / 2), "Labor 28%", font=fs(15), fill=WHITE, anchor="mm")
            od.text(((bx0 + full * 0.50 + bx1) / 2, byy + bh / 2), "Profit room 50%", font=fs(15, bold=False), fill=TEXT, anchor="mm")
            od.text(((bx0 + bx1) / 2, byy + bh + 34), "Prime cost 50% — under the 60% line", font=fs(18), fill=PRIMARY, anchor="mm")
        elif kind == "print":
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Sales Tracker",
                "Everything a basic tracker leaves out — built for coffee")
    rows = [
        ("A daily sales tracker", True, True),
        ("Cup-cost engine (per drink)", False, True),
        ("Beverage-cost % on every item", False, True),
        ("Labor % & prime cost", False, True),
        ("Daypart sales mix", False, True),
        ("Bean & milk usage tracking", False, True),
        ("Inventory & par levels", False, True),
        ("Waste log & ordering sheet", False, True),
        ("Regulars / loyalty tracker", False, True),
        ("Live Café Score dashboard", False, True),
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "SET UP IN AN AFTERNOON", "Dial It In, in 4 Steps",
                "No spreadsheet skills needed — if you can type, you can run it")
    steps = [
        ("1", "Cost your cups", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack, then cost each drink to the cup."),
        ("2", "Build your board", "Add every drink to the Menu Board — beverage-cost % and margin calculate live, so you price for a healthy pour."),
        ("3", "Watch prime cost", "Log daypart & weekly sales and daily labor — the sheet shows your prime cost, the number that makes or breaks a café."),
        ("4", "Plug the leaks", "Track bean & milk usage, inventory, waste & ordering, and watch your Café Score climb as prime cost drops."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "EVERYTHING YOU GET", "One System, Whole Café",
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + Café Score",
        "Cup-cost engine", "Menu board with beverage-cost %",
        "Daypart & weekly sales", "Labor & prime cost",
        "Bean/milk, inventory, waste & ordering", "Start-Here guide + free updates",
    ]
    ...
    who = ["Café & coffee-shop owners", "Baristas & shift leads", "Espresso bars & roasteries",
           "Drive-thru & kiosk coffee", "Bubble tea & juice bars", "New cafés dialing in prime cost"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "COST EVERY CUP · POUR MORE PROFIT", font=fs(27), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Cost every cup, watch your prime cost & keep more of every dollar you pour.",
       fs(23, bold=False), TEXT_MUTED)
    book_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "CAFE COMMAND CENTER", 52, max_w=1200)
    img.convert("RGB").save(out, "PNG", optimize=True)
