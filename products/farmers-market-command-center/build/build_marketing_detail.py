"""Detailed marketing image set for Farmers Market Vendor Command Center™ (07-10)."""
...
from build_marketing import (
    ...
    ROW_ALT, fs, fserif, premium_bg, pill, wordmark, gold_divider, tent_crest,
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR MARKET VENDORS", "Cost It, Sell It, Did It Pay?",
                "Four ways it turns a market stall into a business you can trust")
    feats = [
        ("$", "Live Vendor Score", "donut", (90, "90%", "healthy"),
         "COGS, margin, product pricing, sales, profit & waste roll into one number — so you know your stall's health at a glance."),
        ("%", "Booth P&L Engine", "table", None,
         "Enter what you sold; it subtracts your cost of goods and your booth costs and tells you the booth net and what you made per hour."),
        ("box", "Run the Stall", "stock", None,
         "Product margins, a bake plan, ingredient & packaging costs, a markets log and your regulars & CSA — all in one place."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack — booth P&L, product price list, bake plan, market checklist, waste log & more for the stall."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("Market sales", "$768"), ("− Goods + booth", "−$330"), ("Booth net", "$438")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["SATURDAY MARKET", "VALUE"], [0.0, 0.66]):
            ...
            od = ImageDraw.Draw(img)
            items = [("Monthly sales", "$3,000", MINT_BG, PRIMARY), ("Monthly profit", "$1,595", MINT_BG, PRIMARY),
                     ("Markets / month", "4", MINT_BG, PRIMARY), ("Avg basket", "$12.00", SURFACE, (110, 88, 58))]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    ...
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Price List",
                "Everything a basic stall sheet leaves out — built in")
    rows = [
        ("A product price list", True, True),
        ("A booth P&L — did the day pay?", False, True),
        ("Net per market hour", False, True),
        ("Margin on every product", False, True),
        ("Bake / prep plan", False, True),
        ("Markets log → monthly sales", False, True),
        ("Booth-cost & packaging tracking", False, True),
        ("Regulars, CSA & wholesale", False, True),
        ("Income & expenses for taxes", False, True),
        ("Live Vendor Score dashboard", False, True),
    ...
    d.text((c1, top + hh // 2), "A Basic\nPrice List", font=fs(18), fill=(210, 210, 205), anchor="mm")
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "SET UP IN AN AFTERNOON", "Run Your Stall in 4 Steps",
                "No spreadsheet skills needed — if you can type, you can run it")
    steps = [
        ("1", "Price your products", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack, then price each product with a live margin."),
        ("2", "Plan the bake", "Set how many of each to make for the market, and track your ingredient and packaging costs so your product cost stays honest."),
        ("3", "Run the market day", "After the market, enter what you sold; the booth P&L subtracts your goods and booth costs and shows if the day actually paid."),
        ("4", "Total the month", "Log every market, keep your income and expenses for taxes, log waste, and watch your monthly profit and Vendor Score climb."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "EVERYTHING YOU GET", "One System, Whole Stall",
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + Vendor Score",
        "Booth P&L engine (did it pay?)", "Product price & margin list",
        "Bake plan & ingredient costs", "Markets log → monthly sales",
        "Packaging, customers & waste logs", "Start-Here quick-start guide",
    ]
    ...
    who = ["Farmers-market bakers", "Jam, honey & preserve makers", "Produce & flower growers",
           "Cottage-food & craft sellers", "CSA & farm-stand sellers", "Anyone with a market stall"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "COST IT · SELL IT · DID IT PAY?", font=fs(27), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Cost every product, price the whole stall, and know if the market day paid.",
       fs(23, bold=False), TEXT_MUTED)
    tent_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "FARMERS MARKET", 54, max_w=1100)
    img.convert("RGB").save(out, "PNG", optimize=True)
