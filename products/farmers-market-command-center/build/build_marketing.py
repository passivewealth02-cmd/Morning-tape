"""Marketing image set for Farmers Market Vendor Command Center™ (6 images, 2000x2000)."""
...
TABS = ["Dashboard", "Market Day", "Products", "Ingredient Costs", "Bake Plan", "Markets",
        "Booth Costs", "Packaging", "Customers", "Waste Log",
        "Income & Expenses", "Monthly Summary", "Settings"]

FILE_LABEL = "Farmers_Market_Command_Center.xlsx — Harvest Lane · Sage"
...
def tent_crest(c, cx, cy, r=56, glow=True):
    """Brand crest — a market-stall canopy: the icon of the farmers-market vendor."""
    if glow:
    ...
    ov = Image.new("RGBA", c.size, (0, 0, 0, 0)); d = ImageDraw.Draw(ov)
    # canopy roof — striped
    x0, x1 = cx - r * 0.48, cx + r * 0.48
    roof_top = cy - r * 0.34; roof_bot = cy - r * 0.02
    n = 6; sw = (x1 - x0) / n
    for k in range(n):
        col = GOLD_HI if k % 2 == 0 else WHITE
        d.rectangle((x0 + k * sw, roof_top, x0 + (k + 1) * sw, roof_bot), fill=col)
    d.rectangle((x0, roof_top, x1, roof_top + r * 0.05), fill=(150, 120, 70))
    # scalloped valance (triangles hanging from roof_bot)
    for k in range(n):
        col = GOLD_HI if k % 2 else WHITE
        vx0 = x0 + k * sw; vx1 = x0 + (k + 1) * sw
        d.polygon([(vx0, roof_bot), (vx1, roof_bot), ((vx0 + vx1) / 2, roof_bot + r * 0.12)], fill=col)
    # poles
    for px in (x0 + r * 0.03, x1 - r * 0.03):
        d.line((px, roof_bot, px, cy + r * 0.42), fill=WHITE, width=3)
    # counter / table
    d.rectangle((x0 - r * 0.02, cy + r * 0.30, x1 + r * 0.02, cy + r * 0.42), fill=(150, 120, 70))
    # a couple of goods on the counter
    for gx in (cx - r * 0.18, cx + r * 0.02, cx + r * 0.22):
        d.ellipse((gx - r * 0.07, cy + r * 0.16, gx + r * 0.07, cy + r * 0.30), fill=HIGHLIGHT)
    c.alpha_composite(ov)


# aliases (shared render helpers may reference these names across products)
toque_crest = tent_crest
bento_crest = tent_crest
cupcake_crest = tent_crest
clock_crest = tent_crest
clipboard_crest = tent_crest
cloche_crest = tent_crest
ghost_crest = tent_crest
mug_crest = tent_crest
book_crest = tent_crest
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "MARKET STALL", font=fs(15), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("UNITS SOLD", "90", "this market"),
    ("MARKET SALES", "$768", "this day"),
    ("COGS %", "24%", "of sales"),
    ("BOOTH NET", "$438", "after costs"),
    ("NET / HOUR", "$54.75", "you made"),
    ("TOP SELLER", "Sourdough", "$176"),
    ("AVG BASKET", "$12.00", "per customer"),
    ("MONTHLY SALES", "$3,000", "4 markets"),
    ("MARKETS / MO", "4", "this month"),
    ("MONTHLY PROFIT", "$1,595", "after costs"),
    ("WASTE", "2.4%", "of sales"),
    ("VENDOR SCORE", "90%", "healthy"),
]

SALES_MONTHS = [("Jul", 3000), ("Aug", 3200), ("Sep", 3450), ("Oct", 3100), ("Nov", 2600), ("Dec", 3600)]


def content_dashboard(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Market Stall Dashboard", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 64), "Harvest Lane Market Co. · Sage  ·  cost every product, and know if the market day paid", font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 210, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 105, y0 + 44), "● $438 booth net", font=fs(15), fill=PRIMARY, anchor="mm")
    ...
    d.text((gx, cy_top), "VENDOR HEALTH · WHERE THE DAY GOES · SALES BY MONTH", font=fs(20), fill=ACCENT, anchor="lt")
    ...
    d.text((px + 16, panels_y + 14), "Vendor Health", font=fs(17), fill=ACCENT, anchor="lt")
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("COGS on target", 1.0, "100%"), ("Margin healthy", 1.0, "100%"), ("Products priced", 1.0, "100%"),
           ("Sales vs goal", 1.0, "100%"), ("Profitable", 1.0, "100%"), ("Waste low", 0.40, "40%")],
          color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Where the Day Goes", font=fs(16), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.44, panels_y + panel_h * 0.50, min(panel_h * 0.27, pw * 0.27),
          [(57, PRIMARY), (24, ACCENT), (19, GOLD_LT)], "57%", "booth net")
    legend(d, px + pw * 0.05, panels_y + panel_h - 108, [(PRIMARY, "Booth net 57%"), (ACCENT, "COGS 24%"), (GOLD_LT, "Booth costs 19%")], 14, 30)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Sales by Month", font=fs(17), fill=ACCENT, anchor="lt")
    vbars(img, d, (px + 18, panels_y + 60, px + pw - 14, panels_y + panel_h - 10), SALES_MONTHS, 3600)
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Vendor Score", font=fs(17), fill=ACCENT, anchor="lt")
...
def content_products(img, cbox):
    rows = [
        ("Sourdough Loaf", "$1.85", "$8", "$6.15", "77%"),
        ("Cinnamon Rolls (6)", "$2.40", "$12", "$9.60", "80%"),
        ("Baguette", "$1.10", "$5", "$3.90", "78%"),
        ("Focaccia", "$1.60", "$7", "$5.40", "77%"),
        ("Jam (jar)", "$2.80", "$9", "$6.20", "69%"),
        ("Honey (jar)", "$4.50", "$14", "$9.50", "68%"),
    ]
    _table(img, cbox, "Product Price List",
           "Cost, price and the margin on every product — price the whole stall for profit.",
           ["PRODUCT", "COST", "PRICE", "MARGIN", "MARGIN %"],
           [0.0, 0.44, 0.60, 0.74, 0.88], rows)


def content_boothpnl(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Market Day — Did It Pay?", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 62), "The booth P&L every vendor needs: sales, minus goods, minus the stall.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("Market sales  ·  90 units", "$768", PRIMARY), ("− Cost of goods sold", "−$187", DANGER),
            ("− Booth costs (stall, fuel, help)", "−$143", DANGER)]
    ...
    d.text((x0 + pad + 14, ry + rh / 2), "= BOOTH NET", font=fs(18), fill=PRIMARY, anchor="lm")
    d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.80, ry + rh / 2), "$438", font=fs(18), fill=PRIMARY, anchor="mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2), "÷ 8 hours  =  $54.75 an hour — the day paid", font=fs(22), fill=WHITE, anchor="mm")
    img.alpha_composite(ov)


def content_markets(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Markets → Monthly Sales", font=fs(32), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 62), "Log each market day — where you sold and the take — and monthly sales roll up.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("Sat · wk 1  ·  Downtown", "$768", PRIMARY), ("Sat · wk 2  ·  Downtown", "$690", PRIMARY),
            ("Sat · wk 3  ·  Riverside", "$815", PRIMARY), ("Sat · wk 4  ·  Downtown", "$727", PRIMARY)]
    callout_h = 150; gap = 44; rh = 118
    n_units = 1 + len(rows) + 1
    top_region = y0 + 110
    block_h = rh * n_units + gap + callout_h
    ty = top_region + max(((y1 - pad) - top_region - block_h) / 2, 0)
    grad_round(img, (x0 + pad, ty, x1 - pad, ty + rh), 8, PRIMARY_LT, PRIMARY_DK)
    for h, fx in zip(["MARKET DAY", "DAY SALES"], [0.0, 0.80]):
        d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * fx, ty + rh / 2), h, font=fs(16), fill=WHITE, anchor="lm" if fx == 0 else "mm")
    for i, (label, val, col) in enumerate(rows):
        ry = ty + rh + i * rh
        if i % 2:
    ...
    ry = ty + rh + len(rows) * rh
    d.rectangle((x0 + pad, ry, x1 - pad, ry + rh), fill=SURFACE)
    d.text((x0 + pad + 14, ry + rh / 2), "= MONTHLY SALES  ·  4 markets", font=fs(18), fill=PRIMARY, anchor="lm")
    d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.80, ry + rh / 2), "$3,000", font=fs(18), fill=PRIMARY, anchor="mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2), "$1,595 profit after a month of markets", font=fs(22), fill=WHITE, anchor="mm")
    img.alpha_composite(ov)
...
def render_hero(out):
    ...
    tent_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE COMPLETE MARKET-STALL SYSTEM", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "FARMERS MARKET", 78, max_w=1600)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 546), "Product margins, a booth P&L that tells you if the day paid, bake plans & books — one system.",
       fs(20, bold=False), (224, 213, 190))
    chips = [("14", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("BOOTH", "P&L ENGINE")]
    cw = 440
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not a price list — a complete cost-it, sell-it, did-it-pay system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Start Here", "how it all works"), ("Dashboard", "sales, profit & score"),
        ("Market Day", "did the day pay?"), ("Products", "margin on every item"),
        ("Ingredient Costs", "your buying list"), ("Bake Plan", "what to make"),
        ("Markets", "every day → monthly sales"), ("Booth Costs", "stall fees by market"),
        ("Packaging", "the small costs"), ("Customers", "regulars & CSA"),
        ("Waste Log", "the quiet leaks"), ("Income & Expenses", "your monthly books"),
        ("Monthly Summary", "watch the season swing"), ("Settings", "set it once"),
        ("+ 12 Printable Pages", "booth P&L & checklists"), ("Live Vendor Score", "it all rolls up"),
    ]
    cols = 4
    margin = 88
    gx, gy = 22, 20
...
def render_boothpnl(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "DID THE DAY PAY?", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "The Booth P&L Engine", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "The number every vendor needs — sales minus goods minus the stall, and what you made per hour",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 1, content_boothpnl)
    img.convert("RGB").save(out, "PNG", optimize=True)


def render_markets(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "EVERY MARKET COUNTS", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "Markets Become Monthly Sales", fserif(40), WHITE)
    tc(d, (SIZE // 2, 300), "Log each market day and watch your monthly sales — and your profit — roll up on their own",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 5, content_markets)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_engine(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 110, "COST IT · SELL IT", font=fs(32), pad_x=52, pad_y=22)
    tc(d, (SIZE // 2, 224), "The Market-Vendor Engine", fserif(44), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 1, content_boothpnl)
    app_window(img, (60, 1180, SIZE - 60, SIZE - 60), 2, content_products)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_printables(out):
    ...
    d = ImageDraw.Draw(img)
    tent_crest(img, SIZE // 2, 110, r=48)
    pill(img, SIZE // 2, 214, "PLUS 12 PRINTABLE PAGES", font=fs(30), pad_x=48, pad_y=20)
    tc(d, (SIZE // 2, 300), "Print, Fill & Keep — For the Stall", fserif(44), WHITE)
    gold_divider(img, SIZE // 2, 362, width=480)
    tc(d, (SIZE // 2, 404), "A matching print-ready pack — booth P&L, price list, bake plan, market checklist & more",
       fs(21, bold=False), (226, 214, 190))
    labels = ["Booth P&L", "Product Price List", "Bake Plan", "Ingredient List",
              "Markets Log", "Booth Costs", "Packaging", "Customer List",
              "Waste Log", "Income & Expenses", "Monthly Summary", "Market Day Checklist"]
...
def main():
    ...
    targets = [
        ...
        ("03_boothpnl.png", render_boothpnl),
        ("04_markets.png", render_markets),
