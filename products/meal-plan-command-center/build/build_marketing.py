"""Marketing image set for Meal Planning & Grocery Budget Command Center™ (6 images, 2000x2000)."""
...
TABS = ["Dashboard", "Cost Per Meal", "Weekly Plan", "Recipes", "Price Book", "Grocery List", "Pantry",
        "Budget", "Eating Out", "Savings", "Meal Ideas", "Monthly Summary", "Settings"]

FILE_LABEL = "Meal_Plan_Command_Center.xlsx — Hearth & Harvest · Nora"
...
def basket_crest(c, cx, cy, r=56, glow=True):
    """Brand crest — a grocery basket: the icon of the smart-shopping kitchen."""
    if glow:
    ...
    ov = Image.new("RGBA", c.size, (0, 0, 0, 0)); d = ImageDraw.Draw(ov)
    # basket handle
    d.arc((cx - r * 0.28, cy - r * 0.42, cx + r * 0.28, cy + r * 0.02), 180, 360, fill=GOLD_HI, width=4)
    # basket body (trapezoid)
    d.polygon([(cx - r * 0.42, cy - r * 0.12), (cx + r * 0.42, cy - r * 0.12),
               (cx + r * 0.32, cy + r * 0.40), (cx - r * 0.32, cy + r * 0.40)], fill=GOLD_HI, outline=(150, 120, 70))
    # basket weave lines
    for k in range(1, 3):
        yy = cy - r * 0.12 + k * r * 0.17
        d.line((cx - r * 0.40 + k * 3, yy, cx + r * 0.40 - k * 3, yy), fill=(150, 120, 70), width=2)
    for k in (-1, 0, 1):
        d.line((cx + k * r * 0.20, cy - r * 0.12, cx + k * r * 0.16, cy + r * 0.40), fill=(150, 120, 70), width=2)
    # a little produce peeking out
    d.ellipse((cx - r * 0.20, cy - r * 0.24, cx - r * 0.04, cy - r * 0.08), fill=HIGHLIGHT)
    d.ellipse((cx + r * 0.02, cy - r * 0.26, cx + r * 0.20, cy - r * 0.08), fill=DANGER)
    c.alpha_composite(ov)


# aliases (shared render helpers may reference these names across products)
compass_crest = basket_crest
candle_crest = basket_crest
camera_crest = basket_crest
house_crest = basket_crest
...
def app_window(img, box, active_idx, content_fn, file_label=FILE_LABEL):
    ...
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    bx = sb[0] + 26
    od.text((bx, sb[1] + 26), "KITCHEN", font=fs(15), fill=GOLD_HI, anchor="lt")
...
KPIS = [
    ("RECIPE COST", "$10.00", "flagship"),
    ("SERVINGS", "4", "per recipe"),
    ("COST / SERVING", "$2.50", "per plate"),
    ("EAT-OUT / SERVING", "$12.50", "takeout"),
    ("SAVED / SERVING", "$10.00", "vs eating out"),
    ("WEEKLY PLAN", "$66", "7 dinners"),
    ("GROCERY BUDGET", "$600", "per month"),
    ("SPENT", "$540", "this month"),
    ("BUDGET USED", "90%", "in the green"),
    ("SERVINGS COOKED", "120", "this month"),
    ("MONTHLY SAVINGS", "$960", "vs eating out"),
    ("KITCHEN SCORE", "90%", "healthy"),
]

SAVE_MONTHS = [("Feb", 720), ("Mar", 800), ("Apr", 860), ("May", 900), ("Jun", 940), ("Jul", 960)]
...
def content_dashboard(img, cbox):
    ...
    d.text((x0 + pad, y0 + 64), "Hearth & Harvest · Nora  ·  plan the week, know the cost, save on every meal", font=fs(19, bold=False), fill=TEXT_MUTED, anchor="lt")
    d.rounded_rectangle((x1 - pad - 210, y0 + 26, x1 - pad, y0 + 62), radius=18, fill=MINT_BG)
    d.text((x1 - pad - 105, y0 + 44), "● $2.50 / serving", font=fs(14), fill=PRIMARY, anchor="mm")
    ...
    d.text((gx, cy_top), "KITCHEN HEALTH · WHERE THE PLATE $ GOES · SAVED BY MONTH", font=fs(19), fill=ACCENT, anchor="lt")
    ...
    hbars(img, d, (px + 20, panels_y + 50, px + pw - 16, panels_y + panel_h - 16),
          [("Under budget", 1.0, "100%"), ("Cooking saves", 1.0, "100%"), ("Week planned", 1.0, "100%"),
           ("Cheap per serving", 1.0, "100%"), ("Price book built", 1.0, "100%"), ("Eating out rare", 0.40, "40%")],
          color=(GOLD_HI, GOLD))
    px = gx + (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Cook vs Eat Out", font=fs(15), fill=ACCENT, anchor="lt")
    donut(d, px + pw * 0.44, panels_y + panel_h * 0.50, min(panel_h * 0.27, pw * 0.27),
          [(20, PRIMARY), (80, ACCENT)], "1/5", "the cost")
    legend(d, px + pw * 0.06, panels_y + panel_h - 96, [(PRIMARY, "Cook $2.50"), (ACCENT, "Eat out $12.50")], 15, 32)
    px = gx + 2 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
    d.text((px + 16, panels_y + 14), "Saved by Month", font=fs(17), fill=ACCENT, anchor="lt")
    vbars(img, d, (px + 18, panels_y + 60, px + pw - 14, panels_y + panel_h - 10), SAVE_MONTHS, 960)
    px = gx + 3 * (pw + gap)
    d.rounded_rectangle((px, panels_y, px + pw, panels_y + panel_h), radius=12, fill=WHITE, outline=GRID, width=2)
...
def content_weekly(img, cbox):
    rows = [
        ("Mon", "Sheet-Pan Chicken & Veg", "4", "$10.00"),
        ("Tue", "Pasta Primavera", "4", "$8.00"),
        ("Wed", "Beef Tacos", "4", "$12.00"),
        ("Thu", "Veggie Chili", "6", "$9.00"),
        ("Fri", "Fried Rice", "4", "$7.00"),
        ("Sat", "Salmon & Rice", "4", "$14.00"),
        ("Sun", "Lentil Soup", "6", "$6.00"),
    ]
    _table(img, cbox, "Weekly Plan",
           "Seven dinners, planned and priced — the whole point of a cheaper, calmer week.",
           ["DAY", "DINNER", "SERVES", "COST"],
           [0.0, 0.16, 0.70, 0.87], rows,
           total_row=("Week", "7 dinners planned", "", "$66.00"))


def content_costmeal(img, cbox):
    x0, y0, x1, y1 = cbox; pad = 30
    d = ImageDraw.Draw(img); d.rectangle(cbox, fill=BG)
    d.text((x0 + pad, y0 + 22), "Cost Per Meal — Per Plate", font=fs(30), fill=PRIMARY, anchor="lt")
    d.text((x0 + pad, y0 + 60), "A recipe's ingredients, over its servings, is what a home-cooked plate really costs.",
           font=fs(18, bold=False), fill=TEXT_MUTED, anchor="lt")
    rows = [("Ingredients · Sheet-Pan Chicken & Veg", "$10.00", PRIMARY),
            ("÷ Servings", "4", PRIMARY)]
    ...
    d.text((x0 + pad + 14, ry + rh / 2), "= COST PER SERVING", font=fs(18), fill=PRIMARY, anchor="lm")
    d.text((x0 + pad + 14 + (x1 - x0 - 2 * pad) * 0.80, ry + rh / 2), "$2.50", font=fs(18), fill=PRIMARY, anchor="mm")
    by = ry + rh + gap
    grad_round(img, (x0 + pad, by, x1 - pad, by + callout_h), 14, PRIMARY_LT, PRIMARY_DK, outline=GOLD_LT, width=3)
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0)); od = ImageDraw.Draw(ov)
    od.text(((x0 + x1) / 2, by + callout_h / 2), "vs $12.50 eating out  =  you save $10.00 a plate", font=fs(22), fill=WHITE, anchor="mm")
    img.alpha_composite(ov)
...
def render_hero(out):
    ...
    basket_crest(img, SIZE // 2, 132, r=56)
    pill(img, SIZE // 2, 256, "THE COMPLETE MEAL-PLAN & GROCERY SYSTEM", font=fs(20), pad_x=40, pad_y=20)
    wordmark(img, SIZE // 2, 400, "MEAL PLAN", 88, max_w=1550)
    gold_divider(img, SIZE // 2, 500, width=520)
    tc(d, (SIZE // 2, 546), "A cost-per-meal engine, weekly plan, price book, grocery list & budget — one system.",
       fs(20, bold=False), (224, 213, 190))
    chips = [("14", "CONNECTED TABS"), ("12", "PRINTABLE PAGES"), ("COST", "ENGINE")]
    cw = 440
...
def render_inside(out):
    ...
    tc(d, (SIZE // 2, 352), "Not a meal planner — a complete plan-it, cost-it, save-on-it system",
       fs(24, bold=False), (226, 214, 190))
    cards = [
        ("Start Here", "how it all works"), ("Dashboard", "cost & score"),
        ("Cost Per Meal", "per-plate engine"), ("Weekly Plan", "7 dinners priced"),
        ("Recipes", "your rotation"), ("Price Book", "cheapest by store"),
        ("Grocery List", "one week's shop"), ("Pantry", "shop from home first"),
        ("Budget", "stay in the green"), ("Eating Out", "the pricey habit"),
        ("Savings", "cook vs takeout"), ("Meal Ideas", "never wonder again"),
        ("Monthly Summary", "watch it pay off"), ("Settings", "set it once"),
        ("+ 12 Printable Pages", "planners & lists"), ("Live Kitchen Score", "it all rolls up"),
    ]
    cols = 4
    margin = 88
    gx, gy = 22, 20
...
def render_costmeal(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "KNOW THE COST", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "The Cost-Per-Meal Engine", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Add a recipe's ingredients, divide by servings, and see your cost per plate versus takeout",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 1, content_costmeal)
    img.convert("RGB").save(out, "PNG", optimize=True)


def render_weekly(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    premium_bg(img, band_h=360)
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 116, "PLAN THE WEEK", font=fs(30), pad_x=48, pad_y=22)
    tc(d, (SIZE // 2, 232), "The Weekly Plan", fserif(44), WHITE)
    tc(d, (SIZE // 2, 300), "Seven dinners on the calendar and priced — a cheaper, calmer week with zero 5pm panic",
       fs(22, bold=False), (226, 214, 190))
    app_window(img, (70, 400, SIZE - 70, SIZE - 70), 2, content_weekly)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_engine(out):
    ...
    d = ImageDraw.Draw(img)
    pill(img, SIZE // 2, 110, "COST IT · PLAN IT", font=fs(32), pad_x=52, pad_y=22)
    tc(d, (SIZE // 2, 224), "The Kitchen Engine", fserif(44), WHITE)
    app_window(img, (60, 330, SIZE - 60, 1150), 1, content_costmeal)
    app_window(img, (60, 1180, SIZE - 60, SIZE - 60), 2, content_weekly)
    img.convert("RGB").save(out, "PNG", optimize=True)
...
def render_printables(out):
    ...
    d = ImageDraw.Draw(img)
    basket_crest(img, SIZE // 2, 110, r=48)
    pill(img, SIZE // 2, 214, "PLUS 12 PRINTABLE PAGES", font=fs(30), pad_x=48, pad_y=20)
    tc(d, (SIZE // 2, 300), "Print, Fill & Stick on the Fridge", fserif(44), WHITE)
    gold_divider(img, SIZE // 2, 362, width=480)
    tc(d, (SIZE // 2, 404), "A matching print-ready pack — weekly plan, grocery list, price book, meal planner & more",
       fs(21, bold=False), (226, 214, 190))
    labels = ["Weekly Meal Plan", "Cost Per Meal", "Grocery List", "Price Book",
              "Recipe Rotation", "Pantry Inventory", "Grocery Budget", "Eating Out Log",
              "Meal Ideas", "Savings Summary", "Monthly Summary", "Kitchen Checklist"]
...
def main():
    ...
    targets = [
        ...
        ("03_costmeal.png", render_costmeal),
        ("04_weekly.png", render_weekly),
