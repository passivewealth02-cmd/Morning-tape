"""Detailed marketing image set for Meal Planning & Grocery Budget Command Center™ (07-10)."""
...
from build_marketing import (
    ...
    ROW_ALT, fs, fserif, premium_bg, pill, wordmark, gold_divider, basket_crest,
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR YOUR KITCHEN", "Plan It, Cost It, Save on It",
                "Four ways it turns dinner into a plan that saves money")
    feats = [
        ("$", "Live Kitchen Score", "donut", (90, "90%", "healthy"),
         "Under budget, cooking saves, week planned, cheap per serving, price book built and eating-out rare roll into one number — your kitchen's health at a glance."),
        ("%", "Cost-Per-Meal Engine", "table", None,
         "A recipe's ingredients over its servings gives your true cost per plate — and how much that beats the takeout price."),
        ("box", "Run the Whole Kitchen", "stock", None,
         "A weekly plan, recipes, a price book, a grocery list, a pantry, a budget and an eating-out log — the whole kitchen in one place."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack — weekly meal plan, grocery list, price book, meal planner & more, for the fridge."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("Ingredients", "$10.00"), ("÷ 4 servings", "$2.50"), ("vs eat out", "$12.50")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["COST PER PLATE", "VALUE"], [0.0, 0.64]):
            ...
            od = ImageDraw.Draw(img)
            items = [("Weekly plan", "$66", MINT_BG, PRIMARY), ("Budget used", "90%", MINT_BG, PRIMARY),
                     ("Monthly savings", "$960", MINT_BG, PRIMARY), ("Eating out rare", "40%", SURFACE, (110, 88, 58))]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Meal Planner",
                "Everything a basic planner leaves out — built in")
    rows = [
        ("A weekly meal plan", True, True),
        ("A cost-per-meal engine", False, True),
        ("What you save vs eating out", False, True),
        ("A recipe cost rotation", False, True),
        ("A store price book", False, True),
        ("Grocery list & pantry", False, True),
        ("A monthly grocery budget", False, True),
        ("Eating-out tracking", False, True),
        ("Savings by month", False, True),
        ("Live Kitchen Score dashboard", False, True),
    ...
    d.text((c1, top + hh // 2), "A Basic\nPlanner", font=fs(18), fill=(210, 210, 205), anchor="mm")
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "SET UP IN AN AFTERNOON", "Plan Your Week in 4 Steps",
                "No spreadsheet skills needed — if you can type, you can run it")
    steps = [
        ("1", "Cost a meal", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack, then cost a recipe from its ingredients and servings."),
        ("2", "Read your savings", "See your cost per plate and exactly how much a home-cooked meal beats the takeout price — every single serving."),
        ("3", "Plan & shop", "Fill your weekly plan, build your grocery list from the price book, and shop the pantry first so nothing is wasted."),
        ("4", "Stay in the green", "Track your budget week by week, log the odd meal out, and watch your monthly savings climb on the dashboard."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    ...
    includes = [
        ...
        "12-page printable PDF pack", "Live dashboard + Kitchen Score",
        "Cost-per-meal engine", "Weekly plan & recipe rotation",
        "Store price book & grocery list", "Pantry & monthly budget",
        "Eating-out log & savings roll-up", "Start-Here quick-start guide",
    ]
    ...
    who = ["Busy families & couples", "Budget & frugal-living fans", "Meal preppers & batch cooks",
           "New cooks setting up a kitchen", "Anyone cutting the grocery bill", "Anyone tired of takeout spend"]
    ...
    img.alpha_composite(ov)
    pill(img, SIZE // 2, 1420, "PLAN IT · COST IT · SAVE ON IT", font=fs(27), pad_x=44, pad_y=22,
         grad=(PRIMARY_LT, PRIMARY_DK))
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Plan the week, know the cost, and save on every meal.",
       fs(23, bold=False), TEXT_MUTED)
    basket_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "MEAL PLAN", 58, max_w=1150)
    img.convert("RGB").save(out, "PNG", optimize=True)
