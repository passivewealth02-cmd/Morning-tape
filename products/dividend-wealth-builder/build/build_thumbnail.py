"""Premium feature-forward hero thumbnail for Dividend Wealth Builder.
...
def coin(c, cx, cy, r=60):
    ...
    d.text((cx, cy), "$", font=fserif(int(r * 1.0)), fill=GOLD_LT, anchor="mm")
    c.alpha_composite(ov)


def benefit_badge(c, cx, cy, big, small, w=440, h=170):
    box = (cx - w // 2, cy - h // 2, cx + w // 2, cy + h // 2)
    shadow(c, box, 22, 26, 90)
    ov = Image.new("RGBA", c.size, (0, 0, 0, 0))
    d = ImageDraw.Draw(ov)
    d.rounded_rectangle(box, radius=22, fill=PRIMARY, outline=GOLD_LT, width=3)
    d.text((cx, cy - h * 0.20), big, font=fserif(60), fill=GOLD_LT, anchor="mm")
...
def feature_card(c, x, y, w, h, title, sub):
    ...
    d.text((x + 92, cyc - 22), title, font=fs(30), fill=PRIMARY, anchor="lm")
...
def dashboard_screen(c, screen):
    ...
    d.text((sx0 + 28, sy0 + head // 2), "DIVIDEND WEALTH BUILDER", font=fs(26), fill=WHITE, anchor="lm")
    d.text((sx1 - 28, sy0 + head // 2), "Portfolio · FIRE Tracker", font=fs(22), fill=GOLD_LT, anchor="rm")
    ...
    kh = 120
    kpis = [
        ("PORTFOLIO VALUE", "$148.4K", PRIMARY),
        ("MONTHLY INCOME", "$612", PRIMARY),
        ("PORTFOLIO YIELD", "4.95%", PRIMARY),
        ("YIELD ON COST", "6.20%", ACCENT),
        ("ANNUAL INCOME", "$7,349", PRIMARY),
        ("TOTAL GAIN", "+$22.1K", PRIMARY),
        ("DIV GROWTH", "7.0%", PRIMARY),
        ("PROGRESS", "62%", PRIMARY),
    ]
    ...
    nav_y = gy + 2 * (kh + gap) + 8
    nav_h = 34
    nav = ["Holdings", "Income Calendar", "Calculations", "Projections", "Settings"]
    ...
    d.text((sx0 + sw / 2, note_y),
           "Holdings · Income Calendar · Sector Allocation · FIRE Projections — all in one file",
...
def render(out):
    ...
    coin(img, SIZE // 2, 150, r=58)
    pill(img, SIZE // 2, 285, "THE ULTIMATE DIVIDEND INCOME TRACKER", font=fs(36), pad_x=56, pad_y=22)

    tc(d, (SIZE // 2, 430), "DIVIDEND WEALTH", fserif(126), PRIMARY)
    tc(d, (SIZE // 2, 548), "BUILDER", fserif(118), PRIMARY)
    gold_divider(img, SIZE // 2, 632, width=560)
    tc(d, (SIZE // 2, 680), "Build $1,000 → $10,000+ a month in passive dividend income",
       fs(26, bold=False), TEXT_MUTED)

    laptop(img, SIZE // 2, 1015, w=1360, h=560)
    benefit_badge(img, 300, 738, "$612", "MONTHLY INCOME")
    benefit_badge(img, SIZE - 300, 738, "2-in-1", "EXCEL + GOOGLE SHEETS")

    tc(d, (SIZE // 2, 1360), "EVERYTHING A DIVIDEND INVESTOR NEEDS", fs(34), ACCENT)
    features = [
        ("Holdings Tracker", "gain/loss · weight · risk"),
        ("Income Calendar", "see your payday months"),
        ("Yield & YoC", "auto-calculated"),
        ("Sector Allocation", "instant diversification"),
        ("FIRE Projections", "30-year forecast"),
        ("Top Payers", "auto-ranked ranking"),
        ("Reinvestment", "compounding built in"),
        ("Goal Tracking", "progress to your target"),
    ]
    cols = 4
    margin = 120
    gx, gy = 24, 24
    ...
    pill(img, SIZE // 2, SIZE - 120,
         "6 SHEETS · INSTANT DOWNLOAD · EXCEL + GOOGLE SHEETS",
