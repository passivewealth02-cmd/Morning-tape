"""Detailed marketing image set for Food Cost & Inventory Command Center™ (07-10)."""
...
def render_features(out):
    ...
    header_band(img, "BUILT FOR OPERATORS", "Where Food Cost Is Won",
                "Four ways it puts your food cost back under control")
    feats = [
        ("$", "Live Inventory Score", "donut", (90, "90%", "healthy"),
         "Food cost, variance, counts, pars, turns & gross margin roll into one number — so you know your food-cost health at a glance."),
        ("%", "Food Cost to the Point", "table", None,
         "Beginning + purchases − ending ÷ sales = your true food-cost %. Your counts, invoices and sales flow straight in — no more guessing."),
        ("box", "Catch Every Leak", "stock", None,
         "Compare theoretical usage to actual to catch shrinkage, value inventory to the dollar, and see exactly where the food dollar goes."),
        ("star", "12 Printable Pages", "print", None,
         "A matching print-ready pack — inventory count sheet, food-cost worksheet, order guide, vendor list, waste log & a count checklist."),
    ]
    margin = 84; gap = 28
    ...
    for i, (badge, title, kind, dat, body) in enumerate(feats):
        ...
        if kind == "donut":
            ...
            rows = [("Begin + Purch − End", "$19,200"), ("÷ Sales", "$64,000"), ("Food cost %", "30.0%")]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
            grad_round(img, (tx0, ty, tx1, ty + 34), 6, PRIMARY_LT, PRIMARY_DK)
            for h, fx in zip(["THE PERIOD", "VALUE"], [0.0, 0.62]):
            ...
            od = ImageDraw.Draw(img)
            items = [("Inventory value", "$11,800", MINT_BG, PRIMARY), ("Food used (COGS)", "$19,200", MINT_BG, PRIMARY),
                     ("Usage variance", "+2.9%", RED_BG, (150, 60, 60)), ("Inventory turns", "1.6", SURFACE, (110, 88, 58))]
            tx0 = x + 44; tx1 = x + cw - 44; ty = vy - 30
...
def render_compare(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "WHY IT'S DIFFERENT", "More Than a Count Sheet",
                "Everything a basic inventory sheet leaves out — built in")
    rows = [
        ("An inventory count sheet", True, True),
        ("Period food-cost % engine", False, True),
        ("Inventory valuation to the dollar", False, True),
        ("Theoretical vs actual variance", False, True),
        ("Par levels & order value", False, True),
        ("Vendor & price tracking", False, True),
        ("Menu costing & food %", False, True),
        ("Waste & category breakdown", False, True),
        ("Everything cross-linked & live", False, True),
        ("Live Inventory Score dashboard", False, True),
    ...
    d.text((c1, top + hh // 2), "A Basic\nCount Sheet", font=fs(18), fill=(210, 210, 205), anchor="mm")
...
def render_howitworks(out):
    img = Image.new("RGBA", (SIZE, SIZE), BG + (255,))
    header_band(img, "SET UP IN AN AFTERNOON", "Control Food Cost in 4 Steps",
                "No spreadsheet skills needed — if you can type, you can run it")
    steps = [
        ("1", "Count your inventory", "Buy once and instantly download the Google Sheets system plus the 12-page printable pack, then count each item — the extended value is your inventory."),
        ("2", "Log purchases & sales", "Enter your invoices and food sales for the period — the totals flow straight into the food-cost engine, no re-typing."),
        ("3", "Read your food cost %", "Beginning + purchases − ending ÷ sales = your true food-cost %, to the point. Compare theoretical usage to actual to catch shrinkage."),
        ("4", "Control the leaks", "Set par levels, track vendor prices, cost your menu and log waste, and watch your Inventory Score climb."),
    ]
    margin = 100; top = 500
    sh = 300; gap = 24
...
def render_value(out):
    ...
    who = ["Restaurants & cafés", "Bars & pubs", "Food trucks & caterers",
           "Bakeries & ghost kitchens", "Chefs & kitchen managers", "Anyone who tracks food cost"]
    ...
    d = ImageDraw.Draw(img)
    tc(d, (SIZE // 2, 1530), "Know your food cost to the point, catch every leak, and keep more of every dollar.",
       fs(23, bold=False), TEXT_MUTED)
    book_crest(img, SIZE // 2, 1700, r=54)
    wordmark(img, SIZE // 2, 1850, "FOOD COST & INVENTORY", 56, max_w=1280)
    img.convert("RGB").save(out, "PNG", optimize=True)
