"""Printable PDF pack for Food Cost & Inventory Command Center™ (12 pages, US Letter)."""
...
def page(title, subtitle):
    ...
    d.text((M, 90), "FOOD COST & INVENTORY COMMAND CENTER™", font=fs(30), fill=GOLD_HI, anchor="lt")
    ...
    d.text((M, H - 108), "Period / Date: ______________", font=fs(24, bold=False), fill=MUTED, anchor="lt")
    d.text((W - M, H - 108), "Food Cost & Inventory Command Center™", font=fs(22, bold=False), fill=ACCENT, anchor="rt")
    return img, d
...
def p01(i):
    img, d = page("Inventory Count Sheet", "Count × unit cost = value")
    field(d, M, TOP, CW / 2 - 30, "Area / station"); field(d, M + CW / 2 + 30, TOP, CW / 2 - 30, "Counted by")
    table(d, M, TOP + 130, CW, ["ITEM", "COUNT", "UNIT COST", "EXT. VALUE"], [0.0, 0.46, 0.64, 0.84], 20, 118)
    i.append(img)


def p02(i):
    img, d = page("Food Cost Worksheet", "Beginning + Purchases − Ending ÷ Sales")
    for k, lab in enumerate(["Beginning inventory", "+ Purchases", "− Ending inventory",
                             "= Food used (COGS)", "÷ Food sales", "= Food cost %"]):
        field(d, M, TOP + k * 200, CW, lab)
    i.append(img)


def p03(i):
    img, d = page("Purchases Log", "Every invoice in, by vendor")
    table(d, M, TOP, CW, ["DATE", "VENDOR", "CATEGORY", "AMOUNT"], [0.0, 0.18, 0.50, 0.82], 18, 120)
    i.append(img)


def p04(i):
    img, d = page("Sales Log", "Food sales for the period")
    table(d, M, TOP, CW, ["PERIOD", "SALES", "FOOD %", "NOTES"], [0.0, 0.28, 0.46, 0.66], 18, 120)
    i.append(img)


def p05(i):
    img, d = page("Usage & Variance", "Theoretical vs actual = shrinkage")
    table(d, M, TOP, CW, ["CATEGORY", "THEORETICAL", "ACTUAL", "VARIANCE %"], [0.0, 0.36, 0.58, 0.80], 14, 155)
    i.append(img)


def p06(i):
    img, d = page("Par & Order Guide", "What to order, and what it costs")
    table(d, M, TOP, CW, ["ITEM", "PAR", "ON HAND", "ORDER", "COST"], [0.0, 0.42, 0.56, 0.72, 0.86], 20, 110)
    i.append(img)


def p07(i):
    img, d = page("Vendor List", "Who you buy from, by category")
    table(d, M, TOP, CW, ["VENDOR", "CATEGORY", "CONTACT", "STATUS"], [0.0, 0.28, 0.54, 0.84], 18, 120)
    i.append(img)


def p08(i):
    img, d = page("Price Comparison", "The cheaper vendor wins")
    table(d, M, TOP, CW, ["ITEM", "VENDOR A", "PRICE A", "VENDOR B", "PRICE B"], [0.0, 0.30, 0.50, 0.64, 0.86], 18, 120)
    i.append(img)


def p09(i):
    img, d = page("Menu Costing", "Plate cost, food % & margin")
    table(d, M, TOP, CW, ["ITEM", "PLATE COST", "PRICE", "FOOD %", "MARGIN"], [0.0, 0.40, 0.58, 0.72, 0.86], 18, 120)
    i.append(img)


def p10(i):
    img, d = page("Waste Log", "The leaks that raise food cost")
    table(d, M, TOP, CW, ["DATE", "ITEM", "REASON", "COST"], [0.0, 0.16, 0.44, 0.82], 20, 118)
    i.append(img)


def p11(i):
    img, d = page("Category Breakdown", "Where the food dollar goes")
    table(d, M, TOP, CW, ["CATEGORY", "FOOD COST $", "% OF TOTAL", "TARGET"], [0.0, 0.34, 0.58, 0.80], 14, 155)
    i.append(img)


def p12(i):
    img, d = page("Weekly Count Checklist", "Same count, every week")
    half = CW / 2 - 30
    for k, st in enumerate(["Walk-in / cooler", "Dry storage"]):
        x = M + k * (half + 60)
...
def main():
    ...
    pdf_path = os.path.join(out_dir, "Food_Cost_Printables.pdf")
